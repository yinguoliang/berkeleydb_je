...
public class INTest extends TestBase {
    ...
    public void setUp() throws Exception {
        ...
        initialINCapacity = envImpl.getConfigManager().getInt(EnvironmentParams.NODE_MAX);
        db = new DatabaseImpl(null, "foo", new DatabaseId(11), envImpl, new DatabaseConfig());
    }
    ...
    @Test
    public void testFindEntry() throws DatabaseException {
        ...
        for (int i = 0; i < N_BYTES_IN_KEY; i++) {
             ...
             * checks the Key.compareTo routine for proper unsigned comparisons.
             */
        ...
        for (int i = 0; i < initialINCapacity; i++) {
            ...
            int insertionFlags = in.insertEntry1(null, keyBytes, null, FAKE_LSN, false);
            ...
            for (int j = 1; j < in.getNEntries(); j++) { // 0th key is virtual
                ...
                assertTrue(in.findEntry(in.getKey(j), true, false) == (j | IN.EXACT_MATCH));
                assertTrue(in.findEntry(in.getKey(j), true, true) == (j | IN.EXACT_MATCH));
    ...
    @Test
    public void testInsertEntry() throws DatabaseException {
    ...
    private void doInsertEntry(boolean withMinMax) throws DatabaseException {
        ...
        if (withMinMax) {
            ...
            assertTrue(in.findEntry(maxBytes, true, false) == (1 | IN.EXACT_MATCH));
            /* Shadowed by the virtual 0'th key. */
            assertTrue(in.findEntry(zeroBytes, true, true) == IN.EXACT_MATCH);
            assertTrue(in.findEntry(maxBytes, true, true) == (1 | IN.EXACT_MATCH));
        }

        Random rnd = new Random();

        try {
            for (int i = 0; i < initialINCapacity - (withMinMax ? 2 : 0); i++) {
                ...
                /*
                 * There's a small chance that we may generate the same sequence
                 * of bytes that are already present.
                 */
                while (true) {
                    rnd.nextBytes(keyBytes);
                    int index = in.findEntry(keyBytes, true, false);
                    if ((index & IN.EXACT_MATCH) != 0 && index >= 0) {
                        continue;
                    }
                    break;
                }
                ...
                if (withMinMax) {
                    assertTrue(in.findEntry(zeroBytes, false, false) == 0);
                    assertTrue(in.findEntry(maxBytes, false, false) == in.getNEntries() - 1);
                    /* Shadowed by the virtual 0'th key. */
                    assertTrue(in.findEntry(zeroBytes, false, true) == 0);
                    assertTrue(in.findEntry(maxBytes, false, true) == in.getNEntries() - 1);

                    assertTrue(in.findEntry(zeroBytes, true, false) == IN.EXACT_MATCH);
                    assertTrue(in.findEntry(maxBytes, true, false) == ((in.getNEntries() - 1) | IN.EXACT_MATCH));
                    /* Shadowed by the virtual 0'th key. */
                    assertTrue(in.findEntry(zeroBytes, true, true) == IN.EXACT_MATCH);
                    assertTrue(in.findEntry(maxBytes, true, true) == ((in.getNEntries() - 1) | IN.EXACT_MATCH));
                } else {
                    assertTrue(in.findEntry(zeroBytes, false, false) == 0);
                    assertTrue(in.findEntry(maxBytes, false, false) == in.getNEntries() - 1);
                    assertTrue(in.findEntry(zeroBytes, false, true) == -1);
                    ...
                    assertTrue(in.findEntry(zeroBytes, true, false) == -1);
                    assertTrue(in.findEntry(maxBytes, true, false) == in.getNEntries() - 1);
                }
        ...
        try {
            ...
            assertSame(EnvironmentFailureReason.UNEXPECTED_STATE_FATAL, e.getReason());
        }
        in.releaseLatch();
    }

    @Test
    public void testDeleteEntry() throws DatabaseException {
    ...
    private void doDeleteEntry(boolean withMinMax) throws DatabaseException {
    ...
    private boolean deleteEntry(IN in, byte[] key) throws DatabaseException {
