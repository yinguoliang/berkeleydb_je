...
public class EvictActionTest extends TestBase {
    ...
    @Test
    public void testNoNeedToEvict() throws Throwable {

        doEvict(80, BIG_CACHE_SIZE, false);
    }

    /**
     * Evict in very controlled circumstances. Check that we first strip BINs
     * and later evict BINS.
     */
    private void doEvict(int floor, int maxMem, boolean shouldEvict) throws Throwable {
    ...
    @Test
    public void testSetCacheSize() throws DatabaseException {
    ...
    @Test
    public void testSetCachePercent() throws DatabaseException {
    ...
    @Test
    public void testThreadedCacheSizeChanges() throws DatabaseException {
    ...
    @Test
    public void testSmallCacheSettings() throws DatabaseException {
        ...
        env.setMutableConfig(config);
        stats = env.getStats(null);
        assertEquals(500 * 1024 - stats.getBufferBytes(), mb.getMinTreeMemoryUsage());
    ...
    /**
     * We now allow eviction of the root IN of a DB, whether the DB is closed or
     * not. Check that basic root eviction works. [#13415]
     */
    @Test
    public void testRootINEviction() throws DatabaseException {
    ...
    /**
     * We now allow eviction of the MapLN and higher level INs in the DB mapping
     * tree when DBs are closed. Check that basic mapping tree IN eviction
     * works. [#13415]
     */
    @Test
    public void testMappingTreeEviction() throws DatabaseException {
        ...
        EnvironmentConfig envConfig = getEnvConfig(80, SMALL_CACHE_SIZE, false /* readonly */);
     ...
     */
    public void testCleaningAfterMappingTreeEviction() throws DatabaseException {
        ...
        EnvironmentConfig envConfig = getEnvConfig(0, SMALL_CACHE_SIZE, false /* readonly */);
        envConfig.setConfigParam(EnvironmentConfig.LOG_FILE_MAX, String.valueOf(1024 * 1024));
         ...
         * single call to cleanLog, the MapLN will be resident due to the nearby
         * LNs for that database, since the cleaner does read-ahead during LN
         * processing. Prior to the bug fix, TestUtils.validateNodeMemUsage
         * (called by forceEviction below) would report a mismatch in the tree
         * admin memory usage.
         */
     ...
     */
    @Test
    public void testReadOnlyRootINEviction() throws DatabaseException {
        ...
        status = db.put(null, bigRecordKey, new DatabaseEntry(new byte[BIG_CACHE_SIZE]));
        assertSame(OperationStatus.SUCCESS, status);
        ...
        EnvironmentConfig envConfig = getEnvConfig(80, SMALL_CACHE_SIZE, true /* readOnly */);
        openEnv(envConfig);
         ...
         */
        IN rootIN = DbInternal.getDbImpl(db1).getTree().getRootIN(CacheMode.DEFAULT);
     ...
     */
    @Test
    public void testAbortOpen() throws DatabaseException {
        ...
        /*
         * Environment.close will not throw an exception, even though the DB has
         * not been closed. The abort took care of cleaning up the handle.
         */
        closeEnv();

        /*
         * Try a non-transactional DB open that throws an exception because we
         * create it exclusively and it already exists. The use count should be
         * decremented.
         */
        env = new Environment(envHome, envConfig);
        dbConfig.setAllowCreate(true);
    ...
    private void forceEviction() throws DatabaseException {
        ...
        for (int i = 0; i < 2; i += 1) {
            ...
            status = c.put(new DatabaseEntry(new byte[1]), new DatabaseEntry(new byte[BIG_CACHE_SIZE]));
            assertSame(OperationStatus.SUCCESS, status);

            /*
             * Evict while cursor pins LN memory, to ensure eviction of other DB
             * INs, including the DB root. When lruOnly=false, root IN eviction
             * may not occur unless a cursor is used to pin the LN.
             */
            env.evictMemory();
     ...
     */
    private void forceReadOnlyEviction(DatabaseEntry key) throws DatabaseException {
    ...
    private void openEnv(int floor, int maxMem) throws DatabaseException {
        ...
        EnvironmentConfig envConfig = getEnvConfig(floor, maxMem, false /* readonly */);
        openEnv(envConfig);
    }
     ...
     * Open an environment and database.
     */
    private EnvironmentConfig getEnvConfig(int floor, int maxMem, boolean readOnly) {
        ...
        if (evictBytes > 0) {
            envConfig.setConfigParam(EnvironmentParams.EVICTOR_EVICT_BYTES.getName(),
    ...
    private void openEnv(EnvironmentConfig envConfig) throws DatabaseException {
    ...
    private void insertData(int nKeys) throws DatabaseException {
    ...
    private void putLargeData(int nKeys, int dataSize) throws DatabaseException {
    ...
    private void verifyData(int nKeys) throws DatabaseException {
    ...
    private void evictAndCheck(boolean shouldEvict, int nKeys) throws DatabaseException {

        EnvironmentImpl envImpl = DbInternal.getNonNullEnvImpl(env);
        MemoryBudget mb = envImpl.getMemoryBudget();

        /*
         * The following batches are run in a single evictMemory() call: 1st
         * eviction will strip DBINs. 2nd will evict DBINs 3rd will evict DINs
         * 4th will strip BINs 5th will evict BINs 6th will evict INs 7th will
         * evict INs
         */
        ...
        TestUtils.validateNodeMemUsage(envImpl, true);
        if (DEBUG) {
            System.out.println("preEvict=" + preEvictMem + " postEvict=" + postEvictMem);
        }

        if (shouldEvict) {
            assertTrue("preEvict=" + preEvictMem + " postEvict=" + postEvictMem + " maxMem=" + mb.getMaxMemory(),
                    (preEvictMem > postEvictMem));
        } else {
            assertTrue("preEvict=" + preEvictMem + " postEvict=" + postEvictMem, (preEvictMem == postEvictMem));
        }
