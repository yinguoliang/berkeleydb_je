...
public class LatchTest extends TestBase {
    ...
    private void createExclusiveLatches() {
        latch1 = LatchFactory.createExclusiveLatch(LatchFactory.createTestLatchContext("LatchTest-latch1"),
                true /* collectStats */);
        latch2 = LatchFactory.createExclusiveLatch(LatchFactory.createTestLatchContext("LatchTest-latch2"),
                true /* collectStats */);
    }
    ...
    @Test
    public void testAcquireAndReacquire() throws Throwable {

        createExclusiveLatches();
        JUnitThread tester = new JUnitThread("testAcquireAndReacquire") {
            @Override
            public void testBody() {
                ...
                try {
                    ...
                    assertSame(EnvironmentFailureReason.UNEXPECTED_STATE, expected.getReason());
                    assertTrue(latch1.getStats().getInt(LATCH_SELF_OWNED) == 1);
                }
    ...
    @Test
    public void testAcquireAndReacquireShared() throws Throwable {

        final SharedLatch latch = LatchFactory
                .createSharedLatch(LatchFactory.createTestLatchContext("LatchTest-latch2"), false);

        JUnitThread tester = new JUnitThread("testAcquireAndReacquireShared") {
            @Override
            public void testBody() {
                ...
                try {
                    ...
                    assertSame(EnvironmentFailureReason.UNEXPECTED_STATE, e.getReason());
                }
            }
        };
     ...
     */
    @Test
    public void testAcquireReleasePerformance() throws Throwable {

        createExclusiveLatches();
        JUnitThread tester = new JUnitThread("testAcquireReleasePerformance") {
            @Override
            public void testBody() {
    ...
    public void doTestWait() throws Throwable {

        final AtomicBoolean t1Acquired = new AtomicBoolean(false);

        tester1 = new JUnitThread("testWait-Thread1") {
            @Override
            public void testBody() {
                /* Acquire a latch. */
                latch1.acquireExclusive();
        ...
        tester2 = new JUnitThread("testWait-Thread2") {
            @Override
            public void testBody() {
                ...
                assertTrue(latch1.getStats().getInt(LATCH_CONTENTION) == ++nAcquiresWithContention);
    ...
    @Test
    public void testAcquireNoWait() throws Throwable {
        ...
        tester2 = new JUnitThread("testWait-Thread2") {
            ...
            public void testBody() {
                ...
                /*
                 * Attempt Acquire with no wait -- should fail since tester1 has
                 * it.
                 */
                debugMsg("Acquiring no wait");
                assertFalse(latch1.acquireExclusiveNoWait());
                assertTrue(latch1.getStats().getInt(LATCH_NOWAIT_UNSUCCESS) == 1);
                ...
                assertTrue(latch1.getStats().getInt(LATCH_NOWAIT_SUCCESS) == 1);

                /*
                 * Attempt Acquire with no wait again -- should throw exception
    ...
    @Test
    public void testMultipleWaiters() throws Throwable {
        ...
        for (int i = 0; i < N_WAITERS; i++) {
            waiterThreads[i] = new MultiWaiterTestThread("testWait-Waiter" + i, i) {
                @Override
                public void testBody() {

                    int waiterNumber = ((MultiWaiterTestThread) Thread.currentThread()).waiterNumber;
                    ...
                    /*
                     * Wait until it's our turn to try to acquire the latch.
                     */
                    ...
                    assertTrue(latch1.getNWaiters() == (N_WAITERS - waiterNumber - 1));
