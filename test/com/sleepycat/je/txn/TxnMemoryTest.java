...
public class TxnMemoryTest extends DualTestCase {
    ...
    private void openEnv() throws DatabaseException {
        ...
        /*
         * ReadCommitted isolation is not allowed by this test because we expect
         * no locks/memory to be freed when using a transaction.
         */
     ...
     */
    @Test
    public void testWriteLocks() throws DatabaseException {

        loadData();

        /*
         * Now update the database transactionally. This should not change the
         * node related memory, but should add txn related cache consumption. If
         * this is a user transaction, we should hold locks and consume more
         * memory.
         */
        for (int t = 0; t < numTxns; t++) {
            for (int i = 0; i < numRecordsPerTxn; i++) {
                ...
                assertEquals(db.put(txns[t], keyEntry, dataEntry), OperationStatus.SUCCESS);
            }
        }
     ...
     */
    @Test
    public void testReadLocks() throws DatabaseException {

        loadData();

        /*
         * Now scan the database. Make sure all locking overhead is released.
         */
        for (int t = 0; t < numTxns; t++) {
    ...
    private void loadData() throws DatabaseException {
        ...
        for (int t = 0; t < numTxns; t++) {
            for (int i = 0; i < numRecordsPerTxn; i++) {
                ...
                assertEquals(db.put(null, keyEntry, dataEntry), OperationStatus.SUCCESS);
            }
        }
        ...
        if (lockMode.equals(LOCK_USERTXN)) {
            ...
            assertTrue("afterTxns=" + afterTxnsCreated + "beforeUpdate=" + beforeAction,
                    (afterTxnsCreated > beforeAction));
        }
    }

    private void closeTxns(boolean writesDone) throws DatabaseException {

        assertTrue(afterAction > afterTxnsCreated);

        /*
         * If this is not a user transactional lock, we should be done with all
         * locking overhead. If it is a user transaction, we only release memory
         * after locks are released at commit or abort.
         */
        if (lockMode.equals(LOCK_USERTXN)) {

            /*
             * Note: expectedLockUsage is annoyingly fragile. If we change the
             * lock implementation, this may not be the right number to check.
             */
            long expectedLockUsage = (numRecordsPerTxn * numTxns * MemoryBudget.THINLOCKIMPL_OVERHEAD);
            ...
            assertTrue("lockMode=" + lockMode + " endMode=" + endMode + " afterTxnEnd=" + afterTxnEnd + " beforeAction="
                    + beforeAction, (afterTxnEnd <= beforeAction));
        }
        if (DEBUG) {
            System.out.println("afterUpdate = " + afterAction + " before=" + beforeAction);
        }
