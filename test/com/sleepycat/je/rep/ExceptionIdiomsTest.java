...
/**
 * Dummy junit test captures illustrative code for HA exception handling idioms.
 */
public class ExceptionIdiomsTest extends TestBase {

    /*
     * The number of seconds to wait between retries when a sufficient number of
     * replicas are not available for a transaction.
     */
     ...
     * environment, and creates a new environment handle so that the application
     * can recover and continue to make progress in the face of errors that
     * might result in environment invalidation.
     *
     ...
     */
    void environmentLoop(File envHome, ReplicationConfig repConfig, EnvironmentConfig envConfig)
        ...
        while (true) {
            ...
            try {
                ...
                networkRestore.execute(insufficientLog, new NetworkRestoreConfig());
                continue;
            } catch (RollbackException rollback) {

                /*
                 * Any transient state that is dependent on the environment must
                 * be re-synchronized to account for transactions that may have
                 * been rolled back.
                 */
                continue;
            } finally {
     ...
     */
    ReplicatedEnvironment getEnvironmentHandle(File envHome, ReplicationConfig repConfig, EnvironmentConfig envConfig)
            throws InterruptedException {
        ...
        /*
         * INSERT APP-SPECIFIC CODE HERE: For example, an applications may throw
         * an exception or retry indefinitely in the above loop.
         */
        throw new IllegalStateException("Unable to open handle after " + REP_HANDLE_RETRY_MAX + " tries");
    }

    /**
     * Initiates transactions based upon the state of the node.
     * <p>
     * The code in the method is single threaded, but the dispatch could be also
     * be used to start threads to perform transactional work.
     *
     * @param repEnv the replicated environment
     * @throws InterruptedException
     */
    private void transactionDispatchLoop(ReplicatedEnvironment repEnv) throws InterruptedException {
        ...
        while (true) {
            ...
            if (state.isUnknown()) {
                 ...
                 * nodes for an election, network connectivity issues, etc. Wait
                 * and retry to allow the problem to be resolved.
                 */
                ...
                /*
                 * The node is no longer in communication with the group. Reopen
                 * the environment, so the application can resume.
                 */
                return;
            }

            /*
             * The node is in the master or replica state. Assumes that a higher
             * level has sent it the appropriate type (read or write) of
             * transaction workload. If that isn't the case, the handler for
             * ReplicaWriteException will cause it to be redirected to the
    ...
    /**
     * Illustrates the handling of exceptions thrown in the process of creating
     * and executing a transaction. It retries the transaction if the exception
     * indicates that a retry is warranted.
     *
     ...
     *             the the outer level.
     */
    private void doTransaction(ReplicatedEnvironment repEnv, boolean readOnly)
            throws InterruptedException, InsufficientLogException, RollbackException {

        boolean success = false;
        long sleepMillis = 0;
        final TransactionConfig txnConfig = readOnly ? new TransactionConfig().setDurability(Durability.READ_ONLY_TXN)
                : null;
        ...
        for (int i = 0; i < TRANSACTION_RETRY_MAX; i++) {
            ...
            try {
                ...
                /*
                 * INSERT APP-SPECIFIC CODE HERE: For example, repeat idempotent
                 * changes to ensure they went through. Note that 'success' is
                 * false at this point, although some applications may consider
                ...
                /*
                 * Attempted a modification while in the Replica state. INSERT
                 * APP-SPECIFIC CODE HERE: Cannot accomplish the changes on this
                 * node, redirect the write to the new master and retry the
                 * transaction there. This could be done by forwarding the
                ...
                /*
                 * Retry the transaction. Note that LockConflictException covers
