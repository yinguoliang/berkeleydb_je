...
public class StateChangeListenerTest extends RepTestBase {
    ...
    public void testEventIdentity() {
        ...
        try {
            ...
            final PassiveListener passiveListener = (PassiveListener) rep1.getStateChangeListener();
    ...
    public void testBasic() throws Exception {
        ...
        for (int i = 0; i < repEnvInfo.length; i++) {
            ...
            assertEquals(repEnvInfo[0].getRepConfig().getNodeName(), event.getMasterNodeName());
        ...
        awaitEvents(listenerLatch, 60, TimeUnit.SECONDS, listeners.get(0).events, 2, UNKNOWN, DETACHED);
        ...
        for (int i = 1; i < repEnvInfo.length; i++) {
            ...
            assertEquals(handleState, listeners.get(i).events.get(1).getState());
        ...
        for (int i = 1; i < repEnvInfo.length; i++) {
            ...
            /* Wait 60s to ensure events can be delivered */
            awaitEvents(listenerLatch, 60, TimeUnit.SECONDS, listeners.get(i).events, numExpectedEvents);
        }
    }
    ...
    public void testSecondary() throws Exception {
         ...
         * generated only if the secondary notices the loss of the master before
         * it is closed down.
         */
        listenerLatch.await(60, TimeUnit.SECONDS);
        if (listener.events.size() == 2) {
            awaitEvents(listenerLatch, 0, TimeUnit.SECONDS, listener.events, 2, REPLICA, DETACHED);
        } else {
            awaitEvents(listenerLatch, 0, TimeUnit.SECONDS, listener.events, 3, REPLICA, UNKNOWN, DETACHED);
        }
    }

    /**
     * Assert that the count down latch reaches zero in the specified amount of
     * time, and confirm that the expected number of events were delivered. If
     * expectedStates are specified, check that the delivered events have the
     * expected states.
     */
    void awaitEvents(CountDownLatch latch, long time, TimeUnit timeUnit, List<StateChangeEvent> events,
                     int numExpectedEvents, State... expectedStates)
            throws InterruptedException {

        final long start = System.currentTimeMillis();
        latch.await(time, timeUnit);
        if (events.size() < numExpectedEvents) {
            fail("Expected " + numExpectedEvents + " events, found " + events.size() + ": " + describeEvents(events));
        }

        if ((expectedStates != null) && (expectedStates.length > 0)) {
            assertEquals("Number of expected states", numExpectedEvents, expectedStates.length);
            for (int i = 0; i < numExpectedEvents; i++) {
                if (!expectedStates[i].equals(events.get(i).getState())) {
                    fail("Expected event " + i + " state " + expectedStates[i] + ", found " + events.get(i).getState()
                            + ", for events: " + describeEvents(events));
                }
            }
        }
        if (logger.isLoggable(INFO)) {
            logger.info("Received awaited events" + ", startTime: " + new Timestamp(start) + ", events: "
    ...
    class Listener implements StateChangeListener {
        ...
        final List<StateChangeEvent> events = Collections.synchronizedList(new LinkedList<StateChangeEvent>());
    ...
    class BadListener implements StateChangeListener {
        ...
        @Override
        public void stateChange(@SuppressWarnings("unused") StateChangeEvent stateChangeEvent) {
