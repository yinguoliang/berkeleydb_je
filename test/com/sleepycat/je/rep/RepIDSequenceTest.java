...
public class RepIDSequenceTest extends TestBase {
    ...
    @Test
    public void testIDSequenceAfterRecovery() throws Exception {
        ...
        /*
         * The node id, db id and txn id should be the same as before, since the
         * test doesn't do any operations during this time.
         */
        assertEquals(lastRepNodeId, repImpl.getNodeSequence().getLastReplicatedNodeId());
        assertEquals(lastDbId, repImpl.getDbTree().getLastReplicatedDbId());
        /*
         ...
         */
        assertEquals(lastTxnId - 1, repImpl.getTxnManager().getLastReplicatedTxnId());
        doOperations(master, DB_NUM + 1, DB_NUM + 1);

        /*
         * The db id and txn id should be smaller than before, since test creats
         * a new database, db id should be 1 smaller than before.
         */
        assertTrue(repImpl.getDbTree().getLastReplicatedDbId() == lastDbId - 1);
        assertTrue(repImpl.getTxnManager().getLastReplicatedTxnId() < lastTxnId);
        master.close();
    }

    /* Verify that ids are in sequence after fail over. */
    @Test
    public void testIDSequenceAfterFailOver() throws Exception {
    ...
    private void doOperations(ReplicatedEnvironment master, int beginId, int endId) throws Exception {
        ...
        for (int i = beginId; i <= endId; i++) {
            Database db = master.openDatabase(null, DB_NAME + new Integer(i).toString(), dbConfig);
            insertData(db);
            db.close();
        }
    }
