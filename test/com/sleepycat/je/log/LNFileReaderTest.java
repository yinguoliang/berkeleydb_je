...
public class LNFileReaderTest extends TestBase {
    ...
    public void testNoFile() throws DatabaseException {
        ...
        LNFileReader reader = new LNFileReader(envImpl, 1000, // read buffer size
        ...
        assertFalse("Empty file should not have entries", reader.readNextEntry());
    }
     ...
     */
    @Test
    public void testBasicRedo() throws Throwable {

        try {
            DbConfigManager cm = envImpl.getConfigManager();
            doTest(50, cm.getInt(EnvironmentParams.LOG_ITERATOR_READ_SIZE), 0, false, true);
        } catch (Throwable t) {
            t.printStackTrace();
            throw t;
        }
     ...
     */
    @Test
    public void testBasicUndo() throws Throwable {

        try {
            DbConfigManager cm = envImpl.getConfigManager();
            doTest(50, cm.getInt(EnvironmentParams.LOG_ITERATOR_READ_SIZE), 0, false, false);
        } catch (Throwable t) {
            t.printStackTrace();
            throw t;
        }
     ...
     */
    @Test
    public void testSmallBuffersRedo() throws IOException, DatabaseException {
     ...
     */
    @Test
    public void testSmallBuffersUndo() throws IOException, DatabaseException {
     ...
     */
    @Test
    public void testMedBuffersRedo() throws IOException, DatabaseException {
     ...
     */
    @Test
    public void testMedBuffersUndo() throws IOException, DatabaseException {
     ...
     */
    @Test
    public void testMiddleStartRedo() throws IOException, DatabaseException {
     ...
     */
    @Test
    public void testMiddleStartUndo() throws IOException, DatabaseException {
     ...
     * 
     * @param numIters each iteration makes 3 log entries (debug record, ln and
     *            mapLN
     ...
     *            mapLNs
     */
    private void doTest(int numIters, int bufferSize, int checkIndex, boolean trackLNs, boolean redo)
        ...
        LNFileReader reader = new LNFileReader(envImpl, bufferSize, startLsn, redo, endOfFileLsn, finishLsn, null,
                DbLsn.NULL_LSN);
        if (trackLNs) {
            addUserLNTargetTypes(reader);
        } else {
     ...
     */
    private long createLogFile(int numIters, boolean trackLNs, boolean redo) throws IOException, DatabaseException {
        ...
        for (int i = 0; i < numIters; i++) {
             ...
             */
            lsn = ln.log(envImpl, dbImpl, userTxn, new WriteLockInfo(), false, key, 0 /* newExpiration */,
                    false /* newExpirationInHours */, false, DbLsn.NULL_LSN, 0, true /* isInsertion */, false,
                    ReplicationContext.NO_REPLICATE).lsn;
            ...
            nonTxnalLN.log(envImpl, dbImpl, null, null, false, key, 0 /* newExpiration */,
                    false /* newExpirationInHours */, false, DbLsn.NULL_LSN, 0, true /* isInsertion */, false,
            ...
            lsn = mapLN.log(envImpl, mapDbImpl, null, null, false, key, 0 /* newExpiration */,
                    false /* newExpirationInHours */, false, DbLsn.NULL_LSN, 0, true /* isInsertion */, false,
                    ReplicationContext.NO_REPLICATE).lsn;
    ...
    private void checkLogFile(LNFileReader reader, int checkIndex, boolean redo) throws DatabaseException {
        ...
        while (reader.readNextEntry()) {
            ...
            /* Check LSN. */
            assertEquals("LSN " + i + " expected " + DbLsn.getNoFormatString(expected.lsn) + " but read "
                    + DbLsn.getNoFormatString(reader.getLastLsn()), expected.lsn, reader.getLastLsn());
            ...
            if (reader.isLN()) {
                ...
                assertEquals("Should be the same type of object", expectedLN.getClass(), lnFromLog.getClass());

                if (DEBUG) {
                    if (!expectedLN.toString().equals(lnFromLog.toString())) {
                        System.out
                                .println("expected = " + expectedLN.toString() + "lnFromLog = " + lnFromLog.toString());
                    }
                }

                /*
                 * Don't expect MapLNs to be equal, since they change as logging
                 * occurs and utilization info changes.
                 */
                if (!(expectedLN instanceof MapLN)) {
                    assertEquals("LN " + i + " should match", expectedLN.toString(), lnFromLog.toString());
                }
                ...
                if (DEBUG) {
                    if (!Arrays.equals(expectedKey, keyFromLog)) {
                        System.out.println("expectedKey=" + expectedKey + " logKey=" + keyFromLog);
                    }
                }

                assertTrue("Key " + i + " should match", Arrays.equals(expectedKey, keyFromLog));

                if (expected.txnId != 0) {
                    assertEquals(expected.txnId, reader.getTxnId().longValue());
                }

            } else {
                /* Should be a txn commit record. */
                assertEquals(expected.txnId, reader.getTxnCommitId());
            }
