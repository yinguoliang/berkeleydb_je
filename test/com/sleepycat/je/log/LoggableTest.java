...
public class LoggableTest extends TestBase {
    ...
    public void testEntryData() throws Throwable {
        ...
        try {
            ...
            database = new DatabaseImpl(null, "foo", new DatabaseId(1), envImpl, new DatabaseConfig());
            ...
            in.insertEntry(null, new byte[] { 0, 0, 1, 0 }, DbLsn.makeLsn(1235, 400));

            in.insertEntry(null, new byte[] { 0, 0, 1, 0 }, DbLsn.makeLsn(0xFFFFFFF0L, 400));
            ...
            bin.insertEntry(null, new byte[] { 1, 0, 1, 0 }, DbLsn.makeLsn(212, 200));

            bin.insertEntry(null, new byte[] { 1, 1, 1, 0 }, DbLsn.makeLsn(229, 300));

            bin.insertEntry(null, new byte[] { 0, 0, 1, 0 }, DbLsn.makeLsn(235, 400));
             ...
             * Root
             */
            DbTree dbTree = new DbTree(envImpl, false /* replicationIntended */, false /* preserveVLSN */);
            DbTree dbTreeFromLog = new DbTree();
            writeAndRead(buffer, LogEntryType.LOG_DBTREE, dbTree, dbTreeFromLog);
            dbTree.close();
            ...
            writeAndRead(buffer, LogEntryType.LOG_NAMELN_TRANSACTIONAL, nameLn, nameLnFromLog);
            ...
            /*
             * Disabled for now because these txns don't compare equal, because
             * one has a name of "main" and the other has a name of null because
             * it was read from the log. Txn txn = new Txn(envImpl, new
             * TransactionConfig()); Txn txnFromLog = new Txn();
             ...
             * TxnCommit
             */
            TxnCommit commit = new TxnCommit(111, DbLsn.makeLsn(10, 10), 179 /* masterNodeId */, 1 /* DTVLSN */);
             ...
             * TxnAbort
             */
            TxnAbort abort = new TxnAbort(111, DbLsn.makeLsn(11, 11), 7654321 /* masterNodeId */, 1 /* DTVLSN */);
             ...
             * Checkpoint end
             */
            CheckpointEnd end = new CheckpointEnd("test", DbLsn.makeLsn(20, 55), envImpl.getRootLsn(),
                    ...
                    envImpl.getTxnManager().getLastReplicatedTxnId(), 177, true /* cleanerFilesToDelete */);
            ...
            RollbackStart rs = new RollbackStart(new VLSN(1001), 99, activeTxnIds);
            RollbackStart rsFromLog = new RollbackStart();
            writeAndRead(buffer, LogEntryType.LOG_ROLLBACK_START, rs, rsFromLog);
            ...
            writeAndRead(buffer, LogEntryType.LOG_ROLLBACK_END, re, reFromLog);
            ...
            writeAndRead(buffer, LogEntryType.LOG_ROLLBACK_END, matchpoint, matchpointFromLog);
            ...
            RestoreRequired rr = new RestoreRequired(RestoreRequired.FailureType.NETWORK_RESTORE, props);

            RestoreRequired rrFromLog = new RestoreRequired();
            writeAndRead(buffer, LogEntryType.LOG_RESTORE_REQUIRED, rr, rrFromLog);
    ...
    /**
     * Helper which takes a dbLoggable, writes it, reads it back and checks for
     * equality and size
     */
    private void writeAndRead(ByteBuffer buffer, LogEntryType entryType, Loggable orig, Loggable fromLog)
            throws Exception {
        ...
        /*
         * Read it and compare sizes. Note that we assume we're testing objects
         * that are readable and writable to the log.
         */
        fromLog.readFromLog(buffer, LogEntryType.LOG_VERSION);
        assertEquals(orig.getLogSize(), fromLog.getLogSize());

        assertEquals("We should have read the whole buffer for " + fromLog.getClass().getName(), buffer.limit(),
                buffer.position());
        ...
        assertEquals("Not equals for " + fromLog.getClass().getName(), sb1.toString(), sb2.toString());
