...
public class PropUtilTest extends TestBase {
    ...
    public void testDurationToMillis() {
        ...
        try {
            PropUtil.durationToMillis(((long) Integer.MAX_VALUE) + 1, TimeUnit.MILLISECONDS);
        } catch (IllegalArgumentException expected) {
        }
        ...
        assertEquals(2, PropUtil.durationToMillis(2000000, TimeUnit.NANOSECONDS));
    }
    ...
    public void testParseDuration() {
        ...
        String[][][] testvals = { { { "1 ns", "1", "1" } }, { { "1", Long.toString(NS_TO_MICRO), "1" } },
                { { "1 us", "1000", "1" } }, { { "1 ms", "1000000", "1" } }, { { "1 nanoseconds", "1", "1" } },
                ...
                /* IEEE abbreviations */
                { { "3000000 NS", "3000000", "3" } }, { { "3000 US", Long.toString(NS_TO_MICRO * 3000), "3" } },
                ...
                { { "1 s", Long.toString(NS_TO_SECOND), Long.toString(MS_TO_SECOND) } },
                { { "1 min", Long.toString(NS_TO_MINUTE), Long.toString(MS_TO_MINUTE) } },
                { { "1 h", Long.toString(NS_TO_HOUR), Long.toString(MS_TO_HOUR) } },
                /* maximum 32 bit for ms */
                { { "2147483647 ms", Long.toString(2147483647L * NS_TO_MILLI), "2147483647" } },
                /* maximum 32 bit ns */
                { { "596 h", Long.toString(596 * NS_TO_HOUR), Long.toString(596 * MS_TO_HOUR) } } };

        String[][][] exceeds32BitMillis = {
                { { "2147483648 ms", Long.toString(2147483648L * NS_TO_MILLI), "2147483648" } },
                { { "597 h", Long.toString(597 * NS_TO_HOUR), Long.toString(597 * MS_TO_HOUR) } } };
        ...
        for (String[][] val : testvals) {
            ...
            assertTrue("expected " + val[0][1] + " got " + val[0][0], valueNano == Long.valueOf(val[0][1]));

            int valueMillis = PropUtil.parseDuration(val[0][0]);
            assertTrue("expected " + val[0][2] + " got " + val[0][0], valueMillis == Long.valueOf(val[0][2]));
        }
    ...
    public void testFormatDuration() {
        assertEquals("30 NANOSECONDS", PropUtil.formatDuration(30, TimeUnit.NANOSECONDS));
        assertEquals("30 MICROSECONDS", PropUtil.formatDuration(30, TimeUnit.MICROSECONDS));
        assertEquals("30 MILLISECONDS", PropUtil.formatDuration(30, TimeUnit.MILLISECONDS));
        assertEquals("30 SECONDS", PropUtil.formatDuration(30, TimeUnit.SECONDS));
    }
}
