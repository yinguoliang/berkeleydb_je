...
public class TestUtils {
    ...
    /**
     * Generate a synthetic base 26 four byte alpha key from an int. The bytes
     * of the key are between 'A' and 'Z', inclusive. 0 maps to 'AAAA', 1 to
     * 'AAAB', etc.
     */
     ...
     * disabled. (Used for debugging, so that the tester can dump the log file.
     */
    ...
    /**
     * Remove je log files from the home directory. Will be disabled if the unit
     * test is run with -DremoveLogFiles=false
     * 
     * @param msg prefix to append to error messages
     * @param envFile environment directory
     */
    public static void removeLogFiles(String msg, File envFile, boolean checkRemove) {
     ...
     */
    public static void removeFiles(String msg, File envFile, String suffix) {
     ...
     * @param envFile environment directory
     * @param suffix files with this suffix will be removed
     * @param checkRemove if true, check the -DremoveLogFiles system property
     *            before removing.
     */
    public static void removeFiles(String msg, File envFile, String suffix, boolean checkRemove) {
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            assert done : msg + " directory = " + envFile + " couldn't delete " + names[i] + " out of "
     ...
     * environment home directory. Note that BadFileFilter looks for this
     * pattern: NNNNNNNN.bad.# InfoFileFilter looks for this pattern: je.info.#
     * 
     * @param envFile environment directory
     */
    public static void removeFiles(File envFile, FilenameFilter filter) {
        ...
        for (int i = 0; i < targetFiles.length; i++) {
            ...
            if (!done) {
                System.out.println("Warning, couldn't delete " + targetFiles[i] + " out of "
    ...
    /**
     * Useful utility for generating byte arrays with a known order. Vary the
     * length just to introduce more variability.
     * 
    ...
    /**
     * Return the value of a test data array generated with getTestArray as an
     * int
     */
    ...
    /*
     * Check that the stored memory count for all INs on the inlist matches
     * their computed count. The environment mem usage check may be run with
     * assertions or not. In a multithreaded environment (or one with daemons
     * running), you can't be sure that the cached size will equal the
     * calculated size. Nodes, txns, and locks are all counted within the memory
     * budget.
     */
    public static long validateNodeMemUsage(EnvironmentImpl envImpl, boolean assertOnError) throws DatabaseException {
        ...
        if (assertOnError) {
            assert (nodeTallyUsage == nodeCacheUsage) : "treeNodeTallyUsage=" + formatter.format(nodeTallyUsage)
            ...
            if (DEBUG) {
                if (nodeCacheUsage != nodeTallyUsage) {
                    ...
                    if ((diff / nodeCacheUsage) > .05) {
                        System.out.println("treeNodeTallyUsage=" + formatter.format(nodeTallyUsage)
                                + " treeNodeCacheUsage=" + formatter.format(nodeCacheUsage));
                    }
                }
            }
        ...
        long adminCacheUsage = envImpl.getMemoryBudget().getTreeAdminMemoryUsage();
        if (assertOnError) {
            assert (adminTallyUsage == adminCacheUsage) : "treeAdminTallyUsage=" + formatter.format(adminTallyUsage)
            ...
            if (DEBUG) {
                if (adminCacheUsage != adminTallyUsage) {
                    ...
                    if ((diff / adminCacheUsage) > .05) {
                        System.out.println("treeAdminTallyUsage=" + formatter.format(adminTallyUsage)
                                + " treeAdminCacheUsage=" + formatter.format(adminCacheUsage));
                    }
                }
            }
    ...
    public static long tallyNodeMemUsage(EnvironmentImpl envImpl) throws DatabaseException {
    ...
    private static TreeMemTally tallyTreeMemUsage(EnvironmentImpl envImpl) throws DatabaseException {
        ...
        for (IN in : envImpl.getInMemoryINs()) {
            in.latch();
            try {
                assert in.verifyMemorySize() : "in nodeId=" + in.getNodeId() + ' ' + in.getClass().getName();
    ...
    /**
     * Called by each unit test to enforce isolation level settings specified in
     * the isolationLevel system property. Other system properties or default
     * settings may be applied in the future.
     */
    public static EnvironmentConfig initEnvConfig() {
        ...
        if (val != null && val.length() > 0) {
            if ("serializable".equals(val)) {
                ...
                throw new IllegalArgumentException("Unknown isolationLevel system property value: " + val);
            }
        }
    ...
    /**
     * If a unit test needs to override the isolation level, it should call this
     * method after calling initEnvConfig.
     */
     ...
     */
    public static void loadLog(Class<?> cls, String resourceName, File envHome) throws IOException {
     ...
     */
    public static void loadLog(Class cls, String resourceName, File envHome, String logFileName) throws IOException {
     ...
     */
    public static void logBINAndIN(Environment env, Cursor cursor) throws DatabaseException {

        logBINAndIN(env, cursor, false /* allowDeltas */);
    }

    public static void logBINAndIN(Environment env, Cursor cursor, boolean allowDeltas) throws DatabaseException {
        ...
        SearchResult result = tree.getParentINForChildIN(bin, false, /* useTargetLevel */
        ...
        binParent.updateEntry(result.index, binLsn, VLSN.NULL_VLSN_SEQUENCE, 0 /* lastLoggedSize */);
        ...
        if (binParent.isRoot()) {
            ...
            result = tree.getParentINForChildIN(binParent, false, /* useTargetLevel */
                    true, CacheMode.DEFAULT);
        }
        ...
        if (inParent != null) {
            inParent.recoverIN(result.index, binParent, inLsn, 0 /* lastLoggedSize */);
     ...
     * Logs the given IN.
     */
    public static long logIN(Environment env, IN in, boolean allowDeltas, boolean provisional, IN parent)
            throws DatabaseException {

        in.latch();
        long lsn = in.log(allowDeltas, provisional, false /* backgroundIO */, parent);
        in.releaseLatch();
        return lsn;
    }
     ...
     */
    public static IN getIN(IN in) throws DatabaseException {

        Tree tree = in.getDatabase().getTree();
        in.latch();

        SearchResult result = tree.getParentINForChildIN(in, false, /* useTargetLevel */
                true, CacheMode.DEFAULT);
    ...
    /**
     * Assert if the tree is not this deep. Use to ensure that data setups are
     * as expected.
     */
    public static boolean checkTreeDepth(Database db, int desiredDepth) throws DatabaseException {
    ...
    /**
     * Skip over the JE version number at the start of the exception message for
     * tests which are looking for a specific message.
     */
    ...
    public static void createEnvHomeWithSubDir(File envHome, int subDirNumber) {
        if (!envHome.exists()) {
            throw new IllegalStateException("Environment home directory doesn't exist.");
        }
    ...
    public static String getSubDirName(int i) {
        if (i < 10) {
            ...
            throw new IllegalArgumentException("The number of sub directories is invalid.");
        }
    }
    ...
    public static ArrayList<String> readWriteJEProperties(File envHome, String configure) throws IOException {
     ...
     */
    public static void reWriteJEProperties(File envHome, ArrayList<String> formerLines) throws IOException {
    ...
    public static Object serializeAndReadObject(File envHome, Object object) throws Exception {
        ...
        if (!output.delete()) {
            throw new IllegalStateException(
                    "Can't delete the output for serialized object after " + "testing is done.");
        }
     ...
     */
    public static class StdErrExceptionListener implements ExceptionListener {

        public void exceptionThrown(ExceptionEvent event) {
            System.err.println(Thread.currentThread() + " received " + event);
        }
    }

    /**
     * Calls Closeable.close for each parameter in the order given, if it is
     * non-null. If one or more close methods throws an Exception, all close
     * methods will still be called and the first Exception will be rethrown. If
     * an Error is thrown by a close method, it will be thrown by this method
     * and no further close methods will be called. An IOException may be thrown
     * by a close method because is declared by Closeable.close; however, the
     * use of RuntimeExceptions is recommended.
     */
     ...
     * (with the current cache overhead). When a test precisely sizes the main
     * cache for a particular data set, especially when the data set is small,
     * it is difficult to predict how much to add for the cache overheads. For
     * example, the off-heap cache adds tree-admin overhead for the LRU lists.
     * This method adjusts the cache size to fit a specified data size, given
     * the overheads currently present. When an off-heap cache is used, the
     * initial LRU list (enough to hold 100k entries) will be pre-allocated to
     * ensure this overhead is counted.
     */
    public static long adjustCacheSize(final Environment env, final long dataSize) {
        ...
        final long overhead = mem.getLocalCacheUsage() - mem.getTreeMemoryUsage();
    ...
    public static long adjustSharedCacheSize(final Environment[] envs, final long dataSize) {
        ...
        for (final Environment env : envs) {
            ...
            overhead += mem.getLocalCacheUsage() - mem.getTreeMemoryUsage();
        }
     ...
     * used, the main cache needs to be a little larger to hold the off-heap LRU
     * lists. Note that all unit tests are run with an off-heap cache.
     */
    public static void adjustCacheSizeForOffHeapCache(Environment env) {
        ...
        config.setCacheSize(config.getCacheSize() + OffHeapCache.MIN_MAIN_CACHE_OVERHEAD);

        env.setMutableConfig(config);

        DbInternal.getNonNullEnvImpl(env).getOffHeapCache().preallocateLRUEntries();
    }
     ...
     * className in its command line. The matching of className is simplistic --
     * if it appears anywhere in the program command line, that's considered a
     * match. Using 'jps' rather than 'ps' would improve matching, but 'jps' is
     * not available in the IBM JDK. For testing purposes, the matching used
    ...
    public static int getPid(final String className) throws IOException {
        ...
        final ProcessBuilder pBuilder = new ProcessBuilder("ps", "-e", "o", "pid=,cmd=");
        ...
        for (String line = reader.readLine(); line != null; line = reader.readLine()) {
            ...
            if (line.contains(className)) {
                ...
                if (pid >= 0) {
                    throw new RuntimeException("Running more than once: " + className);
                }
