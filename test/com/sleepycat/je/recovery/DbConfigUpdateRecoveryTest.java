...
public class DbConfigUpdateRecoveryTest extends TestBase {
     ...
     * [#18262]. An update of a database configuration results in the logging of
     * a NameLN, followed by a MapLN. Since MapLNs are always non-transactional,
     * there is nothing that links the NameLN and the MapLN. If the MapLN is not
     * flushed to disk, and the NameLN alone is within the recovery processing
     * part of the log, we expect recovery to succeed, and that the
     * configuration change will not be persisted. This test will do the
     * following things: 1. Open a database and do database config updates. 2.
     * Write a tracer entry right after the updates so that the file is flipped,
     * and we can be sure that there is no checkpoint after the logged MapLN,
     * and that the NameLN will be in the recovery period. 3. Use the
     * LNFileReader to read the updated NameLNLogEntry, and calculate the lsn of
     * the entry right after the NameLNLogEntry. 4. Close the environment
     * without doing a checkpoint. 5. Use DbTruncateLog to truncate log entries
     * after the updated NameLNLogEntry. 6. Open the Environment again to see if
     * the database config has updated.
     */
    private void doTest(boolean transactional) throws Exception {
        ...
        logManager.logForceFlush(tracerEntry, false, ReplicationContext.NO_REPLICATE);

        /* Use FileReader to get the start lsn for the deleted entry. */
        LNFileReader reader = new LNFileReader(envImpl, 1000, DbLsn.NULL_LSN, true, DbLsn.NULL_LSN, DbLsn.NULL_LSN,
                null, DbLsn.NULL_LSN);
        ...
        truncate.truncateLog(envHome, DbLsn.getFileNumber(deleteLsn), DbLsn.getFileOffset(deleteLsn));
