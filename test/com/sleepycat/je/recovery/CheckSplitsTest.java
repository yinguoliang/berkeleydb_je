...
public class CheckSplitsTest extends CheckBase {
     ...
     */
    @Test
    public void testBasicInsert() throws Throwable {
        ...
        testOneCase(DB_NAME, envConfig, dbConfig,
                ...
                }, envConfig, validateDbConfig);

        /*
         * Now run the test in a stepwise loop, truncate after each log entry.
         * We start the steps before the inserts, so the base expected set is
         * empty.
         */
    ...
    @Test
    public void testBasicInsertDups() throws Throwable {

        useDups = true;
        testBasicInsert();
    }

    private void setupBasicInsertData(Database db) throws DatabaseException {
    ...
    /**
     * SR #10715 Splits must propagate up the tree at split time to avoid
     * logging inconsistent versions of ancestor INs.
     */
    @Test
    public void testSplitPropagation() throws Throwable {
        ...
        testOneCase(DB_NAME, envConfig, dbConfig, new TestGenerator(true) {
            void generateData(Database db) throws DatabaseException {

                setupSplitData(db);
            }
        }, restartConfig, new DatabaseConfig());
    ...
    private void setupSplitData(Database db) throws DatabaseException {
    ...
    /**
     * [#13435] Checks that a DIN can be replayed with a full BIN parent. When a
     * DIN is replayed, it may already be present in the parent BIN. Before
     * fixing this bug, we searched without allowing splits and then called
     * IN.insertEntry, which would throw InconsistentNodeException if the BIN
     * was full. We now search with splits allowed, which avoids the exception;
     * however, it causes a split when one is not needed. Note that an alternate
     * fix would be to revert to an earlier version of
     ...
     */
    @Test
    public void testBINSplitDuringDINReplay() throws Throwable {
     ...
     */
    private void setupBINSplitDuringDINReplay(Database db) throws DatabaseException {
        ...
        for (int i = 2; i <= max; i++) {
            IntegerBinding.intToEntry(i, key);
            IntegerBinding.intToEntry(0, data);
            assertEquals(OperationStatus.SUCCESS, cursor.putNoOverwrite(key, data));
        }
     ...
     * the BIN. The sequence is: LN-a (DupCountLN/) DIN (/DBIN/DupCountLN) LN-b
     * DelDupLN-a (/DupCountLN) DelDupLN-b (/DupCountLN) INDupDelete compress
     * LN-c/etc to fill the BIN BIN LN-a and LN-b are dups (same key). After
     * being compressed away, the BIN is filled completely and flushed by the
     * evictor or checkpointer. During recovery, when we replay the DIN and need
     * to insert it into the full BIN, therefore we need to split. Before the
     * bug fix, we did not search with splits allowed, and got an
     * InconsistentNodeException.
     */
    @Test
    public void testBINSplitDuringDeletedDINReplay() throws Throwable {
    ...
    /**
     * Insert two dups, delete them, and compress to free the BIN entry; then
     * fill the BIN with LNs and flush the BIN.
     */
    private void setupBINSplitDuringDeletedDINReplay(Database db) throws DatabaseException {
