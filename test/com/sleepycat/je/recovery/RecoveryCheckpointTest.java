...
public class RecoveryCheckpointTest extends RecoveryTestBase {
     ...
     */
    @Test
    public void testEmptyCheckpoint() throws Throwable {
     ...
     */
    @Test
    public void testNoCheckpointOnOpenSR11861() throws Throwable {
    ...
    public void testBinDelta() throws Throwable {
        ...
        try {
             ...
             * record. The 1 record insertion will qualify for a delta, because
             * the threshold percentage is 25%, and 25% of 4 is 1.
             */
            int numRecs = 4;
            Map<TestData, Set<TestData>> expectedData = new HashMap<TestData, Set<TestData>>();
            ...
            env.checkpoint(forceConfig);
            insertData(txn, numRecs + 1, numRecs + 1, expectedData, 1, true, NUM_DBS);
            txn.commit();
     ...
     */
    @Test
    public void testActiveWhileCheckpointing() throws Throwable {
    ...
    @Test
    public void testSR11293() throws Throwable {
        ...
        /**
         * The sequence between the two tester threads is: tester2: write 1/1
         * into the database. This causes the initial tree to be created
         * (IN/BIN/LN). Flush that out to the disk with a full checkpoint.
         * Signal tester1 and wait. tester1: Lock the MapLN for "foo" db. Signal
         * tester2 and wait. tester2: Add 2/2 to the tree which causes the BIN
         * to be dirtied. Signal tester1 to continue, perform a full checkpoint
         * which will causes the root IN to be dirtied and flushed.
         * DbTree.modifyDbRoot will block on the MapLN lock held by tester1.
         ...
         * the transaction. tester2: modifyDbRoot finally acquires the write
         * lock on foo-db's MapLN write lock, performs the update to the DbTree
         * and returns from the sync().
         */
        JUnitThread tester1 = new JUnitThread("testSR11293DbTreeLocker") {
            @Override
            public void testBody() {
                try {
                    ...
                    DatabaseId fooId = DbInternal.getDbImpl(db).getId();
        ...
        JUnitThread tester2 = new JUnitThread("testSR11293DbWriter") {
            @Override
            public void testBody() {
                try {
        ...
        recoveryConfig.setConfigParam(EnvironmentParams.ENV_RUN_CHECKPOINTER.getName(), "false");
     ...
     */
    @Test
    public void testReadOnlyCheckpoint() throws DatabaseException {
