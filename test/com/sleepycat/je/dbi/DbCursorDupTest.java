...
public class DbCursorDupTest extends DbCursorTestBase {
    ...
    public void testCursorDupAndCloseDb() throws DatabaseException {
        ...
        OperationStatus status = cursor.getNext(new DatabaseEntry(), new DatabaseEntry(), LockMode.DEFAULT);
    ...
    public void testDupInitialized() throws DatabaseException {
        ...
        assertEquals(OperationStatus.SUCCESS, c1.put(new DatabaseEntry(new byte[0]), new DatabaseEntry(new byte[0])));
        assertEquals(OperationStatus.SUCCESS, c2.getFirst(new DatabaseEntry(), new DatabaseEntry(), null));
        assertEquals(OperationStatus.NOTFOUND, c2.getNext(new DatabaseEntry(), new DatabaseEntry(), null));
        assertEquals(OperationStatus.SUCCESS, c3.getFirst(new DatabaseEntry(), new DatabaseEntry(), null));
        assertEquals(OperationStatus.NOTFOUND, c3.getNext(new DatabaseEntry(), new DatabaseEntry(), null));
    ...
    /**
     * Create some duplicate data. Pass 1, walk over the data and with each
     * iteration, dup() the cursor at the same position. Ensure that the dup
     * points to the same key/data pair. Advance the dup'd cursor and ensure
     * that the data is different (key may be the same since it's a duplicate
     * set). Then dup() the cursor without maintaining position. Ensure that
     * getCurrent() throws a Cursor Not Init'd exception. Pass 2, iterate
     * through the data, and dup the cursor in the same position. Advance the
     * original cursor and ensure that the dup()'d points to the original data
     * and the original cursor points at new data.
     */
    @Test
    public void testCursorDupSamePosition() throws DatabaseException {
        ...
        DataWalker dw = new DataWalker(null) {
            void perData(String foundKey, String foundData) throws DatabaseException {
                ...
                if (cursor2.getNext(keyDbt, dataDbt, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
                    /* Keys can be the same because we have duplicates. */
                    /*
                     * assertFalse(StringUtils.fromUTF8(keyDbt.getData()).
                     * equals(foundKey));
                     */
                    assertFalse(StringUtils.fromUTF8(dataDbt.getData()).equals(foundData));
                }
                cursor2.close();
                try {
        ...
        dw = new DataWalker(null) {
            void perData(String foundKey, String foundData) throws DatabaseException {
                ...
                OperationStatus status = cursor.getNext(keyDbt, dataDbt, LockMode.DEFAULT);
                ...
                if (status == OperationStatus.SUCCESS) {
                    assertFalse(StringUtils.fromUTF8(dataDbt.getData()).equals(foundData));
                    assertFalse(StringUtils.fromUTF8(dataDbt.getData()).equals(c2Data));
                }
                cursor2.close();
            }
        };
