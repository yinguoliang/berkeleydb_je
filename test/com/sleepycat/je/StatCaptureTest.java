...
public class StatCaptureTest extends TestBase {
    ...
    public void testCustomStats() throws Exception {
        ...
        envConfig.setConfigParam(EnvironmentParams.STATS_COLLECT_INTERVAL.getName(), "10 s");
        ...
        File statcsv = new File(envHome.getAbsolutePath() + File.separator + "je.stat.csv");
    ...
    public void testStatsCapture() throws Exception {
        ...
        File envStatFile = new File(envHome.getAbsolutePath() + File.separator + EnvStatsLogger.STATFILENAME + "."
        ...
        env.close();
        env = null;
        assertEquals("Number of rows in env stat file not expected.", getRowCount(envStatFile), 2);

        /* Try to open and close again, now that the environment exists */
        envConfig.setAllowCreate(false);
        //        envConfig.setTxnNoSync(true);
        envConfig.setConfigParam(EnvironmentParams.MAX_MEMORY.getName(), "100000");
        env = new Environment(envHome, envConfig);
        assertEquals("Number of rows in env stat file not expected.", getRowCount(envStatFile), 3);
        ...
        for (int i = 0; i < DATACOUNT; i++) {
            ...
            OperationStatus os = db.get(null, new DatabaseEntry(key), value, LockMode.DEFAULT);
        ...
        assertSame(corsair.getPrev(key, value, null), OperationStatus.SUCCESS);
        assertSame(corsair.getFirst(key, value, null), OperationStatus.SUCCESS);
        corsair.close();
        corsair = null;
        ...
        assertEquals(Long.valueOf(envCreationTime), values.get("Environment:environmentCreationTime"));
    ...
    public void testChangeStatConfig() throws Exception {
        ...
        currentStatFile = new File(
                envHome.getAbsolutePath() + File.separator + StatCapture.STATFILENAME + "." + StatCapture.STATFILEEXT);
        currentConfigStatFile = new File(envHome.getAbsolutePath() + File.separator + EnvStatsLogger.STATFILENAME + "."
                + EnvStatsLogger.STATFILEEXT);

        /* Init the Environment. */
        EnvironmentConfig envConfig = TestUtils.initEnvConfig();
        envConfig.setConfigParam(EnvironmentParams.CHECKPOINTER_BYTES_INTERVAL.getName(), "1000000000");
        envConfig.setTransactional(true);
        envConfig.setAllowCreate(true);
        ...
        try {
            ...
            assertSame("Number of stat files was expected to be zero.", filecount, 0);

            /* Try to open and close again, now that the environment exists */
            envConfig.setAllowCreate(false);

            envConfig.setConfigParam(EnvironmentParams.STATS_COLLECT_INTERVAL.getName(), "1 s");
            env = new Environment(envHome, envConfig);

            /* change config param to collect stats */
            mc = env.getMutableConfig();
            mc.setConfigParam(EnvironmentParams.STATS_COLLECT.getName(), "true");
            ...
            for (int i = 0; i < DATACOUNT; i++) {
                ...
                if (i == 0) {
                    ...
                    Thread.sleep(1000);
                    mc = env.getMutableConfig();
                    mc.setConfigParam(EnvironmentParams.STATS_COLLECT_INTERVAL.getName(), "10 s");
                    env.setMutableConfig(mc);
                    Thread.sleep(5000);
            ...
            rowcount = getRowCount(currentStatFile);
            assertSame("Expected number of rows in stat file is " + "incorrect expected " + (DATACOUNT / 10) + " have "
                    + (rowcount - prevrowcount), (rowcount - prevrowcount), (DATACOUNT / 10));

            /* turn stat capture off, then on to insure that works */
            mc = env.getMutableConfig();
            mc.setConfigParam(EnvironmentParams.STATS_COLLECT.getName(), "false");
            env.setMutableConfig(mc);
            ...
            mc = env.getMutableConfig();
            mc.setConfigParam(EnvironmentParams.STATS_COLLECT_INTERVAL.getName(), "1 s");
            mc.setConfigParam(EnvironmentParams.STATS_FILE_ROW_COUNT.getName(), Integer.toString(DATACOUNT / 4));
            env.setMutableConfig(mc);
            ...
            assertTrue("Number of stat files did not increase " + filecount + " have " + envHome.listFiles(ff).length,
                    filecount < envHome.listFiles(ff).length);

            // check the environment config log
            rowcount = getRowCount(currentConfigStatFile);
            assertEquals("Number of rows in " + currentConfigStatFile.getAbsolutePath() + "not expected.", 6, rowcount);
            db.close();
            db = null;
            env.close();
            env = null;
            ...
            envConfig.setConfigParam(EnvironmentParams.CHECKPOINTER_BYTES_INTERVAL.getName(), "4000000");
            envConfig.setConfigParam(EnvironmentParams.STATS_COLLECT.getName(), "true");
            env = new Environment(envHome, envConfig);

            // check the environment config log
            rowcount = getRowCount(currentConfigStatFile);
            assertEquals("Number of rows in " + currentConfigStatFile.getAbsolutePath() + "not expected.", 7, rowcount);
        } finally {
            if (db != null) {
                db.close();
            }
        ...
        Map<String, Long> values = StatFile.sumItUp(envHome, StatCapture.STATFILENAME);
    ...
    /**
     * Statistics Capture test that moves the statistics file while stat capture
     * is ongoing.
     */
    @Test
    public void testFileErrors() throws Exception {
        ...
        File statFileDir = new File(envHome.getAbsolutePath() + File.separator + "statsdir");
        jeLogFile = new File(envHome.getAbsolutePath() + File.separator + "je.info.0");
        ...
        envConfig.setConfigParam(EnvironmentParams.STATS_COLLECT.getName(), "true");
        envConfig.setConfigParam(EnvironmentParams.STATS_COLLECT_INTERVAL.getName(), "1 s");
        envConfig.setConfigParam(EnvironmentParams.STATS_FILE_DIRECTORY.getName(), statFileDir.getAbsolutePath());
        Environment env = null;
        Database db = null;
        try {
            env = new Environment(envHome, envConfig);
            ...
            for (int i = 0; i < DATACOUNT; i++) {
                ...
                /* wait a second */
                Thread.sleep(1000);
                statFileDir.renameTo(new File(statFileDir.getAbsolutePath() + "1"));
            }
        } finally {
            if (db != null) {
                db.close();
        ...
        assertEquals("Number of rows in env stat file not expected.", getRowCount(jeLogFile), 1);
    }

    /**
     * Test to check if added statistics are projected to the stat file. A
     * manual change to the StatCaptureDefinitions class is required if a new
     * statistic is added and is to be projected.
     */
    @Test
    public void testForMissingStats() throws Exception {
        ...
        try {
            ...
            stats = StatFile.getNameValueMap(env.getStats(fastconfig).getStatGroups());
            for (Map.Entry<String, Stat<?>> entry : stats.entrySet()) {
                String name = entry.getKey();
                assertTrue("Statistic " + name + " returned but not captured.", sd.getDefinition(name) != null);

                if (entry.getValue() instanceof LongMinStat) {
                    assertTrue("Statistic " + name + " returned but not defined as a minStat.",
                            findDef(name, StatCaptureDefinitions.minStats));
                } else if (entry.getValue() instanceof LongMaxStat) {
                    assertTrue("Statistic " + name + " returned but not defined as a maxStat.",
    ...
    public void testRepEnvStats() throws Exception {
        ...
        Database db = null;
        try {
            EnvironmentConfig envConfig = RepTestUtils
                    .createEnvConfig(new Durability(Durability.SyncPolicy.WRITE_NO_SYNC,
            ...
            assertTrue("value not expected ", rs.getReplayMaxCommitProcessingNanos() > 0);
            final long replayTotalCommitLagMs = rs.getReplayTotalCommitLagMs();
            assertTrue("replayTotalCommitLagMs should be greater than zero," + " was " + replayTotalCommitLagMs,
        ...
        assertTrue("BinaryProtocol:nMessagesWritten value not " + "greater than zero.", writtenMessages > 0);

        /* Check slave statistics. */
        statcsv = new File(repEnvHome[1].getAbsolutePath() + File.separator + "je.stat.csv");
        values = StatFile.sumItUp(statcsv);
        Long readMessages = values.get("BinaryProtocol:nMessagesRead");
        assertTrue("BinaryProtocol:nMessagesRead value not greater than zero.", readMessages > 0);
    }
    ...
    public void testNoJoinRepEnvStats() throws Exception {
        ...
        RepEnvInfo[] repEnvInfo = null;
        try {
            File currentFile = new File(envHome.getAbsolutePath() + File.separator + EnvStatsLogger.STATFILENAME + "."
                    + EnvStatsLogger.STATFILEEXT);
            currentFile.delete();

            repEnvInfo = RepTestUtils.setupEnvInfos(envHome, 2);
            ReplicatedEnvironment master = new ReplicatedEnvironment(envHome, repEnvInfo[0].getRepConfig(),
    ...
    public void testMissingRepEnvStats() throws Exception {
        ...
        try {
            ...
            stats = StatFile.getNameValueMap(master.getRepStats(fastconfig).getStatGroups());

            for (Map.Entry<String, Stat<?>> entry : stats.entrySet()) {
                String name = entry.getKey();

                assertTrue("Statistic " + name + " returned but not captured from master.",
                        rsd.getDefinition(name) != null);

                if (entry.getValue() instanceof LongMinStat) {
                    assertTrue("Statistic " + name + "returned but not defined as a minStat.",
                            findDef(name, StatCaptureRepDefinitions.minStats));
                } else if (entry.getValue() instanceof LongMaxStat) {
                    assertTrue("Statistic " + name + "returned but not defined as a maxStat.",
                            findDef(name, StatCaptureRepDefinitions.maxStats));
                }
            }

            ReplicatedEnvironment replica = repEnvInfo[1].getEnv();
            stats = StatFile.getNameValueMap(replica.getRepStats(fastconfig).getStatGroups());

            for (Map.Entry<String, Stat<?>> entry : stats.entrySet()) {
                String name = entry.getKey();

                assertTrue("Statistic " + name + " returned but not captured from replica.",
                        rsd.getDefinition(name) != null);
    ...
    private void verify(Map<String, Long> m1, String m1desc, Map<String, Long> m2, String m2desc) {

        final int ERROR_PERCENT = 5;
        String[] statsToVerify = { "Cache:adminBytes", "Cache:dataBytes", "Cache:nBINsFetch", "Cache:nBINsFetchMiss",
                "Cache:nCachedBINs", "Cache:nINCompactKey", "Cache:nINNoTarget", "Cache:nINSparseTarget",
                "Cache:nLNsFetch", "Cache:nLNsFetchMiss", "Cache:nUpperINsFetch", "Environment:btreeRelatchesRequired",
                "Environment:environmentCreationTime", "I/O:bufferBytes", "I/O:nCacheMiss", "I/O:nFSyncRequests",
                "I/O:nFSyncs", "I/O:nLogBuffers", "I/O:nLogFSyncs", "I/O:nNotResident", "I/O:nRandomReadBytes",
                "I/O:nRandomReads", "I/O:nSequentialReadBytes", "I/O:nSequentialReads", "I/O:nSequentialWriteBytes",
                "I/O:nSequentialWrites", "Op:priSearch", "Op:priInsert", };
        ...
        for (String statname : statsToVerify) {
            ...
            assertTrue(statname + " " + m1desc + " [" + v1 + "] not equal to " + m2desc + " [" + v2 + "]",
                    (av1 - (av1 * ERROR_PERCENT * .01) <= av2) && (av2 <= (av1 + (av1 * ERROR_PERCENT * .01))));
        }
    }
    ...
    private boolean findDef(String name, StatManager.SDef[] definitions) {
        ...
        for (StatManager.SDef def : definitions) {
            if (namePart[0].equals(def.getGroupName()) && namePart[1].equals(def.getDefinition().getName())) {
                return true;
            }
        }
        return false;
    ...
    private class Custom implements CustomStats {
        ...
        public String[] getFieldValues() {
            ...
            if (putCount.get() != 0) {
                ...
                retval[0] = Long.valueOf(val).toString();
            } else {
                retval[0] = "0";
            }
            return retval;
