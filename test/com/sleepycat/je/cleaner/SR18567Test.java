...
/**
 * Test concurrent checkpoint and Database.sync operations. Prior to the fix for
 * [#18567], checkpoint and Database.sync both acquired a shared latch on the
 * parent IN while logging a child IN. During concurrent logging of provisional
 * child BINs for same parent IN, problems can occur when the utilization info
 * in the parent IN is being updated by both threads. For example, the following
 * calls could be concurrent in the two threads for the same parent IN.
 * BIN.afterLog (two different BINs) |--IN.trackProvisionalObsolete (same
 * parent) |-- PackedObsoleteInfo.copyObsoleteInfo (array copy) Concurrent array
 * copies could corrupt the utilization info, and this could result in the
 * exception reported by the OTN user later on, when the parent IN is logged and
 * the obsolete info is copied to the UtilizationTracker:
 * java.lang.IndexOutOfBoundsException at
 * com.sleepycat.bind.tuple.TupleInput.readBoolean(TupleInput.java:186) at
 * com.sleepycat.je.cleaner.PackedObsoleteInfo.countObsoleteInfo(
 * PackedObsoleteInfo.java:60) at
 * com.sleepycat.je.log.LogManager.serialLogInternal(LogManager.java:671) at
 * com.sleepycat.je.log.SyncedLogManager.serialLog(SyncedLogManager.java:40) at
 * com.sleepycat.je.log.LogManager.multiLog(LogManager.java:388) at
 * com.sleepycat.je.recovery.Checkpointer.logSiblings(Checkpointer.java:1285)
 ...
 * Since both threads are modifying the parent IN, it seems obvious that using a
 * shared latch could cause more than one type of problem and should be
 * disallowed.
 */
...
public class SR18567Test extends CleanerTestBase {
    ...
    @Test
    public void testSR18567() throws Throwable {
