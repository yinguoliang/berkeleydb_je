...
/**
 * Test that searches and cursor traversals execute correctly in the face of a
 * BIN with 0 entries, and with tree pruning at key points.
 */
...
public class EmptyBINTest extends TestBase {
    ...
    /*
     * Run all tests in four combinations, using dups, and invoking bin pruning.
     */
    @Parameters
    ...
    public EmptyBINTest(boolean dup, boolean pruneCursor, boolean pruneTree) {
        ...
        if (DEBUG) {
            System.out.println("useDups=" + useDups + " doPruningAtCursorLevel=" + doPruningAtCursorLevel
                    + " doPruningAtTreeLevel=" + doPruningAtTreeLevel);
        }
        this.envHome = SharedTestUtils.getTestDir();
        customName = (useDups ? "DUPS" : "!DUPS") + "_" + (doPruningAtCursorLevel ? "CURSORPRUNE" : "!CURSORPRUNE")
                + "_" + (doPruningAtTreeLevel ? "TREEPRUNE" : "!TREEPRUNE");
    }
    ...
    @Test
    public void testScanFromEndOfFirstBin() throws DatabaseException {

        /*
         * Tree holds <0,1> <2,3,4> <empty> <8,9,10>. | fwd scan starts --- -+
    ...
    @Test
    public void testScanFromLeftSideOfEmptyBin() throws DatabaseException {

        /*
         * Tree holds <0,1> <2,3,4> <empty> <8,9,10>. | scan starts
    ...
    @Test
    public void testScanFromRightSideOfEmptyBin() throws DatabaseException {

        /*
         * Tree holds <0,1> <2,3,4> <empty> <8,9,10>. | backwards scan starts
    ...
    @Test
    public void testScanFromBeginningOfLastBin() throws DatabaseException {
    ...
    @Test
    public void testScanForward() throws DatabaseException {

        /*
         * Tree holds <0,1> <2,3,4> <empty> <8,9,10>. Fwd scan starting with
         * first. Expect 0, 1, 2, 4, 8, 9, 10.
         */
        doScanAcrossEmptyBin(true, // forward
                (byte) -1, new byte[] { 0, 1, 2, 3, 4, 8, 9, 10 });
    }

    @Test
    public void testScanBackwards() throws DatabaseException {

        /*
         * Tree holds <0,1> <2,3,4> <empty> <8,9,10>. Bwd scan starting with
         * last. 10 -> 0
         */
        doScanAcrossEmptyBin(false, // backwards
                (byte) -1, new byte[] { 10, 9, 8, 4, 3, 2, 1, 0 });
    }

    /**
     * Scan over an empty BIN that is in the middle of the tree. [#11778] The
     * tree holds values from 0 - 10. Values 5, 6, 7 have been deleted.
     * 
     ...
     */
    private void doScanAcrossEmptyBin(boolean forward, byte startKey, byte[] expectVals) throws DatabaseException {
        ...
        if (doPruningAtTreeLevel) {
            DbInternal.getDbImpl(db).getTree().setSearchHook(new PruningHook(env));
        }
        ...
        if (startKey < 0) {
            ...
            if ((startKey >= deleteStartVal) && (startKey <= deleteEndVal)) {
                /* Test range query. */
                if (useDups) {
        ...
        do {
            ...
            /* check value. */
            if (DEBUG) {
                System.out.println("=>key=" + key.getData()[0] + " data=" + data.getData()[0]);
            }
            if (useDups) {
    ...
    /**
     * Create a tree with: IN / \ IN IN / \ / \ BIN1 BIN2 BIN3 BIN4 where BIN1
     * has values 0,1 BIN2 has valus 2,3,4 BIN3 has valus 5,6,7 BIN4 has valus
     * 8,9,10 Depending on configuration, the entries in BIN2 or BIN3
     */
    private void openAndInitEmptyMiddleBIN(int deleteStartVal, int deleteEndVal) throws DatabaseException {
     ...
     */
    private void openEnv(boolean transactional, String nodeMax) throws DatabaseException {

        EnvironmentConfig envConfig = TestUtils.initEnvConfig();
        envConfig.setTransactional(transactional);
        envConfig.setConfigParam(EnvironmentParams.ENV_RUN_INCOMPRESSOR.getName(), "true");
        if (nodeMax != null) {
            envConfig.setConfigParam(EnvironmentParams.NODE_MAX.getName(), nodeMax);
            envConfig.setConfigParam(EnvironmentParams.NODE_MAX_DUPTREE.getName(), nodeMax);
        }
    ...
    private static class PruningHook implements TestHook {
        ...
        public void doHook() {
            DbInternal.getNonNullEnvImpl(env).getINCompressor().wakeup();
            Thread.yield();
            try {
                Thread.sleep(100);
            } catch (Throwable T) {
