 ...
 * <p>
 * A single TransactionRunner instance may be used by any number of threads for
 ...
 * before calling doWork(). If LockConflictException is thrown by doWork(), the
 * transaction will be aborted and the process will be repeated up to the
 * maximum number of retries. If another exception is thrown by doWork() or the
 * maximum number of retries has occurred, the transaction will be aborted and
 * the exception will be rethrown by the run() method. If no exception is thrown
 * by doWork(), the transaction will be committed. The run() method will not
 * attempt to commit or abort a transaction if it has already been committed or
 * aborted by doWork().</li>
 ...
 * started before calling doWork(). The transaction that is active when calling
 * the run() method will become the parent of the nested transaction. The nested
 * transaction will be committed or aborted by the run() method following the
 * same rules described above. Note that nested transactions may not be enabled
 * for the JE product, since JE does not support nested transactions.</li>
 ...
 * <li>When the run() method is called and a transaction is active for the
 * current thread and nested transactions are not enabled (the default) the same
 * rules as above apply. All the operations performed by the doWork() method
 * will be part of the currently active transaction.</li>
 * </ul>
 * <p>
...
public class TransactionRunner {
    ...
    /**
     * Creates a transaction runner for a given Berkeley DB environment. The
     * default maximum number of retries ({@link #DEFAULT_MAX_RETRIES}) and a
     * null (default) {@link TransactionConfig} will be used.
     *
    ...
    /**
     * Creates a transaction runner for a given Berkeley DB environment and with
     * a given number of maximum retries.
     *
     * @param env is the environment for running transactions.
     ...
     *            {@link Environment#beginTransaction}, or null to use the
     *            default configuration. The configuration object is not cloned,
     *            and any modifications to it will impact subsequent
     *            transactions.
     */
     ...
     * deadlocks are detected. Calling this method does not impact transactions
     * already running.
     *
    ...
    /**
     * Returns whether nested transactions will be created if <code>run()</code>
     * is called when a transaction is already active for the current thread. By
     * default this property is false.
     *
     * @return whether nested transactions will be created.
     *         <p>
     *         Note that this method always returns false in the JE product,
    ...
    /**
     * Changes whether nested transactions will be created if <code>run()</code>
     ...
     *            <p>
     *            Note that true may not be passed to this method in the JE
     *            product, since nested transactions are not supported by JE.
     *            </p>
     */
     ...
     *             maximum number of retries has occurred. The transaction will
     *             have been aborted by this method.
     * @throws Exception when any other exception is thrown by doWork(). The
     *             exception will first be unwrapped by calling
     *             {@link ExceptionUnwrapper#unwrap}. The transaction will have
     *             been aborted by this method.
     */
                ...
                try {
                    ...
                    if (txn != null && txn == currentTxn.getTransaction()) {
                        try {
                            ...
                            /*
                             * We print this stack trace so that the information
                             * is not lost when we throw the original exception.
                             */
     ...
     * transaction retry policy. The transaction is aborted by the {@link #run
     * run} method before calling this method.
     * <p>
     * The default implementation of this method throws the {@code
     * exception} parameter if it is not an instance of
     * {@link LockConflictException} and otherwise returns the
     * {@code maxRetries} parameter value. This method can be overridden to
     * throw a different exception or return a different number of retries. For
     * example:
     * </p>
     * <ul>
     ...
     * {@code retries} value is greater or equal to the {@code maxRetries} and a
     * {@link LockConflictException} occurs, to override the default behavior
     * which is to throw the {@link LockConflictException}.</li>
     * </ul>
     *
     * @param exception an exception that was thrown by the
     *            {@link TransactionWorker#doWork} method or thrown when
     *            beginning or committing the transaction. If the
     *            {@code retries} value is greater or equal to
     *            {@code maxRetries} when this method returns normally, this
     *            exception will be thrown by the {@link #run run} method.
     * @param retries the current value of a counter that starts out at zero and
     *            is incremented when each retry is performed.
     * @param maxRetries the maximum retries to be performed. By default, this
     *            value is set to {@link #getMaxRetries}. This method may return
     *            a different maximum retries value to override that default.
     * @return the maximum number of retries to perform. The default policy is
     *         to return the {@code maxRetries} parameter value if the
     *         {@code exception} parameter value is an instance of
     *         {@link LockConflictException}.
     * @throws Exception to cause the exception to be thrown by the {@link #run
     *             run} method. The default policy is to throw the
     *             {@code exception} parameter value if it is not an instance of
     *             {@link LockConflictException}.
     * @since 3.4
     */
