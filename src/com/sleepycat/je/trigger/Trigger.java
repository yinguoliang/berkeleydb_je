 ...
 * manage the lifecycle of the database itself, as well as the record operations
 * used to modify the contents of the database.
 * <p>
 * WARNING: Only transient triggers are currently supported, and the documention
 * below has not yet been updated to reflect this fact. The bottom line is that
 * triggers are currently only useful and known to be reliable for maintaining a
 * cache of database information on a replica, where the cache is initialized
 * after opening the database (and configuring the trigger), and where only the
 ...
 * <li>Triggers must be configured on each node in a rep group separately, when
 * needed. Specifically, a trigger configured on a master will not be
 * automatically configured and invoked on the replicas in the group.</li>
 * <li>Because only transient triggers are currently supported, trigger methods
 * are only called after opening a database (when configuring the trigger in the
 * DatabaseConfig), and are not called after closing the database.</li>
 * <li>As a result of the above point, triggers are not called during recovery,
 * and therefore cannot be reliably used to perform write operations using the
 * tranaction passed to the trigger method.</li>
 ...
 * transaction. If the exception is thrown during the replay of a transaction on
 * a replica in an HA application, the environment is invalidated and a new
 * environment handle must be created.
 * </p>
 * <p>
 * A Trigger is associated with a database via {@link DatabaseConfig#setTriggers
...
public interface Trigger {
    ...
    /**
     * Returns the name associated with the trigger. All the triggers associated
     * with a particular database must have unique names.
     *
     ...
     * invocation of the {@link Persistent#remove} remove trigger. If the
     ...
     * non-null. If an existing entry was updated, oldData and newData will be
     * non-null.
     * </p>
     *
     * @param txn the active transaction associated with the operation. The
     ...
     * @param oldData the data before the change, or null if the record did not
     *            exist.
