 ...
 *
 * <p>
 * There are two groups of database attributes: per-database handle attributes,
 * and database-wide attributes. An attribute may be persistent/transient or
 * mutable/immutable:
 * </p>
 *
 * <table border="true">
 ...
 * <!-- {@link DatabaseConfig#getTriggers() triggers}<br>
 * </td> -->
 * </tr>
 * <tr>
 * <td>True</td>
 ...
 * {@link DatabaseConfig#getTransactional() transactional}<br>
 * </td>
 * </tr>
 * <tr>
 * <td>False</td>
 * <td>True</td>
 * <td>{@link DatabaseConfig#getSortedDuplicates() sorted duplicates}<br>
 * </td>
 * </tr>
 * <tr>
 * <td>False</td>
 ...
 *
 * <p>
 * Persistent attributes will be saved in the log and remain in effect every
 * time the environment is reopened. Transient attributes only remain in effect
 * until:
 * </p>
 *
 * <ul>
...
public class DatabaseConfig implements Cloneable {
	 ...
	 *
	 * @param allowCreate
	 *            If true, configure the
	 *            {@link com.sleepycat.je.Environment#openDatabase
	 *            Environment.openDatabase} method to create the database if it
	 ...
	 * Environment.openDatabase} method is configured to create the database if
	 ...
	 *         Environment.openDatabase} method is configured to create the
	 ...
	 *            If true, configure the
	 *            {@link com.sleepycat.je.Environment#openDatabase
	 *            Environment.openDatabase} method to fail if the database
	 *            already exists.
	 *
	 * @return this
	 */
	 ...
	 *
	 * @return true if the {@link com.sleepycat.je.Environment#openDatabase
	 *         Environment.openDatabase} method is configured to fail if the
	 ...
	 *
	 * <p>
	 * When duplicate keys are configured for a database, key prefixing is also
	 * implicitly configured. Without key prefixing, databases with duplicates
	 * would store keys inefficiently. Key prefixing is therefore mandatory for
	 * databases with duplicates.
	 * </p>
	 * 
	 * <p>
	 * Although two records may have the same key, they may not also have the
	 * same data item. Two identical records, that have the same key and data,
	 * may not be stored in a database.
	 * </p>
	 *
	 * <p>
	 * The ordering of duplicates in the database is determined by the duplicate
	 * comparison function. See {@link #setDuplicateComparator}. If the
	 * application does not specify a duplicate comparison function, a default
	 * lexical comparison will be used.
	 * </p>
	 *
	 * <p>
	 ...
	 *            If true, configure the database to support duplicate data
	 *            items. A value of false is illegal to this method, that is,
	 *            once set, the configuration cannot be cleared.
	 *
	 * @return this
	 */
	...
	/**
	 * Configure the database to support key prefixing. Key prefixing causes the
	 * representation of keys in the b-tree internal nodes to be split between
	 * the common prefix of all keys and the suffixes. Using this may result in
	 * a more space-efficient representation in both the in-memory and on-disk
	 * formats, but at some possible performance cost.
	 *
	 * <p>
	 * When duplicate keys are configured for a database, key prefixing is also
	 * implicitly configured. Without key prefixing, databases with duplicates
	 ...
	 *
	 * @param keyPrefixing
	 *            If true, enables keyPrefixing for the database.
	 *
	 * @return this
	 *
	 * @throws IllegalStateException
	 *             if the keyPrefixing argument is false and
	 *             {@link #setSortedDuplicates} has been called to configure
	 *             duplicates. Key prefixing is therefore mandatory for
	 *             databases with duplicates.
	 * 
	 */
	 ...
	 *
	 * <p>
	 * If the call succeeds, the open operation will be recoverable. If the call
	 * fails, no database will have been created.
	 * </p>
	 *
	 * <p>
	 * All future operations on this database, which are not explicitly enclosed
	 * in a transaction by the application, will be enclosed in in a transaction
	 * within the library.
	 * </p>
	 *
	 * @param transactional
	 ...
	 * </p>
	 *
	 * @param nodeMaxEntries
	 *            The maximum children per B+Tree node.
	 *
	 * @return this
	 */
	 ...
	 *             {@link #setNodeMaxEntries} should be used instead.
	 */
	 ...
	 *
	 * <p>
	 * If a comparator needs to be initialized before it is used or needs access
	 * to the environment's ClassLoader property, it may implement the
	 * {@link DatabaseComparator} interface.
	 * </p>
	 *
	 * <p>
	 * The Comparator.compare() method is passed the byte arrays that are stored
	 * in the database. If you know how your data is organized in the byte
	 * array, then you can write a comparison routine that directly examines the
	 * contents of the arrays. Otherwise, you have to reconstruct your original
	 * objects, and then perform the comparison. See the <a href=
	 * "{@docRoot}/../GettingStartedGuide/comparator.html" target="_top">Getting
	 * Started Guide</a> for examples.
	 * </p>
	 *
	 * <p>
	 * <em>WARNING:</em> There are several special considerations that must be
	 * taken into account when implementing a comparator.
	 * <p>
	 * <ul>
	 ...
	 * synchronization.</li>
	 *
	 * <li>Because records are stored in the order determined by the Comparator,
	 * the Comparator's behavior must not change over time and therefore should
	 * not be dependent on any state that may change over time. In addition,
	 * although it is possible to change the comparator for an existing
	 * database, care must be taken that the new comparator provides compatible
	 * results with the previous comparator, or database corruption will occur.
	 * </li>
	 *
	 * <li>JE uses comparators internally in a wide variety of circumstances, so
	 * custom comparators must be sure to return valid values for any two
	 * arbitrary keys. The user must not make any assumptions about the range of
	 * key values that might be compared. For example, it's possible for the
	 * comparator may be used against previously deleted values.</li>
	 * </ul>
	 *
	 * <p>
	 * A special type of comparator is a <em>partial comparator</em>, which
	 ...
	 * fields but uniquely identify each record with a single field. The partial
	 * comparator could then compare only the single identifying field, allowing
	 * the rest of the fields to be updated. A query ({@link Cursor#getSearchKey
	 * Cursor.getSearchKey}, for example) could then be performed by passing a
	 * partial key that contains only the identifying field.
	 *
	 * <p>
	 * <strong>WARNING:</strong> To allow for key updates in situations like
	 * those described above, all partial comparators must implement the
	 * {@link PartialComparator} tag interface. Otherwise, BDB JE will raise an
	 * exception if an attempt is made to update a key in a database whose
	 ...
	 * comparator, which considers two keys to be equal if and only if they have
	 * the same length and they are equal byte-per-byte. All binary equality
	 ...
	 * optimization, described in {@link BinaryEqualityComparator}
	 * <p>
	 ...
	 * JE will store the class name and instantiate the Comparator by class name
	 * (using <code>Class.forName</code> and <code>newInstance</code>) when
	 * subsequently opening the database. Because the Comparator is instantiated
	 * using its default constructor, it should not be dependent on other
	 ...
	 * <strong>WARNING:</strong> To allow for key updates in situations like
	 * those described above, all partial comparators must implement the
	 * {@link PartialComparator} tag interface. See "Upgrading from JE 5.0 or
	 * earlier" in the change log and the {@link PartialComparator} javadoc for
	 * more information.
	 * </p>
	 *
	 * Another special type of comparator is a <em>binary equality</em>
	...
	/**
	 * Sets to true if the database exists and the btree comparator specified in
	 * this configuration object should override the current comparator.
	 *
	 * @param override
	 *            Set to true to override the existing comparator.
	 *
	 * @return this
	 */
	...
	/**
	 * By default, a byte by byte lexicographic comparison is used for duplicate
	 * data items in a duplicate set. To customize the comparison, supply a
	 ...
	 *
	 * <p>
	 * When using a partial duplicates comparator, it is possible to update the
	 * data for a duplicate record, as long as only the non-identifying fields
	 * in the data are changed. See {@link Cursor#putCurrent Cursor.putCurrent}
	 ...
	 *
	 *         Specifies the list of triggers associated with the database;
	 *         triggers are executed in the order specified by this list.
	 *         <p>
	 *         This configuration parameter is only meaningful when configuring
	 *         a <code>Primary</code> database.
	 *         <p>
	 *         The type of the trigger specified in the list must match the type
	 *         of database being configured. For example, the trigger object
	 *         must implement the <code>ReplicatedDatabaseTrigger</code>
	 *         interface if it's used to configure a replicated database.
	 *         <p>
	 *         Some of the incorrect uses of this parameter are detected during
	 *         calls to {@link Environment#openDatabase
	 *         Environment.openDatabase} or
	 *         {@link Environment#openSecondaryDatabase
	 *         Environment.openSecondaryDatabase} and will result in an
	 *         <code>IllegalArgumentException</code>.
	 *
	 * @param triggers
	 *            the list of database triggers to be associated with the
	 *            environment.
	 *
	 * @throws IllegalArgumentException
	 *             If the triggers in the list do not have unique names, have
	 *             conflicting types (e.g. only a subset implement
	 *             {@link ReplicatedDatabaseTrigger ReplicatedDatabaseTrigger}),
	 *             or do not implement {@link ReplicatedDatabaseTrigger
	 ...
	 * @hidden For internal use only.
	 *
	 *         Set to true if the database exists and the
	 *         {@link PersistentTrigger}s in the trigger list specified in this
	 *         configuration object should override those in the current list of
	 *         triggers. Note that any transient triggers that are specified are
	 *         always configured, because they do not override existing
	 *         triggers.
	 *
	 * @param override
	 *            Set to true to override the existing comparator.
	 ...
	 *
	 * <p>
	 * Temporary databases operate internally in deferred-write mode to provide
	 * reduced disk I/O and increased concurrency. But unlike an ordinary
	 * deferred-write database, the information in a temporary database is not
	 * durable or persistent.
	 *
	 * <p>
	 * A temporary database is not flushed to disk when the database is closed
	 * or when a checkpoint is performed, and the Database.sync method may not
	 * be called. When all handles for a temporary database are closed, the
	 * database is automatically removed. If a crash occurs before closing a
	 * temporary database, the database will be automatically removed when the
	 * environment is re-opened.
	 *
	 * <p>
	 * Note that although temporary databases can page to disk if the cache is
	 * not large enough to hold the databases, they are much more efficient if
	 * the database remains in memory. See the JE FAQ on the Oracle Technology
	 * Network site for information on how to estimate the cache size needed by
	 * a given database.
	 *
	 * <p>
	 * See the
	 * {@link <a href="{@docRoot}/../GettingStartedGuide/DB.html#tempdbje">
	 * Getting Started Guide, Database chapter</a>} for a full description of
	 * temporary databases.
	 * <p>
	 * 
	 * @param temporary
	 *            if true, the database will be opened as a temporary database.
	 *
	 * @return this
	 */
	 ...
	 *
	 * <p>
	 * Deferred-write databases have reduced disk I/O and improved concurrency.
	 * Disk I/O is reduced when data records are frequently modified or deleted.
	 * The information in a deferred-write database is not guaranteed to be
	 * durable or persistent until {@link Database#close} or
	 * {@link Database#sync} is called, or a checkpoint is performed. Since the
	 * usual write ahead logging system is relaxed in order to improve
	 * performance, if the environment crashes before a {@link Database#sync} or
	 * {@link Database#close}, none, all, or a unpredictable set of the
	 ...
	 * can be used to initially load a large data set in deferred-write mode and
	 * then switch to transactional mode for subsequent operations.
	 *
	 * <p>
	 * Note that although deferred-write databases can page to disk if the cache
	 ...
	 * Technology Network site for information on how to estimate the cache size
	 * needed by a given database.
	 *
	 * <p>
	 * See the
	 * {@link <a href="{@docRoot}/../GettingStartedGuide/DB.html#dwdatabase">
	 * Getting Started Guide, Database chapter</a>} for a full description of
	 ...
	 * 
	 * @param deferredWrite
	 *            if true, the database will be opened as a deferred-write
	 *            database.
	 *
	 * @return this
	 */
	 ...
	 * duplicates or not, you can set this property to true. In general, this is
	 * used by the JE utilities, to avoid having to know the configuration of a
	 * database. The databases should be opened readOnly when this property is
	 * set to true.
	 *
	 * @param useExistingConfig
	 *            true if this Database should be opened using the existing
	 *            configuration.
	 *
	 * @return this
	 */
	 ...
	 * {@link ReadOptions#setCacheMode(CacheMode)} or
	 * {@link WriteOptions#setCacheMode(CacheMode)}.
	 *
	 * @param cacheMode
	 *            is the default {@code CacheMode} used for operations performed
	 *            on this database. If {@code null} is specified, the
	...
	/**
	 * Configures a database to be replicated or non-replicated, in a replicated
	 * Environment. By default this property is true, meaning that by default a
	 * database is replicated in a replicated Environment.
	 * <p>
	 * In a non-replicated Environment, this property is ignored. All databases
	 ...
	 *         primary-secondary associations for a group of primary and
	 *         secondary databases. The same SecondaryAssociation instance must
	...
	void validate(DatabaseConfig config) throws DatabaseException {
		...
		if (config.overrideBtreeComparator) {
			if (btreeComparator == null) {
				...
				btCmpMatch = Arrays.equals(DatabaseImpl.objectToBytes(btreeComparator, "Btree"),
		...
		if (config.overrideDuplicateComparator) {
			if (duplicateComparator == null) {
				...
				dtCmpMatch = Arrays.equals(DatabaseImpl.objectToBytes(duplicateComparator, "Duplicate"),
		...
		String message = genDatabaseConfigMismatchMessage(txnMatch, roMatch, sdMatch, dwMatch, btCmpMatch, dtCmpMatch);
		throw new IllegalArgumentException(message);
	}

	private String genDatabaseConfigMismatchMessage(boolean txnMatch, boolean roMatch, boolean sdMatch, boolean dwMatch,
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             via setBtreeComparator and setDuplicateComparator
	 */
	 ...
	 *             via setBtreeComparator and setDuplicateComparator
	 */
	private Comparator<byte[]> validateComparator(Class<? extends Comparator<byte[]>> comparatorClass, String type)
			throws IllegalArgumentException {
		...
		if (!Comparator.class.isAssignableFrom(comparatorClass)) {
			throw new IllegalArgumentException(comparatorClass.getName() + " is is not valid as a " + type
		...
		try {
			...
			throw new IllegalArgumentException(
					type + " comparator is not valid. " + "Perhaps you have not implemented a zero-parameter "
	...
	boolean persistentEquals(DatabaseConfig other) {
		...
		if ((duplicateComparator != null)) {
			if (duplicateComparator.getClass() != other.duplicateComparator.getClass()) {
				return false;
			}
		}
	...
	void validateOnDbOpen(String databaseName, boolean dbIsReplicated) {
		...
		if ((getDeferredWrite() && getTemporary()) || (getDeferredWrite() && getTransactional())
		...
		if ((triggers != null) && (triggers.size() > 0)) {
			...
			if (dbIsReplicated && !replicatedTriggers) {
				throw new IllegalArgumentException("For a replicated Database, triggers must implement "
	...
	boolean checkTriggers(List<Trigger> triggerList) {
		...
		for (Trigger trigger : triggerList) {
			...
			/*
			 * Note that we do not disallow the unsupported PersistentTrigger or
			 * ReplicatedDatabaseTrigger intefaces here, to enable the continued
			 * testing of these partially implemented features.
			 *
			 * if (trigger instanceof PersistentTrigger) { throw new
			 * IllegalArgumentException ("PeristentTrigger not supported: " +
			 * trigger.getName()); }
			 */
	 ...
	 *
	 * @param dbImpl
	 *            the underlying DatabaseImpl for a database handle, which
	 *            provides the Database-wide properties
	 *
	 * @param dbHandleConfig
	 *            DatabaseConfig field for the same database handle, which
	 *            provides the per-Database properties.
	 *
	...
	static DatabaseConfig combineConfig(DatabaseImpl dbImpl, DatabaseConfig dbHandleConfig) {
		 ...
		 * properties in dbHandleConfig are consistent with DatabaseImpl.
		 * They're still set here in case the useExistingConfig property is set,
		 * and those field were not initialized.
		 */
		if (dbImpl != null) {
			...
			showConfig.setDuplicateComparatorInternal(dbImpl.getDuplicateComparator(),
