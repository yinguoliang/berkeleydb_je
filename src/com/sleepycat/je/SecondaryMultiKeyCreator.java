 ...
 * {@link SecondaryKeyCreator} interface instead. The table below summarizes how
 ...
 *
 * <p>
 * Note that <code>SecondaryMultiKeyCreator</code> may also be used for single
 * key secondaries (many-to-one and one-to-one); in this case, at most a single
 * key is added to the results set. <code>SecondaryMultiKeyCreator</code> is
 * only slightly less efficient than {@link SecondaryKeyCreator} in that two or
 * three temporary sets must be created to hold the results. @see
 * SecondaryConfig
 * </p>
 *
 * <p>
...
public interface SecondaryMultiKeyCreator {
	 ...
	 * must always return the same results for a given set of input parameters.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param results
	 *            the set to contain the the secondary key DatabaseEntry objects
