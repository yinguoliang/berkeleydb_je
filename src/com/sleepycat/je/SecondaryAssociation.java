 ...
 *         an application that wishes to partition data from a single logical
 *         "table" into multiple primary databases to take advantage of the
 *         performance benefits of {@link Environment#removeDatabase} for a
 *         single partition, while at the same having a single index database
 *         for the entire "table". For read operations via a secondary database,
 *         JE reads the primary key from the secondary database and then must
 *         read the primary record from the primary database. The mapping from a
 *         primary key to its primary database is performed by the user's
 *         implementation of the {@link #getPrimary} method.
 *         <p>
 *         In addition, a primary database may be divided into subsets of keys
 *         where each subset may be efficiently indexed by zero or more
 *         secondary databases. This effectively allows multiple logical
 *         "tables" per primary database. When a primary record is written, the
 *         secondaries for its logical "table" are returned by the user's
 *         implementation of {@link #getSecondaries}. During a primary record
 *         write, because {@link #getSecondaries} is called to determine the
 *         secondary databases, only the key creators/extractors for those
 *         secondaries are invoked. For example, if you have a single primary
 *         database with 10 logical "tables", each of which has 5 distinct
 *         secondary databases, when a record is written only 5 key
 *         creators/extractors will be invoked, not 50.
 *         <p>
 ...
 *         <p>
 *         A {@code SecondaryAssociation} is configured using
 *         {@link DatabaseConfig#setSecondaryAssociation} and
 *         {@link SecondaryConfig#setSecondaryAssociation} for a primary and
 *         secondary database, respectively. When calling
 *         {@link Environment#openSecondaryDatabase}, null must be passed for
 *         the primaryDatabase parameter when a {@code SecondaryAssociation} is
 *         configured.
 *         <p>
 *         Note that when a {@code SecondaryAssociation} is configured,
 *         {@code true} may not be passed to the
 *         {@link SecondaryConfig#setAllowPopulate} method. Population of new
 *         secondary databases in an existing {@code
 * SecondaryAssociation} is done differently, as described below.
 *         <p>
 *         <b>Adding and removing secondary associations</b>
 *         <p>
 *         The state information defining the association between primary and
 *         secondary databases is maintained by the application, and made
 *         available to JE by implementing the {@code SecondaryAssociation}
 *         interface. The application may add/remove databases to/from the
 *         association without any explicit coordination with JE. However,
 *         certain rules need to be followed to maintain data integrity.
 *         <p>
 *         In the simplest case, there is a fixed (never changing) set of
 *         primary and secondary databases and they are added to the association
 *         at the time they are created, before writing or reading records. In
 *         this case, since reads and writes do not occur concurrently with
 ...
 *         </ol>
 *         <p>
 *         In other cases, primary and secondary databases may be added to an
 *         already established association, along with concurrent reads and
 *         writes. The rules for doing so are described below.
 *         <p>
 *         Adding an empty (newly created) primary database is no more complex
 *         than in the simple case above, assuming that writes to the primary
 *         database do not proceed until after it has been added to the
 *         association.
 *         <ol>
 *         <li>Open the new primary database.</li>
 *         <li>Add the primary database to the association such that
 *         {@link #getSecondaries} returns the appropriate list when passed a
 *         key in the new primary database, and {@link #getPrimary} returns the
 *         new database when passed a key it contains.</li>
 *         <li>Begin writing to the new primary database.</li>
 *         </ol>
 *         <p>
 *         Using the procedure above for adding a primary database, records will
 *         be indexed in secondary databases as they are added to the primary
 *         database, and will be immediately available to secondary index
 *         queries.
 *         <p>
 *         Alternatively, records can be added to the primary database without
 *         making them immediately available to secondary index queries by using
 *         the following procedure. Records will be indexed in secondary
 *         databases as they are added but will not be returned by secondary
 *         queries until after the last step.
 *         <ol>
 *         <li>Open the new primary database.</li>
 *         <li>Modify the association such that {@link #getSecondaries} returns
 *         the appropriate list when passed a key in the new primary database,
 *         but {@link #getPrimary} returns null when passed a key it contains.
 *         </li>
 *         <li>Write records in the new primary database.</li>
 *         <li>Modify the association such that {@link #getPrimary} returns the
 *         new database when passed a key it contains.</li>
 *         </ol>
 *         <p>
 *         The following procedure should be used for removing an existing
 *         (non-empty) primary database from an association.
 *         <ol>
 *         <li>Remove the primary database from the association, such that
 *         {@link #getPrimary} returns null for all keys in the primary.</li>
 ...
 *         database, {@link SecondaryDatabase#deleteObsoletePrimaryKeys} should
 *         be called to process all secondary records.</li>
 *         <li>At this point {@link #getPrimary} may throw an exception (rather
 *         than return null) when called for a primary key in the removed
 *         database, if desired.</li>
 *         </ol>
 *         <p>
 *         The following procedure should be used for adding a new (empty)
 *         secondary database to an association, and to populate the secondary
 *         incrementally from its associated primary database(s).
 *         <ol>
 *         <li>Open the secondary database and call
 *         {@link SecondaryDatabase#startIncrementalPopulation}. The secondary
 *         may not be used (yet) for read operations.</li>
 ...
 *         <p>
 *         The following procedure should be used for removing an existing
 *         (non-empty) secondary database from an association.
 *         <ol>
 *         <li>Remove the secondary database from the association, such that it
 *         is not included in the collection returned by {@link #getSecondaries}
 *         .</li>
 *         <li>Disable read and write operations on the database and ensure that
 *         all in-progress operations have completed.</li>
 ...
 *         association information that provide <em>happens-before</em>
 *         semantics. In other words, when the association is changed, other
 *         threads calling the methods in this interface must see the changes as
 *         soon as the change is complete.
 *         <p>
 *         The implementation should use non-blocking data structures to hold
 *         association information to avoid blocking on the methods in this
 *         interface, which may be frequently called from many threads.
 *         <p>
 *         The simplest way to meet the above two requirements is to use
 *         concurrent structures such as those provided in the
