 ...
 * given txn to an arbitrary point. Txn rollback (and TxnChain construction) is
 * done in 2 occasions: 1. During normal processing, when an ongoing txn must be
 * rolled-back due to a syncup operation (see rep/txn/ReplayTxn.java). 2. During
 * recovery, to process a "rollback period" (see RollbackTracker.java) In the JE
 * log, the logrecs that make up a txn are chained, but each logrec contains
 * undo info that refers to the pre-txn version of the associated record, which
 * may not be the immediately previous version, if the txn writes the same
 * record multiple times. For example, a log looks like this: lsn key data
 * abortlsn 100 A 10 null_lsn (first instance of record A) 150 B 100 null_lsn
 * (first instance of record B) ..... txn begins ..... 200 A 20 100 300 A
 * deleted 100 400 B 200 150 500 A 30 100 600 C 10 null_lsn When reading the
 * log, we can find all the records in the transaction. This chain exists:
 * 500->400->300->200->null_lsn To rollback to an arbitrary entry in the
 * transaction, we need a chain of all the records that occupied a given BIN
 * slot during the transaction. chain. The key, data, and comparators are used
 * to determine which records hash to the same slot, mimicking the btree itself.
 * 300 400 500 600 | | | | \ / \ / \ / \ / 200 150 300 null_lsn revertToLsn true
 * revertKD
 */
public class TxnChain {
     ...
     * obj, which refers to the record version to revert to. The list is ordered
     ...
     * recovery. This differs from the constructor used by syncup rollback which
     * is instigated by the txn. In this case, there is no cache of
     * DatabaseImpls.
     */
     ...
     */
    public TxnChain(long lastLoggedLsn, long txnId, long matchpoint, Map<DatabaseId, DatabaseImpl> undoDatabases,
        ...
        /*
         * A map that stores for each record R the revert info for the latest
         * R-logrec seen during the backwards traversal of the txn chain done
         * below.
         */
        ...
        /*
         * Traverse this txn's entire logrec chain and record revert info for
         * each logrec in the chain. Start the traversal with the last logrec
         * generated by this txn and move backwards.
         */
        ...
        try {
            ...
            while (currLsn != DbLsn.NULL_LSN) {
                ...
                if (dbImpl == null) {
                    ...
                    if (undoDatabases != null) {
                        throw EnvironmentFailureException.unexpectedState(envImpl, // fatal error, this is a corruption
                                "DB missing during non-recovery rollback, dbId=" + currLogrec.getDbId() + " txnId="
                                        + txnId);
                    }

                    /*
                     * For recovery rollback, simply skip this entry when the DB
                     * has been deleted. This has no impact on the chain for
                     * other LNs. This LN will not be processed by recovery
                     * rollback, since RollbackTracker.rollback ignores LNs in
                     * deleted DBs. [#22071] [#22052]
                     */
                ...
                try {
                    ...
                    if (ri != null) {
                        ...
                        ri.revertVLSN = (currLogrec.isEmbeddedLN() ? currLogrec.getLN().getVLSNSequence()
                     ...
                     * R-logrec by T and thus set its revert info to refer to
                     * the pre-T version of R.
                     */
                    if (DbLsn.compareTo(currLsn, matchpoint) > 0) {

                        ri = new RevertInfo(currLogrec.getAbortLsn(), currLogrec.getAbortKnownDeleted(),
                                currLogrec.getAbortKey(), currLogrec.getAbortData(), currLogrec.getAbortVLSN(),
                                currLogrec.getAbortExpiration(), currLogrec.isAbortExpirationInHours());
                        ...
                        /*
                         * We are done with record R, so remove it from the map,
                        ...
                        if (lastValidVLSN != null && lastValidVLSN.isNull() && wholeEntry.getHeader().getVLSN() != null
            ...
            throw EnvironmentFailureException.promote(envImpl, EnvironmentFailureReason.LOG_INTEGRITY,
                    "Problem finding intermediates for txn " + txnId + " at lsn " + DbLsn.getNoFormatString(currLsn),
                    e);
        }
    }
    ...
    public static class RevertInfo {
        ...
        RevertInfo(long revertLsn, boolean revertKD, byte[] revertKey, byte[] revertData, long revertVLSN,
                   int revertExpiration, boolean revertExpirationInHours) {
    ...
    public static class CompareSlot implements Comparable<CompareSlot> {
        ...
        public int hashCode() {
             ...
             * be used instead when a CompareSlot is used as a key. Because a
             * comparator may be configured that compares only a part of the
             * key, a hash code cannot take into account the key or data,
             * because hashCode() must return the same value for two objects
             * whenever equals() returns true. We could hash the DB ID alone,
             * but that would not produce an efficient hash table.
             */
