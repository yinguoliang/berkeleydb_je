 ...
 * semantics. Txn (includes Txns marked autoTxn) MasterTxn and ReadonlyTxn
 * instances are truly transactional. They have potentially different
 * transaction begin and end behaviors.
 */
public abstract class Locker {
     ...
     * because the mandatedId mechanism only works if the generateId() method is
     * overridden to use the mandatedId value.
     *
     ...
     * recovery. Other subtypes ask the txn manager for an id or use a specific,
     * mandated id.
     */
    ...
    /**
     * Get the lock timeout period for this locker, in milliseconds WARNING: Be
     * sure to always access the timeout with this accessor, since it is
     * overridden in BuddyLocker.
     */
     ...
     *            milliseconds. A value of 0 disables timeouts for the
     *            transaction.
     ...
     *
     * @param timeout The timeout value for the transaction lifetime, in
     *            microseconds. A value of 0 disables timeouts for the
     *            transaction.
     * @throws IllegalArgumentException via Transaction.setLockTimeout
     */
    ...
    public void setPreempted() {
        ...
        /*
         * Record the stack trace when a lock is stolen. This will provide more
         * "cause" information when it is wrapped in a LockPreemptedException
         * that is thrown later -- see checkPreempted.
         */
     ...
     * thrown if a lock was preempted earlier. This operation is split into two
     * methods, checkPreempted and throwIfPreempted, so that Txn.checkPreempted
     * can call throwIfPreempted for all its BuddyLockers without causing an
     * infinite recursion. This method is overridden by BuddyLocker to forward
     * the call to its parent buddy (the Txn), and by Txn to check all its child
     * buddies.
     *
     * @param allowPreemptedLocker is a locker that is being closed as the
     *            result of a cursor move operation. If the operation is
     *            successful then allowPreemptedLocker will be closed, and the
     *            fact that a lock has been stolen from allowPreemptedLocker can
     *            be ignored.
     */
     ...
     * if a lock was preempted earlier. Creating the LockPreemptedException sets
     * the txn to abort-only.
     *
     * @see #checkPreempted
     */
     ...
     * closingLocker so that a reference to the old closed locker is not held by
     * this object. [#16513]
     *
     * @param closingLocker the old locker that will be closed if the new cursor
     *            (using this locker) is moved successfully.
     */
    ...
    /**
     * Returns whether a transaction is method indicates whether the txn is part
     * of the rep stream. A replicated txn must be used for writing to a
     * replicated DB, and a non-replicated txn must be used for writing to a
     * non-replicated DB. This is critical for avoiding corruption when HA
     * failovers occur [#23234] [#23330]. See guard in LN.logInternal.
     */
     ...
     * locker, or false if writes may only be to replicated DBs. By default
     * (this implementation) local-write is true, since it is allowed for all
     * non-txnal lockers and for all lockers in a standalone environment. This
     * method is overridden and returns false for for user transactions in a
     * replicated environment that are not explicitly configured for
     * local-write. This method is used to describe a locker's configured usage
     * for checking the validity of an API write operation. This is checked by
     * Cursor methods at the beginning of each write operation.
     */
     ...
     * LockNotAvailableException and can therefore be used by nonBlockingLock to
     ...
     * @param noWait is true to override the defaultNoWait setting. If true, or
     *            if defaultNoWait is true, throws LockNotAvailableException if
     *            the lock cannot be granted without waiting.
     ...
     */
    abstract LockResult lockInternal(long lsn, LockType lockType, boolean noWait, boolean jumpAheadOfWaiters,
    ...
    public LockResult lock(long lsn, LockType lockType, boolean noWait, DatabaseImpl database)
        ...
        final LockResult result = lockInternal(lsn, lockType, noWait, false /* jumpAheadOfWaiters */, database);
     ...
     * denied rather than throwing LockNotAvailableException. This method should
     * therefore not be used as the final lock for a user operation, since in
     * that case LockNotAvailableException should be thrown for a denied lock.
     * It is normally used only to probe for a lock internally, and other
     * recourse is taken if the lock is denied.
     * </p>
     *
     * @param lsn is the node to lock.
     * @param lockType is the type of lock to request.
    ...
    public LockResult nonBlockingLock(long lsn, LockType lockType, boolean jumpAheadOfWaiters, DatabaseImpl database) {
        final LockResult result = lockInternal(lsn, lockType, true /* noWait */, jumpAheadOfWaiters, database);
        if (result.getLockGrant() != LockGrantType.DENIED) {
            checkPreempted(closingLocker);
        }
        return result;
     ...
     * locker that currently holds a lock on the old LSN. Lock is non-blocking
     * because no contention is possible on the new LSN. Because this locker is
     * being used by multiple threads, this method may be called for a locker
     * that has been closed or for which the lock on the old LSN has been
     * released. Unlike other locking methods, in this case we simply return
    ...
    public synchronized void lockAfterLsnChange(long oldLsn, long newLsn, DatabaseImpl dbImpl) {
        ...
        final LockResult lockResult = nonBlockingLock(newLsn, lockType, false /* jumpAheadOfWaiters */, dbImpl);

        if (lockResult.getLockGrant() == LockGrantType.DENIED) {
            throw EnvironmentFailureException
                    .unexpectedState("No contention is possible on new LSN: " + DbLsn.getNoFormatString(newLsn)
                            + " old LSN: " + DbLsn.getNoFormatString(oldLsn) + " LockType: " + lockType);
        }
    }

    /**
     * In the case where logging occurs before locking, allow lockers to reject
     * the operation (e.g., if writing on a replica) and also prepare to undo in
     * the (very unlikely) event that logging succeeds but locking fails.
     */
     ...
     * 'this'. For a BuddyLocker, this method may return the buddy.
     */
     ...
     * <p>
     * This method must return a locker that shares locks with this locker,
     * e.g., a ThreadLocker.
     * </p>
     * <p>
    ...
    /**
     * By default the set of buddy lockers is not maintained. This is overridden
     * by Txn.
     */
     ...
     *
     * @param operationOK is whether the operation succeeded, since that may
     *            impact ending behavior. (i.e for an auto Txn)
     */
    public abstract void operationEnd(boolean operationOK) throws DatabaseException;

    /**
     * Should be called by all subclasses when the locker is no longer used. For
     * Txns and auto Txns this is at commit or abort. For non-transactional
     * lockers it is at operationEnd.
     */
    ...
    /**
     * Used to determine whether the locker is usable. FUTURE: Note that this
     * method is overridden by Txn, and Txn.abort sets the state to closed when
     * it begins rather than when it ends, but calls close() (the method above)
     * when it ends. This is not ideal and deserves attention in the future.
     */
    ...
    public void addDeleteInfo(BIN bin) {
        ...
        /*
         * Skip queue addition if a delta will be logged. In this case the slot
         * compression will occur in BIN.beforeLog, when a full version is
         * logged.
         */
     ...
     * public for jca/ra/JELocalTransaction. WARNING: Be sure to always access
     * the timeout with this accessor, since it is overridden in BuddyLocker.
     */
    ...
    /**
     * This method is safe to call without synchronizing and this fact is relied
     * on by LockManager when creating exception messages.
     */
    @Override
