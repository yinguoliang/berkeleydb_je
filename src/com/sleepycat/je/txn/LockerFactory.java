...
public class LockerFactory {
     ...
     */
    public static Locker getWritableLocker(final Environment env, final Transaction userTxn, final boolean isInternalDb,
                                           final boolean dbIsTransactional, final boolean autoTxnIsReplicated) {

        return getWritableLocker(env, userTxn, isInternalDb, dbIsTransactional, autoTxnIsReplicated,
     ...
     *
     * @param autoTxnIsReplicated is true if this transaction is executed on a
     *            rep group master, and needs to be broadcast. Currently, all
     *            application-created transactions are of the type
     *            com.sleepycat.je.txn.Txn, and are replicated if the parent
     *            environment is replicated. Auto Txns are trickier because they
     *            may be created for a local write operation, such as log
     *            cleaning.
     */
    public static Locker getWritableLocker(final Environment env, final Transaction userTxn, final boolean isInternalDb,
                                           final boolean dbIsTransactional, final boolean autoTxnIsReplicated,
        ...
        if (dbIsTransactional && userTxn == null) {
            ...
            return Txn.createAutoTxn(envImpl, autoCommitConfig,
        ...
        /*
         * The user provided a transaction, so the environment and the database
        ...
        /*
         * Use the locker for the given transaction. For read-committed, wrap
         * the given transactional locker in a special locker for that isolation
         * level.
         */
     ...
     */
    public static Locker getReadableLocker(final Database dbHandle, final Transaction userTxn,
                                           final boolean readCommittedIsolation) {

        return getReadableLocker(dbHandle, (userTxn != null) ? DbInternal.getLocker(userTxn) : null,
     ...
     */
    public static Locker getReadableLocker(final Database dbHandle, Locker locker, boolean readCommittedIsolation) {
        ...
        return getReadableLocker(dbHandle.getEnvironment(), locker, autoTxnIsReplicated, readCommittedIsolation);
    }

    /**
     * Get a locker for a read or cursor operation.
     */
    private static Locker getReadableLocker(final Environment env, final Locker locker,
                                            final boolean autoTxnIsReplicated, final boolean readCommittedIsolation) {
     ...
     * non replicated. This method is not called for user txns and should not
     * throw a Java runtime exception (IllegalArgument, etc).
     */
