 ...
 * Sequence handles are opened using the
 * {@link com.sleepycat.je.Database#openSequence Database.openSequence} method.
 */
public class Sequence implements Closeable {
			...
			if (sequenceExists) {
				...
				if (useConfig.getAllowCreate()) {
					if (!isWritableLocker) {
						...
						locker = LockerFactory.getWritableLocker(db.getEnvironment(), txn,
								db.getDbImpl().isInternalDb(), db.isTransactional(), db.getDbImpl().isReplicated(),
								autoCommitConfig);
						cursor = new Cursor(db, locker, null);
					}
	 ...
	 * sequence handle then they will be returned. Otherwise the next value will
	 * be fetched from the database and incremented (decremented) by enough to
	 * cover the <code>delta</code> and the next batch of cached values.
	 *
	 ...
	 *
	 * <p>
	 * The <code>txn</code> handle must be null if the sequence handle was
	 * opened with a non-zero cache size.
	 * </p>
	 *
	 * <p>
	 * For maximum concurrency, a non-zero cache size should be specified prior
	 * to opening the sequence handle, the <code>txn</code> handle should be
	 * <code>null</code>, and
	 * {@link com.sleepycat.je.SequenceConfig#setAutoCommitNoSync
	 ...
	 *
	 * @throws SequenceOverflowException
	 *             if the end of the sequence is reached and wrapping is not
	 *             configured.
	 *
	 * @throws SequenceIntegrityException
	 *             if the sequence record has been deleted.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if the delta is less than or equal to zero, or larger than
	 *             the size of the sequence's range.
	 */
	public synchronized long get(Transaction txn, int delta) throws DatabaseException {
		...
		if ((increment && delta > ((cacheLast - cacheValue) + 1))
			...
			OperationStatus status = OperationStatus.NOTFOUND;
			try {
				locker = LockerFactory.getWritableLocker(db.getEnvironment(), txn, db.getDbImpl().isInternalDb(),
						db.isTransactional(), db.getDbImpl().isReplicated(),
						// autoTxnIsReplicated
						autoCommitConfig);
		...
		if (logger.isLoggable(Level.FINEST)) {
			LoggerUtils.finest(logger, db.getEnvironment().getNonNullEnvImpl(),
	...
	public SequenceStats getStats(StatsConfig config) throws DatabaseException {
		...
		if (!config.getFast()) {
			...
			/*
			 * storedValue may have been updated by another handle since it was
			 * last read by this handle. Fetch the last written value.
		...
		StatGroup stats = new StatGroup(SequenceStatDefinition.GROUP_NAME, SequenceStatDefinition.GROUP_DESC);
