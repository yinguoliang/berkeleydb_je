...
/**
 * The configuration properties of a <code>SecondaryDatabase</code> extend those
 * of a primary <code>Database</code>. The secondary database configuration is
 * specified when calling {@link Environment#openSecondaryDatabase
 ...
 * </pre>
 *
 * @see Environment#openSecondaryDatabase Environment.openSecondaryDatabase @see
 *      SecondaryDatabase
 */
public class SecondaryConfig extends DatabaseConfig {
	 ...
	 *
	 * <p>
	 * Unless the primary database is read-only, a key creator is required when
	 * opening a secondary database. Either a KeyCreator or MultiKeyCreator must
	 * be specified, but both may not be specified.
	 * </p>
	 *
	 * <p>
	 * Unless the primary database is read-only, a key creator is required when
	 ...
	 *
	 * <p>
	 * <em>WARNING:</em> Key creator instances are shared by multiple threads
	 ...
	 * should be used and any caching of computed values must be done with
	 ...
	 *
	 * <p>
	 * If automatic population is allowed, when the secondary database is opened
	 * it is checked to see if it is empty. If it is empty, the primary database
	 * is read in its entirety and keys are added to the secondary database
	 * using the information read from the primary.
	 * </p>
	 *
	 * <p>
	 * If this property is set to true and the database is transactional, the
	 * population of the secondary will be done within the explicit or
	 ...
	 * {@link #setAllowPopulate} has not been called, this method returns false.
	 *
	 ...
	 *
	 * <p>
	 * If this property is non-null, a record must be present in the specified
	 * foreign database for every record in the secondary database, where the
	 * secondary key value is equal to the foreign database key value. Whenever
	 * a record is to be added to the secondary database, the secondary key is
	 * used as a lookup key in the foreign database. If the key is not found in
	 * the foreign database, a {@link ForeignConstraintException} is thrown.
	 * </p>
	 *
	 * <p>
	 * The foreign database must not have duplicates allowed. If duplicates are
	 * allowed, an IllegalArgumentException will be thrown when the secondary
	 * database is opened.
	 * </p>
	 *
	 * @param foreignKeyDatabase
	 *            the database used to check the foreign key integrity
	 *            constraint, or null if no foreign key constraint should be
	 *            checked.
	 *
	 * @return this
	 */
	 ...
	 *            the action taken when a referenced record in the foreign key
	 *            database is deleted.
	 *
	...
	/**
	 * Specifies the user-supplied object used for setting single-valued foreign
	 * keys to null.
	 *
	 * <p>
	 * This method may <em>not</em> be used along with
	 * {@link #setMultiKeyCreator}. When using a multi-key creator, use
	 * {@link #setForeignMultiKeyNullifier} instead.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <em>WARNING:</em> Key nullifier instances are shared by multiple threads
	 * and key nullifier methods are called without any special synchronization.
	 * Therefore, key creators must be thread safe. In general no shared state
	 * should be used and any caching of computed values must be done with
	 * proper synchronization.
	 * </p>
	 *
	 * @param foreignKeyNullifier
	 *            the user-supplied object used for setting single-valued
	 ...
	 * </p>
	 *
	 * @param foreignMultiKeyNullifier
	 *            the user-supplied object used for setting multi-valued foreign
	 *            keys to null.
	 *
	 ...
	 *
	 *         Specifies whether the key extractor/creator will only use the
	 *         primary key.
	 *
	 *         <p>
	 *         Specifying that only the primary key is needed can be used to
	 *         optimize primary database updates and deletions. If a primary
	 *         record is updated or deleted, and all associated secondaries have
	 *         this property set to true, then the existing primary record will
	 *         not be read. This potentially saves an I/O. When this property is
	 *         <em>not</em> set to true (it is false by default), the existing
	 *         primary record must be read if it is not already in cache, order
	 *         to pass the primary data to the key extractor/creator.
	 *         </p>
	 *
	 *         <p>
	 *         Note that if this property is true, either null or a non-null
	 *         value may be passed to the key extractor/creator for the primary
	 *         data parameter. The key extractor/creator is expected to ignore
	 *         this parameter.
	 *         <p>
	 *
	 * @param extractFromPrimaryKeyOnly
	 *            whether the key extractor/creator will only use the primary
	 *            key.
	 *
	 * @return this
	 */
	 ...
	 * @hidden For internal use only.
	 *
	 *         Returns whether the key extractor/creator will only use the
	 *         primary key. If {@link #setExtractFromPrimaryKeyOnly} has not
	 ...
	 * operation significantly if the <code>createSecondaryKey</code> operation
	 * is expensive.
	 * </p>
	 *
	 * <p>
	 ...
	 * {@link #setImmutableSecondaryKey} has not been called, this method
	 * returns false.
	 *
	 ...
	 *             via JEConnection.openSecondaryDatabase.
	 */
	@Override
	...
	String genSecondaryConfigMismatchMessage(DatabaseConfig config, boolean kcMatch, boolean mkcMatch, boolean fkdMatch,
			boolean fkdaMatch, boolean fknMatch, boolean fmknMatch, boolean imskMatch) {
		StringBuilder ret = new StringBuilder("The following SecondaryConfig parameters for the\n"
