 ...
 * the next (adjacent) buffer is made available for writing. The buffer pool has
 * a dirty list of buffers. A buffer becomes a member of the dirty list when the
 * currentWriteBuffer is moved to another buffer. Buffers are removed from the
 * dirty list when they are written. The dirtyStart/dirtyEnd variables indicate
 * the list of dirty buffers. A value of -1 for either variables indicates that
 * there are no dirty buffers. These variable are synchronized via the
 * LogBufferPool.bufferPoolLatch. The LogManager.logWriteLatch (aka LWL) is used
 * to serialize access to the currentWriteBuffer, so that entries are added in
 * write/LSN order. A buffer used for writing is accessed by the getWriteBuffer
 * method. This method must be called while holding the LWL. The buffer returned
 * is not latched. A LogBuffer has a pin count (LogBuffer.writePinCount)
 * associated with it. The pin count is incremented when space is allocated in
 * the buffer. The allocation of space is serialized under the LWL. Threads will
 * add data to the buffer by latching the buffer, but without holding the LWL.
 ...
 * does not latch the buffer. Read access to a log buffer is allowed only if the
 * buffer is latched and the pin count is zero. A thread that attempt to access
 * a log buffer for reading will latch and check the pin count. If the pin count
 * is not zero, the latch is released and the process is retried. The thread
 * attempting to access the log buffer for reading may be delayed. The worst
 * case is when the reader has to wait until the buffer is filled (the pin count
 * would be zero).
 *
 * @see LogBuffer
 */
class LogBufferPool {
     ...
     * These dirty buffers do not include the current write buffer. These fields
     * are changed under buffer pool latch.
     */
    private int                   dirtyStart   = -1;
    private int                   dirtyEnd     = -1;

    /*
     * Buffer that holds the current log end. All writes go to this buffer.The
     * members are protected by the LogManager.logWriteLatch.
     */
     ...
     * of latching is: bufferPoolLatch.acquire() LogBuffer read latch acquire();
     * bufferPoolLatch.release(); LogBuffer read latch release() bufferPoolLatch
     * is also used to protect assignment to the currentWriteBuffer field.
     */
    private final Latch           bufferPoolLatch;

    /*
     * A minimum LSN property for the pool that can be checked without latching,
     * to reduce contention by readers. An LSN less than minBufferLsn is
     * guaranteed not to be in the pool. An LSN greater or equal to minBufferLsn
     * may or may not be in the pool, and latching is necessary to determine
     * this. Initializing minBufferLsn to zero ensures that we will latch and
     * check the pool until it is initialized with a valid LSN. [#19642]
     */
    ...
    LogBufferPool(FileManager fileManager, EnvironmentImpl envImpl) throws DatabaseException {
        ...
        bufferPoolLatch = LatchFactory.createExclusiveLatch(envImpl, DEBUG_NAME + "_FullLatch",
                true /* collectStats */);
     ...
     * This method is called after the memory budget has been calculated. The
     * LWL must be held when the buffer pool is not being constructed.
     */
    void reset(DbConfigManager configManager) throws DatabaseException {
         ...
         * when an existing pool is in use: - The old pool will no longer be
         * referenced. - Buffers being read in the old pool will be no longer
         * referenced after the read operation is complete. - The
         * currentWriteBuffer field is not changed here; it will be no longer
         * referenced after it is written to the file and a new
         * currentWriteBuffer is assigned. - The logBufferSize can be changed
         * now because it is only used for allocating new buffers; it is not
         * used as the size of the currentWriteBuffer.
         */
    ...
    /**
     * Get a log buffer for writing an entry of sizeNeeded bytes. If sizeNeeded
     * will fit in currentWriteBuffer, currentWriteBuffer is returned without
     * requiring any flushing. The caller can allocate the entry in the buffer
     * returned. If sizeNeeded won't fit in currentWriteBuffer, but is LTE the
     * LogBuffer capacity, we bump the buffer to get an empty
     * currentWriteBuffer. If there are no free write buffers, then all dirty
     * buffers must be flushed. The caller can allocate the entry in the buffer
     * returned. If sizeNeeded is greater than the LogBuffer capacity, flush all
     * dirty buffers and return an empty (but too small) currentWriteBuffer. The
     * caller must then write the entry to the file directly. The LWL must be
     * held.
     *
    ...
    LogBuffer getWriteBuffer(int sizeNeeded, boolean flippedFile) throws IOException, DatabaseException {
        ...
        if (flippedFile) {
             ...
             * currentWriteBuffer. TODO: Why pass true for flushWriteQueue
             * before doing an fsync?
             */
            ...
            /*
             * Try to bump the current write buffer since there was not enough
             * space in the current write buffer.
             */

            if (!bumpCurrent(sizeNeeded) || !currentWriteBuffer.hasRoom(sizeNeeded)) {

                /*
                 * We could not bump because there was no free buffer, or the
                 * item is larger than the buffer size. Write the dirties to
                 * free a buffer up, or to flush in preparation for writing a
                 * temporary buffer.
                 */
    ...
    /**
     * Bump current write buffer and write the dirty buffers. The LWL must be
     * held to ensure that, if there are no free buffers, we can write the dirty
     * buffers and have a free one, which is required to bump the current write
     * buffer.
     *
     * @param sizeNeeded used only if running in memory. Size of the log buffer
     *            buffer that is needed.
     * @param flushWriteQueue true if data is written to log, otherwise data may
     *            be placed on the write queue.
     */
    void bumpAndWriteDirty(int sizeNeeded, boolean flushWriteQueue) {
        ...
        if (!bumpCurrent(sizeNeeded)) {
            ...
            /*
             * Could not bump the current write buffer; no clean buffers. Write
             * the current dirty buffers so we can bump.
             */
            ...
            if (bumpCurrent(sizeNeeded)) {
                ...
                /*
                 * Since we have the log write latch we should be able to bump
                 * the current buffer.
                 */
                writeDirty(flushWriteQueue);
            } else {
            ...
            /*
             * Since we have the log write latch we should be able to bump the
             * current buffer.
             */
            writeDirty(flushWriteQueue);
        }
     ...
     * The slots are are a circular buffer. The bufferPoolLatch must be held.
     *
    ...
    /**
     * Writes the dirty log buffers. No latches need be held. Note that if no
     * buffers are dirty, nothing will be written, even when data is buffered in
     * the write queue and flushWriteQueue is true. If we were to allow a
     * condition where 1) the write queue is non-empty, 2) there are no dirty
     * log buffers, and 3) the current write buffer is empty, then a flushNoSync
     * at that time won't flush the write queue. This should never happen
     * because: a) flushNoSync leaves the write queue empty, b)
     * LogManager.serialLogWork leaves the current write buffer non-empty, and
     * c) writeDirty(false) doesn't change the state of the current write
     * buffer. TODO: Confirm this with a more detailed analysis.
     *
     * @param flushWriteQueue true then data is written to file otherwise the
     *            data may be placed on the FileManager WriteQueue.
     */
    ...
    /**
     * Writes a log buffer. The LWL or buffer latch must be held.
     *
    ...
    private void writeBufferToFile(LogBuffer latchedBuffer, boolean flushWriteQueue) {
        ...
        try {
            ...
            try {
                 ...
                 * exception is unexpectedly thrown.
                 */
                if (t instanceof EnvironmentFailureException) {

                    /*
                     * If we've already invalidated the environment, re-throw so
                    ...
                    throw EnvironmentFailureException.unexpectedException(envImpl, (EnvironmentFailureException) t);
     ...
     * write buffer if the buffer is empty. The LWL must be held.
     *
     * @param sizeNeeded used only if running in memory. Size of the log buffer
     *            that is needed.
     * @return false when the buffer needs flushing, but there are no free
     *         buffers. Returns true when when running in memory, when the
     *         buffer is empty, or when the buffer is non-empty and is bumped.
     */
     ...
     * getReadBufferByLsn. [#19642] The LWL must be held.
     */
    ...
    /**
     * Find a buffer that contains the given LSN location. No latches need be
     * held.
     *
    ...
    LogBuffer getReadBufferByLsn(long lsn) throws DatabaseException {
        ...
        try {
            /*
             * TODO: Check currentWriteBuffer last, because we will have to wait
             * for its pin count to go to zero, which may require waiting until
             * it is full.
             */
