 ...
 * themselves to a byte buffer in an earlier log format, for use by instances of
 * {@link ReplicableLogEntry} that need to support an earlier log format during
 * replication. See [#22336].
 * <p>
 * Classes that implement {@code Loggable} should implement this interface if
 * they are included in replication data.
 * <p>
 * Implementing classes should document the version of the class's most recent
 * format change. Log entry classes that contain {@code
...
public interface VersionedWriteLoggable extends Loggable {
    ...
    /**
     * Returns the number of bytes needed to store this object in the format for
     ...
     * specified log version. Earlier log versions only need to be supported for
     * log entries with format changes made in
     * {@link LogEntryType#LOG_VERSION_REPLICATE_OLDER} or greater.
     *
