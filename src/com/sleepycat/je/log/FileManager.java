...
public class FileManager {
    ...
    /*
     * The number of writes that have been performed. public so that unit tests
     * can diddle them.
     */
     ...
     * (seek/read/write/fsync). OS's like Windows and Solaris do not since they
     * are able to handle concurrent IO operations on a single file.
     */
    ...
    /*
     * For IO stats, this is a measure of what is "close enough" to constitute a
     * sequential IO vs a random IO. 1MB for now. Generally a seek within a few
     * tracks of the current disk track is "fast" and only requires a single
     * rotational latency.
     */
        ...
        try {
            ...
            if (!envImpl.isMemOnly()) {
                ...
                if (!lockEnvironment(isReadOnly, false)) {
                    throw new EnvironmentLockedException(envImpl, "The environment cannot be locked for "
                            + (isReadOnly ? "shared" : "single writer") + " access.");
                }
            }
            ...
            final Boolean logFileDeleteDetect = configManager.getBoolean(EnvironmentParams.LOG_DETECT_FILE_DELETE);
     ...
     * @param prevOffset value to use for the prevOffset of the next entry. If
     *            the beginning of the file, this is 0.
     */
    ...
    /**
     * For assertions that check whether a file is valid or has been deleted via
     * log cleaning.
     */
            ...
            if (nDataDirs != 0) {
                ...
                if (dbEnvDataDirsIdx != (num % nDataDirs)) {
                    throw EnvironmentFailureException
                            .unexpectedState("Found file " + name + " but it should have been in " + "data directory "
                                    + (dbEnvDataDirsIdx + 1) + ". Perhaps it was moved or restored incorrectly?");
                }
            }
        }
        return nums;
     ...
     * 
     * @param curFile the file we're at right now. Note that it may not exist,
     *            if it's been cleaned and renamed.
     * @param forward if true, we want the next larger file, if false we want
     *            the previous file
     ...
     *
     * @return index into dbEnvDataDirs of this fileName's data directory. -1 if
     *         multiple data directories are not being used.
     */
     ...
     * @return array of file names Used by unit tests so package protection.
     */
    ...
    /**
     * Find .jdb files which are >= the minimimum file number and <= the maximum
     * file number. Return names sorted in ascending fashion.
     *
    ...
    private static Comparator<File>   fileComparator   = new Comparator<File>() {
                                                           ...
                                                           private String getFileNum(File file) {
                                                               ...
                                                               return fname
                                                                       .substring(fname.indexOf(File.separator) + 1);
                                                           }
    ...
    private void checkNoDataDirs() {
        String[] dataDirNames = dbEnvHome.list(new FilenameFilter() {
        ...
        if (dataDirNames != null && dataDirNames.length != 0) {
            throw EnvironmentFailureException.unexpectedState(
                    EnvironmentParams.LOG_N_DATA_DIRECTORIES.getName() + " was not set and expected to find no"
                            + " data directories, but found " + dataDirNames.length + " data directories instead.");
        }
    }
    ...
    public File[] gatherDataDirs() {
        ...
        if (dataDirNames.length != nDataDirs) {
            throw EnvironmentFailureException.unexpectedState(
                    EnvironmentParams.LOG_N_DATA_DIRECTORIES.getName() + " was set and expected to find " + nDataDirs
                            + " data directories, but found " + dataDirNames.length + " instead.");
        }
        ...
        for (String fn : dataDirNames) {
            ...
            try {
                ...
                if (subdirNum != ddNum) {
                    throw EnvironmentFailureException.unexpectedState("Expected to find data subdir: data"
                            + paddedDirNum(ddNum) + " but found data" + subdirNumStr + " instead.");
    ...
    /*
     * Return the full file name of a specified log file name, including the sub
     * directories names if needed.
     */
    ...
    /**
     * HexFormatter generates a 0 padded string starting with 0x. We want the
     * right most 8 digits, so start at 10.
     */
     ...
     *            The subDir must already exist. May be null to leave the file
     *            in its current data directory.
     * @return renamed File if successful, or null if File.renameTo returns
     *         false, which can occur on Windows if the file was recently
     *         closed.
     */
    public File renameFile(final long fileNum, final String newSuffix, final String subDir) throws IOException {
        ...
        while (true) {
            ...
            /*
             * If CLEANER_EXPUNGE is false, then the cleaner will rename the
             * .jdb file. The rename action will first delete the old file and
             * then create the new file. So we should also record the file
             * rename action here.
             */
            if (fdd != null) {
     ...
     *
     * @param fileNum the file we want to move
     * @return true if successful, false if File.delete returns false, which can
     *         occur on Windows if the file was recently closed.
     */
    public boolean deleteFile(final long fileNum) throws IOException, DatabaseException {
         ...
         * The file name gotten from WatchKey is the relative file name, so we
         * should also get the relative file name here.
         */
        if (fdd != null) {
    ...
    public FileHandle getFileHandle(long fileNum) throws FileNotFoundException, ChecksumException, DatabaseException {
        ...
        try {
            while (true) {
                 ...
                 * handle that we get back is really still open after we latch
                 * it down below.
                 */
                 ...
                 * threads that are opening other files while we open this file.
                 * The latch on the handle blocks other threads waiting to open
                 * the same file, which is necessary.
                 */
                ...
                if (newHandle) {
                    ...
                    if (!fileHandle.latchNoWait()) {
                         ...
                         * just make a new transient handle. It doesn't need to
                         * be latched, but it does need to be closed.
                         */
                        final FileHandle existingHandle = fileHandle;
                        fileHandle = new FileHandle(envImpl, fileId, getFileNumberString(fileId)) {
                            @Override
    ...
    /**
     * Creates a new FileHandle and adds it to the cache, but does not open the
     * file.
     * 
     ...
     *            null. If non-null it is used to avoid the cost of reading the
     *            file header.
     */
    private void openFileHandle(FileHandle fileHandle, FileMode mode, FileHandle existingHandle)
        ...
        try {
                ...
                if (mode.isWritable()) {
                    ...
                    if ((headerPrevOffset == 0) && (fileNum > 1) && syncAtFileEnd) {
                        ...
                        throw EnvironmentFailureException.unexpectedState(envImpl,
                                "Zero prevOffset fileNum=0x" + Long.toHexString(fileNum) + " lastLsn="
                                        + DbLsn.getNoFormatString(lastLsn) + " perFileLastUsedLsn=" + perFileLastUsedLsn
            ...
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_READ,
    ...
    private int readAndValidateFileHeader(RandomAccessFile file, String fileName, long fileNum)
        ...
        LogEntry headerEntry = logManager.getLogEntryAllowChecksumException(DbLsn.makeLsn(fileNum, 0), file,
     ...
     */
    private void writeFileHeader(RandomAccessFile file, String fileName, FileHeader header, long fileNum)
        ...
        ByteBuffer headerBuf = envImpl.getLogManager().putIntoBuffer(headerLogEntry, 0); // prevLogEntryOffset
        ...
        if (bytesWritten != headerLogEntry.getSize() + LogEntryHeader.MIN_HEADER_SIZE) {
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_INTEGRITY, "File " + fileName
                    + " was created with an incomplete header. Only " + bytesWritten + " bytes were written.");
        }
    }
     ...
     * @param flushWriteQueue true if this write can not be queued on the Write
     *            Queue.
     */
    void writeLogBuffer(LogBuffer fullBuffer, boolean flushWriteQueue) throws DatabaseException {
        ...
        if (firstLsn != DbLsn.NULL_LSN) {
            ...
            try {
                ...
                writeToFile(file, data, DbLsn.getFileOffset(firstLsn), DbLsn.getFileNumber(firstLsn), flushWriteQueue);
            } catch (ClosedChannelException e) {
     ...
     */
    private int writeToFile(RandomAccessFile file, ByteBuffer data, long destOffset, long fileNum,
         ...
         * because two threads may call seek() on the same file object. If the
         * Write Queue is enabled, attempt to get the fsync latch. If we can't
         * get it, then an fsync or write is in progress and we'd block anyway.
         * In that case, queue the write operation.
         */
     ...
     * data exists in this file. There's no need to incur extra costs such as
     * checks of the file length, nor to return status as to whether this file
     * contains the data.
     */
     ...
     */
    boolean readFromFile(RandomAccessFile file, ByteBuffer readBuffer, long offset, long fileNo,
        ...
        try {
            ...
            /*
             * Nothing queued, all data for this file must be in the file. Note
             * that there's no synchronization between the check of the write
             * queue above, and this check of file length. It's possible that a
             * newly written log entry could show up between the statements, and
             * enter the write queue just after we finish the check. Because of
             * this, callers of this method must abide by one of three
             * conditions: 1. They guarantee that the attempt to read a chunk of
             * new data comes after the new data has been logged by the
             * LogManager. 2. The files are quiescent when the read is going on.
             * 3. The caller is sure the data is in this file. The replication
             * feeder reader abides by (1) while all other file readers abide by
             * (2). Callers which are fetching specific log entries fall under
             * (3).
             */
    ...
    private void readFromFileInternal(RandomAccessFile file, ByteBuffer readBuffer, long offset, long fileNum)
            throws IOException {
    ...
    private void printLogBuffer(ByteBuffer entryBuffer, long lsn) {
        ...
        while (entryBuffer.remaining() > 0) {
            ...
            System.out.println("LOGREC " + recType.toStringNoVersion() + " at LSN " + DbLsn.toString(lsn)
    ...
    private void verifyChecksum(ByteBuffer entryBuffer, LogEntryHeader header, long lsn, String comment)
    ...
    public void startFileCacheWarmer(final long recoveryStartLsn) {
        ...
        fileCacheWarmer = new FileCacheWarmer(envImpl, recoveryStartLsn, lastUsedLsn, warmUpSize, bufSize);
    ...
    private void stopFileCacheWarmer() {
        ...
        /*
         * Use fcw local var because fileCacheWarmer can be set to null by other
         * threads calling clearFileCacheWarmer, namely the cache warmer thread.
         */
    ...
    private boolean checkEnvHomePermissionsSingleEnvDir(File dbEnvHome, boolean rdOnly) throws DatabaseException {
        ...
        if (envDirIsReadOnly && !rdOnly) {
            ...
            /*
             * Use the absolute path in the exception message, to make a
             * mis-specified relative path problem more obvious.
             */
            throw new IllegalArgumentException("The Environment directory " + dbEnvHome.getAbsolutePath()
                    + " is not writable, but the " + "Environment was opened for read-write access.");
        }
     ...
     * the log file gap. Used by replication hard recovery and the DbTruncateLog
     * utility, see SR [#19463].
     */
    public void truncateLog(long fileNum, long offset) throws IOException, DatabaseException {

        /*
         * Truncate the log files following by this log file in descending order
         * to avoid the log entry gap, see SR [#19463].
         */
    ...
    /**
     * Mark the specified log entries as invisible and obsolete. The entries are
     * written here, but are fsync'ed later. If there is any problem or
     * exception during the setting, the method will throw an
     * EnvironmentFailureException. These changes are made directly to the file,
     * but recently logged log entries may also be resident in the log buffers.
     * The caller must take care to call LogManager.flush() before this method,
     * to ensure that all entries are on disk. In addition, we must ensure that
     * after this step, the affected log entries will only be read via a
     * FileReader, and will not be faulted in by the LogManager. Entries may be
     * present in the log and in the log buffers, but only the on disk version
     * is modified by this method. The LogManager can read directly from the log
     * buffers and may read the incorrect, non-invisible version of the log
     * entry, rather than the invisible version from the file. This should not
     * be an issue, because invisible log entries should be detached from the
     * in-memory tree before they are made invisible.
     *
    ...
    public void makeInvisible(long fileNum, List<Long> lsns) {
        ...
        try {
            ...
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_CHECKSUM,
        ...
        try {
            for (Long lsn : lsns) {
                if (DbLsn.getFileNumber(lsn) != fileNum) {
                     ...
                     * away. But since it causes replication syncup to fail, the
                     * environment will shutdown, which is the effect we want.
                     */
                    throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.UNEXPECTED_STATE,
            ...
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_WRITE,
     ...
     * fsync in that case, 0x0 got fsynced. If a write is attempted and an fsync
     * is already in progress, then the information pertaining to the data to be
     * written (data, offset, length) is saved away in the "queuedWrites" array.
     * When the fsync completes, the queuedWrites buffer is emptied. This
     * ensures that writes continue to execute on file systems which block all
     * IO calls during an fsync() call (e.g. ext3).
     */
    class LogEndFileDescriptor {
        ...
        /*
         * Holds all data for writes which have been queued due to their being
         * blocked by an fsync when the original write was attempted. The next
         * thread to execute an fsync or write will execute any queued writes in
         * this buffer. Latch order is fsyncFileSynchronizer, followed by the
         * queuedWrites mutex [ synchronized (queuedWrites) {} ]. Default
         * protection for unit tests.
         */
         ...
         * readBuffer with those bytes. We theorize that this is needed because
         * HA will be reading at the very end of the log and those writes, if
         * enqueued, may no longer be in LogBuffers in the LogBufferPool. This
         * might happen in the case of lots of concurrent non-synchronous writes
         * (with synchronous commits) which become enqueued in the queuedWrites
         * cache, but cycle out of the LBP. In general, using synchronous
         * commits with HA is a bad idea. Default protection for unit tests.
         * @return true if more data was available. If so, the read buffer will
         * be filled up.
         */
        /* private */
         ...
         * held when this is called. Default protection for unit tests.
         */
        /* private */
        boolean enqueueWrite(final long fileNum, final byte[] data, final long destOffset, final int arrayOffset,
                             final int size)
                throws DatabaseException {
        ...
        private void enqueueWrite1(final long fileNum, final byte[] data, final long destOffset, final int arrayOffset,
                                   final int size)
             ...
             * Latch is held. NOTE: We believe the commented out second
             * condition is safe to add to the code if we ever see contention
             * with this call to dequeuePendingWrites against an fsync. Here is
             * the reasoning: queuedWritesPosition is changed in two places: (1)
             * enqueueWrite1 where it is incremented, and (2)
             * dequeuePendingWrites1 where it is zeroed. Both of these places
             * are proected by the queuedWrites mutex. The zero'ing (2) will
             * only make the dequeue unnecessary so the extra commented out
             * check below is safe since it will only result in eliminating an
             * unnecessary dequeuePendingWrites call.
             */
            ...
            synchronized (queuedWrites) {
                ...
                if (overflow) {
                     ...
                     * holding the fsyncFileSynchronizer latch first which would
                     * be latching out of order relative to the queuedWrites
                     * mutex.
                     */
                ...
                if (curPos + qwStartingOffset != destOffset) {
                    throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_INTEGRITY,
                            "non-consecutive writes queued. " + "qwPos=" + queuedWritesPosition + " write destOffset="
                                    + destOffset);
                }
        ...
        /**
         * getWritableFile must be called under the log write latch. Typically,
         * endOfLogRWFile is not null. Hence the fsyncFileSynchronizer does not
         * need to be locked (which would block the write queue from operating.
         */
        private RandomAccessFile getWritableFile(final long fileNumber, final boolean doLock) {
            try {
                if (endOfLogRWFile == null) {
                     ...
                     * latch. Protect both the RWFile and SyncFile under this
                     * lock, to avoid a race for creating the file and writing
                     * the header. [#20732]
                     */
                    if (doLock) {
                        fsyncFileSynchronizer.lock();
        ...
        private void force() throws DatabaseException, IOException {
             ...
             * when we switch files. If there is no current end file descriptor,
             * we know that the log file has flipped to a new file since the
             * fsync was issued.
             */
            fsyncFileSynchronizer.lock();
            try {
                ...
                if (file != null) {
                    ...
                    try {
                         ...
                         * See SR [#10463].
                         */
                        throw new ThreadInterruptedException(envImpl, "Channel closed, may be due to thread interrupt",
                                e);
                    }
                    ...
                    if (nFSyncMaxTime.setMax(fSyncMs) && fSyncTimeLimit != 0 && fSyncMs > fSyncTimeLimit) {
                        ...
                        LoggerUtils.warning(envImpl.getLogger(), envImpl,
                                String.format("FSync time of %d ms exceeds limit (%d ms)", fSyncMs, fSyncTimeLimit));
                    }
    ...
    private void generateLogWriteException(RandomAccessFile file, ByteBuffer data, long destOffset, long fileNum)
    ...
    /**
     * The factory interface for creating RandomAccessFiles. For production use,
     * the default factory is always used and a DefaultRandomAccessFile is
    ...
    public interface FileFactory {
         ...
         *            program that opens multiple environments. Not for
         *            production use.
    ...
    /**
     * The RandomAccessFile for production use. Tests that override the default
     * FileFactory should return a RandomAccessFile that subclasses this class
     * to inherit workarounds such as the overridden length method.
     */
    public static class DefaultRandomAccessFile extends RandomAccessFile {
         ...
         * file pointer if interrupted in the middle. It is synchronized here to
