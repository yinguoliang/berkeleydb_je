...
public abstract class FileReader {
	...
	public FileReader(EnvironmentImpl envImpl, int readBufferSize, boolean forward, long startLsn,
			Long singleFileNumber, long endOfFileLsn, long finishLsn) throws DatabaseException {
	 ...
	 *             if a ChecksumException, FileNotFoundException, or another
	...
	protected void setBackwardPosition()
		...
		if ((currentEntryPrevOffset != 0) && window.containsOffset(currentEntryPrevOffset)) {
			 ...
			 *
			 * 1) The target log entry is in a different file from the current
			 * window's file. Move the window to the previous file and start the
			 * read from the target LSN.
			 *
			 * 2) The target log entry is the same file but the log entry is
			 * larger than the read chunk size. Start the next read buffer from
			 * the target LSN. It's going to take multiple reads to get the log
			 * entry, and we might as well get as much as possible.
			 *
			 * 3) In the same file, and the log entry fits within one read
			 * buffer. Try to position the next buffer chunk so the target entry
			 * is held within the buffer, all the way at the end. That way,
			 * since we're reading backwards, there will be more buffered data
			 * available for following reads.
			 */
			long nextFile;
	 ...
	 *
	 * @param entryBuffer
	 *            A ByteBuffer that the logrec data and is positioned at the
	 *            start of the logrec body (i.e., just after the logrec header).
	 *
	...
	protected static class ReadWindow {
		 ...
		 * false.
		 *
		 * In all cases, leave the the read buffer pointing at the target offset
		 * and in a state that's ready to support reads, even if there is
