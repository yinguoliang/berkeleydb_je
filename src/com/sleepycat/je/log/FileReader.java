 ...
 * methods that allow their users to examine the contents of the logrec that the
 * iterator is currently positioned at, and (b) implement the (non-public)
 * isTarget() and processEntry() methods that may filter-out logrecs that do not
 * need to be seen by the caller of readNextEntry(), or perform specific actions
 * before each logrec is "returned" to the caller of readNextEntry().
 */
public abstract class FileReader {
    ...
    /*
     * The ReadWindow is a data buffer that acts as a sliding window view of the
     * log. It is positioned against the log and filled up with data.
     */
    ...
    /*
     * true if at end of the log. TODO: assess whether this is redundant with
     * the EOFException, and could be streamlined.
     */
     ...
     * currentEntryPrevOffset is the same as currentEntryHeader.getPrevOffset(),
     * but it's initialized and used before a header is read. Only used for
     * backward scanning.
     */
     ...
     * entry. Only used for forward scanning.
     */
     ...
     */
    public FileReader(EnvironmentImpl envImpl, int readBufferSize, boolean forward, long startLsn,
    ...
    /**
     * Scans the log files until either it has reached the end of the log or has
     * hit an invalid portion.
     *
     ...
     * EnvironmentFailureException and invalidating the enviornment. This allows
     * users of this class (see cleaner.FileProcessor), and subclasses that
     * override readNextEntry (see ScavengerFileReader and LastFileReader), to
     * handle these exceptions specially.
     */
        ...
        try {
            while ((!eof) && (!foundEntry)) {
                ...
                if (currentEntryHeader.isVariableLength()) {
                     ...
                     * the entry is a target for this reader. This has to be
                     * done before we read the variable portion of the header,
                     * because readData() only guarantees that it returns a
                     * dataBuffer that contains the next bytes that are needed,
                     * and has no guarantee that it holds any bytes that were
                     * previously read. The act of calling readData() to obtain
                     * the optional portion may reset the dataBuffer, and nudge
                     * the invariant part of the header out of the buffer
                    ...
                    /*
                     * Add to checksum while the buffer is positioned at the
                     * start of the new bytes.
                     */
                ...
                /*
                 * We've read the header of the next logrec. Move up our offsets
                 * if we're moving forward. If we're moving backwards, we set
                 * our offset before we read the header, because we knew where
                 * the entry started.
                 */
                if (forward) {
                    currentEntryOffset = nextEntryOffset;
                    nextEntryOffset += currentEntryHeader.getSize() + // header
                                                                      // size
    ...
    private void reportProblem(Exception e) {
        ...
        sb.append("Halted log file reading at file 0x").append(Long.toHexString(window.currentFileNum()))
                .append(" offset 0x").append(Long.toHexString(nextEntryOffset)).append(" offset(decimal)=")
                .append(nextEntryOffset).append(" prev=0x").append(Long.toHexString(currentEntryPrevOffset));

        if (currentEntryHeader != null) {
            LogEntryType problemType = LogEntryType.findType(currentEntryHeader.getType());
            sb.append(":\nentry=").append(problemType).append("type=").append(currentEntryHeader.getType())
                    .append(",version=").append(currentEntryHeader.getVersion()).append(")\nprev=0x")
                    .append(Long.toHexString(currentEntryPrevOffset)).append("\nsize=")
                    .append(currentEntryHeader.getItemSize()).append("\nNext entry should be at 0x")
    ...
    private void getLogEntryInReadBuffer()
         ...
         * we go backwards, we need to jump the buffer position. These methods
     ...
     * it's in the window. All we have to do is to check if we've gone past the
     * specified end point.
     * 
     * @throws DatabaseException
    ...
    protected void setBackwardPosition()
         ...
         * currentEntryOffset is the entry we just read (or the end of the file
        ...
        if ((currentEntryPrevOffset != 0) && window.containsOffset(currentEntryPrevOffset)) {
            ...
            if (finishLsn != DbLsn.NULL_LSN) {
                if (DbLsn.compareTo(nextLsn, finishLsn) == -1) {
                    throw new EOFException("finish=" + DbLsn.getNoFormatString(finishLsn) + "next="
            ...
            /*
             * The start of the log entry is not in this read buffer so we must
             * fill the buffer again. 1) The target log entry is in a different
             * file from the current window's file. Move the window to the
             * previous file and start the read from the target LSN. 2) The
             * target log entry is the same file but the log entry is larger
             * than the read chunk size. Start the next read buffer from the
             * target LSN. It's going to take multiple reads to get the log
             * entry, and we might as well get as much as possible. 3) In the
             * same file, and the log entry fits within one read buffer. Try to
             * position the next buffer chunk so the target entry is held within
             * the buffer, all the way at the end. That way, since we're reading
             * backwards, there will be more buffered data available for
             * following reads.
             */
            long nextFile;
            ...
            if (currentEntryPrevOffset == 0) {
                ...
                /*
                 * Check finishLSN before proceeding, in case we should stop the
                 * search before attempting to set the file reader to a position
                 * in the previous file. In [#22407] we threw a spurious EFE
                 * complaining that we cannot read backwards over a cleaned file
                 * because the previous file had been cleaned away.
                 */
                if (finishLsn != DbLsn.NULL_LSN && prevFileNum < DbLsn.getFileNumber(finishLsn)) {
                    throw new EOFException("finish=" + DbLsn.getNoFormatString(finishLsn) + " nextFile=0x"
                ...
                /*
                 * Case 2: The entry is in the same file, but is bigger than one
                 * buffer. Position it at the front of the buffer.
                 */
    ...
    /**
     * Read the basic log entry header, leaving the buffer mark at the beginning
     * of the checksummed header data.
     */
     ...
     * the data buffer is positioned just past the end of the invariant portion
     * of the log entry header.
     * 
     * @throws DatabaseException
     */
    ...
    private void startChecksum(ByteBuffer dataBuffer, boolean isChecksumTarget) throws ChecksumException {
        ...
        if (currentEntryHeader.isInvisible()) {
             ...
             */
            LogEntryHeader.turnOffInvisible(dataBuffer, originalPosition - LogEntryHeader.MIN_HEADER_SIZE);
        }
    ...
    private void validateChecksum(ByteBuffer dataBuffer, boolean isChecksumTarget) throws ChecksumException {
        ...
        cksumValidator.validate(currentEntryHeader.getChecksum(), window.currentFileNum(), currentEntryOffset);
    }
     ...
     * @param collectData is true if we need to actually look at the data. If
     *            false, we know we're skipping this entry, and all we need to
        ...
        while ((alreadyRead < amountToRead) && !eof) {
            ...
            if (window.hasRemaining()) {
                ...
                if (collectData) {
                     ...
                     */
                    int positionIncrement = (window.remaining() > bytesNeeded) ? bytesNeeded : window.remaining();
     ...
     * only, it may perform some actions and then decide whether the rest of the
     * logrec should be de-serialized or just skipped.
     * 
     * @return true if this reader should process the current logrec further,
     *         via the processEntry() method. A logrec must be passed to
     *         processEntry if the full logrec (not just the header) must be
     *         de-serialized for further processing. Return false if no further
     *         processing is needed, in which case the current logrec will be
     *         skipped (i.e, not returned to the caller of readNextEntry().
     ...
     *
     * @param entryBuffer A ByteBuffer that the logrec data and is positioned at
     *            the start of the logrec body (i.e., just after the logrec
     *            header).
    ...
    public static class EOFException extends Exception {
        ...
        /*
         * @param message The message is used to hold debugging information.
         */
    ...
    public boolean entryIsReplicated() {
        ...
        if (currentEntryHeader == null) {
            throw EnvironmentFailureException
                    .unexpectedState("entryIsReplicated should not be used before reader is " + "initialized");
        }
    ...
    protected void handleGapInBackwardsScan(long prevFileNum) {
        throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_INTEGRITY,
                "Cannot read backward over cleaned file" + " from " + window.currentFileNum() + " to " + prevFileNum);
    }
    ...
    protected static class ReadWindow {
         ...
         * fileNum = 9 startOffset = 100 endOffset = 300 Note that the end point
         * is not inclusive; endOffset is > the readBuffer's end.
         */
        ...
        public void positionBuffer(long targetOffset) {
            ...
            assert containsOffset(targetOffset) : this + " doesn't contain " + DbLsn.getNoFormatString(targetOffset);
        ...
        /*
         * Reposition to the specified file, and fill starting at startOffset.
         * Position the window's buffer to point at the log entry indicated by
         * targetOffset
         */
        public void slideAndFill(long windowfileNum, long windowStartOffset, long targetOffset, boolean forward)
            ...
            try {
                ...
                if (!foundData && !forward) {
                    throw EnvironmentFailureException.unexpectedState("Detected a log file gap when reading backwards. "
                            + "Target position = " + DbLsn.getNoFormatString(DbLsn.makeLsn(windowfileNum, targetOffset))
                            + " starting position = "
                            + DbLsn.getNoFormatString(DbLsn.makeLsn(windowfileNum, windowStartOffset))
                            + " end position = " + DbLsn.getNoFormatString(DbLsn.makeLsn(windowfileNum, endOffset)));
                }
            } finally {
                fileHandle.release();
            }
        ...
        /**
         * Fill up the read buffer with more data, moving along to the following
         * file (next largest number) if needed.
         * 
        ...
        protected boolean fillNext(boolean singleFile, int bytesNeeded)
            ...
            try {
                ...
                if (fillFromFile(fileHandle, startOffset)) {
                    /*
                     * Successfully filled the read buffer, but didn't move to a
                     * new file.
                     */
                    return false;
                }
         ...
         * false. In all cases, leave the the read buffer pointing at the target
         * offset and in a state that's ready to support reads, even if there is
         * nothing in the buffer. Note that the target offset, which may not be
         * the same as starting offset.
        ...
        protected boolean fillFromFile(FileHandle fileHandle, long targetOffset) throws DatabaseException {
            ...
            readBuffer.clear();
            if (fileManager.readFromFile(fileHandle.getFile(), readBuffer, startOffset, fileHandle.getFileNum(),
