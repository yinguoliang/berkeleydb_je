...
/**
 * Parameters passed when an item is logged. This class is used as a simple
 * struct for passing multiple params, and does not need getters and setters.
 */
public class LogParams {
    ...
    /**
     * Set of obsolete LSNs which are counted when logging a commit entry. This
     * information includes the DatabaseImpl for each LSN, and the nodeDb field
     * does not apply.
     */
    ...
    /**
     * Whether it is possible that the previous version of this log entry is
     * already marked obsolete. In general, the latest version of any IN or LN
     * is alive, so that logging a new version requires making the last version
     * obsolete. Utilization tracking generally asserts that this last version
     * is not already obsolete. When partial rollbacks are used, some of the
     * original intermediate versions may have been pruned away, leaving a
     * current previous that was already marked obsolete. For example, a
     * transaction might have done: LNA (version 1) LNA (version 2) -- now
     * version 1 is obsolete -- if we do a partial rollback to version1, verison
     * 2 is removed -- we start retransmitting LNA (version 2) When we log this
     * LNA/version2, this previous LNA (version1) is already obsolete.
