 ...
 * log. The caller may set "dumpCorruptedBounds" to true if information about
 * the start and finish of the corrupted portion should be displayed on stderr.
...
abstract public class ScavengerFileReader extends FileReader {
     ...
     */
    public ScavengerFileReader(EnvironmentImpl env, int readBufferSize, long startLsn, long finishLsn,
                               long endOfFileLsn)
            throws DatabaseException {

        super(env, readBufferSize, false, startLsn, null, // single file number
    ...
    public boolean readNextEntry() {
        ...
        try {
            ...
            resyncReader(DbLsn.makeLsn(window.currentFileNum(), saveCurrentEntryOffset), dumpCorruptedBounds);
    ...
    protected void handleGapInBackwardsScan(long prevFileNum) {
        if (!resyncReader(DbLsn.makeLsn(prevFileNum, DbLsn.MAX_FILE_OFFSET), false)) {
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_INTEGRITY,
                    "Cannot read backward over cleaned file" + " from " + window.currentFileNum() + " to "
                            + prevFileNum);
        }
    }
    ...
    protected boolean isTargetEntry() {
        if (currentEntryHeader.isInvisible()) {
            ...
            /*
             * This log entry is supposed to be effectivly truncated, so we know
             * this data is not alive.
             */
            return false;
        }
