 ...
 * <p>
 * The log entry on disk consists of a log header defined by LogManager and the
 * specific contents of the log entry.
 * <p>
 * Log entries that support replication are required to implement
 * {@link ReplicableLogEntry}.
 */
public interface LogEntry extends Cloneable {
     ...
     * entries as needed to ensure that a usable main object is available. For
     * an OldBINDeltaLogEntry, fetches the full BIN and merges the delta
     * information. This is necessary to return a Node main object. However, for
     * the new BINDeltaLogEntry, the full BIN is not fetched, since the partial
     * BIN (the delta) is usable as a Node.
     */
    ...
    /**
     * @return true if these two log entries are logically the same. Used for
     *         replication.
     */
