 ...
 * information about the database operation which instigated the logging of this
 * NameLN. This additional information is used to support replication of
 * database operations in a replication group. Database operations pose a
 * special problem for replication because unlike data record put and get calls,
 * they can result in multiple log entries that are not all members of a single
 * transaction. Create and truncate are the problem operations because they end
 * up logging new MapLNs, and our implementation does not treat MapLNs as
 * transactional. Database operations challenge two replication assumptions: (a)
 * that all logical operations can be repeated on the client node based on the
 * contents of a single log entry, and (b) that non-txnal log entries like
 * MapLNs need not be replicated. Specifically, here's what is logged for
 * database operations. create: 1. new NameLN_TX 2. new MapLN, which has the
 * database config info. 3. txn commit of autocommit or user txn. rename: 1.
 * deleted NameLN_TX 2. new NameLN_TX 3. txn commit from autocommit or user txn
 * truncate: 1. new MapLN w/new id 2. modify the existing NameLN with new id
 * (old database is deleted by usual commit-time processing) 3. txn commit from
 * autocommit or user txn delete 1. deleted NameLN_TX (old database gets deleted
 * by usual commit-time processing) 2. txn commit from autocommit or user txn
 * Extra information is needed for create and truncate, which both log
 * information within the MapLN. Rename and delete only log NameLNs, so they can
 * be replicated on the client using the normal replication messages. The extra
 * fields which follow the usual LNLogEntry fields are: operationType - the type
 * of database operation. In a single node system, this is local information
 * implicit in the code path. databaseConfig (optional) - For creates, database
 * configuration info databaseId (optional)- For truncates, the old db id, so we
 * know which MapLN to delete.
 */
public class NameLNLogEntry extends LNLogEntry<NameLN> {
    ...
    /*
     * operationType, truncateOldDbId and replicatedCreateConfig are logged as
     * part of the entry.
     */
     ...
     */
    public NameLNLogEntry(LogEntryType entryType, DatabaseId dbId, Txn txn, long abortLsn, boolean abortKD, byte[] key,
                          NameLN nameLN, ReplicationContext repContext) {

        super(entryType, dbId, txn, abortLsn, abortKD, null/* abortKey */, null/* abortData */,
                VLSN.NULL_VLSN_SEQUENCE/* abortVLSN */, 0 /* abortExpiration */, false /* abortExpirationInHours */,
                key, nameLN, false/* newEmbeddedLN */, 0 /* expiration */, false /* expirationInHours */);
    ...
    public boolean logicalEquals(LogEntry other) {
        ...
        if (replicatedCreateConfig != null) {
            if (!replicatedCreateConfig.logicalEquals(otherEntry.replicatedCreateConfig))
                return false;
        }
        return true;
    }
