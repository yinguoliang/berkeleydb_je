...
public class SecondaryCursor extends Cursor {
	 ...
	 *
	 * @param key
	 *            the secondary key returned as output. Its byte array does not
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if an invalid parameter is specified, for example, if a
	 *             DatabaseEntry parameter is null or does not contain a
	 ...
	 *
	 * @param pKey
	 *            the primary key used as input and returned as output. It must
	 ...
	 *
	 * When the primary record cannot be read (see readPrimaryAfterGet), advance
	 * over the unavailable record, according to the search type.
	 */
	 ...
	 *
	 * When the primary record cannot be read (see readPrimaryAfterGet), advance
	 * over the unavailable record.
	 */
	private OperationResult position(final DatabaseEntry key, final DatabaseEntry pKey, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final boolean first, final boolean lockPrimaryOnly) {
	...
	private OperationResult retrieveNext(final DatabaseEntry key, final DatabaseEntry pKey, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final GetMode getMode, final boolean lockPrimaryOnly) {
	 ...
	 * changes to the serializable algorithms. Currently we live with the fact
	 * that secondary access with serializable isolation is deadlock prone.
	 *
	 ...
	 * until we read (and lock) the primary record. However, the natural way to
	 * read via a secondary DB would be to read (and lock) the secondary record
	 * first to obtain the primary key, and then read (and lock) the primary
	 * record. Because this would obtain locks in the reverse order as write
	 * ops, a different approach is used for secondary reads.
	 *
	 * In order to avoid deadlocks, for non-serializable isolation we change the
	 * natural lock order for reads -- we only lock the primary record and then
	 * check the secondary record's reference to primary record. The initial
	 * read of the secondary DB is performed without acquiring locks
	 ...
	 * Cursor). Note that there is no need to lock the secondary before checking
	 * its reference to the primary, because during the check the secondary is
	 * protected from changes by the lock on the primary. If we discover that
	 * the secondary record has been deleted (for example, due to an update to
	 * the primary after the secondary dirty-read and before the primary locking
	 * read), the record will not be returned to the caller (it will be skipped)
	 * and we will advance to the next record according to the operation type.
	 * In this case the lock on the primary record is released.
	 *
	 * In addition, the READ_UNCOMMITTED_ALL mode is used for the dirty-read of
	 * the secondary DB. This ensures that we do not skip uncommitted deleted
	 * records. See LockMode.READ_UNCOMMITTED_ALL and Cursor.readPrimaryAfterGet
	 * for further details.
	 *
	 * For a secondary DB with dups, READ_UNCOMMITTED_ALL will return a deleted
	 * record for an open txn, and we'll discover the deletion when reading (and
	 * locking) the primary record. The primary lookup is wasted in that case,
	 * but this should be infrequent. For a secondary DB without dups,
	 * READ_UNCOMMITTED_ALL will block during the secondary read in this case (a
	 * deleted record for an open txn) in order to obtain the data (the primary
	 * key).
	 *
	...
	private OperationResult readPrimaryAfterGet(final DatabaseEntry key, final DatabaseEntry pKey,
			final DatabaseEntry data, final LockMode lockMode, final boolean secDirtyRead,
			final boolean lockPrimaryOnly, final OperationResult origResult) {
