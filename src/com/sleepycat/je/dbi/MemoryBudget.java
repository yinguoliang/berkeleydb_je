...
public class MemoryBudget implements EnvConfigObserver {
    ...
    /*
     * CLEANUP_DONE can be set to false for unit test debugging that is still in
     * progress. When we do the final regression, this should be removed to be
     * assured that it is never false.
     */
    public static boolean       CLEANUP_DONE                         = false;

    /*
     * These DEBUG variables are public so unit tests can easily turn them on
     * and off for different sections of code.
     */
     ...
     * behave predictably. _32 values are the same on Windows and Solaris. _64
     * values are from Linux (were previously 1.5.0_05 on Solaris). _OOPS are on
     * a 64b JVM with -XX:+UseCompressedOops The integer following the // below
     * is the Sizeof argument used to compute the value.
     */
    ...
    // 26
    private final static int    CHECKPOINT_REFERENCE_SIZE_32         = 40 + HASHSET_ENTRY_OVERHEAD_32;
    ...
    static {
        ...
        final boolean useCompressedOops = COMPRESSED_OOPS_KNOWN ? COMPRESSED_OOPS_KNOWN_ON : COMPRESSED_OOPS_REQUESTED;
     ...
     * worth the cost of synchronizing every time we access them. The worst that
     * can happen is that we may invoke eviction unnecessarily.
     */
    ...
    /*
     * Amount of memory cached for admininstrative structures that are sometimes
     * housed within tree nodes. Right now, that's DbFileSummaryMap, which is
     * sometimes referenced by a MapLN by way of a DatabaseImpl, and sometimes
     * is just referenced by a DatabaseImpl without a MapLN (the id and name
     * databases.)
     */
    ...
    /*
     * Memory available to JE, based on je.maxMemory and the memory available to
     * this process.
     */
    ...
    private long calcMaxMemory(DbConfigManager configManager) {
        ...
        if (newMaxMemory != 0) {
            ...
            if (jvmMemory < newMaxMemory) {
                throw new IllegalArgumentException(EnvironmentParams.MAX_MEMORY.getName() + " has a value of "
                        + newMaxMemory + " but the JVM is only configured for " + jvmMemory
                        + ". Consider using je.maxMemoryPercent.");
            }
            if (newMaxMemory < MIN_MAX_MEMORY_SIZE) {
                throw new IllegalArgumentException(EnvironmentParams.MAX_MEMORY.getName() + " is " + newMaxMemory
                        + " which is less than the minimum: " + MIN_MAX_MEMORY_SIZE);
            }
        } else {
     ...
     *            budget for a new environment. Note that a new environment has
     *            not yet been added to the set of shared cache environments.
     */
    void reset(long newMaxMemory, boolean newEnv, DbConfigManager configManager) throws DatabaseException {
         ...
         * below we make buffer and tracker sizes a fixed percentage (7% and 2%,
         * by default) of the total shared cache size. The math for this starts
         * by dividing the total size by number of environments to get
         ...
         * Note that when we resize the shared cache, we resize the buffer pools
         * and tracker budgets for all environments. Resizing the tracker budget
         * has no overhead, but resizing the buffer pools causes new buffers to
         * be allocated. If reallocation of the log buffers is not desirable,
         * the user can configure a byte amount rather than a percentage.
         */
        long myCachePortion;
        if (envImpl.getSharedCache()) {
         ...
         * If it is set, use that explicit setting. No point in having more log
         * buffers than the maximum size. If this starting point results in
         * overly large log buffers, reduce the log buffer budget again.
         */
        ...
        long newCriticalThreshold = (newMaxMemory
                * envImpl.getConfigManager().getInt(EnvironmentParams.EVICTOR_CRITICAL_PERCENTAGE)) / 100;

        long newTrackerBudget = (myCachePortion
                * envImpl.getConfigManager().getInt(EnvironmentParams.CLEANER_DETAIL_MAX_MEMORY_PERCENTAGE)) / 100;

        long newMinTreeMemoryUsage = Math.min(configManager.getLong(EnvironmentParams.MIN_TREE_MEMORY),
    ...
    /**
     * Returns whether eviction of INList information is allowed. To prevent
     * extreme cache churn, eviction of Btree information is prohibited unless
     * the tree memory usage is above this minimum value.
     */
    ...
    public void updateLockMemoryUsage(long increment, int lockTableIndex) {
        if (increment != 0) {
            ...
            assert lockMemoryUsage.get() >= 0 : makeErrorMessage("lockMem", lockMemoryUsage.get(), increment);
            if (DEBUG_LOCK) {
                if (increment > 0) {
                    System.err.println(
                            "INC-------- =" + increment + " lock[" + lockTableIndex + "] " + lockMemoryUsage.get());
                } else {
                    System.err.println(
    ...
    public long getLocalCacheUsage() {
        return (logBufferBudget + treeMemoryUsage.get() + dosMemoryUsage.get() + adminMemoryUsage.get()
                + treeAdminMemoryUsage.get() + getLockMemoryUsage());
    }

    long getVariableCacheUsage() {
        return (treeMemoryUsage.get() + dosMemoryUsage.get() + adminMemoryUsage.get() + treeAdminMemoryUsage.get()
                + getLockMemoryUsage());
    }
     ...
     * overhead that can be used for the first N elements -- however many fit in
     * 4 bytes. On 64b Java, there is no extra space included. In all cases,
     * space is allocated in 8 byte chunks.
     */
    ...
    StatGroup loadStats() {
        ...
        new LongStat(stats, MB_SHARED_CACHE_TOTAL_BYTES, totals.isSharedCache() ? totals.getCacheUsage() : 0);
    ...
    /**
     * Common base class for shared and private totals. This abstraction allows
     * most other classes to be unaware of whether we're using a SharedEvictor
     * or PrivateEvictor.
     */
