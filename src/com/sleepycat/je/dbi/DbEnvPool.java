 ...
 * close, supporting this from multiple threads by synchronizing on the pool. To
 * avoid multiple environment openings from being blocked by recovery in
 * getEnvironment(), the EnvironmentImpl constructor is broken into two parts,
 * with the second part (EnvironmentImpl.finishInit) doing the recovery. When
 * synchronizing on two or more of the following objects the synchronization
 * order must be as follows. Synchronization is not performed in constructors,
 * of course, because no other thread can access the object. Synchronization
 * order: Environment, DbEnvPool, EnvironmentImpl, Evictor
 */
public class DbEnvPool {
     ...
     */
    public EnvironmentImpl getEnvironment(File envHome, EnvironmentConfig config, boolean checkImmutableParams,
        ...
        synchronized (this) {
            ...
            if (envImpl != null) {
                ...
                if (!envImpl.isReplicated() && (repConfigProxy != null)) {
                    throw new UnsupportedOperationException("This environment was previously opened as a "
                            + "standalone environment. It cannot be re-opened for " + "replication.");
                }

                /*
                 * If the envImpl instance returned is replicated, but users are
                 * actually creating a standalone environment and it is not read
                 * only, then throw out an UnsupportedOperationException.
                 */
                if (envImpl.isReplicated() && (repConfigProxy == null) && !config.getReadOnly()) {
                    throw new UnsupportedOperationException("This environment was previously opened for "
                            + "replication. It cannot be re-opened in read/write " + "mode for standalone operation.");
                }
                ...
                if (checkImmutableParams) {
                     ...
                     * An exception is thrown if the check fails. Don't do this
                     * check if we create the environment here because the
                     * creation might modify the parameters, which would create
                     * a Catch-22 in terms of validation. For example,
                     * je.maxMemory will be overridden if the JVM's -mx flag is
                     * less than that setting, so the new resolved config
                 ...
                 */
                envImpl = (repConfigProxy == null) ? new EnvironmentImpl(envHome, config, sharedCacheEnv)
         ...
         * environment. Note that finishInit must be called even if an existing
         * envImpl was found, because initialization (recovery) for that envImpl
         * may not be complete. finishInit is synchronized and this ensures that
         * recovery will be complete when it returns. If this environment
         * finishInit() fails in any way, make sure it is removed from the envs
         * map. If it isn't, it will block all future attempts to create the
         * environment.
         */
     ...
     */
    private EnvironmentImpl loadRepImpl(File envHome, EnvironmentConfig config, EnvironmentImpl sharedCacheEnv,
                                        RepConfigProxy repConfigProxy)
        ...
        try {
            ...
            return (EnvironmentImpl) repClass.getConstructor(envHome.getClass(), config.getClass(),
                    Class.forName(envImplName), Class.forName(repProxy))
     ...
     * setMutableConfig operation while synchronized on the DbEnvPool. In theory
     * we shouldn't need to synchronize here when envImpl.getSharedCache() is
     * false; however, we synchronize unconditionally to standardize the
     * synchronization order and avoid accidental deadlocks.
     */
    synchronized void setMutableConfig(EnvironmentImpl envImpl, EnvironmentMutableConfig mutableConfig)
            throws DatabaseException {
     ...
     * synchronized on the DbEnvPool. Synchronization on this DbEnvPool during
     * the close is used to protect its data structures. Unfortunately, this
     * means that a long checkpoint during a close will block other closes and
     * opens. We may want to improve this in the future. However, at least there
     * is a user workaround: perform a full checkpoint before closing the
     * environment.
     */
    ...
    private void removeEnvironment(EnvironmentImpl envImpl) {
        ...
        /*
         * Latch notes may only be cleared when there is no possibility that any
         * environment is open.
         */
        if (envs.isEmpty()) {
            LatchSupport.clear();
     ...
     *            the total should remain unchanged. A total greater than zero
     *            is given when it has changed via setMutableConfig.
     * @param skipEnv is an environment that should not be reset, or null.
     *            Non-null is passed when an environment has already been reset
     *            because it was just created or the target of setMutableConfig.
     */
    private void resetSharedCache(long newMaxMemory, EnvironmentImpl skipEnv) throws DatabaseException {
        ...
        for (EnvironmentImpl envImpl : sharedCacheEnvs) {
             ...
             * not yet been removed. They aren't usable, and we expect them to
             * be closed and removed very soon.
             */
