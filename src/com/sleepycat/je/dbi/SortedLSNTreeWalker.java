 ...
 * latter may require random access. SortedLSN walking does not obey cursor and
 * locking constraints, and therefore can only be guaranteed consistent for a
 * quiescent tree which is not being modified by user or daemon threads. The
 * class walks over the tree using sorted LSN fetching for parts of the tree
 * that are not in memory. It returns LSNs for each node in the tree,
 * <b>except</b> the root IN, in an arbitrary order (i.e. not key order). The
 * caller is responsible for getting the root IN's LSN explicitly.
 * <p>
 ...
 * processing. LSNs of the children of those nodes are retrieved and the process
 * repeated until there are no more nodes to be fetched for this database's
 * tree. LSNs are accumulated in batches in this phase so that memory
 * consumption is not excessive. For instance, if batches were not used then the
 * LSNs of all of the BINs would need to be held in memory.
 */
public class SortedLSNTreeWalker {
    ...
    public interface TreeNodeProcessor {
        void processLSN(long childLSN, LogEntryType childType, Node theNode, byte[] lnKey, int lastLoggedSize)
     ...
     * TreeNodeProcessor callback method. Even if true, dup LNs are not fetched
     * because they are normally never used (see accumulateDupLNs).
     */
     ...
     * index of each LSN processed by the tree walker. Since fetchLSN is called
     * with an arbitrary LSN, and since when we fetch (for preload) we need to
     * setup the parent to refer to the node which we are prefetching, we need
     * to have the parent in hand at the time of the call to fetchLSN. This map
     * allows us to keep a reference to that parent so that we can call
     * fetchNode on that parent. It is also necessary to maintain this map for
     * cases other than preload() so that during multi-db walks (i.e. multi db
     * preload), we can associate an arbitrary LSN back to the parent IN and
     * therefore connect a fetch'ed Node into the proper place in the tree. LSN
     * -> INEntry
     */
     ...
     * in LSN order as usual. When the full BIN is fetched, the DeltaINEntry is
     * used to apply the delta and place the result in the tree.
     */
    ...
    /*
     * @param dbImpls an array of DatabaseImpls which should be walked over in
     * disk order. This array must be parallel to the rootLsns array in that
     * rootLsns[i] must be the root LSN for dbImpls[i].
     * @param setDbState if true, indicate when the INList harvest has completed
     * for a particular DatabaseImpl.
     * @param rootLsns is passed in addition to the dbImpls, because the root
     * may be nulled out on the dbImpl before walk() is called.
     ...
     * should be ignored.
     */
    public SortedLSNTreeWalker(DatabaseImpl[] dbImpls, boolean setDbState, long[] rootLsns, TreeNodeProcessor callback,
                               List<DatabaseException> savedExceptions, ExceptionPredicate excPredicate) {

        if (dbImpls == null || dbImpls.length < 1) {
            throw EnvironmentFailureException
                    .unexpectedState("DatabaseImpls array is null or 0-length for " + "SortedLSNTreeWalker");
        }
        ...
        for (DatabaseImpl di : dbImpls) {
            ...
            if (ei == null) {
                throw EnvironmentFailureException
                        .unexpectedState("environmentImpl is null for target db " + di.getDebugName());
            }

            if (ei != this.envImpl) {
                throw new IllegalArgumentException("Environment.preload() must be called with Databases "
                        + "which are all in the same Environment. (" + di.getDebugName() + ")");
            }
        }
     ...
     * non-resident children. On entering this method, parent is latched and
     * remains latched on exit.
     */
    private void accumulateLSNs(final IN parent, final LSNAccumulator pendingLSNs, final IN ohBinParent,
         ...
         * depends on the dup format. Preload works with the old dup format and
         * the new. In the new dup format (or after dup conversion), BINs
         * contain only LNs and no DBINs exist. In the old dup format, DBINs
         * contain only LN children, but BINs may contain a mix of LNs and DINs.
         */
         ...
         */
        final boolean accumulateChildren = !allChildrenAreLNs || (dups ? accumulateDupLNs : accumulateLNs);
        ...
        for (int i = 0; i < parent.getNEntries(); i += 1) {
            ...
            final byte[] lnKey = (allChildrenAreLNs || (childCached && child.isLN())) ? parent.getKey(i) : null;
            ...
            if (parentBin != null && parentBin.isDefunct(i)) {
                 ...
                 * LSNs to be sorted and fetched in phase 2. But don't do this
                 * if the child is an embedded LN.
                 */
                if (!parent.isEmbeddedLN(i)) {
                    ...
                    processChild(DbLsn.NULL_LSN, null /* child */, lnKey, 0 /* lastLoggedSize */, pendingLSNs, null,
                            -1);
                }
                 ...
                 */
                processChild(lsn, null /* child */, lnKey, parent.getLastLoggedSize(i), pendingLSNs, null, -1);
            }

            /*
             * If we've exceeded the batch size then process the current batch
             * and start a new one.
             */
    ...
    private void processChild(final long lsn, final Node child, final byte[] lnKey, final int lastLoggedSize,
                              final LSNAccumulator pendingLSNs, final IN ohBinParent, final int ohBinIndex) {
         ...
         */
        callProcessLSNHandleExceptions(lsn, (!childCached ? LogEntryType.LOG_INS_LN
                /* Any LN type will do */ : child.getGenericLogType()), child, lnKey, lastLoggedSize);
    ...
    private void fetchAndProcessLSN(long lsn, LSNAccumulator pendingLSNs) {
        ...
        try {
            callProcessLSNHandleExceptions(lsn, result.node.getGenericLogType(), result.node, lnKeyEntry.getData(),
    ...
    private void callProcessLSNHandleExceptions(long childLSN, LogEntryType childType, Node theNode, byte[] lnKey,
        ...
        try {
            ...
            if (excPredicate == null || !excPredicate.ignoreException(e)) {
                dbe = new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_FILE_NOT_FOUND, e);
            }
     ...
     * getResidentRootIN and/or getRootIN to modify behavior. getResidentRootIN
     * is called first,
     */
    ...
    private static class FetchResult {
        ...
        FetchResult(final Node node, final int lastLoggedSize, final IN ohBinParent, final int ohBinIndex) {
            this.node = node;
    ...
    private FetchResult fetchLSN(long lsn, DatabaseEntry lnKeyEntry, LSNAccumulator pendingLSNs) {
        ...
        try {
            /*
             * When the indexed slot contains an off-heap BIN, the node to fetch
             * is an LN within the off-heap BIN or the full BIN to merge with an
             * off-heap BIN-delta.
             */
            ...
            if (residentNode != null) {
                /*
                 * If the resident node is not a delta then concurrent activity
                 * (e.g., log cleaning) must have loaded the node. Just return
                 * it and continue.
                 */
            ...
            try {
                ...
                final String msg = (fetchAndInsertIntoTree() ? "Preload failed" : "SortedLSNTreeWalker failed")
                        + " dbId=" + dbImpl.getId() + " isOffHeapBinInTree=" + isOffHeapBinInTree + " isLnInOffHeapBin="
                        + isLnInOffHeapBin + " deltaObject=" + (deltaObject != null) + " residentNode="
                        + (residentNode != null);
            ...
            /*
             * If there is a delta, then this is the full BIN to which the delta
             * must be applied. The delta LSN is the last logged LSN.
             */
            ...
            if (fetchAndInsertIntoTree()) {
                ...
                final boolean storeOffHeap = preloadIntoOffHeapCache
                        && memBudget.getCacheMemoryUsage() > memBudget.getMaxMemory();

                /*
                 * Note that UINs are always stored in the main cache even if it
                 * is full. The idea is that LNs and BINs should be evicted from
                 * main to make room. When the main cache fills with UINs, and
                 * an off-heap cache is also being filled, we currently allow
                 * the main cache to overflow.
                 */
                if (isOffHeapBinInTree || (storeOffHeap && !ret.isUpperIN())) {
                    if (ret.isLN()) {
                        /*
                         * Store LN off-heap. If an oh LN was added to an oh BIN
                         * we must re-store the oh BIN as well. This is
