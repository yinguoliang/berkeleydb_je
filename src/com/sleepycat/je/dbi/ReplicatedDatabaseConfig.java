 ...
 * persistent. This class is logged as part of a nameLN so that databases can be
 * created on replica nodes with the correct configuration.
 */
public class ReplicatedDatabaseConfig extends BasicVersionedWriteLoggable {
    ...
    /** For writing */
    ReplicatedDatabaseConfig(byte flags, int maxTreeEntriesPerNode, byte[] btreeComparatorBytes,
                             byte[] duplicateComparatorBytes, byte[][] triggerBytes) {
    ...
    /**
     * Create a database config for use on the replica which contains all the
     * configuration options that were conveyed by way of this class.
     */
    public DatabaseConfig getReplicaConfig(EnvironmentImpl envImpl) {
         ...
         * Replica has been.
         */
        replicaConfig.setKeyPrefixing(DatabaseImpl.getKeyPrefixing(flags) || DatabaseImpl.getSortedDuplicates(flags));
        ...
        DatabaseImpl.ComparatorReader reader = new DatabaseImpl.ComparatorReader(btreeComparatorBytes, "Btree",
        ...
        reader = new DatabaseImpl.ComparatorReader(duplicateComparatorBytes, "Duplicate", envImpl.getClassLoader());
        if (reader.isClass()) {
        ...
        replicaConfig.setTriggers(TriggerUtils.unmarshallTriggers(null, triggerBytes, envImpl.getClassLoader()));
    ...
    public int getLogSize(final int logVersion, final boolean forReplication) {
        return 1 + // flags, 1 byte
                LogUtils.getPackedIntLogSize(maxTreeEntriesPerNode) + LogUtils.getByteArrayLogSize(btreeComparatorBytes)
                + LogUtils.getByteArrayLogSize(duplicateComparatorBytes) + TriggerUtils.logSize(triggerBytes);
    }
    ...
    public void dumpLog(StringBuilder sb, boolean verbose) {
        ...
        sb.append("\" dupcmpSet=\"").append(duplicateComparatorBytes != LogUtils.ZERO_LENGTH_BYTE_ARRAY).append("\"");
    ...
    public boolean logicalEquals(Loggable other) {
        ...
        if (!Arrays.equals(duplicateComparatorBytes, otherConfig.duplicateComparatorBytes)) {
            return false;
        }
