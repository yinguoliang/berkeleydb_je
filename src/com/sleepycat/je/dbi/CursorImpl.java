...
public class CursorImpl implements Cloneable {
    ...
    /*
     * Cursor location in the dbImpl, represented by a BIN and an index in the
     * BIN. The bin is null if not established, and the index is negative if not
     * established.
     */
    ...
    /**
     * Creates a cursor. A cursor always retains transactional locks when it is
     * reset or closed. Non-transaction locks may be retained or not, depending
     * on the retainNonTxnLocks parameter value. Normally a user-created
     * non-transactional Cursor releases locks on reset and close, and a
     * ThreadLocker is normally used. However, by passing true for
     * retainNonTxnLocks a ThreadLocker can be made to retain locks; this
     * capability is used by SecondaryCursor.readPrimaryAfterGet. For internal
     * (non-user) cursors, a BasicLocker is often used and locks are retained.
     * In these internal use cases the caller explicitly calls
     ...
     * BasicLocker from being released when the cursor is closed. BasicLocker is
     * also used for NameLN operations while opening a Database handle. Database
     * handle locks must be retained, even if the Database is opened
     * non-transactionally.
     *
     * @param retainNonTxnLocks is true if non-transactional locks should be
     *            retained (not released automatically) when the cursor is reset
     *            or closed.
    ...
    public CursorImpl(DatabaseImpl dbImpl, Locker locker, boolean retainNonTxnLocks, boolean isSecondaryCursor) {
         ...
         * (mainly for internal databases). When the CursorImpl is created by a
         * Cursor, CursorImpl.setCacheMode will be called.
         */
     ...
     *            cursor, and addCursor is called on the new cursor to register
     *            it with the current BIN. If false, the new cursor will be
     *            uninitialized.
     */
     ...
     *
     * @param closingCursor the old cursor that will be closed if the new cursor
    ...
    /**
     * Called when a cursor move operation is complete. Clears the closingLocker
     * so that a reference to the old closed locker is not held.
     */
    ...
    /**
     * Returns the key at the current position, regardless of whether the record
     * is defunct. Does not lock. The key returned is not a copy and may not be
     * returned directly to the user without copying it first. The cursor must
     * be initialized. TODO: The returned byte array is normally, but not always
     * a copied, and then copied again into the user's DatabaseEntry. If this
     * method always returns a copy, the extra copy into DatabaseEntry could be
     * avoided.
     */
     ...
     * Close a cursor.
     *
     * @param newCursor is another cursor that is kept open by the parent Cursor
     *            object, or null if no other cursor is kept open.
     */
    ...
    private void performPriorBINEviction() {
        ...
        if (cacheMode == CacheMode.DEFAULT || cacheMode == CacheMode.KEEP_HOT || cacheMode == CacheMode.EVICT_LN) {
            return;
        }
     ...
     * but eviction should be enabled for user cursors. Eviction is disabled by
     * default.
     */
    ...
    public void criticalEviction() {
         ...
         * Operations using these modes for a non-dirty BIN generally do not add
         * any net memory to the cache, so they shouldn't have to perform
         * critical eviction or block while another thread performs eviction.
         */
        if (allowEviction && ((bin != null && bin.getDirty()) || (cacheMode != CacheMode.UNCHANGED
                && cacheMode != CacheMode.EVICT_BIN && cacheMode != CacheMode.MAKE_COLD))) {
     ...
     * CursorImpl.close(). By default, the CacheMode returned by
     * DatabaseImpl.getCacheMode is used, and the defaults specified by the user
     * for the Database or Environment are applied. However, the default mode
     * can be overridden by the user by calling Cursor.setCacheMode, and the
     * mode may be changed prior to each operation, if desired. To implement a
     * per-operation CacheMode, two CacheMode fields are maintained.
     * Cursor.cacheMode is the mode to use for the next operation.
     ...
     * or reset. This method must be called with the BIN latched but may release
     * it, namely when the BIN is evicted.
     */
    ...
    private boolean shouldEmbedLN(byte[] data) {
        ...
        return data.length <= dbImpl.getEnv().getMaxEmbeddedLN() && !dbImpl.getSortedDuplicates()
                && !dbImpl.getDbType().isInternal();
    }

    /**
     * Delete the item pointed to by the cursor. If the item is already defunct,
     * return KEYEMPTY. Returns with nothing latched.
     */
    public OperationResult deleteCurrentRecord(ReplicationContext repContext) {
        ...
        try {
             ...
             */
            final LockStanding lockStanding = lockLN(LockType.WRITE, true /* allowUncontended */, false /* noWait */);
             ...
             * are true: - CLEANER_FETCH_OBSOLETE_SIZE is configured and
             * lastLoggedSize is unknown - this database does not use the
             * standard LN class and we cannot call DbType.createdDeletedLN
             * further below For other cases, we are careful not to fetch, in
             * order to avoid a random read during a delete operation.
             */
            LN ln;
            if ((currLoggedSize == 0 && !currEmbeddedLN && envImpl.getCleaner().getFetchObsoleteSize(dbImpl))
            ...
            logItem = ln.optionalLog(envImpl, dbImpl, locker, wli, currEmbeddedLN /* newEmbeddedLN */,
                    currKey /* newKey */, expiration, expirationInHours, currEmbeddedLN, currLsn, currLoggedSize,
                    false/* isInsertion */, repContext);

            /*
             * Now update the parent BIN to reference the logrec written above,
             * set the PD flag on, and do the BIN memory counting.
             */
             ...
             * must be in the tree so that a logrec will be generated when a
             * db.sync() occurs later (that logrec is needed for crash recovery,
             * because BINs are not replayed during crash recovery). If the LN
             * child is cached, it is desirable to evict it because as long as
             * the PD flag is on, the record's data will never be accessed. But
             * for DW DBs we should not evict the dirty LN since it will be
    ...
    /**
     * Modify the current record with the given data, and optionally replace the
     * key.
     *
     ...
     */
    public OperationResult updateCurrentRecord(DatabaseEntry key, DatabaseEntry data, ExpirationInfo expInfo,
                                               DatabaseEntry returnOldData, DatabaseEntry returnNewData,
        ...
        try {
            ...
            if (!lockStanding.recordExists()) {
                ...
                result = updateRecordInternal((key != null ? Key.makeKey(key) : null), data, expInfo, returnOldData,
                        returnNewData, lockStanding, repContext);
            }
    ...
    /**
     * Insert the given record (key + LN) in the tree or return false if the key
     * is already present. The cursor must initially be uninitialized. This
     * method is called directly internally for putting tree map LNs and file
     * summary LNs.
     */
    public boolean insertRecord(byte[] key, LN ln, boolean blindInsertion, ReplicationContext repContext) {
        ...
        try {
            final Pair<LockStanding, OperationResult> result = insertRecordInternal(key, ln, null /* expirationInfo */,
                    blindInsertion, null /* returnNewData */, repContext);
    ...
    /**
     * Insert or update a given record. The method searches for the record using
     * its key. It will perform an update if the record is found, otherwise an
     * insertion. The cursor must initially be uninitialized. Called by all the
     * Cursor.putXXX() ops, except putCurrent().
     *
     ...
     *         updates. Is null only if an expired LN was purged and a partial
     *         'data' param was supplied.
     */
    public OperationResult insertOrUpdateRecord(final DatabaseEntry key, final DatabaseEntry data, final LN ln,
        ...
        try {
             ...
             * whose key is equal to "key", with the LSN of that slot locked in
             * WRITE mode, and with the containing BIN latched.
             */
            Pair<LockStanding, OperationResult> insertResult = null;
            insertResult = insertRecordInternal(keyCopy, ln, expInfo, false, returnNewData, repContext);
     ...
     * non-defunct record does not exist already with the given key. The cursor
     * must initially be uninitialized. On return, this.bin is latched.
     *
     * @return a non-null pair of LockStanding and OperationResult. +
     *         LockStanding will be non-null if a slot with the given key
     *         already exists, whether or not we reuse the slot for this record
     *         (i.e., whether or not the result is non-null). In other words, we
     *         always lock the record in an existing slot for the give key. +
     *         OperationResult will be non-null if we inserted a slot or reused
     *         a slot having a defunct record, or null if the insertion failed
     *         because a non-defunct record exists with the given key.
     */
    private Pair<LockStanding, OperationResult> insertRecordInternal(final byte[] key, final LN ln,
         ...
         * registered with the BIN, then splits won't be allowed. This is a good
         * reason to use non-sticky cursors for insertions, especially
         ...
         * reject the operation (e.g., if writing on a replica) and also prepare
         * to undo in the (very unlikely) event that logging succeeds but
         * locking fails. Call this method BEFORE slot insertion, in case it
         * throws an exception which would leave the slot with a null LSN. For
         * Txn, creates the writeInfo map (if not done already), and inserts
         * dbImpl in the undoDatabases map. Noop for other non-HA lockers.
         */
        locker.preLogWithoutLock(dbImpl);

        /*
         * If the key exists already, insertEntry1() does not insert, but
         * returns the index of the existing key. If bin is a delta and it does
         * not contain the key, then: (a) if blindInsertion is false,
         * insertEntry1() will mutate it to a full BIN and check again if the
         * key exists or not. (b) if blindInsertion is true, insertEntry1() will
         * not mutate the delta; it will just insert the key into the delta.
         * This is OK, because blindInsertion will be true only if we know
         * already that the key does not exist in the tree.
         */
        ...
        if ((insertIndex & IN.INSERT_SUCCESS) == 0) {
            ...
            /*
             * Lock the LSN for the existing LN slot, and check defunct-ness. An
             * uncontended lock request is permitted because we are holding the
             * bin latch. If no locker holds a lock on the slot, then no lock is
             * taken by this cursor either.
             */
            lockStanding = lockLN(LockType.WRITE, true /* allowUncontended */, false /* noWait */);
            ...
            /*
             * The record in the current slot is defunct. Note: it may have been
             * made defunct by this.locker itself.
             */
            currLsn = lockStanding.lsn;
            currEmbeddedLN = bin.isEmbeddedLN(index);

            /*
             * Create a new WriteLockInfo or get an existing one for the LSN of
             * the current slot, and set its abortLSN and abortKD fields, if
             * needed, i.e, if it is not the current txn the one who created
        ...
        try {
            logItem = ln.optionalLog(envImpl, dbImpl, locker, wli, newEmbeddedLN, key, expInfo.expiration,
                    expInfo.expirationInHours, currEmbeddedLN, currLsn, 0/* currSize */, true/* isInsertion */,
                    repContext);
        } finally {
            if (logItem == null && !isSlotReuse) {
                 ...
                 * prevent an exception during a future fetchLN() call, we set
                 * the KD flag. We do not call BIN.deleteEntry because it does
                 * not adjust cursors. We do not add this entry to the
        ...
        if (lockStanding == null) {
            /*
             * No slot reuse; straight insertion. Update LSN in BIN slot. The LN
             ...
             * This can happen for FileSummaryLN's only (it is a noop for other
             * kinds of LNs). To avoid violating assertions (e.g., in
             * IN.changeMemorySize), we must must finish the memory adjustment
             * while the BIN is still latched. [#20069] This special handling
             * does not apply to slot reuse, because the updateEntry() version
             * used in the slot reuse case will recalc the BIN memory from
             * scratch, and as a result, will take into account the extra
             * marshaled memory. [#20845]
             */
             ...
             * new key is non-identical to the key in the slot but is considered
             * equal by the btree comparator.
             */
            bin.insertRecord(index, ln, logItem.lsn, logItem.size, key, data, expInfo.expiration,
         ...
         */
        if (dbImpl.getSortedDuplicates() && !dbImpl.isDeferredWriteMode() && bin.getTarget(index) != null) {
            bin.evictLN(index);
        }

        traceInsert(Level.FINER, bin, logItem.lsn, index);

        return new Pair<>(lockStanding, DbInternal.makeResult(expInfo.expiration, expInfo.expirationInHours));
    }

    /**
     * Update the record where the cursor is currently positioned at. The cursor
     * is registered with this position, the associated bin is latched, the BIN
     * slot is not defunct, and it has been locked in WRITE mode.
     *
     * @param returnOldData if non-null, will be filled in with the pre-existing
     *            record's data. However, if an expired LN was purged, it will
     *            not be filled in and the caller should expect this; see
     *            {@link Cursor#putNotify}.
     * @return OperationResult, or null if an expired LN was purged and a
     *         partial 'data' param was supplied.
     */
    private OperationResult updateRecordInternal(final byte[] key, final DatabaseEntry data,
                                                 final ExpirationInfo expInfo, final DatabaseEntry returnOldData,
                                                 final DatabaseEntry returnNewData, final LockStanding lockStanding,
                                                 final ReplicationContext repContext) {
        ...
        /*
         * Must fetch LN if it is not embedded and any of the following are
         * true: - returnOldData is non-null: data needs to be returned - data
         * is a partial entry: needs to be resolved -
         * CLEANER_FETCH_OBSOLETE_SIZE is configured and lastLoggedSize is
         * unknown - this database does not use the standard LN class and we
         * cannot call DbType.createdUpdatedLN further below (this is the case
         * for NameLNs, MapLNs, and FileSummaryLNs). For other cases, we are
         * careful not to fetch, in order to avoid a random read during an
         * update operation.
         */
        LN ln;
        if (returnOldData != null || data.getPartial()
                || (currLoggedSize == 0 && !currEmbeddedLN && envImpl.getCleaner().getFetchObsoleteSize(dbImpl))
        ...
        /*
         * If the key is changed (according to the comparator), we assume it is
         * actually the data that has changed for a duplicate's DB.
         */
        if (key != null && Key.compareKeys(currKey, key, dbImpl.getKeyComparator()) != 0) {
            throw new DuplicateDataException("Can't replace a duplicate with new data that is not "
                    + "equal to the existing data according to the duplicate " + " comparator.");
        }
        ...
        /*
         * Create a new WriteLockInfo or get an existing one for the LSN of the
         * current slot, and set its abortLSN and abortKD fields, if needed,
         * i.e, if it is not the current txn the one who created this LSN. The
         * abortLSN and abortKD fields of the wli will be included in the new
         * logrec.
         */
        final WriteLockInfo wli = lockStanding.prepareForUpdate(bin, index);

        /* Log the new record version and lock its new LSN . */
        logItem = ln.optionalLog(envImpl, dbImpl, locker, wli, newEmbeddedLN, (key != null ? key : currKey), expiration,
                expirationInHours, currEmbeddedLN, currLsn, currLoggedSize, false/* isInsertion */, repContext);
         ...
         */
        bin.updateRecord(index, oldLNMemSize, logItem.lsn, ln.getVLSNSequence(), logItem.size, key,
                (newEmbeddedLN ? newData : null), expiration, expirationInHours);

        /*
         * If the LN child is not cached, attach it to the tree if the DB is a
         * DW one or if the record is not embedded in the BIN. For DW DBs, we
         * must attach the LN even if the record in embedded, because no logrec
         * was generated above, and as a result, the LN must be in the tree so
         * that a logrec will be generated when a db.sync() occurs later (that
         * logrec is needed for crash recovery, because BINs are not replayed
         * during crash recovery). If the LN child is cached, it is desirable to
         * evict it if the record is embedded because it will never be fetched
         * again. But for DW DBs we should not evict a dirty LN since it will be
         * logged unnecessarily.
         */
        final boolean shouldCache = (dbImpl.isDeferredWriteMode() || (!dbImpl.getSortedDuplicates() && !newEmbeddedLN));
     ...
     * if this record is defunct. The cursor must initially be uninitialized.
        ...
        try {
            ...
            if (bin != null) {
                ...
                if (bin.getNEntries() == 0) {
                     ...
                     * moving to the first non-defunct entry.
                     */
                    found = true;
                    index = -1;
                } else {
                    ...
                    if (treeStatsAccumulator != null && !bin.isEntryKnownDeleted(index)
                            && !bin.isEntryPendingDeleted(index)) {
                        treeStatsAccumulator.incrementLNCount();
                    }

                    /*
                     * Even if the entry is defunct, just leave our position
                     * here and return.
                     */
                    found = true;
                }
     ...
     * to the given search key. To be more precise, let K1 be search key. The
     * method positions the cursor on the BIN that should contain K1. If the BIN
     * does contain K1, this.index is set to the containing slot. Otherwise,
     * this.index is set to the right-most slot whose key is < K1, or to -1 if
     * K1 is < than all keys in the BIN. The cursor must initially be
     * uninitialized. The method returns with the BIN latched, unless an
     * exception is raised. The method returns an integer that encodes the
     * search outcome: If the FOUND bit is not set, the tree is completely empty
     * (has no BINs). If the FOUND bit is set, the EXACT_KEY bit says whether K1
     * was found or not and the FOUND_LAST bit says whether the cursor is
     * positioned to the very last slot of the BTree (note that this state can
     * only be counted on as long as the BIN is latched). Even if the search
     * returns an exact result, the record may be defunct. The caller must
    ...
    public int searchRange(DatabaseEntry searchKey, Comparator<byte[]> comparator) {
        ...
        try {
            ...
            bin = dbImpl.getTree().search(key, Tree.SearchType.NORMAL, binBoundary, cacheMode, comparator);
            ...
            if (bin != null) {
                ...
                if (bin.isBINDelta() && comparator != null) {
                     ...
                     * used. Otherwise, if we positioned the cursor on the delta
                     * using the non-null comparator, we would not be able to
                     * adjust its position correctly later when the delta gets
                     * mutated for some reason (because at that later time, the
                     * comparator used here would not be known).
                     */
                ...
                if (bin.isBINDelta() && (index < 0 || (index & IN.EXACT_MATCH) == 0 || binBoundary.isLastBin)) {

                    /*
                     * Note: if binBoundary.isLastBin, we must mutate the BIN in
                     * order to compute the foundLast flag below.
                     */
                    bin.mutateToFullBIN(false /* leaveFreeSlot */);
                 ...
                 * to reposition "this" inside mutateToFullBIN(), which would be
                 * both unnecessary and wrong given that this.index could have
                 * the IN.EXACT_MATCH still on.
                 */
                addCursor(bin);
            }
            ...
            return ((foundSomething ? FOUND : 0) | (foundExactKey ? EXACT_KEY : 0) | (foundLast ? FOUND_LAST : 0));
     ...
     * lockLNAndCheckDefunct() method. The cursor must initially be
     * uninitialized. The method returns with the BIN latched, unless an
     * exception is raised. In all cases, the method registers the cursor with
     * the BIN that contains or should contain the search key.
     *
     * @return the LockStanding for the found record, or null if no record was
     *         found.
     */
    public LockStanding searchExact(final DatabaseEntry searchKey, final LockType lockType, final boolean dirtyReadAll,
    ...
    /**
     * Lock and copy current record into the key and data DatabaseEntry. When
     * calling this method, this.bin should not be latched already. On return,
     * this.bin is unlatched.
     */
    public OperationResult lockAndGetCurrent(DatabaseEntry foundKey, DatabaseEntry foundData, final LockType lockType) {

        return lockAndGetCurrent(foundKey, foundData, lockType, false, false, true);
    }

    /**
     * Let S be the slot where this cursor is currently positioned on. If S does
     * not hold a "valid" record, return null. Otherwise, lock the record in S
     * with the specified lock type(which may be NONE), copy its key and data
     * into the key and data DatabaseEntries, and return SUCCESS. Whether the
     * slot contains a "valid" record or not depends on the slot's KD/PD flags,
     * the lockType and dirtyReadAll parameters, and whether the record has
     * expired. For details see {@link #lockLNAndCheckDefunct}. On entry, the
     * isLatched param says whether this.bin is latched or not. On return,
     * this.bin is unlatched if the unlatch param is true or an exception is
     * thrown.
     *
     * @return OperationResult, or null if the LN has been cleaned and cannot be
     *         fetched.
     */
    public OperationResult lockAndGetCurrent(DatabaseEntry foundKey, DatabaseEntry foundData, final LockType lockType,
                                             final boolean dirtyReadAll, final boolean isLatched,
        ...
        try {
            ...
            /*
             * If we encounter a deleted slot, opportunistically add the BIN to
             * the compressor queue. We do not queue expired slots to avoid
             ...
             * running the size of this BIN to 0 but not having yet removed it
             * from the tree. The index may be negative if we're at an
             * intermediate stage in an higher level operation (e.g., the
             * starting search for a range scan op), and we expect a higher
             * level method to do a next or prev operation after this returns
             * KEYEMPTY. [#11700]
             */
            ...
            final boolean dataRequested = (foundData != null
     ...
     * "defunct" below we mean S is KD/PD or expired. 1. If S is not defunct,
     * return the LockStanding obj. In this case, we know that S holds a valid
     * (non-defunct) record. 2. If S is defunct, and the lock type is not NONE,
     * return null. In this case, we know that the record that used to be in S
     * is definitely defunct. 3. If S is defunct, the lock kind is NONE, and
     * dirtyReadAll is false, return null. This case corresponds to the
     * READ_UNCOMMITTED LockMode. The record in S is defunct, but the deleting
     * txn may be active still, and if it aborts later, the record will be
     * restored. To avoid a potentially blocking lock, in READ_UNCOMMITTED mode
     * we consider the record to be non-existing and return null. 4. If S is
     * defunct, the lock kind is NONE, and dirtyReadAll is true, lock the record
     * in READ mode. This case corresponds to the READ_UNCOMMITTED_ALL LockMode,
     * which requires that we do not skip "provisionally defunct" records. There
     * are two sub-cases: 4a. If dataRequested is true, we wait until the
     * deleting txn finishes. In this case the READ lock is blocking. If after
     * the lock is granted S is still defunct, release the lock and return null.
     * Otherwise, release the lock and return the LockStanding obj. 4b. If
     * dataRequested is false, then we check whether the deleting txn is still
     * open by requested a non-blocking READ lock. If the lock is granted then
     * the writing txn is closed or this cursor's locker is the writer, and we
     * proceed as if the READ lock was granted in 4a. If the lock is denied then
     * the deleting txn is still open, and we return the LockStanding obj so
     * that the record is not skipped. The BIN must be latched on entry and is
     * latched on exit.
     *
     ...
     *            record data, meaning that a blocking lock must be used for
     *            dirtyReadAll. Is ignored if dirtyReadAll is false. Is always
     *            false for a dup DB, since data is never requested for dup DB
     *            ops at the CursorImpl level.
     */
    private LockStanding lockLNAndCheckDefunct(final LockType lockType, final boolean dirtyReadAll,
                                               final boolean dataRequested) {
        ...
        if (lockType != LockType.NONE) {
            ...
            /*
             * The record was committed by another locker, or has been performed
             * by this locker.
             */
            return null;
        }
    ...
    public OperationResult getCurrent(final DatabaseEntry foundKey, final DatabaseEntry foundData) {
        ...
        final long vlsn = (ln != null ? ln.getVLSNSequence() : bin.getVLSN(index, false /* allowFetch */, cacheMode));
    ...
    /**
     * Returns the VLSN and LSN for the record at the current position. Must be
     * called when the cursor is positioned on a record. If this method is
     * called on a secondary cursor, the version of the associated primary
     * record is returned. In that case, the allowFetch parameter is ignored,
     * and the version is available only if the primary record was retrieved
    ...
    public RecordVersion getCurrentVersion(boolean allowFetch) {
        ...
        /*
         * For a secondary cursor, the cached version is all we have. See
         * setSecondaryCurrentVersion.
         */
        if (isSecondaryCursor) {
            if (currentRecordVersion == null) {
                throw new IllegalStateException("Record version is available via a SecondaryCursor only "
        ...
        /*
         * Use cached version if available. Do not use cached version if it does
         * not contain a VLSN, and VLSNs are preserved, and fetching is allowed;
         * instead, try to fetch it below.
         */
        if (currentRecordVersion != null) {
            if ((currentRecordVersion.getVLSN() != VLSN.NULL_VLSN_SEQUENCE) || !allowFetch
     ...
     * cursor is positioned on a record. When called on a secondary cursor that
     * was used to return the primary data, the size of the primary record is
     * returned by this method. Otherwise the size of the record at this cursor
    ...
    /**
     * When the primary record is read during a secondary operation, this method
     * is called to copy the primary version and storage size here. This allows
     * the secondary cursor API to return the version and size of the primary
     * record. Note that a secondary record does not have a version of its own.
     *
     ...
     * operation at the current cursor position. NOTE: this method does not work
     * (returns 0) if primary deletions are performed via a secondary
     * (SecondaryDatabase/SecondaryCursor.delete).
     *
    ...
    public boolean advanceCursor(DatabaseEntry key, DatabaseEntry data) {
        ...
        try {
            getNext(key, data, LockType.NONE, false /* dirtyReadAll */, true /* forward */, false /* isLatched */,
     ...
     * they are described in the lockLNAndCheckDefunct() method. This will cross
     * BIN boundaries. On return, no latches are held. If no exceptions, the
     * cursor is registered with its new location.
     *
     ...
     *            is out of range.
     */
    public OperationResult getNext(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
                                   boolean dirtyReadAll, boolean forward, boolean isLatched,
        ...
        try {
            while (bin != null) {
                ...
                if ((forward && ++index < bin.getNEntries()) || (!forward && --index > -1)) {
                    ...
                    OperationResult ret = lockAndGetCurrent(foundKey, foundData, lockType, dirtyReadAll,
                    ...
                    /*
                     * Make sure that the current BIN will not be pruned away if
                     * it is or becomes empty after it gets unlatched by
                     * Tree.getNextBin() or Tree.getPrevBin(). The operation of
                     * these Tree methods relies on the current BIN not getting
                     * pruned.
                     */
                    anchorBIN = bin;
                    anchorBIN.pin();
     ...
     * operation from the prior position. Something may have been added to the
     * original cursor (cursorImpl) while we were getting the next BIN.
     * cursorImpl would have been adjusted properly but we would have skipped a
     * BIN in the process. This can happen when all INs are unlatched in
     * Tree.getNextBin. It can also happen without a split, simply due to
     * inserted entries in the previous BIN.
     *
     * @return true if an unaccounted for insertion happened. TODO:
     *         Unfortunately, this method doesn't cover all cases where a
     *         phantom may have been inserted. Another case is described below.
     *         IN-0 ---------------------- | | 50 | 100 | |
     *         ---------------------- / / \ \ / \ /---- ----\ IN-1 / \ IN-2
     *         ---------------- ---------------- | 60 | 70 | 80 | | | | |
     *         ---------------- ---------------- / | \ / / | \ /
     *         ---------------- ----------------- | 81 | 83 | 85 | | 110 | | |
     *         ---------------- ----------------- BIN-3 BIN-4 Initially, the
     *         tree looks as above and a cursor (C) is located on the last slot
     *         of BIN-3. For simplicity, assume no duplicates and no
     *         serializable isolation. Also assume that C is a sticky cursor. 1.
     *         Thread 1 calls C.getNext(), which calls
     *         retrieveNextAllowPhantoms(), which duplicates C's cursorImpl, and
     *         calls dup.getNext(). dup.getNext() latches BIN-3, sets
     *         dup.binToBeRemoved to BIN-3 and then calls
     *         Tree.getNextBin(BIN-3). Tree.getNextBin(BIN-3) does the
     *         following: - sets searchKey to 85 - calls
     *         Tree.getParentINForChildIN(BIN-3). -
     *         Tree.getParentINForChildIN(BIN-3) unlatches BIN-3 and searches
     *         for BIN-3 parent, thus reaching IN-1. - IN-1.findEntry(85) sets
     *         "index" to 2, - "index" is incremented, - "moreEntriesThisIn" is
     *         set to false, - "next" is set to IN-1, . -
     *         Tree.getParentINForChildIN(IN-1) is called and unlatches IN-1.
     *         Assume at this point thread 1 looses the cpu. 2. Thread 2 inserts
     *         keys 90 and 95, causing a split of both BIN-3 and IN-1. So the
     *         tree now looks like this: IN-0 --------------------------- | | 50
     *         | 80 | 100 | | --------------------------- / / | \ \ / | \
     *         /--------- | ----------\ IN-1 / | \ IN-2 / IN-5 | \ -----------
     *         ----------- ---------------- | 60 | 70 | | 80 | 90 | | | | |
     *         ----------- ----------- ---------------- / | / \ / / | / \ /
     *         ---------------- ----------- ----------------- | 81 | 83 | 85 | |
     *         90 | 95 | | 110 | | | ---------------- -----------
     *         ----------------- BIN-3 BIN-6 BIN-4 Notice that C.cursorImpl
     *         still points to the last slot of BIN-3. 3. Thread 1 resumes: -
     *         Tree.getParentINForChildIN(IN-1) reaches IN-0. -
     *         IN-0.findEntry(85) sets "index" to 2, - "index" is incremented, -
     ...
     *         skipped Now we are back in dup.getNext(): - dup.bin is set to
     *         BIN-4, dup.index to -1, and dup is added to BIN-4 - the while
     *         loop repeats, dup.index is set to 0, the 1st slot of BIN-4 is
     *         locked, and dup.getNext() returns SUCCESS. Now we are back in
     *         C.retrieveNextAllowPhantoms(): - C.checkForInsertion() is called
     *         - C.cursorImpl and dup are on different BINs, but the condition:
     *         origBIN.getNEntries() - 1 > origCursor.getIndex() is false, so
     *         C.checkForInsertion() returns false. The end result is that BIN-6
     *         has been missed. This is not be a "bug" for non-serializable
     *         isolation, but the above scenario applies to serializable
     *         isolation as well, and in that case, BIN-6 should really not be
     *         missed. This could be solved by re-implementing
    ...
    public boolean checkForInsertion(final GetMode getMode, final CursorImpl dupCursor) {
        ...
        if (origCursor.bin != dupCursor.bin) {
            ...
            /*
             * We jumped to the next BIN during getNext(). Be sure to operate on
             * the BIN returned by latchBIN, not a cached var [#21121]. Note
             * that a cursor BIN can change after the check above, but that's
             * not relevant; what we're trying to detect are BIN changes during
             * the operation that has already completed. Note that we can call
             * isDefunct without locking. If we see a non-committed defunct
             * entry, we'll just iterate around in the caller. So a false
             * positive is ok.
             */
            origCursor.latchBIN();
     ...
     * because maxCount is reached or RangeConstraint.inBounds returns false. If
     * a maxCount is passed, this allows advancing the cursor quickly by N
     * entries. If a rangeConstraint is passed, this allows returning the entry
     * count after advancing until the predicate returns false, e.g., the number
     * of entries in a key range. In either case, the number of entries advanced
     * is returned. Optimized to scan using level two of the tree when possible,
     * to avoid calling getNextBin/getPrevBin for every BIN of the database. All
     * BINs beneath a level two IN can be skipped quickly, with the level two
     * parent IN latched, when all of its children BINs are resident and can be
     ...
     * the parent IN latched for long time periods. Although this method
     * positions the cursor on the last non-defunct entry seen (before the
     * boundary condition is satisfied), because it does not lock the LN it is
     * possible that it is made defunct by another thread after the BIN is
     * unlatched.
     *
     * @param forward is true to skip forward, false to skip backward.
     * @param maxCount is the maximum number of non-defunct entries to skip, and
     *            may be LTE zero if no maximum is enforced.
     * @param rangeConstraint is a predicate that returns false at a position
     *            where advancement should stop, or null if no predicate is
     *            enforced.
    ...
    public long skip(boolean forward, long maxCount, RangeConstraint rangeConstraint) {
        ...
        try {
             ...
             * throws because the BIN latch is already held. It should have been
             * released by skipInternal and therefore an unexpected exception
             * must have been throw and the error handling must be incorrect.
             */
    ...
    /**
     * Use this cursor to reference the current BIN in the traversal, to prevent
     * the current BIN from being compressed away. But set the given
     ...
     * returns.
     */
    private long skipInternal(boolean forward, long maxCount, RangeConstraint rangeConstraint,
        ...
        try {
            while (true) {
                ...
                count = skipEntries(forward, maxCount, rangeConstraint, finalPositionCursor, curBin, curIndex, count);
                 ...
                 */
                final byte[] idKey = (curBin.getNEntries() == 0 ? curBin.getIdentifierKey()
                        : (forward ? curBin.getKey(curBin.getNEntries() - 1) : curBin.getKey(0)));

                final SearchResult result = tree.getParentINForChildIN(curBin, false, /* useTargetLevel */
                ...
                if (!result.exactParentFound) {
                    throw EnvironmentFailureException.unexpectedState(
                            "Cannot get parent of BIN id=" + curBin.getNodeId() + " key=" + Arrays.toString(idKey));
                }
                ...
                if (forward ? (parentIndex < parent.getNEntries() - 1) : (parentIndex > 0)) {
                    ...
                    for (parentIndex += incr;; parentIndex += incr) {
                        ...
                        /*
                         * Latch next child BIN, if cached and unlatched. Note
                         * that although 2 BINs are latched here, this can't
                         * cause deadlocks because the 2nd latch is no-wait.
                         */
                        ...
                        count = skipEntries(forward, maxCount, rangeConstraint, finalPositionCursor, curBin,
                ...
                /*
                 * Only the prevBin is still latched here. Move to the next BIN
                 * the "hard" way (i.e., via full tree searches).
                 */
                curBin = forward ? tree.getNextBin(prevBin, CacheMode.DEFAULT)
     ...
     * bounds is exceeded.
     */
    private long skipEntries(boolean forward, long maxCount, RangeConstraint rangeConstraint,
                             CursorImpl finalPositionCursor, BIN curBin, int curIndex, long count) {
     ...
     * inconsistent. Used by CountEstimator.
     */
    public List<TrackingInfo> getAncestorPath() {
        ...
        final SearchResult result = tree.getParentINForChildIN(origBin, false, /* useTargetLevel */
         ...
         * total entries and adjust the index accordingly. Add TrackingInfo for
         * child BIN.
         */
        try {
     ...
     * insert lock on it. If there are no more records following the given key,
     * lock the special EOF node for the dbImpl.
     */
        ...
        try {
            while (true) {
                ...
                if ((searchResult & FOUND) != 0 && (searchResult & FOUND_LAST) == 0) {
                     ...
                     * K1 (or this.index is -1 if K1 is < than all keys in the
                     * BIN). Furthermore, "this" is NOT positioned on the very
                     * last slot of the BTree. Call getNext() to advance "this"
                     * to the next *valid* (i.e., not defunct) slot and lock
                     * that slot in RANGE_INSERT mode. Normally, getNext() will
                     * move the cursor to the 1st slot with a key K2 > K1.
                     * However, it is possible that K2 <= K1 (see the comments
                     * in Cursor.searchRangeAdvanceAndCheckKey() about how this
                    ...
                    OperationResult result = getNext(tempKey, tempData, LockType.RANGE_INSERT, false, true, true,
                            null /* rangeConstraint */);
    ...
    /**
     * Holds the result of a lockLN operation. A lock may not actually be held
     * (getLockResult may return null) if an uncontended lock is allowed.
     */
    public static class LockStanding {
         ...
         * the LN and updating the BIN. It returns a WriteLockInfo that is meant
         * to be passed to the LN logging method, where its info will be
         * included in the LN log entry and also copied into the new
         * WriteLockInfo that will be created for the new LSN. If the locker is
         * not transactional, or the current LSN has not been write-locked
         * before by this locker, a new WriteLockInfo is created here and its
         * abortLsn and abortKD fields are set. (note: even though lockLN() is
         * called before prepareForUpdate(), it may not actually acquire a lock
         * because of the uncontended optimization). Otherwise, a WriteLockInfo
         * exists already. It may have been created by the lockLN() call during
         * the current updating op, or a lockLN() call during an earlier
         * updating op by the same txn. In the later case, the abortLsn and
         * abortKD have been set already and should not be overwriten here.
         */
        public WriteLockInfo prepareForUpdate(BIN bin, int idx) {
            ...
            if (wri == null) {
                ...
                lockResult.setAbortInfo(lsn, abortKD, abortKey, abortData, abortVLSN, abortExpiration,
                        abortExpirationInHours, db);
            }
            return wri;
        }
         ...
         * copied into the WriteLockInfo for the new LSN. This method is static
         * because lockLN is never called prior to logging an LN for a newly
         * inserted slot.
         */
    ...
    /**
     * Locks the LN at the cursor position. Attempts to use a non-blocking lock
     * to avoid unlatching/relatching. Retries if necessary, to handle the case
     * where the LSN is changed while the BIN is unlatched. Because it
     * re-latches the BIN to check the LSN, this serializes access to the LSN
     * for locking, guaranteeing that two lockers cannot obtain conflicting
     * locks on the old and new LSNs. Preconditions: The BIN must be latched.
     * Postconditions: The BIN is latched. LN Locking Rules ---------------- The
     * lock ID for an LN is its LSN in the parent BIN slot. Because the LSN
     * changes when logging the LN, only two methods of locking an LN may be
     * used to support concurrent access: 1. This method may be called to lock
     * the old LSN. For read operations, that is all that is necessary. For
     * write operations, the new LSN must be locked after logging it, which is
     * done by all the LN logging methods. Be sure to pass a non-null locker to
     * the LN logging method to lock the LN, unless locking is not desired. 2. A
     * non-blocking lock may be obtained on the old LSN (using
     * Locker.nonBlockingLock rather than this method), as long as the lock is
     * released before the BIN latch is released. In this case a null locker is
     * passed to the LN logging method; locking the new LSN is unnecessary
     * because no other thread can access the new LSN until the BIN latch is
     * released. The first method is used for all user operations. The second
     * method is used by the cleaner, when flushing dirty deferred-write LNs,
     * and by certain btree operations. Uncontended Lock Optimization
     * ----------------------------- The allowUncontended param is passed as
     * true for update and delete operations as an optimization for the case
     * where no lock on the old LSN is held by any locker. In this case we don't
     * need to lock the old LSN at all, as long as we log the new LSN before
     * releasing the BIN latch. 1. Latch BIN 2. Determine that no lock/waiter
     * exists for oldLsn 3. Log LN and get lsn 4. Lock lsn 5. Update BIN 6.
     * Release BIN latch The oldLsn is never locked, saving operations on the
     * lock table. The assumption is that another locker will first have to
     * latch the BIN to get oldLsn, before requesting a lock. A potential
     * problem is that the other locker may release the BIN latch before
     * requesting the lock. This Operation Another Operation --------------
     * ----------------- Latch BIN, get oldLsn, release BIN latch Step 1 and 2
     * Request lock for oldLsn, granted Step 3 and 4 Both operations now believe
     * they have an exclusive lock, but they have locks on different LSNs.
     ...
     * retry if the LSN changes while acquiring the lock. Because it re-latches
     * the BIN to check the LSN, this will serialize access to the LSN for
     * locking, guaranteeing that two conflicting locks cannot be granted on the
     * old and new LSNs. Deferred-Write Locking ---------------------- When one
     * of the LN optionalLog methods is called, a deferred-write LN is dirtied
     * but not actually logged. In order to lock an LN that has been inserted
     * but not yet assigned a true LSN, a transient LSNs is assigned. These LSNs
     * serve to lock the LN but never appear in the log. See
     * LN.assignTransientLsn. A deferred-write LN is logged when its parent BIN
     * is logged, or when the LN is evicted. This will replace transient LSNs
     * with durable LSNs. If a lock is held by a cursor on a deferred-write LN
     * when it is logged, the same lock is acquired on the new LSN by the
     * cursor. See lockAfterLsnChange. Cleaner Migration Locking
     * ------------------------- The cleaner takes a non-blocking read lock on
     * the old LSN before migrating/logging the LN, while holding the BIN latch.
     * It does not take a lock on the new LSN, since it does not need to retain
     * a lock after releasing the BIN latch. Because a read, not write, lock is
     * taken, other read locks may be held during migration. After logging, the
     * cleaner calls lockAfterLsnChange to lock the new LSN on behalf of other
     * lockers. For more info on migration locking, see HandleLocker. Expired
     * Record Locking ---------------------- To support repeatable-read
     * semantics when a record expires after being locked, we must check whether
     * a record was previously locked before attempting to lock it. If it was
     * previously locked, then it is treated as not expired, even if its
     * expiration time has passed. By was previously "locked" here we mean that
     * any lock type is held, or shared with its owner, by this cursor's locker.
     * Since a read lock will prevent modification of the expiration time, any
     * lock type is adequate. A shared lock is considered adequate to account
     * for the case where multiple lockers are used internally for a single
     * virtual locker, as seen by the user. This is the case when using a
     * read-committed locker or a thread-locker, for example. To avoid
     * unnecessary added overhead, we do not check whether a record was
     * previously locked except when expiration is imminent, which is defined as
     * expiring within {@link EnvironmentParams#ENV_TTL_MAX_TXN_TIME}. The
     * ENV_TTL_MAX_TXN_TIME buffer is used because the expiration time may pass
     * while waiting for a lock. Another case to account for is when the
     * expiration time of the record changes while waiting for the lock. This
     * can happen if the record is updated or an update is aborted. In this case
     * we can assume that the was not previously locked, since that would have
     * prevented the update. Note that when an uncontended lock applies, the
     * expiration of the record with the current LSN cannot change. It is
     * possible that the update or deletion requesting the uncontended lock will
     * be aborted, and the LSN of an expired record will be reinstated in the
     * BIN, but this does not create a special case. Historical Notes
     * ---------------- In JE 4.1 and earlier, each LN had a node ID that was
     * used for locking, rather than using the LSN. The node ID changed only if
     * a deleted slot was reused. The node ID was stored in the LN, requiring
     * that the LN be fetched when locking the LN. With LSN locking a fetch is
     * not needed. When LN node IDs were used, deferred-write LNs were not
     * assigned an LSN until they were actually logged. Deferred-write LNs were
     * initially assigned a null LSN and transient LSNs were not needed.
     *
     ...
     * @param noWait is true to perform a no-wait lock request while keeping the
     *            BIN latched. The caller must check the lock result to see
     ...
     */
    public LockStanding lockLN(final LockType lockType, final boolean allowUncontended, final boolean noWait)
        ...
        /*
         * We can avoid taking a lock if uncontended. However, we must call
         * preLogWithoutLock to prevent logging on a replica, and as good
         * measure to prepare for undo.
         */
        ...
        if (envImpl.expiresWithin(prevExpiration, prevExpirationInHours, dbImpl.getEnv().getTtlMaxTxnTime())) {
         ...
         * no-wait, use the standard lock method so LockNotAvailableException is
         * thrown; there is no need to try a non-blocking lock twice. Even for
         * dirty-read (LockType.NONE) we must call Locker.lock() since it checks
         * the locker state and may throw LockPreemptedException.
         */
        if (locker.getDefaultNoWait()) {
            ...
            standing.lockResult = locker.nonBlockingLock(standing.lsn, lockType, false /* jumpAheadOfWaiters */,
                    dbImpl);
        }
        ...
        /*
         * Unlatch, get a blocking lock, latch, and get the current LSN from the
         * slot. If the LSN changes while unlatched, revert the lock and repeat.
         */
        while (true) {
            ...
            if (standing.lsn == newLsn) {
                ...
                /*
                 * If the expiration time changes while unlatched, then it could
                 * not have been previously locked.
                 */
                if (prevExpiration != bin.getExpiration(index) || prevExpirationInHours != bin.isExpirationInHours()) {
     ...
     * is left latched by this method. We release the lock on the oldLsn to
     * prevent locks from accumulating over time on a HandleLocker, as the
     * cleaner migrates LNs, because Database handle locks are legitimately very
     * long-lived. It is important to first acquire all lsn locks and then
     * release the oldLsn locks. Releasing an oldLsn lock might allow another
     * locker to acquire it, and then acquiring another lsn lock may encounter a
     * conflict. [#20617]
     *
    ...
    private void verifyCursor(BIN bin) {
        ...
        if (!bin.getCursorSet().contains(this)) {
            throw new EnvironmentFailureException(dbImpl.getEnv(), EnvironmentFailureReason.UNEXPECTED_STATE,
    ...
    /**
     * Checks that LN deletedness matches KD/PD flag state, at least when the LN
     * is resident. Should only be called under an assertion.
     */
    private boolean verifyPendingDeleted(LockType lockType) {
        ...
        assert ((lnDeleted && (kd || pd)) || (!lnDeleted && !pd)) : "Deleted state mismatch LNDeleted = " + lnDeleted
                + " PD = " + pd + " KD = " + kd;
        return true;
    }
    ...
    /**
     * Enumerates all records in a dbImpl non-transactionally and calls the
     * withCursor method for each record. Stops the enumeration if the callback
     * returns false.
     *
     ...
     *            multiple operations, but may be false if eviction is
     *            disallowed in a particular context.
     * @param withCursor callback object.
     */
    public static void traverseDbWithCursor(DatabaseImpl db, LockType lockType, boolean allowEviction,
        ...
        try {
            ...
            if (cursor.positionFirstOrLast(true /* first */)) {
                ...
                OperationResult result = cursor.lockAndGetCurrent(key, data, lockType, false /* dirtyReadAll */,
                ...
                while (!done) {
                     ...
                     * first record is defunct, but we can call getNext below to
                     * move forward.
                     */
                    if (result != null) {
                    ...
                    if (!done) {
                        result = cursor.getNext(key, data, lockType, false /* dirtyReadAll */, true /* forward */,
