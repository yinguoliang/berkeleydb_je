...
/**
 * Uses com.sun.management (non-portable) APIs to detect whether compressed oops
 * is actually in effect. Uses reflection so that isEnabled simply returns null
 * if the com.sun.management classes are not available, rather than causing a
 * class loading error during static initialization, which would prevent the
 * process from running. For the IBM J9 environment, which doesn't support the
...
class CompressedOopsDetector {
    ...
    private static Boolean isEnabledInternal() throws Throwable {
        ...
        final Object hotspotMBean = ManagementFactory.newPlatformMXBeanProxy(ManagementFactory.getPlatformMBeanServer(),
