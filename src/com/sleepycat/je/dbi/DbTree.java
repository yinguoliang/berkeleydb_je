 ...
 * databaseId->DatabaseImpl. For example, suppose we have two databases, foo and
 * bar. We have the following structure: nameDatabase idDatabase IN IN | | BIN
 * BIN +-------------+--------+ +---------------+--------+ . | | . | | NameLNs
 * NameLN NameLN MapLNs for MapLN MapLN for internal key=bar key=foo internal
 * dbs key=53 key=79 dbs data= data= data= data= dbId79 dbId53 DatabaseImpl
 * DatabaseImpl | | Tree for foo Tree for bar | | root IN root IN Databases,
 * Cursors, the cleaner, compressor, and other entities have references to
 * DatabaseImpls. It's important that object identity is properly maintained,
 * and that all constituents reference the same DatabaseImpl for the same db,
 * lest they develop disparate views of the in-memory database; corruption would
 * ensue. To ensure that, all entities must obtain their DatabaseImpl by going
 * through the idDatabase. DDL type operations such as create, rename, remove
 * and truncate get their transactional semantics by transactionally locking the
 * NameLN appropriately. A read-lock on the NameLN, called a handle lock, is
 * maintained for all DBs opened via the public API (openDatabase). This
 * prevents them from being renamed or removed while open. See HandleLocker for
 * details. However, for internal database operations, no handle lock on the
 * NameLN is acquired and MapLNs are locked with short-lived non-transactional
 * Lockers. An entity that is trying to get a reference to the DatabaseImpl gets
 * a short lived read lock just for the fetch of the MapLN, and a DatabaseImpl
 * usage count is incremented to prevent eviction; see getDb and releaseDb. A
 * write lock on the MapLN is taken when the database is created, deleted, or
 * when the MapLN is evicted. (see DatabaseImpl.isInUse()) The nameDatabase
 * operates pretty much as a regular application database in terms of eviction
 * and recovery. The idDatabase requires special treatment for both eviction and
 * recovery. The issues around eviction of the idDatabase center on the need to
 * ensure that there are no other current references to the DatabaseImpl other
 * than that held by the mapLN. The presence of a current reference would both
 * make the DatabaseImpl not GC'able, and more importantly, would lead to object
 * identity confusion later on. For example, if the MapLN is evicted while there
 * is a current reference to its DatabaseImpl, and then refetched, there will be
 * two in-memory versions of the DatabaseImpl. Since locks on the idDatabase are
 * short lived, DatabaseImpl.useCount acts as a reference count of active
 * current references. DatabaseImpl.useCount must be modified and read in
 * conjunction with appropriate locking on the MapLN. See DatabaseImpl.isInUse()
 * for details. This reference count checking is only needed when the entire
 * MapLN is evicted. It's possible to evict only the root IN of the database in
 * question, since that doesn't interfere with the DatabaseImpl object identity.
 ...
 * change during a reader operation (operation by a thread that has called getDb
 * and not yet called releaseDb). Why not just hold a MapLN read lock during a
 * reader operation?
 * -------------------------------------------------------------- Originally
 * this was not done because of cleaner performance. We afraid that * either of
 * the following solutions would not perform well: + If we get (and release) a
 * MapLN lock for every entry in a log file, this adds a lot of per-entry
 * overhead. + If we hold the MapLN read lock for the duration of a log file
 * cleaning (the assumption is that many entries are for the same DB), then we
 * block checkpoints during that period, when they call modifyDbRoot. Therefore,
 * the usage count is incremented once per DB encountered during log cleaning,
 * and the count is decremented at the end. This caching approach is also used
 * by the HA replayer. In both cases, we do not want to lock the MapLN every
 * entry/operation, and we do not want to block checkpoints or other callers of
 * modifyDbRoot. It is acceptable, however, to block DB naming operations. In
 * addition we allow modifyDbRoot to run when even the usage count is non-zero,
 * which would not be possible using a read-write locking strategy. I'm not sure
 * why this was done originally, perhaps to avoid blocking. But currently, it is
 * necessary to prevent a self-deadlock. All callers of modifyDbRoot first call
 * getDb, which increments the usage count. So if modifyDbRoot was to check the
 * usage count and retry if non-zero (like deleteMapLN), then it would loop
 * forever. Why are the retry loops necessary in the DbTree methods?
 * -------------------------------------------------------- Three methods that
 * access the MapLN perform retries (forever) when there is a lock conflict:
 * getDb, modifyDbRoot and deleteMapLN. Initially the retry loops were added to
 * compensate for certain slow operations. To solve that problem, perhaps there
 * are alternative solutions (increasing the lock timeout). However, the
 * deleteMapLN retry loop is necessary to avoid deleting it when the DB is in
 * use by reader operations. Tendency to livelock -------------------- Because
 * MapLN locks are short lived, but a reader operation may hold a
 * MapLN/DatabaseImpl for a longer period by incrementing the usage count, there
 * is the possibility of livelock. One strategy for avoiding livelock is to
 * avoid algorithms where multiple threads continuously call getDb and
...
public class DbTree implements Loggable {
    ...
    /**
     * Returns the DbType for a given DB name. Note that we allow dbName to be
     * null, because it may be null when the 'debug database name' is not yet
     * known to DatabaseImpl. This works because the debug name is always known
     * immediately for internal DBs.
     */
    ...
    /*
     * Database Ids: We need to ensure that local and replicated databases use
     * different number spaces for their ids, so there can't be any possible
     * conflicts. Local, non replicated databases use positive values,
     * replicated databases use negative values. Values -1 thru NEG_DB_ID_START
     * are reserved for future special use.
     */
     ...
     * replication. The behavior is as follows: Env is Env is Persistent
     * Follow-on action replicated brand new value of DbTree.isReplicated 0 1
     * n/a replicated bit = 0; 0 0 0 none 0 0 1 true for r/o, false for r/w 1 1
     * n/a replicated bit = 1 1 0 0 require config of all dbs 1 0 1 none
     */
     ...
     * replicated environment. The behaviors are as follows: Value of Value of
     * the What happens Can open Can open RepConfig. DbTree when we call as r/o
     * as r/2 allowConvert replicated bit ReplicatedEnv() Environment
     * Environment later on? later on? throw exception, Yes, because Yes,
     * because complain that the env is not env is not false false env is not
     * converted converted replicated Yes, always ok No, this is open a now a
     * true false do conversion replicated replicated env with r/o env Ignore
     * true or open as a replicated allowConvert brand new env the usual way Yes
     * No Environment
     */
    ...
    public DbTree(EnvironmentImpl env, boolean replicationIntended, boolean preserveVLSN) throws DatabaseException {
        ...
        /*
         * Sequences must be initialized before any databases are created. 0 and
         * 1 are reserved, so we start at 2. We've put -1 to NEG_DB_ID_START
         * asided for the future.
         */
        ...
        idDatabase = new DatabaseImpl(null, DbType.ID.getInternalName(), new DatabaseId(0), env, idConfig);
        /* Force a reset if enabled globally. */
        idDatabase.clearKeyPrefixing();

        DatabaseConfig nameConfig = new DatabaseConfig();
        nameConfig.setKeyPrefixing(false);
        nameDatabase = new DatabaseImpl(null, DbType.NAME.getInternalName(), new DatabaseId(1), env, nameConfig);
        /* Force a reset if enabled globally. */
        nameDatabase.clearKeyPrefixing();
    ...
    public void updateFromReplay(DatabaseId replayDbId) {
        ...
        if (replayVal > 0 && !envImpl.isRepConverted()) {
            throw EnvironmentFailureException
                    .unexpectedState("replay database id is unexpectedly positive " + replayDbId);
        }
    ...
    /**
     * Initialize the db tree during recovery, after instantiating the tree from
     * the log. a. set up references to the environment impl b. check for
     * replication rules.
     */
    ...
    /**
     * Creates a new database object given a database name. Increments the use
     * count of the new DB to prevent it from being evicted. releaseDb should be
     * called when the returned object is no longer used, to allow it to be
     * evicted. See DatabaseImpl.isInUse. [#13415]
     */
    public DatabaseImpl createDb(Locker locker, String databaseName, DatabaseConfig dbConfig, HandleLocker handleLocker)
            throws DatabaseException {

        return doCreateDb(locker, databaseName, dbConfig, handleLocker, null, // replicatedLN
                null); // repContext, to be decided by new db
    }

    /**
     * Create a database for internal use. It may or may not be replicated.
     * Since DatabaseConfig.replicated is true by default, be sure to set it to
     * false if this is a internal, not replicated database.
     */
    public DatabaseImpl createInternalDb(Locker locker, String databaseName, DatabaseConfig dbConfig)
        ...
        DatabaseImpl ret = doCreateDb(locker, databaseName, dbConfig, null, // handleLocker,
                null, // replicatedLN
                null); // repContext, to be decided by new db
        /* Force a reset if enabled globally. */
    ...
    public DatabaseImpl createReplicaDb(Locker locker, String databaseName, DatabaseConfig dbConfig,
                                        NameLN replicatedLN, ReplicationContext repContext)
            throws DatabaseException {

        return doCreateDb(locker, databaseName, dbConfig, null, // databaseHndle
                replicatedLN, repContext);
    }

    /**
     * Create a database. Increments the use count of the new DB to prevent it
     * from being evicted. releaseDb should be called when the returned object
     * is no longer used, to allow it to be evicted. See DatabaseImpl.isInUse.
     * [#13415] Do not evict (do not call CursorImpl.setAllowEviction(true))
     * during low level DbTree operation. [#15176]
     */
    private DatabaseImpl doCreateDb(Locker nameLocker, String databaseName, DatabaseConfig dbConfig,
                                    HandleLocker handleLocker, NameLN replicatedLN, ReplicationContext repContext)
            throws DatabaseException {
        ...
        if (replicatedLN != null) {
            ...
            /*
             * This database has been created locally, either because this is a
             * non-replicated node or this is the replicated group master.
             */
        ...
        try {
            ...
            nameCursor.insertRecord(StringUtils.toUTF8(databaseName), // key
            ...
            idCursor.insertRecord(newId.getBytes() /* key */, new MapLN(newDb) /* ln */, false /* blindInsertion */,
             ...
             * by another concurrent operation, for example, truncation. Note
             * that IDs are not conserved in doTruncateDb when a failure occurs.
             * This inconsistency is historical and may or may not be the best
             * approach. [#18642]
             */
            if (!operationOk) {
     ...
     * database existed when openDatabase was called. Here we additionally lock
     * the NameLN for read on behalf of the handleLocker, which is kept by the
     * Database handle. The lock must be acquired while the BIN is latched, so
     * the locker will be updated if the LSN changes. There is no lock
     * contention possible because the HandleLocker shares locks with the
     * nameCursor locker, and jumpAheadOfWaiters=true is passed in case another
     * locker is waiting on a write lock. If the lock is denied, checkPreempted
     * is called on the nameCursor locker, in case the lock is denied because
     * the nameCursor's lock was preempted. If so, DatabasePreemptedException
     * will be thrown.
     *
    ...
    private void acquireHandleLock(CursorImpl nameCursor, HandleLocker handleLocker) {
        ...
        try {
            ...
            final LockResult lockResult = handleLocker.nonBlockingLock(lsn, LockType.READ,
                    true /* jumpAheadOfWaiters */, nameDatabase);
    ...
    /**
     * Write a MapLN to the log in order to: - propagate a root change - save
     * per-db utilization information - save database config information. Any
     * MapLN writes must be done through this method, in order to ensure that
     * the root latch is taken, and updates to the rootIN are properly
     * safeguarded. See MapN.java for more detail.
     *
     * @param db the database whose root is held by this MapLN
     * @param ifBeforeLsn if argument is not NULL_LSN, only do the write if this
     *            MapLN's current LSN is before isBeforeLSN.
        ...
        if (db.getId().equals(ID_DB_ID) || db.getId().equals(NAME_DB_ID)) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (ifBeforeLsn == DbLsn.NULL_LSN || DbLsn.compareTo(cursor.getCurrentLsn(), ifBeforeLsn) < 0) {
    ...
    private static class RewriteMapLN implements WithRootLatched {
        ...
        public IN doWork(@SuppressWarnings("unused") ChildReference root) throws DatabaseException {
            ...
            cursor.updateCurrentRecord(null /* replaceKey */, dataDbt, null /* expirationInfo */, null /* foundData */,
                    null /* returnNewData */, ReplicationContext.NO_REPLICATE);
            return null;
        }
    }
     ...
     * creation, removal, truncation, or configuration update of a replicated DB
     * on a replica. It will throw ReplicaWriteException on a replica if this
     * operation would assign a new VLSN. [#20543]
     */
     ...
     * NameLN that represents this database and write locks it. Do not evict (do
     * not call CursorImpl.setAllowEviction(true)) during low level DbTree
     * operation. [#15176]
     *
     * @throws IllegalStateException via
     *             Environment.remove/rename/truncateDatabase
     */
    private NameLockResult lockNameLN(Locker locker, String databaseName, String action, GetRepContext getRepContext)
            throws DatabaseNotFoundException, NeedRepLockerException {

        /*
         * We have to return both a cursor on the naming tree and a reference to
         * the found DatabaseImpl.
         */
        ...
        try {
            ...
            /*
             * Check for an incorrect locker created via auto-commit. This check
             * is made after we have the DatabaseImpl and can check whether it
             * is replicated. See Environment.DbNameOperation.
             */
            if (envImpl.isReplicated() && result.dbImpl.isReplicated() && locker.getTxnLocker() != null
                    && locker.getTxnLocker().isAutoTxn() && !locker.isReplicated()) {
            ...
            if (locker.getImportunate()) {
                ...
                final String msg = "Database " + databaseName + " has been forcibly closed in order to apply a"
                ...
                if (handleCount > 0) {
                    throw new IllegalStateException("Can't " + action + " database " + databaseName + ", " + handleCount
     ...
     * NameLNLogEntry and replicated. So when there is a DatabaseConfig changes,
     * we'll update the NameLN for the database, which will log a new
     * NameLNLogEntry so that the rep stream will transfer it to the replicas
    ...
    public void updateNameLN(Locker locker, String dbName, final DbOpReplicationContext repContext)
        ...
        try {
            result = lockNameLN(locker, dbName, "updateConfig", new GetRepContext() {

                public ReplicationContext get(DatabaseImpl dbImpl) {
                    return (repContext != null) ? repContext : dbImpl.getOperationRepContext(UPDATE_CONFIG, null);
                }
            });
        ...
        try {
            ...
            /* Log a NameLN. */
            DatabaseEntry dataDbt = new DatabaseEntry(new byte[0]);
            nameCursor.updateCurrentRecord(null /* replaceKey */, dataDbt, null /* expirationInfo */,
     ...
     */
    private DatabaseImpl doRenameDb(Locker locker, String databaseName, String newName, NameLN replicatedLN,
                                    final DbOpReplicationContext repContext)
            throws DatabaseNotFoundException, NeedRepLockerException {

        final NameLockResult result = lockNameLN(locker, databaseName, "rename", new GetRepContext() {
        ...
        try {
            ...
            nameCursor.insertRecord(StringUtils.toUTF8(newName), useLN, false /* blindInsertion */, useRepContext);
     ...
     * @see #doRenameDb
     */
    public DatabaseImpl renameReplicaDb(Locker locker, String databaseName, String newName, NameLN replicatedLN,
     ...
     * @throws DatabaseNotFoundException if the operation fails because the
     *             given DB name is not found, or the non-null checkId argument
     *             does not match the database identified by databaseName.
     */
    private DatabaseImpl doRemoveDb(Locker locker, String databaseName, DatabaseId checkId,
                                    final DbOpReplicationContext repContext)
            throws DatabaseNotFoundException, NeedRepLockerException {

        CursorImpl nameCursor = null;

        final NameLockResult result = lockNameLN(locker, databaseName, "remove", new GetRepContext() {
        ...
        try {
            ...
            /*
             * Schedule database for final deletion during commit. This should
             * be the last action taken, since this will take effect immediately
             * for non-txnal lockers. Do not call releaseDb here on
             * result.dbImpl, since that is taken care of by markDeleteAtTxnEnd.
             */
     ...
     * @see #doRemoveDb
     */
    public void removeReplicaDb(Locker locker, String databaseName, DatabaseId checkId,
                                DbOpReplicationContext repContext)
            throws DatabaseNotFoundException {
     ...
     * database in its place. Do not evict (do not call
     * CursorImpl.setAllowEviction(true)) during low level DbTree operation.
     * [#15176]
     *
     ...
     *             given DB name is not found.
     */
    public TruncateDbResult doTruncateDb(Locker locker, String databaseName, boolean returnCount, NameLN replicatedLN,
                                         final DbOpReplicationContext repContext)
            throws DatabaseNotFoundException, NeedRepLockerException {

        assert ((replicatedLN != null) ? (repContext != null) : true);

        final NameLockResult result = lockNameLN(locker, databaseName, "truncate", new GetRepContext() {

            public ReplicationContext get(DatabaseImpl dbImpl) {
                return (repContext != null) ? repContext : dbImpl.getOperationRepContext(TRUNCATE, dbImpl.getId());
            }
        });
        ...
        try {
            ...
            final DatabaseId newId = (replicatedLN != null) ? replicatedLN.getId()
                    : new DatabaseId(
                            isReplicatedId(oldDb.getId().getId()) ? getNextReplicatedDbId() : getNextLocalDbId());
            ...
            try {
                ...
                idCursor.insertRecord(newId.getBytes() /* key */, new MapLN(newDb), false /* blindInsertion */,
                        ReplicationContext.NO_REPLICATE);
             ...
             * effect immediately for non-txnal lockers. Do not call releaseDb
             * here on oldDb or newDb, since that is taken care of by
             * markDeleteAtTxnEnd.
             */
     ...
     * @see #doTruncateDb
     */
    public TruncateDbResult truncateReplicaDb(Locker locker, String databaseName, boolean returnCount,
                                              NameLN replicatedLN, DbOpReplicationContext repContext)
            throws DatabaseNotFoundException {
    ...
    /*
     * Remove the mapLN that refers to this database. Do not evict (do not call
     * CursorImpl.setAllowEviction(true)) during low level DbTree operation.
     * [#15176]
     */
    void deleteMapLN(DatabaseId id) throws DatabaseException {
        ...
        /*
         * Retry indefinitely in the face of lock timeouts since the lock on the
         * MapLN is only supposed to be held for short periods.
         */
        boolean done = false;
        while (!done) {
            Locker idDbLocker = null;
            ...
            try {
                ...
                if (found) {
                     ...
                     * (checkpointing, cleaning, etc), release the lock (done in
                     * the finally block) and retry. [#15805]
                     */
    ...
    /**
     * Get a database object given a database name. Increments the use count of
     * the given DB to prevent it from being evicted. releaseDb should be called
     * when the returned object is no longer used, to allow it to be evicted.
     * See DatabaseImpl.isInUse. [#13415] Do not evict (do not call
     * CursorImpl.setAllowEviction(true)) during low level DbTree operation.
     * [#15176]
     *
     * @param nameLocker is used to access the NameLN. As always, a NullTxn is
     *            used to access the MapLN.
     * @param databaseName target database
     * @return null if database doesn't exist
     */
    public DatabaseImpl getDb(Locker nameLocker, String databaseName, HandleLocker handleLocker, boolean writeLock)
            throws DatabaseException {
     ...
     * the given map.
     */
    public DatabaseImpl getDb(DatabaseId dbId, long lockTimeout, Map<DatabaseId, DatabaseImpl> dbCache)
            throws DatabaseException {
     ...
     * use, or -1 to use the default timeout. A timeout should normally only be
     * specified by daemons with their own timeout configuration. public for
     * unit tests. Increments the use count of the given DB to prevent it from
     * being evicted. releaseDb should be called when the returned object is no
    ...
    public DatabaseImpl getDb(DatabaseId dbId, long lockTimeout, String dbNameIfAvailable) throws DatabaseException {
        ...
        if (dbId.equals(idDatabase.getId())) {
            ...
            /*
             * Retry indefinitely in the face of lock timeouts. Deadlocks may be
             * due to conflicts with modifyDbRoot.
             */
            while (true) {
                Locker locker = null;
                CursorImpl idCursor = null;
     ...
     * the use count reaches zero. Must be called to release a DatabaseImpl that
     * was returned by a method in this class. See DatabaseImpl.isInUse.
     * [#13415]
     */
    ...
    private void setDebugNameForDatabaseImpl(DatabaseImpl dbImpl, String dbName) throws DatabaseException {
        ...
        if (dbImpl != null) {
            if (dbName != null) {
                 ...
                 * setting up the tree. Only do name lookup if it will be fairly
                 * fast. Debugging info isn't important enough to cause long
                 * lookups during log cleaning, for example. [#21015]
                 */
     ...
     */
    public boolean verify(final VerifyConfig config, @SuppressWarnings("unused") PrintStream out)
            throws DatabaseException {
        ...
        synchronized (envImpl.getINCompressor()) {
            ...
            class Traversal implements CursorImpl.WithCursor {
                ...
                public boolean withCursor(CursorImpl cursor, @SuppressWarnings("unused") DatabaseEntry key,
            ...
            CursorImpl.traverseDbWithCursor(idDatabase, lockType, true /* allowEviction */, traversal);
            if (!traversal.allOk) {
                ret = false;
            }
        }
     ...
     * Database.getName. Do not evict (do not call
     * CursorImpl.setAllowEviction(true)) during low level DbTree operation.
     * [#15176]
     */
    public String getDbName(final DatabaseId id) throws DatabaseException {
        ...
        class Traversal implements CursorImpl.WithCursor {
            ...
            public boolean withCursor(CursorImpl cursor, DatabaseEntry key,
                                      @SuppressWarnings("unused") DatabaseEntry data)
                    throws DatabaseException {
        ...
        CursorImpl.traverseDbWithCursor(nameDatabase, LockType.NONE, false /* allowEviction */, traversal);
    ...
    public List<String> getDbNames() throws DatabaseException {
        ...
        CursorImpl.traverseDbWithCursor(nameDatabase, LockType.NONE, true /* allowEviction */,
                new CursorImpl.WithCursor() {
                    public boolean withCursor(@SuppressWarnings("unused") CursorImpl cursor, DatabaseEntry key,
                                              @SuppressWarnings("unused") DatabaseEntry data)
                            throws DatabaseException {
    ...
    /**
     * Return a list of the names of internally used databases that don't get
     * looked up through the naming tree.
     */
    ...
    /**
     * Release resources and update memory budget. Should only be called when
     * this dbtree is closed and will never be accessed again.
     */
    public void close() {
    ...
    public int getLogSize() {
        return LogUtils.getLongLogSize() + // lastAllocatedLocalDbId
                LogUtils.getLongLogSize() + // lastAllocatedReplicatedDbId
                idDatabase.getLogSize() + nameDatabase.getLogSize() + 1; // 1 byte of flags
    }
    ...
    public void writeToLog(ByteBuffer logBuffer) {
         ...
         * change between the getLogSize and writeToLog calls. Since the root is
         * infrequently logged, the simplest solution is to use fixed size
         * values. [#18540]
         */
