...
public class TransactionConfig implements Cloneable {
	 ...
	 *
	 *         Maps the existing sync settings to the equivalent durability
	 *         settings. Figure out what we should do on commit.
	 *         TransactionConfig could be set with conflicting values; take the
	 *         most stringent ones first. All environment level defaults were
	 *         applied by the caller.
	 *
	 *         ConfigSync ConfigWriteNoSync ConfigNoSync default 0 0 0 sync 0 0
	 *         1 nosync 0 1 0 write nosync 0 1 1 write nosync 1 0 0 sync 1 0 1
	 *         sync 1 1 0 sync 1 1 1 sync
	 *
	 ...
	 *
	 * @param noSync
	 *            If true, transactions exhibit the ACI (atomicity, consistency,
	 *            and isolation) properties, but not D (durability); that is,
	 *            database integrity will be maintained, but if the application
	 *            or system fails, it is possible some number of the most
	 *            recently committed transactions may be undone during recovery.
	 *            The number of transactions at risk is governed by how many log
	 *            updates can fit into the log buffer, how often the operating
	 ...
	 *            If true, transactions exhibit the ACI (atomicity, consistency,
	 *            and isolation) properties, but not D (durability); that is,
	 *            database integrity will be maintained, but if the operating
	 *            system fails, it is possible some number of the most recently
	 *            committed transactions may be undone during recovery. The
	 *            number of transactions at risk is governed by how often the
	 *            operating system flushes dirty buffers to disk, and how often
	 ...
	 *
	 * @param noWait
	 *            If true, transactions will not wait if a lock request cannot
	 *            be immediately granted, instead
	 *            {@link com.sleepycat.je.LockNotAvailableException
	 ...
	 *
	 * @param readCommitted
	 *            If true, configure the transaction for read committed
	 *            isolation.
	 *
	 ...
	 *
	 * <p>
	 * If a write operation is attempted using a read-only transaction, an
	 * {@code UnsupportedOperationException} will be thrown.
	 * </p>
	 *
	 * <p>
	 * For a read-only transaction, the transaction's {@code Durability} is
	 * ignored, even when it is explicitly specified using
	 * {@link #setDurability(Durability)}.
	 * </p>
	 *
	 * <p>
	 * In a {@link com.sleepycat.je.rep.ReplicatedEnvironment}, a read-only
	 * transaction implicitly uses
	 * {@link com.sleepycat.je.Durability.ReplicaAckPolicy#NONE}. A read-only
	 * transaction on a Master will thus not be held up, or throw
	 * {@link com.sleepycat.je.rep.InsufficientReplicasException}, if the Master
	 * is not in contact with a sufficient number of Replicas at the time the
	 ...
	 * {@link Database}s in a {@link com.sleepycat.je.rep.ReplicatedEnvironment}
	 * .
	 *
	 * <p>
	 * In a replicated environment, a given transaction may be used to write to
	 * either replicated databases or non-replicated databases, but not both. If
	 * a write operation to a replicated database is attempted when local-write
	 * is true, or to a non-replicated database when local-write is false, an
	 * {@code UnsupportedOperationException} will be thrown.
	 * </p>
	 *
	 * <p>
	 * Note that for auto-commit transactions (when the {@code Transaction}
	 ...
	 * local-write is always true for a non-replicated database, and always
	 * false for a replicated database.
	 * </p>
	 *
	 * <p>
	 * In a replicated environment, a local-write transaction implicitly uses
	 ...
	 *
	 * <p>
	 * By default the local-write setting is false, meaning that the transaction
	 * may only write to replicated Databases in a replicated environment.
	 * </p>
	 *
	 * <p>
	 * This configuration setting is ignored in a non-replicated Environment
	 * since no databases are replicated.
	 * </p>
	 *
	 * @return this
	 *
	 ...
	 *             via TransactionConfig and EnvironmentMutableConfig setters
	 */
	static void checkMixedMode(final boolean sync, final boolean noSync, final boolean writeNoSync,
			final Durability durability) throws IllegalArgumentException {
