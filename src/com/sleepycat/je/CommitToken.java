 ...
 * created on the master, it's meaningful to compare commit tokens, as described
 * in the {@link #compareTo(CommitToken)} method below. CommitTokens are
 * obtained from {@link Transaction#getCommitToken()}
 *
...
public class CommitToken implements Serializable, Comparable<CommitToken> {
	...
	/**
	 * @hidden For internal use only. Creates a CommitToken suitable for use in
	 *         a consistency policy.
	 *
	 * @param repenvUUID
	 *            identifies the replicated environment associated with the
	 *            <code>vlsn</code>
	 * @param vlsn
	 ...
	 * compare commit tokens across environments, since they represent states in
	 * unrelated serialized transaction streams.
	 * <p>
	 * CommitToken(1) < CommitToken(2) implies that CommitToken(1) represents a
	 * state of the database that preceded the state defined by CommitToken(2).
	 * 
	 * @throws IllegalArgumentException
	 *             if two tokens from different environments are compared.
	 */
	public int compareTo(CommitToken other) {
		if (!repenvUUID.equals(other.repenvUUID)) {
			throw new IllegalArgumentException("Comparisons across environments are not meaningful. "
					+ "This environment: " + repenvUUID + " other environment: " + other.getRepenvUUID());
		}
