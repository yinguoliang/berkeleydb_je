 ...
 * comparator that allows for the keys of a database to be updated, but only if
 * the updates are not significant with respect to uniqueness and ordering. Also
 * described is the fact that comparators must be used with great caution, since
 * a badly behaved comparator can cause B-tree corruption.
 * <p>
 ...
 * difficult to understand. In addition, as of JE 6, this tag interface must be
 * added to all partial comparator classes so that JE can correctly perform
 * transaction aborts, while maintaining the last committed key or duplicate
 * data values properly. In addition, for a database with duplicates configured,
 * a partial comparator (implementing this tag interface) will disable
 * optimizations in JE 6 that drastically reduce cleaner costs.
 * <p>
