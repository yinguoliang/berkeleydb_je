...
public class CursorConfig implements Cloneable {
     ...
     * isolation. This ensures the stability of the current data item read by
     * the cursor but permits data read by this cursor to be modified or deleted
    ...
    /**
     * Configures read operations performed by the cursor to return modified but
    ...
    /**
     * Returns true if read operations performed by the cursor are configured to
     * return modified but not yet committed data.
     *
     * @return true if read operations performed by the cursor are configured to
     *         return modified but not yet committed data.
     * @see LockMode#READ_UNCOMMITTED
     */
    ...
    /**
     * Configures read operations performed by the cursor to obey read committed
     * isolation. Read committed isolation provides for cursor stability but not
     ...
     *
     * @param readCommitted If true, configure read operations performed by the
     *            cursor to obey read committed isolation.
     ...
     * returns {@link OperationStatus#NOTFOUND}. By default, a cursor is
     * "sticky", meaning that the prior position is maintained by cursor
     * movement operations, and the cursor stays at the prior position when the
     * operation does not succeed. For example, if {@link Cursor#getFirst} is
     * called successfully, and then {@link Cursor#getNext} is called, if
     * {@code getNext} returns {@code NOTFOUND} the cursor will remain
     ...
     * position. In the example above, a lock on the first record will still be
     * held after {@code getNext} returns {@code NOTFOUND}.
     * <p>
     * If the cursor is configured to be non-sticky, the prior position is not
     * maintained, and this has certain performance advantages:
     * <ul>
     * <li>Some processing is avoided because the prior position is not
     * maintained.</li>
     * <li>The lock on the record at the prior position is released before
     * acquiring the lock on the record at the new position (when the cursor
     * movement operation succeeds.) This can help to prevent deadlocks in
     * certain situations. Namely, if the cursor's isolation mode allows locks
     * to be released when moving to a new position, then only one lock at a
     * time will be held by the cursor. Holding multiple locks at a time can
     * cause deadlocks, when locks are acquired in different orders by different
     * threads, for example, when one cursor is scanning forward and another
     * cursor is scanning backward. Note that this optimization does not apply
     * to repeatable-read or serializable isolation, since these modes require
     * that locks are not released by cursor movement operations.</li>
     * </ul>
     * <p>
     ...
     * <p>
     * Also note that in certain circumstances, internal algorithms require that
     * the prior position is retained, and the operation will behave as if the
     * cursor is sticky. Specifically, these are only the following methods, and
     * only when called on a database with duplicates configured:
     * <ul>
     ...
     * </ul>
     *
     * @param nonSticky if false (the default), the prior position is maintained
     *            by cursor movement operations, and the cursor stays at the
     *            prior position when {@code NOTFOUND} is returned. If true, the
     *            prior position is not maintained, and the cursor is
     *            reinitialized when {@code NOTFOUND} is returned.
     * @return this
     */
