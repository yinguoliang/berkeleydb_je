 ...
 * this <a href="{@docRoot}/../GettingStartedGuide/logfilesrevealed.html"
 * > overview</a> of the logging system. Log files are removed as their contents
 * become obsolete.</li>
...
public class EnvironmentStats implements Serializable {
	 ...
	 * memory at the expense of potentially hotter clean nodes. Specifically, a
	 * node that is selected for eviction from level-1 (the "mixed" LRU) will be
	 ...
	 * nodes referring to off-heap LNs are selected based on LRU. When there are
	 * no more such nodes then dirty nodes with no off-heap LNs are selected,
	 * based on LRU.
	 * <p>
	 ...
	 *
	 * The user may want to change the size of the evictor pool or the number of
	 ...
	 * mutated, it is is copied to a new, smaller off-heap block. After mutating
	 * an off-heap BIN, it is moved to the hot end of the LRU list.
	 *
	 * <p>
	 * If this environment uses the shared cache, the return value is the total
	 * for all environments that are sharing the cache.
	 ...
	 * another evictor thread. This can occur because there is a short period of
	 * time where a targeted node has been removed from the LRU by the evictor
	 ...
	 *
	 * LNs are loaded when requested by CRUD operations or other internal btree
	 * operations.
	 *
	 * <p>
	 * If this environment uses the shared cache, the return value is the total
	 ...
	 *
	 * BINs are loaded when needed by CRUD operations or other internal btree
	 * operations.
	 *
	 * <p>
	 * If this environment uses the shared cache, the return value is the total
	 ...
	 *
	 * There is one block for each off-heap BIN and one for each off-heap LN. So
	 * the total number of blocks is the sum of {@link #getOffHeapCachedLNs} and
	 ...
	 *
	 * The off-heap LRU list is stored in the Java heap. Each entry occupies 20
	 * bytes of memory when compressed oops are used, or 24 bytes otherwise.
