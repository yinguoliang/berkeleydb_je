 ...
 * statistics through {@link EnvironmentStats#toString()} displays the values in
 * these categories, as does viewing the stats through the
 * {@link <a href="{@docRoot}/../jconsole/JConsole-plugin.html">JEMonitor
 * mbean</a>}. Viewing the stats with {@link EnvironmentStats#toStringVerbose()}
 * will provide more detailed descriptions of the stats and stat categories.
 * <p>
 ...
 * <li><b>Log Cleaning</b>: Maintenance of JE's append only storage system. See
 * this <a href=
 * "{@docRoot}/../GettingStartedGuide/logfilesrevealed.html" > overview</a> of
 * the logging system. Log files are removed as their contents become obsolete.
 * </li>
 ...
 * </li>
 * <li><b>Environment</b>: Miscellaneous environment wide statistics.</li>
...
public class EnvironmentStats implements Serializable {
    ...
    public EnvironmentStats() {
        ...
        offHeapStats = new StatGroup(OffHeapStatDefinition.GROUP_NAME, OffHeapStatDefinition.GROUP_DESC);
        ckptStats = new StatGroup(CheckpointStatDefinition.GROUP_NAME, CheckpointStatDefinition.GROUP_DESC);
        cleanerStats = new StatGroup(CleanerStatDefinition.GROUP_NAME, CleanerStatDefinition.GROUP_DESC);
        ...
        envImplStats = new StatGroup(DbiStatDefinition.ENV_GROUP_NAME, DbiStatDefinition.ENV_GROUP_DESC);
        throughputStats = new StatGroup(DbiStatDefinition.THROUGHPUT_GROUP_NAME,
    ...
    public List<StatGroup> getStatGroups() {
        return Arrays.asList(logStats, cacheStats, offHeapStats, cleanerStats, incompStats, ckptStats, envImplStats,
                lockStats, throughputStats);
    }
    ...
    public void setStatGroup(StatGroup sg) {
        ...
        if (sg.getName().equals(INCompStatDefinition.GROUP_NAME)) {
            ...
            throw EnvironmentFailureException
                    .unexpectedState("Invalid stat group name in setStatGroup " + sg.getName());
        }
    }
    ...
    /**
     * @deprecated in 7.0, always returns zero. Use
     *             {@link #getCurrentMinUtilization()} and
     *             {@link #getCurrentMaxUtilization()} to monitor cleaner
     *             behavior.
     */
     ...
     * following reasons:
     * <ul>
     * <li>A checkpoint has not yet completed. Files are deleted only at the end
     * of each checkpoint.</li>
     * <li>A read-only process is running, which prevents file deletion.</li>
     * <li>A file is protected from deletion by an in-progress
     * {@link com.sleepycat.je.util.DbBackup}.</li>
     ...
     * files) that is utilized or active. The remaining portion of the log is
     * obsolete. The log cleaner is responsible for keeping the log utilization
     * below the configured threshold,
     * {@link EnvironmentConfig#CLEANER_MIN_UTILIZATION}.
     * <p>
     * This statistic is computed every time the log cleaner examines the
     * utilization of the log, in order to determine whether cleaning is needed.
     * The frequency can be configured using
     ...
     * <p>
     * Note that the size of the utilized data in the log is always greater than
     * the amount of user data (total size of keys and data). The active Btree
     * internal nodes and other metadata are also included.
     * <p>
     *
     * @return the current minimum utilization, or -1 if the utilization has not
     ...
     * <p>
     *
     * @return the current maximum utilization, or -1 if the utilization has not
     *         been calculated for this environment since it was last opened.
     * @since 6.5
     */
     ...
     * two-pass runs} but not including {@link #getNCleanerRevisalRuns() revisal
     * runs}. The {@link #getCurrentMinUtilization() minimum} and
     ...
     * {@link #getNCleanerRuns() total cleaner runs}. The
     * {@link #getCurrentMinUtilization() minimum} and
     * {@link #getCurrentMaxUtilization() maximum} utilization values are used
     * to drive cleaning.
     * <p>
     ...
     * expired size. The minimum and maximum utilization are the lower and upper
     ...
     * <p>
     * If they overlap completely then the minimum utilization is correct, while
     * if there is no overlap then the maximum utilization is correct. Both
     * utilization values trigger cleaning, but when there is significant
     ...
     * recomputed expiration histogram, which has size information that now does
     * not overlap with the strictly obsolete data, is stored for future use. By
     * storing the revised histogram, the cleaner can select the most
    ...
    /**
     * Number of cleaner runs that ended in revising expiration info, but not in
     * any cleaning.
     *
    ...
    /**
     * The accumulated number of obsolete LNs that were expired. Note that this
     * does not included embedded LNs (those having a data size less than
     * {@link EnvironmentConfig#TREE_MAX_EMBEDDED_LN}), because embedded LNs are
     ...
     * <p>
     * Note that the log entries prior to this position may not yet have been
     * flushed to disk. Flushing can be forced using a Sync or WriteNoSync
     * commit, or a checkpoint.
     * </p>
     */
    ...
    /**
     * The number of fsyncs issued through the group commit manager. A subset of
     * nLogFsyncs.
     */
    ...
    /**
     * The number of disk reads which required repositioning the disk head more
     * than 1MB from the previous file position. Reads in a different *.jdb log
     * file then the last IO constitute a random read.
     * <p>
    ...
    /**
     * The number of bytes read which required repositioning the disk head more
     * than 1MB from the previous file position. Reads in a different *.jdb log
     * file then the last IO constitute a random read.
     * <p>
     ...
     * head more than 1MB from the previous file position. Reads in a different
     ...
     * different *.jdb log file (i.e. a file "flip") then the last IO constitute
     * a random write.
     * <p>
     * This number is approximate and may differ from the actual number of
    ...
    /**
     * The number of bytes written which did not require repositioning the disk
     * head more than 1MB from the previous file position. Writes in a different
     * *.jdb log file then the last IO constitute a random write.
     * <p>
     * This number is approximate vary depending on the type of disks and file
     * system, disk geometry, and file system cache size.
    ...
    /**
     * The number of file read operations which were fulfilled by reading out of
    ...
    /**
     * The number of writes operations which would overflow the Write Queue and
     * could not be queued.
     */
    ...
    /**
     * The number of requests for database objects not contained within the in
    ...
    /**
     * @deprecated This statistic has no meaning after the implementation of the
     *             new evictor in JE 6.0. The method returns 0 always.
     */
     ...
     * dirty nodes evicted from the main cache and stored off-heap. Can be used
     * to determine how much logging and its associated costs (cleaning, etc)
     * are being caused by eviction.
     */
    ...
    /**
     * Number of nodes back into the LRU without any action taken on them. For
     * example, a target BIN will immediatelly be put back if it is pinned, or
     * is marked "hot", or none of it LNs are evictable."
     */
    ...
    /**
     * Number of BINs (bottom internal nodes) requested by btree operations. Can
    ...
    /**
     * The ratio between the number of BINs (full or deltas) fetched to satisfy
     * btree operations and the number of BINs requested by btree operations.
     */
    ...
    /**
     * Number of INs (internal nodes) in cache. The cache holds INs and BINS, so
     * this indicates the proportion used by each type of node. When used on
    ...
    /**
     * Number of INs that use a compact sparse array representation to point to
     * child nodes in the cache. This helps provide some insight into what is
     ...
     * nodes are in the cache. This helps provide some insight into what is
     * resident in the cache.
     */
     ...
     * object representation overhead. This helps provide some insight into what
     * is resident in the cache. In addition, if this number is low, and the
     * application may want to try using Database.setKeyPrefix to enable key
     * prefixing, and to see if that lets JE use the compact key representation
     * more often.
     */
    ...
    /*
     * The size of the dirty LRU at the time that stats were last collected. JE
     * uses a 2-level LRU policy that aims to keep dirty BTree nodes in memory
     * at the expense of potentially hotter clean nodes. Specifically, a node
     * that is selected for eviction from level-1 (the "mixed" LRU) will be
     * moved to level-2 (the "dirty" LRU) if it is dirty. Nodes in level-2 are
    ...
    /*
     * The size of the mixed LRU at the time that stats were last collected. JE
     * uses a 2-level LRU policy that aims to keep dirty BTree nodes in memory
     * at the expense of potentially hotter clean nodes. Specifically, a node
     * that is selected for eviction from level-1 (the "mixed" LRU) will be
     * moved to level-2 (the "dirty" LRU) if it is dirty. Nodes in level-2 are
    ...
    /**
     * Number of bytes evicted in the course of executing operations that use an
     * explicitly set {@link CacheMode}. It serves as an indicator of which part
    ...
    /**
     * Number of bytes evicted in the course of executing operations that will
     * cause the cache to go over budget. It serves as an indicator of which
    ...
    /**
     * Number of bytes evicted by JE deamon threads. It serves as an indicator
     * of which part of the system is doing eviction work.
     */
     ...
     * used by all environments that are sharing the cache. If this environment
     * does not use the shared cache, this method returns zero.
     * <p>
     * To get the configured maximum cache size, see
     * {@link EnvironmentMutableConfig#getCacheSize}.
     * </p>
     */
     ...
     * environment.
     * <p>
     * This method returns the sum of {@link #getDataBytes},
     * {@link #getAdminBytes}, {@link #getLockBytes} and {@link #getBufferBytes}
     * .
     * </p>
     * <p>
     * To get the configured maximum cache size, see
    ...
    /**
     * The amount of JE cache consumed by disk-ordered scans, in bytes. If this
     * environment uses the shared cache, this method returns only the amount
     * used by this environment.
     */
     ...
     *             used for data and use {@link #getAdminBytes},
     *             {@link #getLockBytes} and {@link #getBufferBytes} to get
     *             other components of the total cache usage (
     ...
     * the machine or VM. In practice, we have not seen this occur because Linux
     * will automatically kill processes that are rapidly allocating memory when
     * available memory is very low.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
     */
     ...
     * is available. Even so, the off-heap evictor normally prevents overflowing
     * of the off-heap cache by freeing memory before it is needed.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
     ...
     * evictor pool, but were refused because all off-heap eviction threads were
     * busy. The user may want to change the size of the evictor pool through
     * the EnvironmentConfig.OFFHEAP_* properties.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of BINs selected as off-heap eviction targets. Nodes are selected
     * as targets by the evictor based on LRU, always selecting from the cold
     * end of the LRU list. First, non-dirty nodes and nodes referring to
     * off-heap LNs are selected based on LRU. When there are no more such nodes
     * then dirty nodes with no off-heap LNs are selected, based on LRU.
     * <p>
     ...
     * potentially causing long operation latency in application threads. The
     * user may want to change the size of the evictor pool or the number of
     * eviction bytes through the EnvironmentConfig.OFFHEAP_* properties.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of BINs (dirty and non-dirty) evicted from the off-heap cache. An
     * evicted BIN is completely removed from the off-heap cache and LRU list.
     * If it is dirty, it must be logged. A BIN is evicted only if it has no
     * off-heap child LNs and it cannot be mutated to a BIN-delta.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of target BINs evicted from the off-heap cache that were dirty and
     * therefore were logged. Can be used to determine how much logging and its
     ...
     * main cache BIN is stripped, it is removed from the off-heap LRU. When an
     * off-heap BIN is stripped, it is either modified in place to remove the LN
     * references (this is done when a small number of LNs are referenced and
     * the wasted space is small), or is copied to a new, smaller off-heap block
     * with no LN references.
     * <p>
    ...
    /**
     * Number of off-heap target BINs mutated to BIN-deltas. Mutation to a
     * BIN-delta is performed for full BINs that do not refer to any off-heap
     * LNs and can be represented as BIN-deltas in cache and on disk (see
     * {@link EnvironmentConfig#TREE_BIN_DELTA}). When a BIN is mutated, it is
     * is copied to a new, smaller off-heap block. After mutating an off-heap
     * BIN, it is moved to the hot end of the LRU list.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of off-heap target BINs on which no action was taken. For example,
     * a node will be skipped if it has been moved to the hot end of the LRU
     * list by another thread, or more rarely, already processed by another
     * evictor thread. This can occur because there is a short period of time
     * where a targeted node has been removed from the LRU by the evictor
     * thread, but not yet latched.
     * <p>
    ...
    /**
     * Number of LNs loaded from the off-heap cache. LNs are loaded when
     * requested by CRUD operations or other internal btree operations.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of LNs stored into the off-heap cache. LNs are stored off-heap
     * when they are evicted from the main cache. Note that when
     * {@link CacheMode#EVICT_LN} is used, the LN resides in the main cache for
     * a very short period since it is evicted after the CRUD operation is
     * complete.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
    ...
    /**
     * Number of BINs loaded from the off-heap cache. BINs are loaded when
     * needed by CRUD operations or other internal btree operations.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
    ...
    /**
     * Number of BINs stored into the off-heap cache. BINs are stored off-heap
     * when they are evicted from the main cache. Note that when
     * {@link CacheMode#EVICT_BIN} is used, the BIN resides in the main cache
     * for a very short period since it is evicted after the CRUD operation is
     * complete.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
    ...
    /**
     * Total number of estimated bytes in the off-heap cache. This includes the
     * estimated overhead for off-heap memory blocks, as well as their contents.
     * <p>
     * If this environment uses the shared cache, the return value is the total
     * for all environments that are sharing the cache.
     * </p>
     * <p>
     * To get the configured maximum off-heap cache size, see
     * {@link EnvironmentMutableConfig#getOffHeapCacheSize()}.
     * </p>
     */
    ...
    /**
     * Total number of memory blocks in the off-heap cache. There is one block
     * for each off-heap BIN and one for each off-heap LN. So the total number
     * of blocks is the sum of {@link #getOffHeapCachedLNs} and
    ...
    /**
     * Number of entries in the off-heap LRU list. The off-heap LRU list is
     * stored in the Java heap. Each entry occupies 20 bytes of memory when
     * compressed oops are used, or 24 bytes otherwise. This memory is not
     * considered part of the JE main cache, and is not included in main cache
     * statistics.
     * <p>
     ...
     * avoids an LRU entry per off-heap LN, which would use excessive amounts of
     * space in the Java heap. Similarly, when an off-heap BIN refers to
    ...
    /**
     * Returns the number of latch upgrades (relatches) required while operating
     * on this Environment. Latch upgrades are required when an operation
     * assumes that a shared (read) latch will be sufficient but later
     * determines that an exclusive (write) latch will actually be required.
     *
    ...
    /**
     * Total lock owners in lock table. Only provided when
     * {@link com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
    ...
    /**
     * Total read locks currently held. Only provided when
     * {@link com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
    ...
    /**
     * Total locks currently in lock table. Only provided when
     * {@link com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
    ...
    /**
     * Total transactions waiting for locks. Only provided when
     * {@link com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
    ...
    /**
     * Total write locks currently held. Only provided when
     * {@link com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
    ...
    /**
     * Number of acquires of lock table latch when it was already owned by the
     * caller.
     */
     ...
     * performed in BIN deltas (these are updates performed via the various put
     * methods).
     */
     ...
     * Cursor.get} or
     * {@link Database#get(Transaction, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * Database.get} call with {@link Get#SEARCH}, {@link Get#SEARCH_GTE},
     * {@link Get#SEARCH_BOTH}, or {@link Get#SEARCH_BOTH_GTE}.</li>
     * <li>A successful
     * {@link SecondaryCursor#get(DatabaseEntry, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * SecondaryCursor.get} or
     * {@link SecondaryDatabase#get(Transaction, DatabaseEntry, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * SecondaryDatabase.get} call when the primary data is requested (via the
     * {@code data} param). This call internally performs a key search operation
     * in the primary DB in order to return the data.</li>
     * </ul>
     */
     ...
     * <p>
     * This operation corresponds to a call to
     * {@link Cursor#get(DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * Cursor.get} or
     * {@link Database#get(Transaction, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * Database.get} with {@link Get#SEARCH}, {@link Get#SEARCH_GTE},
     * {@link Get#SEARCH_BOTH}, or {@link Get#SEARCH_BOTH_GTE}, when the
     * specified key is not found in the DB.
     */
     ...
     * <p>
     * This operation corresponds to a successful call to
     * {@link SecondaryCursor#get(DatabaseEntry, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * SecondaryCursor.get} or
     * {@link SecondaryDatabase#get(Transaction, DatabaseEntry, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * SecondaryDatabase.get} with {@link Get#SEARCH}, {@link Get#SEARCH_GTE},
     * {@link Get#SEARCH_BOTH}, or {@link Get#SEARCH_BOTH_GTE}.
     * <p>
     ...
     * Cursor.get} or
     * {@link Database#get(Transaction, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * Database.get} with {@link Get#FIRST}, {@link Get#LAST}, {@link Get#NEXT},
     * {@link Get#NEXT_DUP}, {@link Get#NEXT_NO_DUP}, {@link Get#PREV},
     * {@link Get#PREV_DUP} or {@link Get#PREV_NO_DUP}.
     */
     ...
     * SecondaryCursor.get} or
     * {@link SecondaryDatabase#get(Transaction, DatabaseEntry, DatabaseEntry, DatabaseEntry, Get, ReadOptions)
     * SecondaryDatabase.get} with {@link Get#FIRST}, {@link Get#LAST},
     ...
     * Cursor.put} or
     * {@link Database#put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Database.put} in one of the following cases:
     * <ul>
     * <li>When {@link Put#NO_OVERWRITE} or {@link Put#NO_DUP_DATA} is
     * specified.</li>
     * <li>When {@link Put#OVERWRITE} is specified and the key was inserted
     * because it previously did not exist in the DB.</li>
     * </ul>
     */
     ...
     * Cursor.put} or
     * {@link Database#put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Database.put} with {@link Put#NO_OVERWRITE} or {@link Put#NO_DUP_DATA},
     * when the key could not be inserted because it previously existed in the
     * DB.
     */
     ...
     * Cursor.put} or
     * {@link Database#put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Database.put}, for a primary DB with an associated secondary DB. A
     * secondary record is inserted when inserting a primary record with a
     * non-null secondary key, or when updating a primary record and the
     * secondary key is changed to to a non-null value that is different than
     ...
     * Database.put} in one of the following cases:
     * <ul>
     * <li>When {@link Put#OVERWRITE} is specified and the key previously
     * existed in the DB.</li>
     * <li>When calling {@code Cursor.put} with {@link Put#CURRENT}.</li>
     * </ul>
     */
     ...
     * Cursor.put} or
     * {@link Database#put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Database.put}, when a primary record is updated and its TTL is changed.
     * The associated secondary records must also be updated to reflect the
     * change in the TTL.
     * <p>
     * Note: Operations are currently counted as secondary DB (rather than
     * primary DB) operations only if the DB has been opened by the application
     ...
     * <p>
     * This operation corresponds to a successful call to {@link Cursor#delete()
     * Cursor.delete},
     * {@link Database#delete(Transaction, DatabaseEntry, WriteOptions)
     * Database.delete}, {@link SecondaryCursor#delete() SecondaryCursor.delete}
     * or
     * {@link SecondaryDatabase#delete(Transaction, DatabaseEntry, WriteOptions)
     ...
     * This operation corresponds to one of the following API calls:
     * <ul>
     * <li>A successful call to {@link Cursor#delete() Cursor.delete} or
     * {@link Database#delete(Transaction, DatabaseEntry, WriteOptions)
     * Database.delete}, that deletes a primary record containing a non-null
     * secondary key.</li>
     * <li>A successful call to {@link SecondaryCursor#delete()
     * SecondaryCursor.delete} or
     * {@link SecondaryDatabase#delete(Transaction, DatabaseEntry, WriteOptions)
     * SecondaryDatabase.delete}.</li>
     * <li>A successful call to
     * {@link Cursor#put(DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Cursor.put} or
     * {@link Database#put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)
     * Database.put} that updates a primary record and changes its previously
     * non-null secondary key to null.</li>
     * </ul>
     * <p>
     * Note: Operations are currently counted as secondary DB (rather than
     * primary DB) operations only if the DB has been opened by the application
