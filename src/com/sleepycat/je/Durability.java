...
public class Durability {
	 ...
	 *
	 * The replicated environment policies default to COMMIT_NO_SYNC for commits
	 * of replicated transactions that need acknowledgment and SIMPLE_MAJORITY
	 * for the acknowledgment policy.
	 */
	 ...
	 *
	 * <p>
	 * Note that SECONDARY nodes are not included in the set of replicas that
	 * must acknowledge transaction commits.
	 */
	public enum ReplicaAckPolicy {
		 ...
		 *
		 * @param groupSize
		 *            the number of ELECTABLE replicas in the replication group
		 *
	 ...
	 *
	 * @param localSync
	 *            the SyncPolicy to be used when committing the transaction
	 *            locally.
	 * @param replicaSync
	 *            the SyncPolicy to be used remotely, as part of a transaction
	 *            acknowledgment, at a Replica node.
	 * @param replicaAck
	 ...
	 *             if the durabilityString is invalid.
	 */
	public static Durability parse(String durabilityString) {
		...
		SyncPolicy replicaSync = tokenizer.hasMoreTokens() ? SyncPolicy.valueOf(tokenizer.nextToken())
				: SyncPolicy.NO_SYNC;
		ReplicaAckPolicy replicaAck = tokenizer.hasMoreTokens() ? ReplicaAckPolicy.valueOf(tokenizer.nextToken())
