...
public class Cursor implements ForwardCursor {
	 ...
	 *
	 * @param cacheMode
	 *            is the default {@code CacheMode} used for subsequent
	 ...
	 *
	 *         A RangeConstraint is used by search-range and next/previous
	 *         methods to prevent keys that are not inside the range from being
	 *         returned.
	 *
	 *         This method is not yet part of the public API because it has not
	 *         been designed with future-proofing or generality in mind, and has
	 *         not been reviewed.
	 */
	 ...
	 *
	 * @param samePosition
	 *            If true, the newly created cursor is initialized to refer to
	 *            the same position in the database as the original cursor (if
	 *            any) and hold the same locks (if any). If false, or the
	 *            original cursor does not hold a database position and locks,
	 *            the returned cursor is uninitialized and will behave like a
	 *            newly created cursor.
	 *
	 * @return A new cursor with the same transaction and locker ID as the
	 *         original cursor.
	 *
	 * @throws com.sleepycat.je.rep.DatabasePreemptedException
	 *             in a replicated environment if the master has truncated,
	 *             removed or renamed the database.
	 *
	 * @throws OperationFailureException
	 *             if this exception occurred earlier and caused the transaction
	 *             to be invalidated.
	 *
	 * @throws EnvironmentFailureException
	 *             if an unexpected, internal or environment-wide failure
	 ...
	 *
	 * @param options
	 *            the WriteOptions, or null to use default options.
	 *
	 ...
	 *             if the database is transactional but this cursor was not
	 *             opened with a non-null transaction parameter, or the database
	 *             is read-only.
	 *
	 * @throws IllegalStateException
	 *             if the cursor or database has been closed, or the cursor is
	 *             uninitialized (not positioned on a record), or the
	 ...
	 *
	 * @param key
	 *            the key used as <a href="DatabaseEntry.html#inParam">input</a>
	 ...
	 *
	 * @param putType
	 *            the Put operation type. May not be null.
	 *
	 * @param options
	 *            the WriteOptions, or null to use default options.
	 *
	 ...
	 *             if putType is Put.CURRENT and the old and new data are not
	 *             equal according to the configured duplicate comparator or
	 *             default comparator.
	 *
	 * @throws OperationFailureException
	 *             if one of the
	 ...
	 *             if the database is transactional but this cursor was not
	 *             opened with a non-null transaction parameter, or the database
	 *             is read-only, or putType is Put.NO_DUP_DATA and the database
	 *             is not configured for duplicates.
	 *
	 * @throws IllegalStateException
	 *             if the cursor or database has been closed, or the
	 *             non-transactional cursor was created in a different thread.
	 *
	 * @throws IllegalArgumentException
	 *             if an invalid parameter is specified. This includes passing a
	 *             null putType, a null input key/data parameter, an input
	 *             key/data parameter with a null data array, a partial key/data
	 *             input parameter.
	 *
	 * @since 7.0
	 */
	 ...
	 *             if the database is transactional but this cursor was not
	 *             opened with a non-null transaction parameter, or the database
	 *             is read-only, or the database is not configured for
	 *             duplicates.
	 *
	 * @throws IllegalStateException
	 *             if the cursor or database has been closed, or the
	 ...
	 *
	 * @throws DuplicateDataException
	 *             if the old and new data are not equal according to the
	 ...
	 *
	 * @param key
	 *            the key input or output parameter, depending on getType.
	 *
	 * @param data
	 *            the data input or output parameter, depending on getType.
	 *
	 * @param getType
	 *            the Get operation type. May not be null.
	 *
	 * @param options
	 ...
	 *
	 * @throws IllegalStateException
	 *             if the cursor or database has been closed, the cursor is
	 *             uninitialized (not positioned on a record) and this is not
	 *             permitted (see above), or the non-transactional cursor was
	 *             created in a different thread.
	 *
	 * @throws IllegalArgumentException
	 *             if an invalid parameter is specified. This includes passing a
	 *             null getType, a null input key/data parameter, an input
	 *             key/data parameter with a null data array, a partial key/data
	 *             input parameter, and specifying a
	 ...
	 * efficiency reasons: in Cursor delete we must separately fetch the key and
	 * data, while in Database delete we know the key and have to search anyway
	 * so we can get the old data when we search. The two algorithms need to be
	 * kept in sync.
	 */
	...
	OperationResult putForReplay(final DatabaseEntry key, final DatabaseEntry data, final LN ln, final int expiration,
			final boolean expirationInHours, final PutMode putMode, final ReplicationContext repContext) {
	 ...
	 * call putNoOverwrite concurrently, only one of them will succeed. In other
	 * words, if putNoOverwrite is called for all dup insertions, there will
	 * always be at most one dup per key.
	 *
	 ...
	 * ever, used by applications (for dup DBs that is). It exists primarily for
	 * compatibility with the DB core API.
	 */
	 ...
	 * because secondary updates are part of the replication stream. However, it
	 * is fragile because other operations, when no Database handle is used,
	 ...
	 *
	 * @param putMode
	 *            One of OVERWRITE, NO_OVERWITE, CURRENT. (NO_DUPS_DATA has been
	 *            converted to NO_OVERWRITE). Note: OVERWRITE may perform an
	 *            insertion or an update, NO_OVERWRITE performs insertion only,
	 *            and CURRENT updates the slot where the cursor is currently
	 *            positioned at.
	 *
	 * @param key
	 *            The new key value for the BIN slot S to be inserted/updated.
	 ...
	 *
	 * @param data
	 *            The new data for the LN associated with the BIN slot. For dups
	 *            DBs it is EMPTY_DUPS_DATA. Note: for dups DBs the original,
	 *            user-provided "data" must not be partial.
	 *
	 * @param ln
	 *            LN to be inserted, if insertion is allowed by putMode. null
	 *            for CURRENT (since insertion is not allowed), not null for
	 *            other modes.
	 */
	private OperationResult putNotify(DatabaseEntry key, final DatabaseEntry data, final LN ln,
		...
		try {
			 ...
			 * fetched.
			 *
			 * The latter case is acceptable because the old data is needed only
			 * to delete secondary records, and if the LN expired then those
			 * secondary records will also expire naturally. The old
	 ...
	 *
	 * @param putMode
	 *            is either OVERWRITE, NO_OEVERWRITE, or BLIND_INSERTION
	 *
	 * @param key
	 *            The new key value for the BIN slot S to be inserted/updated.
	 ...
	 *
	 * @param data
	 *            In case of update, the new data to (perhaps partially) replace
	 *            the data of the LN associated with the BIN slot. For dups DBs
	 *            it is EMPTY_DUPS_DATA. Note: for dups DBs the original,
	 *            user-provided "data" must not be partial.
	 *
	 * @param ln
	 *            is normally a new LN node that is created for insertion, and
	 *            will be discarded if an update occurs. However, HA will pass
	 *            an existing node.
	 *
	 * @param returnOldData
	 *            To receive, in case of update, the old LN data (before the
	 *            update). It is needed only by DBs with indexes/triggers; will
	 *            be null otherwise.
	 *
	 * @param returnNewData
	 *            To receive the full data of the new or updated LN. It is
	...
	private OperationResult putNoNotify(final DatabaseEntry key, final DatabaseEntry data, final LN ln,
			...
			final DatabaseEntry returnOldData, final DatabaseEntry returnNewData, final ReplicationContext repContext) {
	 ...
	 *
	 * @param key
	 *            The new key value for the BIN slot S to be updated. Cannot be
	 ...
	 *
	 * @param data
	 *            The new data to (perhaps partially) replace the data of the LN
	 *            associated with the BIN slot. For dups DBs it is
	 *            EMPTY_DUPS_DATA. Note: for dups DBs the original,
	 *            user-provided "data" must not be partial.
	 *
	 * @param returnOldData
	 *            To receive the old LN data (before the update). It is needed
	 *            only by DBs with indexes/triggers; will be null otherwise.
	 *
	 * @param returnNewData
	 *            To receive the full data of the updated LN. It is needed only
	 *            by DBs with indexes/triggers and only if "data" is partial;
	 *            will be null otherwise. Note: "returnNewData" may be different
	...
	private OperationResult putCurrentNoNotify(final DatabaseEntry key, final DatabaseEntry data,
			final DatabaseEntry returnOldData, final DatabaseEntry returnNewData, final CacheMode cacheMode,
			final ExpirationInfo expInfo, final ReplicationContext repContext) {
	...
	private OperationResult positionHandleDups(final DatabaseEntry key, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final boolean first) {
	...
	private OperationResult positionAllowPhantoms(final DatabaseEntry key, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final boolean rangeLock, final boolean first) {
	...
	private OperationResult retrieveNextHandleDups(final DatabaseEntry key, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final GetMode getMode) {
	...
	private OperationResult retrieveNextNoDups(final DatabaseEntry key, final DatabaseEntry data,
			final LockMode lockMode, final CacheMode cacheMode, final GetMode getModeParam) {
	...
	private OperationResult retrieveNextCheckForInsertion(final DatabaseEntry key, final DatabaseEntry data,
			final LockType lockType, final CacheMode cacheMode, final GetMode getMode) {
	 ...
	 * duplicates-set of "key". So there is no way to get an exact key match by
	 * a BTree search. Instead, we do a constrained range search: we forbid the
	 * cursor to advance past the duplicates-set of "key" by using an
	 ...
	 *
	 * Do range search for combined key. Compare result to prefix to see whether
	 * we went out of the bounds of the duplicate set, i.e., whether null should
	 * be returned.
	 */
	...
	private OperationResult searchNoDups(final DatabaseEntry key, final DatabaseEntry data, final LockMode lockMode,
			final CacheMode cacheMode, final SearchMode searchMode, final Comparator<byte[]> comparator) {
	 ...
	 *
	 * Note: On SUCCESS, if this is a sticky cursor, any non-transactional locks
	 * held by this cursor before calling this method are released.
	 *
	 * Note: this method is never called when the desired isolation is
	 * "serializable", because in order to do next-slot-locking, a range search
	 * is required.
	 *
	 * @param key
	 *            It is used as the search key, as well as to receive the key of
	 ...
	 *
	 * @param data
	 *            A DatabaseEntry to compare against the LN of the slot found by
	 *            the search (if searchMode == BOTH) as well as to receive the
	 *            data of that LN. If the DB contains duplicates, it is equal to
	 *            NO_RETURN_DATA, because the LN will be emtpy (the full record
	 ...
	 * and "data" respectively (if "key"/"data" request so). Note that the fact
	 * that the slot is valid implies that it has been locked.
	 *
	 * Note: On return from this method no latches are held by this cursor.
	 *
	 * Note: If the method returns NOTFOUND or raises an exception, any non-
	 ...
	 *
	 * @param data
	 *            A DatabaseEntry to receive the data of the LN associated with
	 ...
	 *
	 * @param comparator
	 *            Comparator to use to compare the search key against the BTree
	 *            keys.
	 *
	 ...
	 *             if the search should be restarted by the caller.
	 */
	private OperationResult searchRange(final DatabaseEntry key, final DatabaseEntry data, final LockMode lockMode,
			final CacheMode cacheMode, Comparator<byte[]> comparator) throws RangeRestartException {
	 ...
	 * its KD flags are set.
	 *
	 * If such a slot is found, copy its key and it associated LN into "key" and
	 * "data" respectively (if "key"/"data" request so). Note that the fact that
	 * the slot is valid implies that it has been locked. If the key of the
	 * found slot is == K1, it is locked in a non-range lock. If the key is >
	 * K1, the slot is locked in a range lock.
	 *
	 ...
	 *            full record is contained in the key).
	 *
	 * @param searchLockType
	 *            LockType to use for locking the slot if its key is == search
	 *            key. Normally, this is a READ or WRITE lock.
	 *
	 * @param advanceLockType
	 *            LockType to use for locking the slot if its key is > search
	 *            key. Normally, this is a READ_RANGE or WRITE_RANGE lock.
	 *
	 * @param comparator
	 *            Comparator to use to compare the search key against the BTree
	 *            keys.
	 *
	 * @param searchMode
	 *            If SET or BOTH, we are actually looking for an exact match on
	 *            K1. If so and K1 is not in the BTree, we want the cursor to
	 *            advance temporarily to the next slot in order to range-lock
	 *            it, but then return NOTFOUND. NOTFOUND is returned also if K1
	 *            is found, but searchMode is BOTH and the data associated with
	 *            the K1 slot does not match the given data.
	 *
	...
	private OperationResult searchRangeSerializable(final DatabaseEntry key, final DatabaseEntry data,
			final LockType searchLockType, final LockType advanceLockType, final Comparator<byte[]> comparator,
		 ...
		 * another transaction can insert new slots at the end of the DB and
		 * then commit. I think the fix is to request the eof lock in
		 * non-blocking mode with the BIN latched and restart the search if the
		 * lock is denied.
		 */
		if (mustLockEOF) {
			cursorImpl.lockEof(LockType.RANGE_READ);
	...
	private OperationResult searchRangeAdvanceAndCheckKey(final CursorImpl dup, final DatabaseEntry key,
			final DatabaseEntry data, final LockType lockType, final boolean dirtyReadAll,
			Comparator<byte[]> comparator, final RangeConstraint rangeConstraint) throws RangeRestartException {
	 ...
	 * Cursors [#21258].
	 *
	 * When a true status is returned by this method, the caller should return a
	 * successful result. When false is returned, the caller should treat this
	 * as a deleted record and either skip the record (in the case of position,
	 * search, and retrieveNext) or return failure/null (in the case of
	 * getCurrent). False can be returned only when read-uncommitted is used or
	 * the primary record has expired.
	 *
	 * @param priDb
	 *            primary database as input.
	 *
	 * @param key
	 *            secondary key as input.
	 *
	 * @param pKey
	 *            key as input.
	 *
	 * @param data
	 *            the data returned as output.
	 *
	 * @param lockMode
	 *            the lock mode to use for the primary read; if null, use the
	 *            default lock mode.
	 *
	 * @param secDirtyRead
	 *            whether we used dirty-read for reading the secondary record.
	 *            It is true if the user's configured isolation mode (or
	 ...
	 *
	 * @param lockPrimaryOnly
	 *            If false, then we are not using dirty-read for secondary
	 *            deadlock avoidance. If true, this secondary cursor's reference
	 *            to the primary will be checked after the primary record has
	 *            been locked.
	 *
	 ...
	 *         is used).
	 *
	 * @throws SecondaryIntegrityException
	 *             to indicate a corrupt secondary reference if the primary
	 *             record is deleted (as opposed to expired) and
	...
	boolean readPrimaryAfterGet(final Database priDb, final DatabaseEntry key, final DatabaseEntry pKey,
			DatabaseEntry data, final LockMode lockMode, final boolean secDirtyRead, final boolean lockPrimaryOnly) {
	 ...
	 * locks the secondary record if necessary.
	 *
	 * This is used for deadlock avoidance with secondary DBs. The initial
	 * secondary index read is done without locking. After the primary has been
	 * locked, we check here to insure that the primary/secondary relationship
	 * is still in place. There are two cases:
	 *
	 ...
	 * (the data of the secondary record) must additionally be compared to the
	 * original search key. This detects the case where the secondary record was
	 * updated to refer to a different primary key.
	 *
	...
	private boolean ensureReferenceToPrimary(final DatabaseEntry matchPriKey, final LockType lockType) {
		 ...
		 *
		 * If this technique were used with serializable isolation then checking
		 * the PD/KD flags wouldn't be sufficient -- locking the secondary would
		 * be necessary to prevent phantoms. With serializable isolation, a lock
		 * on the secondary record is acquired up front by SecondaryCursor.
		 */
		cursorImpl.latchBIN();
		try {
	 ...
	 *
	 * @param twoPartKey1
	 *            combined key with zero offset and size equal to the data array
	 *            length.
	 *
	 * @param keyBytes2
	 *            combined key byte array.
	 */
	 ...
	 * this is a sticky cursor or forceClone is true.
	 *
	 * If the cursor is initialized, the actions taken here depend on whether
	 * cloning is required (either because this is a sticky cursor or because
	 * forceClone is true).
	 *
	 ...
	 * marked uninitialized. - this.cursorImpl is returned.
	 *
	 * Note: In cases where only non-transactional locks are held, releasing
	 * them before the move prevents more than one lock from being held during a
	 * cursor move, which helps to avoid deadlocks.
	 *
	 * (b) Cloning: - this.cursorImpl is cloned. - If same position is true, the
	 * clone is registered at the same position as this.cursorImpl. - If same
	 * position is false, the clone is marked uninitialized. - If
	 * this.cursorImpl uses a locker that may acquire non-txn locks and
	 * retainNonTxnLocks is false, the clone cursorImpl gets a new locker of the
	 * same kind as this.cursorImpl. This allows for the non-txn locks acquired
	 * by the clone to be released independently from the non-txn locks of
	 * this.cursorImpl. - The clone cursorImpl is returned.
	 *
	 ...
	 *
	 * @param samePosition
	 *            If true, this cursor's position is used for the new cursor and
	 *            addCursor is called on the new cursor; if non-sticky, this
	 *            cursor's position is unchanged. If false, the new cursor will
	 *            be uninitialized; if non-sticky, this cursor is reset.
	 *
	 * @param forceClone
	 *            is true to clone an initialized cursor even if non-sticky is
	 *            configured. Used when cloning is needed to support internal
	 *            algorithms, namely when the algorithm may restart the
	 *            operation and samePosition is true.
	 *
	 ...
	 *
	 * (a) No cloning: - If the op is successfull, only critical eviction is
	 * done. - If the op is not successfull, this.cursorImpl is "reset", i.e.,
	 * (1) it is deregistered from its current position, (2) cachemode eviction
	 * is performed, (3) non-txn locks are released, if retainNonTxnLocks is
	 * false, and (4) this.cursorImpl is marked unintialized.
	 *
	 * (b) Cloning: - If the op is successful, this.cursorImpl is closed and
	 * then it is set to the clone cursorImpl. - If the op is not successfull,
	 * the clone cursorImpl is closed. - In either case, closing a cursorImpl
	 * involves deregistering it from its current position, performing cachemode
	 * eviction, releasing its non-transactional locks and closing its locker,
	 * if retainNonTxnLocks is false and the locker is not a Txn, and finally
	 * marking the cursorImpl as closed.
	 *
	 * In all cases, critical eviction is performed after each cursor operation.
	 * This is done by CursorImpl.reset() and close(), or is done here
	 * explicitly when the cursor is not cloned.
	 */
	 ...
	 * non-transactional usage access by multiple threads (see ThreadLocker). In
	 * the future we may wish to use the getTxnSynchronizer for synchronizing
	 * non-transactional access as well; however, note that a new locker is
