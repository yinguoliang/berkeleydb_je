...
public abstract class BaseUtilizationTracker {
     ...
     * be updated in real time under the log write latch. The collection and the
     * objects in the collection should not be modified by the caller.
     * </p>
     */
     ...
     * </p>
     */
    final void countObsolete(long lsn, Object databaseKey, LogEntryType type, int size, boolean countPerFile,
                             boolean countPerDb, boolean trackOffset, boolean checkDupOffsets) {
     ...
     * </p>
     *
     * @param dbFileSummaries the map of Long file number to DbFileSummary for a
     *            database that is being deleted.
     * @param mapLnLsn is the LSN of the MapLN when recovery is replaying the
     *            truncate/remove, or NULL_LSN when called outside of recovery;
     *            obsolete totals should only be counted when this LSN is prior
     *            to the LSN of the FileSummaryLN for the file being counted.
     */
    public void countObsoleteDb(DbFileSummaryMap dbFileSummaries, long mapLnLsn) {
        ...
        for (Map.Entry<Long, DbFileSummary> entry : dbFileSummaries.entrySet()) {
            ...
            if (isFileUncounted(fileNum, mapLnLsn)) {
                 ...
                 * includes LNs that become obsolete now, plus those that became
                 * obsolete previously but whose size was not counted. The
                 * obsoleteLNSizeCounted field is updated accordingly below. In
                 * other words, DB obsolescence is self-healing with respect to
                 * obsolete LN sizes. [#19144]
                 */
                int lnObsoleteSizeCounted = lnObsoleteCount
                        + (dbFileSummary.obsoleteLNCount - dbFileSummary.obsoleteLNSizeCounted);
                fileSummary.obsoleteLNSizeCounted += lnObsoleteSizeCounted;

                /*
                 * Do not update the DbFileSummary. It will be flushed when the
                 * MapLN is deleted. If later replayed during recovery, we will
                 * call this method to update the per-file utilization.
                 */
            }
        }
    }
    ...
    /**
     * Returns whether file summary information for the given LSN is not already
     * counted. Outside of recovery, always returns true. For recovery, is
     * overridden by RecoveryUtilizationTracker and returns whether the
     * FileSummaryLN for the given file is prior to the given LSN. .
     */
     ...
     * <p>
     * This method is implemented by subclasses which interpret the databaseKey
     ...
     * <p>
     * We keep the active file summary in the tracked file map, but we remove
     * older files to prevent unbounded growth of the map.
     * </p>
     * <p>
     ...
     * is tracked for node types and BIN-deltas. A null type is assumed to be an
     * LN.
     */
    public static boolean trackObsoleteInfo(LogEntryType type) {
        return type == null || type.isNodeType() || type.equals(LogEntryType.LOG_BIN_DELTA)
