 ...
 * created for each file, and then flushed to disk when starting a new file as a
 * FileExpirationLN.
 * <p>
 * An ExpirationTracker instance is used to track expired data when performing
 * the first pass of two pass cleaning, although in that case it is only used by
 * one thread, so the optimizations are irrelevant.
 * <p>
 * The {@link #serialize}} method is called to represent the histogram in a
 * single byte array. This array is the record "data" in a FileExpirationLN. It
 * is also stored in memory, in the UtilizationProfile, and used during cleaning
 * to calculate the number of expired bytes per file.
 */
public class ExpirationTracker {
    ...
    /**
     * We wait for pendingTrackCalls to go to zero before flushing the tracker
     * to its database.
     */
    ...
    private void track(int expiration, final boolean expirationInHours, final int size) {
         ...
         * "install" the new map in the volatile field only after adding the new
         * counter.
         */
        if (counter == null) {
            synchronized (this) {
                /*
                 * Check again while synchronized, since another thread may have
                 * added it. This "double check" is safe because the 'map' field
                 * is volatile.
                 */
    ...
    /**
     * Increment the number of calls to {@link #track(int, boolean, int)} that
     * must be made before the tracked data can be flushed to its database.
     */
    ...
    /**
     * Returns whether to wait for outstanding calls to
     * {@link #track(int, boolean, int)} before flushing the tracked data to its
     * database.
     */
    ...
    static int getExpiredBytes(final byte[] serializedForm, final int dayLimit, final int hourLimit) {
        final int expLimit = ExpirationTracker.isExpirationInHours(serializedForm) ? hourLimit : dayLimit;
    ...
    /**
     * Converts this object to a serialized form that is compact and can be used
     * to quickly find the total bytes after a given time. Returns an empty
     * array if no data in this file has an expiration time. The serialized form
     * is a series of {interval,byteSize} pairs that is ordered by expiration
     * time and run length encoded. The interval and byteSize are packed
     * integers. The interval is the delta between the current and previous
     * expiration value. All expiration values are in days if all values are on
     * a day boundary; otherwise they are in hours. Days are used, when
     * possible, to reduce the size of the delta, using less space due to the
    ...
    /**
     * Returns whether the given serialized form has expired values in hours. If
     * false is returned, all values expired on day boundaries.
     */
