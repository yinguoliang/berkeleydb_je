...
abstract class BaseLocalUtilizationTracker extends BaseUtilizationTracker {
    ...
    /**
     * Map of per-database utilization info. In LocalUtilizationTracker:
     * IdentityHashMap of DatabaseImpl to DbFileSummaryMap In
     * RecoveryUtilizationTracker: HashMap of DatabaseId to DbFileSummaryMap
     */
     ...
     * DatabaseId or DatabaseImpl. The releaseDatabaseImpl must be called with
     * the DatabaseImpl returned by this method.
     */
    abstract DatabaseImpl databaseKeyToDatabaseImpl(Object databaseKey) throws DatabaseException;

    /**
     * Must be called after calling databaseKeyToDatabaseImpl. The db parameter
     * may be null, in which case no action is taken. If DbTree.getDb is called
     * by the implementation of databaseKeyToDatabaseImpl, then DbTree.releaseDb
     * must be called by the implementation of this method.
     */
    abstract void releaseDatabaseImpl(DatabaseImpl db);

    /**
     * Allocates DbFileSummary information locally in this object rather than in
     * the DatabaseImpl.
     *
    ...
    DbFileSummary getDbFileSummary(Object databaseKey, long fileNum) {
        if (databaseKey != null) {
             ...
             */
            return fileMap.get(Long.valueOf(fileNum), true /* adjustMemBudget */, false /* checkResurrected */,
                    env.getFileManager());
        } else {
            return null;
        }
    ...
    /**
     * Deallocates all DbFileSummary objects for the given database key. For use
     * by subclasses.
     */
