 ...
 * A database handle.
 * <p>
 * Database attributes are specified in the
 * {@link com.sleepycat.je.DatabaseConfig DatabaseConfig} class. Database
 * handles are free-threaded and may be used concurrently by multiple threads.
 * </p>
 * <p>
...
public class Database implements Closeable {
     ...
     * checking it and before accessing the databaseImpl. But the check makes an
     * NPE unlikely.
     */
    ...
    /*
     * The envHandle field cannot be declared as final because it is initialized
     * by methods called by the ctor. However, after construction it is non-null
     ...
     * should normally only be accessed via the checkOpen and getDbImpl methods.
     * It is guaranteed to be non-null if state == DbState.OPEN.
     */
    private DatabaseImpl                       databaseImpl;

    /*
     * Used to store per-Database handle properties: allow create, exclusive
     * create, read only and use existing config. Other Database-wide properties
     * are stored in DatabaseImpl.
     */
    ...
    /*
     * Locker that owns the NameLN lock held while the Database is open. The
     * handleLocker field is set to null during close to avoid OOME. It is only
     * accessed during close, while synchronized, so checking for null is
     * unnecessary.
     */
     ...
     * contains it. Otherwise, it contains an internal SecondaryAssociation that
     * uses the simpleAssocSecondaries to store associations between a single
     * primary and its secondaries.
     */
    SecondaryAssociation                       secAssoc;
    Collection<SecondaryDatabase>              simpleAssocSecondaries;

    /*
     * Secondaries whose keys have values contrained to the primary keys in this
     * database.
     */
     ...
     */
    DatabaseImpl initNew(final Environment env, final Locker locker, final String databaseName,
     ...
     */
    void initExisting(final Environment env, final Locker locker, final DatabaseImpl dbImpl, final String databaseName,
                      final DatabaseConfig dbConfig) {
     ...
     */
    private void validateConfigAgainstExistingDb(Locker locker, final String databaseName, final DatabaseConfig config,
                                                 final DatabaseImpl dbImpl) {
        /*
         * The sortedDuplicates, temporary, and replicated properties are
         * persistent and immutable. But they do not need to be specified if the
         * useExistingConfig property is set.
         */
        if (!config.getUseExistingConfig()) {
            validatePropertyMatches("sortedDuplicates", dbImpl.getSortedDuplicates(), config.getSortedDuplicates());
        ...
        if (dbImpl.hasOpenHandles()) {
            if (!config.getUseExistingConfig()) {
                validatePropertyMatches("transactional", dbImpl.isTransactional(), config.getTransactional());
                validatePropertyMatches("deferredWrite", dbImpl.isDurableDeferredWrite(), config.getDeferredWrite());
            }
        } else {
        ...
        /*
         * If this database handle uses the existing config, we shouldn't search
         * for and write any changed attributes to the log.
         */
        if (config.getUseExistingConfig()) {
            return;
        }
        ...
        if (config.getOverrideBtreeComparator()) {
            dbImplModified |= dbImpl.setBtreeComparator(config.getBtreeComparator(),
                    config.getBtreeComparatorByClassName());
        }

        if (config.getOverrideDuplicateComparator()) {
            dbImplModified |= dbImpl.setDuplicateComparator(config.getDuplicateComparator(),
            ...
            try {
                ...
                throw new IllegalStateException(
                        "DatabaseConfig properties may not be updated when the "
                                + "database is already open; first close other open " + "handles for this database.",
                        e);
            }
     ...
     * <p>
     * Any BIN-deltas in cache must be mutated to full BINs, since BIN-deltas
     * are not allowed in DW mode. This can be expensive, and is only a
     * workaround for the underlying problem that BIN-deltas are not supported
     * in DW mode. The mutation is necessary when a db transitions from non-DW
     * to DW mode. In that case there may be BIN-deltas in cache, even if the DB
     * has not yet been opened since BIN-deltas may be placed in the Btree by
     * recovery. This method is not optimized because ultimately BIN-deltas will
     * be supported in DW mode or DW mode will be discontinued.
     * <p>
     * At the time this method is called, the DW flag on this object has already
     * been set. Therefore, this workaround doesn't guarantee that internal
     * operations on the db (e.g., compression or eviction) won't operation on a
     * BIN-delta in a DW DB. Therefore, we cannot assume/assert that a cached
     * BIN-delta does not exist in a DW DB. Such assertions were removed as part
     * of this workaround.
     * <p>
     * Setting the DW flag before mutating the BINs is necessary to ensure that
     * no new BIN-deltas appear in cache during this process. BINs loaded into
     * cache for a DW db are mutated to full BINs in IN.postFetchInit.
     *
     ...
     */
    private void validatePropertyMatches(final String propName, final boolean existingValue, final boolean newValue) {
        if (newValue != existingValue) {
            throw new IllegalArgumentException("You can't open a Database with a " + propName + " configuration of "
                    + newValue + " if the underlying database was created with a " + propName + " setting of "
                    + existingValue + '.');
        }
    }
     ...
     * Database} handle concurrently, only a single thread may call this method.
     * <p>
    ...
    public void close() {
        try {
            closeInternal(true /* doSyncDw */, true /* deleteTempDb */, DbState.CLOSED, null /* preemptedException */);
        } catch (Error E) {
    ...
    private void closeNoSync() {
        try {
            closeInternal(false /* doSyncDw */, true /* deleteTempDb */, DbState.CLOSED, null /* preemptedException */);
        } catch (Error E) {
            envHandle.invalidate(E);
            throw E;
        }
    ...
    synchronized void setPreempted(final String dbName, final String msg) {
        ...
        final OperationFailureException preemptedException = dbImpl.getEnv().createDatabasePreemptedException(msg,
                dbName, this);

        closeInternal(false /* doSyncDw */, false /* deleteTempDb */, DbState.PREEMPTED, preemptedException);
    }

    /**
     * Invalidates the handle when the transaction used to open the database is
     * aborted. Note that this method (unlike close) does not perform sync and
     * removal of DW DBs. A DW DB cannot be transactional.
     */
    synchronized void invalidate() {
        closeInternal(false /* doSyncDw */, false /* deleteTempDb */, DbState.INVALID, null /* preemptedException */);
    }
    ...
    private void closeInternal(final boolean doSyncDw, final boolean deleteTempDb, final DbState newState,
     ...
     * null. The app may hold the Database references longer than expected. In
     ...
     */
    synchronized void minimalClose(final DbState newState, final OperationFailureException preemptedException) {
    ...
    private void closeInternalWork(final boolean doSyncDw, final boolean deleteTempDb, final DbState newState,
                                   final OperationFailureException preemptedException) {
        ...
        synchronized (this) {
            ...
            /*
             * The state should be changed ASAP during close, so that addCursor
             * and removeCursor will see the updated state ASAP.
             */
            state = newState;
            preemptedCause = preemptedException;
            ...
            if (newState == DbState.CLOSED) {
                ...
                if (simpleAssocSecondaries != null && simpleAssocSecondaries.size() > 0) {
                    handleRefErrors.append(" ").append(simpleAssocSecondaries.size())
                            .append(" associated SecondaryDatabases.");
                }
                if (foreignKeySecondaries != null && foreignKeySecondaries.size() > 0) {
                    handleRefErrors.append(" ").append(foreignKeySecondaries.size())
            ...
            /*
             * If the handle was preempted, we mark the locker as only-abortable
             * with the DatabasePreemptedException. If the handle locker is a
             * user txn, this causes the DatabasePreemptedException to be thrown
             * if the user attempts to commit, or continue to use, the txn,
             * rather than throwing a LockConflictException. [#17015]
             */
        ...
        /*
         * Notify the database when a handle is closed. This should not be done
         * while synchronized since it may perform database removal or sync.
         * Statements above are synchronized but this section must not be. Note
         * that handleClosed may throw an exception, so any following code may
         * not be executed.
         */
        ...
        if (handleRefErrors.length() > 0) {
            throw new IllegalStateException(
                    "Database closed while still referenced by other handles." + handleRefErrors.toString());
        }
    ...
    /**
     * Flushes any cached information for this database to disk; only applicable
     * for deferred-write databases.
     * <p>
     * Note that deferred-write databases are automatically flushed to disk when
     ...
     * @throws SequenceExistsException if the sequence record already exists and
     *             the {@code SequenceConfig ExclusiveCreate} parameter is true.
     * @throws SequenceNotFoundException if the sequence record does not exist
     *             and the {@code SequenceConfig AllowCreate} parameter is
     *             false.
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#readFailures">
     *             Read Operation Failures</a> occurs. If the sequence does not
     *             exist and the {@link SequenceConfig#setAllowCreate
     ...
     *             another thread during this method invocation, or the database
     *             has been closed.
     * @throws IllegalArgumentException if an invalid parameter is specified,
     *             for example, an invalid {@code SequenceConfig} parameter.
     */
    public Sequence openSequence(final Transaction txn, final DatabaseEntry key, final SequenceConfig config) {
        try {
            checkEnv();
    ...
    /**
     * Removes the sequence from the database. This method should not be called
     ...
     *            the cursor, or null if the operations should not be
     *            transaction protected. If the database is non-transactional,
     *            null must be specified. For a transactional database, the
     *            transaction is optional for read-only access and required for
     *            read-write access.
     ...
     *             {@link #populateSecondaries(Transaction, DatabaseEntry, DatabaseEntry, long, CacheMode)}
     *             .
     */
    public void populateSecondaries(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data) {

        populateSecondaries(txn, key, data, 0 /* expirationTime */, null);
    }

    /**
     * @hidden For internal use only. Given the {@code key}, {@code data} and
     *         {@code expirationTime} for a locked primary DB record, update the
     *         corresponding secondary database (index) records, for secondaries
     *         enabled for incremental population.
     *         <p>
     *         The secondaries associated the primary record are determined by
     *         calling {@link SecondaryAssociation#getSecondaries}. For each of
     *         these secondaries,
     *         {@link SecondaryDatabase#isIncrementalPopulationEnabled} is
     *         called to determine whether incremental population is enabled. If
     *         so, appropriate secondary records are inserted and deleted so
     *         that the index accurately reflects the current state of the
     *         primary record.
     *         <p>
     *         Note that for a given primary record, this method will not modify
     *         the secondary database if the secondary has already been updated
     *         for the primary record, due to concurrent primary write
     *         operations. Due to this behavior, certain integrity checks are
     *         not performed as documented in
     *         {@link SecondaryDatabase#startIncrementalPopulation}.
     *         <p>
     *         The primary record must be locked (read or write locked) when
     *         this method is called. Therefore, the caller should not use
     *         dirty-read to read the primary record. The simplest way to ensure
     *         that the primary record is locked is to use a cursor to read
     *         primary records, and call this method while the cursor is still
     *         positioned on the primary record.
     *         <p>
     *         It is the caller's responsibility to pass all primary records to
     *         this method that contain index keys for a secondary DB being
     *         incrementally populated, before calling
     *         {@link SecondaryDatabase#endIncrementalPopulation} on that
     *         secondary DB.
     * @param txn is the transaction to be used to write secondary records. If
     *            null and the secondary DBs are transactional, auto-commit will
     *            be used.
     ...
     *            {@link OperationResult#getExpirationTime()} when reading the
     *            primary record.
     * @param cacheMode the CacheMode to use, or null for the Database default.
     */
    public void populateSecondaries(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data,
                                    final long expirationTime, CacheMode cacheMode) {
        try {
            checkEnv();
            DatabaseUtil.checkForNullDbt(key, "key", true);
            ...
            try {
                for (final SecondaryDatabase secDb : secondaries) {
                    if (secDb.isIncrementalPopulationEnabled()) {
                        ...
                        secDb.updateSecondary(locker, null /* cursor */, key /* priKey */, null /* oldData */,
                                data /* newData */, cacheMode, expirationTime, false /* expirationUpdated */,
                                expirationTime);
                    }
                }
                success = true;
     ...
     * @throws IllegalStateException if the database has been closed.
     * @throws IllegalArgumentException if an invalid parameter is specified.
     *             This includes passing a null input key parameter, an input
     *             key parameter with a null data array, or a partial key input
     *             parameter.
     * @since 7.0
     */
    public OperationResult delete(final Transaction txn, final DatabaseEntry key, final WriteOptions options) {
        try {
            checkEnv();
            final DatabaseImpl dbImpl = checkOpen();
     ...
     * triggers, update secondaries and enforce foreign key constraints. Deletes
     * all duplicates.
     */
    OperationResult deleteInternal(final Locker locker, final DatabaseEntry key, final CacheMode cacheMode) {
        ...
        try (final Cursor cursor = new Cursor(this, locker, null)) {
            ...
            final LockMode lockMode = cursor.isSerializableIsolation(LockMode.RMW) ? LockMode.RMW
                    : LockMode.READ_UNCOMMITTED_ALL;

            OperationResult searchResult = cursor.search(key, noData, lockMode, cacheMode, SearchMode.SET, false);
            ...
            while (searchResult != null) {
                ...
                final OperationResult deleteResult = cursor.deleteInternal(dbImpl.getRepContext(), cacheMode);
     ...
     * {@link #delete(Transaction, DatabaseEntry, WriteOptions)}.
     * </p>
     *
     * @param txn For a transactional database, an explicit transaction may be
     *            specified, or null may be specified to use auto-commit. For a
     ...
     * @param key the key used as <a href="DatabaseEntry.html#inParam">input</a>
     *            .
     * @return The method will return
     *         {@link com.sleepycat.je.OperationStatus#NOTFOUND
     *         OperationStatus.NOTFOUND} if the given key is not found in the
     *         database; otherwise
     ...
     * non-null OperationResult will be returned. If the operation fails because
     * the record requested is not found, null is returned.
     * </p>
     * <p>
     ...
     * parameters</a>. See the individual {@link Get} operations for more
     * information.
     * </p>
     * <div>
     ...
     *
     * @param txn For a transactional database, an explicit transaction may be
     *            specified to transaction-protect the operation, or null may be
     *            specified to perform the operation without transaction
     *            protection. For a non-transactional database, null must be
     *            specified.
     ...
     * @since 7.0
     */
    public OperationResult get(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data,
                               final Get getType, ReadOptions options) {
        ...
        try {
            ...
            final Locker locker = LockerFactory.getReadableLocker(this, txn, cursorConfig.getReadCommitted());
     ...
     * <p>
     * Calling this method is equivalent to calling
     * {@link #get(Transaction, DatabaseEntry, DatabaseEntry, Get, ReadOptions)}
     * with {@link Get#SEARCH}.
     * </p>
     *
     * @param txn For a transactional database, an explicit transaction may be
     *            specified to transaction-protect the operation, or null may be
     ...
     * @throws IllegalArgumentException if an invalid parameter is specified.
     */
    public OperationStatus getSearchBoth(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data,
                                         LockMode lockMode) {
        final OperationResult result = get(txn, key, data, Get.SEARCH_BOTH, DbInternal.getReadOptions(lockMode));
     ...
     *             putType is Put.NO_DUP_DATA and the database is not configured
     *             for duplicates.
     * @throws IllegalStateException if the database has been closed.
     * @throws IllegalArgumentException if an invalid parameter is specified.
     *             This includes passing a null putType, a null input key/data
     *             parameter, an input key/data parameter with a null data
     *             array, a partial key/data input parameter, or when putType is
     *             Put.CURRENT.
     * @since 7.0
     */
    public OperationResult put(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data,
                               final Put putType, final WriteOptions options) {
     ...
     * <p>
     * Calling this method is equivalent to calling
     * {@link #put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)}
     * with {@link Put#OVERWRITE}.
     * </p>
     * <p>
     * If the key already appears in the database and duplicates are not
     * configured, the data associated with the key will be replaced. If the key
     * already appears in the database and sorted duplicates are configured, the
     * new data value is inserted at the correct sorted location.
     * </p>
     *
     * @param txn For a transactional database, an explicit transaction may be
     *            specified, or null may be specified to use auto-commit. For a
     ...
     * Calling this method is equivalent to calling
     * {@link #put(Transaction, DatabaseEntry, DatabaseEntry, Put, WriteOptions)}
     * with {@link Put#NO_OVERWRITE}.
     * </p>
     * <p>
     * This method will return {@link com.sleepycat.je.OperationStatus#KEYEXIST
     * OpeationStatus.KEYEXIST} if the key already exists in the database, even
     ...
     * @throws IllegalStateException if the database has been closed.
     */
    public OperationStatus putNoDupData(final Transaction txn, final DatabaseEntry key, final DatabaseEntry data) {
     ...
     * <p>
     * Each cursor in the <code>cursors</code> array must have been initialized
     * to refer to the key on which the underlying database should be joined.
     * Typically, this initialization is done by calling
     * {@link Cursor#getSearchKey Cursor.getSearchKey}.
     * </p>
     * <p>
     * Once the cursors have been passed to this method, they should not be
     * accessed or modified until the newly created join cursor has been closed,
     * or else inconsistent results may be returned. However, the position of
     * the cursors will not be changed by this method or by the methods of the
     * join cursor.
     * </p>
     *
     * @param cursors an array of cursors associated with this primary database.
     *            In a replicated environment, an explicit transaction must be
     *            specified when opening each cursor, unless read-uncommitted
     *            isolation is isolation is specified via the
     *            {@link CursorConfig} or {@link LockMode} parameter.
     ...
     * being loaded into the JE cache. Executing preload during any other types
     * of operations may result in JE exceeding its allocated cache size.
     * preload() effectively locks the entire database and therefore will lock
     ...
     *            preloading. Preloading stops once this limit has been reached.
     *            If maxMillisecs is 0, preloading can go on indefinitely or
     *            until maxBytes (if non-0) is reached.
     * @throws OperationFailureException if one of the
     *             <a href="OperationFailureException.html#readFailures">Read
     *             Operation Failures</a> occurs.
     * @throws EnvironmentFailureException if an unexpected, internal or
     ...
     * preload operation exceeds
     * {@link com.sleepycat.je.rep.ReplicationConfig#FEEDER_TIMEOUT}.
     * <p>
     * While this method preloads a single database, {@link Environment#preload}
     * lets you preload multiple databases.
     *
     * @param config The PreloadConfig object that specifies the parameters of
     *            the preload.
     * @return A PreloadStats object with various statistics about the preload()
     *         operation.
     * @throws OperationFailureException if one of the
     *             <a href="OperationFailureException.html#readFailures">Read
     *             Operation Failures</a> occurs.
     ...
     * guaranteed to be accurate if there are concurrent updates. Note that this
     * method does scan a significant portion of the database and should be
     * considered a fairly expensive operation. This operation uses the an
     * internal infrastructure and algorithm that is similar to the one used for
     * the {@link DiskOrderedCursor}. Specifically, it will disable deletion of
     * log files by the JE log cleaner during its execution and will consume a
     * certain amount of memory (but without affecting the memory that is
     * available for the JE cache). To avoid excessive memory consumption (and a
     * potential {@code OutOfMemoryError}) this method places an internal limit
     * on its memory consumption. If this limit is reached, the method will
     * still work properly, but its performance will degrade. To specify a
     * different memory limit than the one used by this method, use the
     * {@link Database#count(long memoryLimit)} method. Currently, the internal
     * memory limit is calculated as 10% of the difference between the max JVM
     * memory (the value returned by Runtime.getRuntime().maxMemory()) and the
     * configured JE cache size.
     *
     ...
     * certain amount of memory (but without affecting the memory that is
     * available for the JE cache). To avoid excessive memory consumption (and a
     * potential {@code OutOfMemoryError}) this method takes as input an upper
     * bound on the memory it may consume. If this limit is reached, the method
     * will still work properly, but its performance will degrade.
     *
     * @param memoryLimit The maximum memory (in bytes) that may be consumed by
     *            this method.
     * @return The count of key/data pairs in the database.
     * @throws OperationFailureException if one of the
     *             <a href="OperationFailureException.html#readFailures">Read
     ...
     * the database environment underlying the {@link com.sleepycat.je.Database
     * Database}.
     * <p>
     * This method may be called at any time during the life of the application.
     * </p>
     *
     * @return The {@link com.sleepycat.je.Environment Environment} handle for
     *         the database environment underlying the
    ...
    private int doCompareKeys(final DatabaseEntry entry1, final DatabaseEntry entry2, final boolean duplicates) {
        try {
            checkEnv();
            final DatabaseImpl dbImpl = checkOpen();
    ...
    /**
     * Returns the non-null, underlying getDbImpl. This method should always be
     * called to access the databaseImpl, to guard against NPE when the database
     * has been closed after the initial checks. However, callers should
     * additionally call checkOpen at API entry points to reject the operation
     * as soon as possible. Plus, if the database has been closed, this method
     * may return non-null because the databaseImpl field is not volatile.
     *
    ...
    DatabaseImpl getDbImpl() {
        ...
        /*
         * checkOpen should have thrown an exception, but we'll throw again here
         * just in case.
         */
    ...
    void removeCursor(final ForwardCursor ignore) throws DatabaseException {
        ...
        /*
         * Do not call checkOpen if the handle was preempted, to allow closing a
         * cursor after an operation failure. [#17015]
         */
    ...
    void checkLockModeWithoutTxn(final Transaction userTxn, final LockMode lockMode) {
        if (userTxn == null && LockMode.RMW.equals(lockMode)) {
            throw new IllegalArgumentException(lockMode + " is meaningless and can not be specified "
                    + "when a null (autocommit) transaction is used. There "
                    + "will never be a follow on operation which will promote " + "the lock to WRITE.");
        }
    }
     ...
     */
    void trace(final Level level, final String methodName, final String getOrPutType, final Transaction txn,
     ...
     * to construct the message if the level is not enabled.
     */
    void trace(final Level level, final String methodName, final Transaction txn, final DatabaseEntry key,
     ...
     * to construct the message if the level is not enabled.
     */
    void trace(final Level level, final String methodName, final Transaction txn, final Object config) {
    ...
    /**
     * Creates a SecondaryIntegrityException using the information given. This
     * method is in the Database class, rather than in SecondaryDatabase, to
     * support joins with plain Cursors [#21258].
     */
    SecondaryIntegrityException secondaryRefersToMissingPrimaryKey(final Locker locker, final DatabaseEntry secKey,
                                                                   final DatabaseEntry priKey,
                                                                   final long expirationTime) {

        return new SecondaryIntegrityException(locker, "Secondary refers to a missing key in the primary database",
