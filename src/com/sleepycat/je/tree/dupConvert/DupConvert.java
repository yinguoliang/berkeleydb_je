 ...
 * are represented by a two-part (key + data) key, and empty data. In JE 4.1 and
 * earlier, the key and data were separate as with non-dup DBs. Uses the
 * DbTree.DUPS_CONVERTED_BIT to determine whether conversion of the environment
 * is necessary. When all databases are successfully converted, this bit is set
 * and the mapping tree is flushed. See EnvironmentImpl.convertDupDatabases.
 ...
 * and the entire database is flushed using Database.sync. The conversion of
 * each database is atomic -- either all INs or none are converted and made
 * durable. This is accomplished by putting the database into Deferred Write
 * mode so that splits won't log and eviction will be provisional (eviction will
 * not flush the root IN if it is dirty). The Deferred Write mode is cleared
 * after conversion is complete and Database.sync has been called. The memory
 * budget is updated during conversion and daemon eviction is invoked
 * periodically. This provides support for arbitrarily large DBs. Uses preload
 * to load all dup trees (DINs/DBINs) prior to conversion, to minimize random
 * I/O. See EnvironmentConfig.ENV_DUP_CONVERT_PRELOAD_ALL. The preload config
 * does not specify loading of LNs, because we do not need to load LNs from
 * DBINs. The fact that DBIN LNs are not loaded is the main reason that
 * conversion is quick. LNs are converted lazily instead; see
 ...
 * key. Even when LN loading is not configured, it turns out that preload does
 * load BIN (not DBIN) LNs in a dup DB, which is what we want. The singleton LNs
 ...
 * The DIN, DBIN and DupCount LSN are counted obsolete during conversion using a
 * local utilization tracker. The tracker must not be flushed until the
 ...
 * tracker is not substantial. Database Conversion Algorithm
 * ----------------------------- 1. Set Deferred Write mode for the database.
 * Preload the database, including INs/BINs/DINs/DBINs, but not LNs except for
 * singleton LNs (LNs with a BIN parent). 2. Convert all IN/BIN keys to
 * "prefix keys", which are defined by the DupKeyData class. This allows tree
 * searches and slot insertions to work correctly as the conversion is
 * performed. 3. Traverse through the BIN slots in forward order. 4. If a
 * singleton LN is encountered, ensure it is loaded. IN.fetchLN() automatically
 * updates the slot key if the LNLogEntry's key is different from the one
 * already in the slot. Because LNLogEntry's key is converted on the fly, a
 * two-part key is set in the slot as a side effect of fetching the LN. 5. If a
 * DIN is encountered, first delete the BIN slot containing the DIN. Then
 * iterate through all LNs in the DBINs of this dup tree, assign each a two-part
 * key, and insert the slot into a BIN. The LSN and state flags of the DBIN slot
 * are copied to the new BIN slot. 6. If a deleted singleton (BIN) LN is
 * encountered, delete the slot rather than converting the key. If a deleted
 * DBIN LN is encountered, simply discard it. 7. Count the DIN and DupCount LSN
 * obsolete for each DIN encountered, using a local utilization tracker. 8. When
 * all BIN slots have been processed, set the DatabaseImpl.DUPS_CONVERTED flag,
 * call Database.sync to flush all INs and the MapLN, clear Deferred Write mode,
 * and flush the local utilization tracker.
 */
public class DupConvert {
    ...
    public DupConvert(final EnvironmentImpl envImpl, final DbTree dbTree) {
        ...
        this.preloadConfig = (envImpl.getDupConvertPreloadConfig() != null) ? envImpl.getDupConvertPreloadConfig()
                : (new PreloadConfig());
    }
    ...
    /**
     * Advances the bin/index fields to the next BIN slot. When moving past the
     * last BIN slot, the bin field is set to null and false is returned.
     ...
     * Enter/leave with bin field latched, although bin field may change. When a
     * singleton LN is converted, leaves with bin/index fields unchanged. When a
     * dup tree is converted, leaves with bin/index fields set to last inserted
     * slot. This is the slot of the highest key in the dup tree.
     */
    private void convertBinSlot() {
        if (DEBUG) {
            System.out.println("DupConvert BIN LSN " + DbLsn.getNoFormatString(bin.getLsn(index)) + " index " + index
         ...
         * so that the DIN slot key doesn't interfere with insertions. The DIN
         * is evicted and memory usage is decremented. This is not exactly
         * correct because we keep a local reference to the DIN until the dup
         * tree is converted, but we tolerate this temporary inaccuracy.
         */
     ...
     * deleted. Returns false if it is not deleted, or if it is deleted but part
     * of an unclosed, resurrected txn. Enter/leave with bin field latched.
     */
    private boolean isLNDeleted(BIN checkBin, int checkIndex) {
        ...
        try {
            final LockResult lockRet = lockingTxn.nonBlockingLock(lsn, LockType.READ, false /* jumpAheadOfWaiters */,
    ...
    /**
     * Converts the given DIN and its descendants. Enter/leave with bin field
     * latched, although bin field will change to last inserted slot.
     */
    private void convertDin(final DIN din, final byte[] binKey) {
        ...
        try {
                ...
                if (child instanceof DBIN) {
                    ...
                    try {
                        ...
                        if (dbin.getLastLoggedLsn() != DbLsn.NULL_LSN) {
                            localTracker.countObsoleteNodeInexact(dbin.getLastLoggedLsn(), dbin.getLogType(), 0,
                                    dbin.getDatabase());
                        }
                    } finally {
                        dbin.releaseLatch();
            ...
            if (din.getLastLoggedLsn() != DbLsn.NULL_LSN) {
                localTracker.countObsoleteNodeInexact(din.getLastLoggedLsn(), din.getLogType(), 0, din.getDatabase());
            }
            final ChildReference dupCountRef = din.getDupCountLNRef();
            if (dupCountRef != null && dupCountRef.getLsn() != DbLsn.NULL_LSN) {
                localTracker.countObsoleteNodeInexact(dupCountRef.getLsn(), LogEntryType.LOG_DUPCOUNTLN, 0,
                        din.getDatabase());
            }
        } finally {
            din.releaseLatch();
    ...
    /**
     * Converts the given DBIN slot, leaving bin/index set to the inserted BIN
     * slot. Enter/leave with bin field latched, although bin field may change.
     * If slot is inserted into current bin, leave bin field unchanged and set
     * index field to inserted slot. If slot is inserted into a different bin,
     * set bin/index fields to inserted slot.
     */
    private void convertDbinSlot(final DBIN dbin, final int dbinIndex, final byte[] binKey) {
        ...
        /*
         * If the current BIN can hold the new slot, don't bother to do a search
         * to find it.
         */
        ...
        final int newIndex = bin.insertEntry1(null/* ln */, newKey, null/* data */, dbin.getLsn(dbinIndex),
     ...
     * an IN from disk via IN.postFetchInit. The conversion of IN keys is
     * invoked from the IN class when an IN is fetched, rather than invoked from
     * the DupConvert class directly, for performance and simplicity. If it were
     * invoked from the DupConvert class, we would have to iterate over all INs
     * in a separate initial pass. This is both more time consuming, and more
     * complex to implement properly so that eviction is possible. Instead,
     * conversion occurs when an old format IN is loaded. Enter/leave with 'in'
     * unlatched.
     */
