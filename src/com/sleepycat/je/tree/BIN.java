...
/**
 * A BIN represents a Bottom Internal Node in the JE tree. BIN-deltas ==========
 ...
 * the BIN-delta flag is set (IN.isBINDelta). On disk, the NewBINDelta log entry
 * type (class BINDeltaLogEntry) is the only thing that distinguishes it from a
 * full BIN, which has the BIN log entry type. BIN-deltas provides two benefits:
 * Reduced writing and reduced memory usage. Reduced Writing ---------------
 * Logging a BIN-delta rather a full BIN reduces writing significantly. The
 * cost, however, is that two reads are necessary to reconstruct a full BIN from
 * scratch. The reduced writing is worth this cost, particularly because less
 * writing means less log cleaning. A BIN-delta is logged when 25% or less
 * (configured with EnvironmentConfig TREE_BIN_DELTA) of the slots in a BIN are
 * dirty. When a BIN-delta is logged, the dirty flag is cleared on the the BIN
 * in cache. If more slots are dirtied and another BIN-delta is logged, it will
 * contain all entries dirtied since the last full BIN was logged. In other
 * words, BIN-deltas are cumulative and not chained, to avoid reading many (more
 * than two) log entries to reconstruct a full BIN. The dirty flag on each slot
 * is cleared only when a full BIN is logged. In addition to the cost of
 * fetching two entries on a BIN cache miss, another drawback of the current
 * approach is that dirtiness propagates upward in the Btree due to BIN-delta
 * logging, causing repeated logging of upper INs. The slot of the parent IN
 * contains the LSN of the most recent BIN-delta or full BIN that was logged. A
 * BINDeltaLogEntry in turn contains the LSN of the last full BIN logged.
 * Historical note: The pre-JE 5 implementation of OldBINDeltas worked
 * differently and had a different cost/benefit trade-off. When an OldBINDelta
 * was logged, its dirty flag was not cleared, causing it to be logged
 * repeatedly at every checkpoint. A full BIN was logged after 10 deltas, to
 * prevent endless logging of the same BIN. One benefit of this approach is that
 * the BIN's parent IN was not dirtied when logging the OldBINDelta, preventing
 * dirtiness from propagating upward. Another benefit is that the OldBINDelta
 * was only processed by recovery, and did not have to be fetched to reconstruct
 * a full BIN from scratch on a cache miss. But the cost (the logging of an
 * OldBINDelta every checkpoint, even when it hadn't changed since the last time
 * logged) outweighed the benefits. When the current approach was implemented in
 * JE 5, performance improved due to less logging. In JE 6, deltas were also
 * maintained in the Btree cache. This was done to provide the reduced memory
 * benefits described in the next section. The log format for a delta was also
 * changed. The OldBINDelta log format is different (not the same as the BIN
 * format) and is supported for backward compatibility as the
 * OldBINDeltaLogEntry. Its log entry type name is still BINDelta, which is why
 * the new type is named NewBINDelta (for backward compatibility, log entry type
 * names cannot be changed.) This is also why the spelling "BIN-delta" is used
 * to refer to deltas in the new approach. The old BINDelta class was renamed to
 * OldBINDelta and there is no longer a class named BINDelta. Reduced Memory
 * Usage -------------------- In the Btree cache, a BIN may be represented as a
 * full BIN or a BIN-delta. Eviction will mutate a full BIN to a BIN-delta in
 * preference to discarding the entire BIN. A BIN-delta in cache occupies less
 * memory than a full BIN, and can be exploited as follows: - When a full BIN is
 * needed, it can be constructed with only one fetch rather than two, reducing
 * IO overall. IN.fetchIN implements this optimization. - Certain operations can
 * sometimes be performed using the BIN-delta alone, allowing such operations on
 * a given data set to take place using less less IO (for a given cache size).
 ...
 * the following operations using the BIN-delta alone. - Consider recording
 * deletions in a BIN-delta. Currently, slot deletion prohibits a BIN-delta from
 * being logged. To record deletion in BIN-deltas, slot deletion will have to be
 * deferred until a full BIN is logged. - User reads by key, updates and
 * deletions can be implemented if the key happens to appear in the BIN-delta. -
 * The Cleaner can migrate an LN if its key happens to appear in the BIN-delta.
 * This is similar to a user update operation, but in a different code path. -
 ...
 * whether the key exists. - Recovery LN redo could also apply insertions,
 * updates and inserts in the manner described. - Add idempotent put/delete
 * operations, which can always be applied in a BIN-delta. - Store a hash of the
 * keys in the full BIN in the BIN-delta and use it to perform the following in
 * the delta: - putIfAbsent (true insertion) - get/delete/putIfPresent
 * operations that return NOTFOUND - to avoid accumulating unnecessary deletions
 * However, some internal operations do currently exploit BIN-deltas to avoid
 * unnecessary IO. The following are currently implemented. - The Evictor and
 * Checkpointer log a BIN-delta that is present in the cache, without having to
 * fetch the full BIN. - The Cleaner can use the BIN-delta to avoid fetching
 * when processing a BIN log entry (delta or full) and the BIN is not present in
 * cache, To support BIB-delta-aware operations, the IN.fetchIN() and
 * IN.getTarget() methods may return a BIN delta. IN.getTarget() will return
 * whatever object is cached under the parent IN, and IN.fetchIN() will do a
 * single I/O to fetch the most recently log record for the requested BIN, which
 * may be a full BIN or a delta. Callers of these methods must be prepared to
 * handle a BIN delta; either doing their operation directly on the delta, if
...
public class BIN extends IN {
     ...
     * minLength is 1 because log sizes are unpredictable. allowSparseRep is
     * false because all slots have log sizes and less mutation is better.
     */
    private static final INLongRep.EmptyRep EMPTY_LAST_LOGGED_SIZES   = new INLongRep.EmptyRep(1, false);

    /**
     * Used as the "empty rep" for the INLongRep vlsnCache field. minLength is 5
     * because VLSNS grow that large fairly quickly, and less mutation is
     * better. The value 5 accomodates data set sizes up to 100 billion. If we
     * want to improve memory utilization for smaller data sets or reduce
     * mutation for larger data sets, we could dynamically determine a value
     * based on the last assigned VLSN. allowSparseRep is false because either
     * all slots typically have VLSNs, or none do, and less mutation is better.
     */
    private static final INLongRep.EmptyRep EMPTY_VLSNS               = new INLongRep.EmptyRep(5, false);

    /**
     * Used as the "empty rep" for the INLongRep offHeapLNIds field. minLength
     * is 8 because memory IDs are 64-bit pointers. allowSparseRep is true
     * because some workloads will only load LN IDs for a subset of the LNs in
     * the BIN.
     */
     ...
     * offset from a base day/hour, to fit in one byte. allowSparseRep is true
     * because some workloads only set TTLs on some of the LNs in a BIN.
     */
    private static final INLongRep.EmptyRep EMPTY_EXPIRATION          = new INLongRep.EmptyRep(1, true);

    /*
     * The set of cursors that are currently referring to this BIN. This field
     * is set to null when there are no cursors on this BIN.
     */
    ...
    /*
     * If this is a delta, fullBinNEntries stores the number of entries in the
     * full version of the BIN. This is a persistent field for BIN-delta logrecs
     * only, and for log versions >= 10.
     */
    private int                             fullBinNEntries           = -1;

    /*
     * If this is a delta, fullBinMaxEntries stores the max number of entries
     * (capacity) in the full version of the BIN. This is a persistent field for
     * BIN-delta logrecs only, and for log versions >= 10.
     */
    private int                             fullBinMaxEntries         = -1;

    /*
     * If "this" is a BIN-delta, bloomFilter is a bloom-filter representation of
     * the set of keys in the clean slots of the full version of the same BIN.
     * It is used to allow blind put operations in deltas, by answering the
     * question whether the put key is in the full BIN or not. See the javadoc
     * of the TREE_BIN_DELTA_BLIND_PUTS config param for more info. This is a
     ...
     * preservation and caching are configured. A VLSN is added to the cache
     * when an LN is evicted from a BIN. When the LN is resident, there is no
     * need for caching because the LN contains the VLSN. See BIN.setTarget.
     * This strategy works because an LN is always cached during a read or write
     * operation, and only evicted after that, based on eviction policies. For
     * embedded LNs a VLSN is added to the cache every time the record is
     * logged. Furthermore, the vlsn cache is made persistent for such LNs. An
     * EMPTY_REP is used initially until the need arises to add a non-zero
     ...
     * if the size is unknown. We use INLongRep in spite of the fact that sizes
     * are int not long; INLongRep will store the minimum number of bytes. An
     * EMPTY_REP is used initially until the need arises to add a non-zero
     * value.
     */
    ...
    /**
     * Returns the VLSN. VLSN.NULL_VLSN_SEQUENCE (-1) is returned in two cases:
     * 1) This is a standalone environment. 2) The VLSN is not cached (perhaps
     * VLSN caching is not configured), and the allowFetch param is false.
     ...
     * immediately, since it is only needed for counting an LN obsolete during
     * an update or deletion. This method may not be called until after the
     * database is initialized, i,e., it may not be called during readFromLog.
     */
    @Override
    ...
    /**
     * Sets last logged size if necessary. This method does not dirty the IN
     * because the caller methods dirty it, for example, when setting the LSN,
     * key, or node. This method is sometimes called to add the logged size for
     * a pre log version 9 BIN, for example, during fetchTarget and preload.
     * This makes the logged size available for obsolete counting but does not
     * dirty the IN, since that could cause an unexpected write of the IN being
     * read.
     *
     * @param lastLoggedSize is positive if the size is known, zero if the size
     *            is unknown, or -1 if the size should not be changed because
     *            logging of the LN was deferred.
     */
    @Override
    ...
    /**
     * Sets the size without checking whether it is necessary. This method is
     * used when reading from the log because the databaseImpl is not yet
     * initialized and isLastLoggedSizeStored cannot be called. It is also
     * called for efficiency reasons when it is known that storing the logged
     * size is necessary, for example, when copying values between slots.
     */
    @Override
    ...
    public boolean isProbablyExpired(int idx) {
        ...
        return getEnv().expiresWithin(getExpiration(idx), isExpirationInHours(), getEnv().getTtlClockTolerance());
    }
     ...
     * @param newSiblingLow
     * @param newSiblingHigh - the low and high entry of "this" that were moved
     *            into newSibling.
     */
    @Override
    void adjustCursors(IN newSibling, int newSiblingLow, int newSiblingHigh) {
        ...
        while (iter.hasNext()) {
             ...
             * points. case 1: We need to set the cursor's "bin" reference to
             * point at the new sibling, but we don't need to adjust its index
             * since that continues to be correct post-split.
             * +=======================================+ | new sibling |
             * existing node | +=======================================+ cursor
             * ^ case 2: We only need to adjust the cursor's index since it
             * continues to point to the current BIN post-split.
             * +=======================================+ | new sibling |
             * existing node | +=======================================+ cursor
             * ^ case 3: Do nothing. The cursor continues to point at the
             * correct BIN and index. +=======================================+
             * | existing Node | new sibling |
             * +=======================================+ cursor ^ case 4: Adjust
             * the "bin" pointer to point at the new sibling BIN and also adjust
             * the index. +=======================================+ | existing
             * Node | new sibling | +=======================================+
             * cursor ^
             */
     ...
     * Caller is responsible for latching and unlatching this node. If the slot
     * containing the identifier is removed, the identifier key will be changed
     * to the key in the first remaining slot. Normally when a slot is removed,
     * the IN is dirtied. However, during compression the BIN is not dirtied
     * when a slot is removed. This is safe for the reasons described below.
     * Note that the BIN being compressed is always a full BIN, not a delta. +
     * If the BIN is not dirty and it does not become dirty before shutdown,
     * i.e., it is not logged, then it is possible that this compression will be
     * "lost". However, the state of the slot on disk is expired/deleted, and
     * when the BIN is later fetched from disk, this state will be restored and
     * the compression will be performed again. + If the slot is dirty, the BIN
     * may also be dirty or may become dirty later, and be logged. Logging a
     * delta would cause the information in the dirty slot to be lost.
     * Therefore, when a dirty slot is removed, we set a flag that prohibits the
     * next BIN logged from being a delta. This optimization (that we don't
     * dirty the BIN and we allow logging a delta after removing a non-dirty
     * slot) has one minor and one major impact: 1. When a slot is removed for a
     * deleted record, normally the slot and the BIN will be dirty. Although it
     * is unusual, we may encounter a non-dirty slot for a deleted record. This
     * happens if the slot could not be removed by this method when a full BIN
     * is logged, due to a lock or a cursor, and we compress the full BIN later.
     ...
     *            to consolidate multiple tracking operations. If null, the
     *            tracking is performed immediately in this method.
     * @return true if all deleted and expired slots were compressed, or false
     *         if one or more slots could not be compressed because we were
     *         unable to obtain a lock. A false return value means
     *         "try again later".
     */
    public boolean compress(boolean compressDirtySlots, final LocalUtilizationTracker localTracker) {

        /*
         * If the environment is not yet recovered we can't rely on locks being
         * set up to safeguard active data and so we can't compress safely.
         */
        ...
        for (int i = 0; i < getNEntries(); i++) {
             ...
             * removal of a slot for a record that is read-locked elsewhere. In
             * both cases it is more efficient to call isLockUncontended than to
             * actually lock the LN, since we would release the lock
             * immediately. We must be able to lock the LN even if
             * isKnownDeleted is true, because locks protect the aborts. (Aborts
             * may execute multiple operations, where each operation latches and
             * unlatches. It's the LN lock that protects the integrity of the
             * whole multi-step process.) For example, during abort, there may
             * be cases where we have deleted and then added an LN during the
             * same txn. This means that to undo/abort it, we first delete the
             * LN (leaving knownDeleted set), and then add it back into the
             * tree. We want to make sure the entry is in the BIN when we do the
             * insert back in.
             */
            ...
            if (entryKeys.compareKeys(getIdentifierKey(), keyPrefix, i, haveEmbeddedData(i), getKeyComparator()) == 0) {

                /*
                 * We're about to remove the entry with the idKey so the node
                 * will need a new idkey.
                 */
                setNewIdKey = true;
            }

            /*
             * When we compress a deleted slot in a deferred-write DB, we must
             * either log a dirty LN or count it obsolete. However, if we
             * compress an expired slot, neither is necessary; see TTL.
             */
            if (!expired && db.isDeferredWriteMode()) {
                ...
                if (ln != null && ln.isDirty() && !DbLsn.isTransient(lsn)) {
                    ...
                    if (db.isTemporary()) {
                         ...
                         * obsolete without logging. There is no requirement for
                         * the dirty deleted LN to be durable past recovery.
                         * There is no danger of the last logged LN being
                         * accessed again (after log cleaning, for example),
                         * since temp DBs do not survive recovery.
                         */
                        if (localTracker != null) {
                            localTracker.countObsoleteNode(lsn, ln.getGenericLogType(), getLastLoggedSize(i), db);
                        } else {
                            envImpl.getLogManager().countObsoleteNode(lsn, ln.getGenericLogType(), getLastLoggedSize(i),
                                    db, true /* countExact */);
                        }
                    } else {
         ...
         * to enlarging the BIN during recovery (see reconstituteBIN) or because
         * the configured capacity was changed.
         */
    ...
    /**
     * This method is called opportunistically at certain places where a deleted
     ...
     * subtree. It may not have any LN children. We assume that this is only
     * called under an assert.
     */
    @Override
    ...
    protected long printMemorySize() {
        ...
        System.out.format("BIN: %d vlsns: %d logSizes: %d expiration: %d offHeapLNIds: %d %n", binTotal,
                vlsnCacheOverhead, logSizesOverhead, expirationOverhead, offHeapLNIdOverhead);
     ...
     * deleted slots will also mark the BIN dirty. The BIN should be latched by
     * the caller.
     *
     * @return a long number encoding (a) the number of evicted bytes, and (b)
     *         whether this BIN is evictable. (b) will be false if the BIN has
     ...
     * cause LNs to be logged, which will mark this BIN dirty. The BIN should be
     ...
     *
     * @param ifFetchedCold If true, evict the LN only if it has the FetchedCold
     *            flag set.
    ...
    private long evictLNInternal(int index, boolean ifFetchedCold) throws DatabaseException {
        ...
        /*
         * Log target if necessary. Do not allow eviction since we evict here
         * and that would cause double-counting of the memory freed.
         */
    ...
    private void logDirtyLN(int idx, LN ln, boolean allowEviction) throws DatabaseException {
        ...
        if (force || ln.isDirty()) {
             ...
             * stream, because this is a deferred-write DB. No reason to include
             * the previous record version in this logrec because this logrec
             * will never be undone (DW databases are non-transactional)
             */
            final LogItem logItem = ln.log(envImpl, dbImpl, null /* locker */, null /* writeLockInfo */,
                    isEmbeddedLN(idx), getKey(idx), getExpiration(idx), isExpirationInHours(), isEmbeddedLN(idx),
                    currLsn, getLastLoggedSize(idx), false/* isInsertion */, true /* backgroundIO */,
    ...
    /*
     * It is called in 3 cases listed below. In all cases, if blind puts are not
     * allowed, the method returns null. 1. A full BIN is being mutated to an
     * in-memory delta. A new filter will be created here and will be stored in
     * the delta by the caller. 2. A full BIN is being logged as a delta. A new
     * filter will be created here and will be written in the delta logrec by
     * the caller. 3. An in-memory BIN-delta is being logged. If the delta has a
     * bloom filter already, that filter will be returned and written into the
     * logrec. The delta may not have a filter already because it was read from
     * an older-version logfile; in this case we return null.
     */
    ...
    boolean isDeltaProhibited() {
        return (getProhibitNextDelta() || getDatabase().isDeferredWriteMode() || getLastFullLsn() == DbLsn.NULL_LSN);
    }

    /**
     * Decide whether to log a full or partial BIN, depending on the ratio of
     * the delta size to full BIN size. Other factors are taken into account: +
     * a delta cannot be logged if the BIN has never been logged before + deltas
     * are not currently supported for DeferredWrite databases + this particular
     * delta may have been prohibited because the cleaner is migrating the BIN
     * or a dirty slot has been removed + if there are no dirty slots, we might
     * as well log a full BIN The restriction on using BIN-deltas for
     * deferred-write DBs is for reasons that are probably no longer relevant.
     * However, we have not tested deltas with DW, so we still prohibit them.
     * Because BIN-deltas may be in cache at the time a DB is opened in DW mode,
     * a workaround is currently necessary: see
     * Database.mutateDeferredWriteBINDeltas.
     *
    ...
    public boolean shouldLogDelta() {
        ...
        if (isBINDelta()) {
            /*
             * Cannot assert that db is not in DeferredWrite mode. See
             * Database.mutateDeferredWriteBINDeltas.
             */
    ...
    /**
     * Mutate to a delta (discard non-dirty entries and resize arrays). This
     * method must be called with this node latched exclusively, and
     ...
     * It is currently used by the DiskOrderedScanner only. The method does not
     * clone the targets array.
     */
     ...
     */
    private void initBINDelta(final BIN destBIN, final int nDeltas, final int capacity, final boolean copyTargets) {
        ...
        destBIN.resetContent(capacity, nDeltas, baseFileNumber, compactLSNs, longLSNs, states, keyPrefix, keys, targets,
                sizes, memIds, vlsns, expiration, isExpirationInHours());
    ...
    /**
     * Replaces the contents of this BIN with the given contents. Used in
     * mutating a full BIN to a BIN-delta or for creating a new BIN delta with
     * the given content.
     */
    private void resetContent(final int capacity, final int newNEntries, final long baseFileNumber,
                              final byte[] compactLSNs, final long[] longLSNs, final byte[] states,
                              final byte[] keyPrefix, final byte[][] keys, final Node[] targets,
                              final int[] loggedSizes, final long[] memIds, final long[] vlsns, final int[] expiration,
     ...
     * merged result to replace the contents of this BIN. This method must be
     * called with this node latched exclusively. If 'this' is not a delta, this
     * method does nothing.
     */
    @Override
     ...
     * param and then replacing this BIN's contents with it. This method must be
     * called with this node latched exclusively. 'this' must be a delta. After
     * mutation, the full BIN is compressed and compacted. The compression is
     * particularly important, since BIN-deltas in cache cannot be compressed.
     ...
     * full BIN that has just been read from the log, and it is not part of the
            ...
            for (CursorImpl cursor : cursorSet) {
                ...
                if (keys[i] != null) {
                    ...
                    if ((index & IN.EXACT_MATCH) == 0) {
                        throw EnvironmentFailureException.unexpectedState(getEnv(),
     ...
     * applying to it the deltas in this BIN delta. The new BIN is not added to
     * the INList or the BTree. Called from
     * DiskOrderedScanner.fetchAndProcessBINs() and
     ...
     *            resulting full BIN, without first checking whether the full
     *            BIN must be split, and performing the split if necessary. If
     *            this param is true, the returned BIN will contain at least one
     *            free slot. If this param is false, a BIN with no free slots
     *            may be returned. For example, it is important that false is
     *            passed when a split will be performed, since if true were
     *            passed, the BIN would grow beyond its bounds unnecessarily.
     */
    public void reconstituteBIN(DatabaseImpl dbImpl, BIN fullBIN, boolean leaveFreeSlot) {
        ...
        try {
             ...
             * following scenario: Non-dirty slots were compressed away earlier,
             * leaving room for inserted records, and a delta was logged with
             * the inserted records. The full version of the BIN (after
             * compression) was not logged, because the BIN is not dirtied when
             * non-dirty slots were compressed away. If we don't compress here,
             * there may not be room in the original BIN for the slots inserted
             * when applying the deltas. However, during recovery we can't
             * compress because locking is not used during recovery, and the
             * compressor may delete a slot for a record that is part of an
             * active transaction. In addition, even when compression is
             * performed here, it is possible that it doesn't compress all
             * deleted/expired slots that were compressed originally in the
             * scenario described, for one of the following reasons: + The
             * record is locked temporarily by a read operation that will skip
             * the record. Note that the compressor uses non-blocking locking. +
             * If expiration has been disabled, or the system clock has been
             * changed, slots that were expired originally may not be expired
             * now. Therefore, in all cases we enlarge the BIN if necessary to
             * hold all slots to be inserted when applying the delta. An extra
             * slot is added if leaveFreeSlot is true, to handle cases where
             * mutation to a full BIN is performed after calling
             * Tree.searchSplitsAllowed, or one of the methods that calls it
             * such as Tree.findBinForInsert and Tree.getParentBINForChildLN. If
             * the search returns a BIN-delta without splitting, and then we
            ...
            for (int i = 0; i < getNEntries(); i++) {
                ...
                fullBIN.applyDelta(getKey(i), getData(i), getLsn(i), getState(i), getLastLoggedSize(i),
                        getOffHeapLNId(i), getCachedVLSN(i), getTarget(i), getExpiration(i), isExpirationInHours());
            }
    ...
    /**
     * Apply (insert, update) a given delta slot in this full BIN. Note: also
     * called from OldBINDelta class.
     */
    void applyDelta(final byte[] key, final byte[] data, final long lsn, final byte state, final int lastLoggedSize,
                    final long ohLnId, final long vlsn, final Node child, final int expiration,
                    final boolean expirationInHours) {

        /*
         * The delta is the authoritative version of the entry. In all cases, it
         * should supersede the entry in the full BIN. This is true even if the
         * BIN Delta's entry is knownDeleted or if the full BIN's version is
         * knownDeleted. Therefore we use the flavor of findEntry that will
        ...
        if (foundIndex >= 0 && (foundIndex & IN.EXACT_MATCH) != 0) {
             ...
             * entry even when it is known or pending deleted, since the deleted
             * (and dirty) entry will be needed to log the next delta. [#20737]
             */
