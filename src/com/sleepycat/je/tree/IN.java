 ...
 * ===================================================================
 *
 * PD: set for all LN entries that are deleted, even before the LN is committed.
 * Is used as an authoritative (transactionally correct) indication that an LN
 * is deleted. PD will be cleared if the txn for the deleted LN is aborted.
 *
 ...
 *
 * First notice that IN.fetchLN will allow a FileNotFoundException when the PD
 * or KD flag is set on the entry. And it will allow a NULL_LSN when the KD flag
 * is set.
 *
 * KD was implemented first, and was originally used when the cleaner attempts
 * to migrate an LN and discovers it is deleted (see Cleaner.migrateLN). We need
 * KD because the INCompressor may not have run, and may not have compressed the
 * BIN. There's the danger that we'll try to fetch that entry, and that the file
 * was deleted by the cleaner.
 *
 ...
 * we mark the entry KD so it won't cause a fetch error later. In this case the
 * entry LSN is NULL_LSN. See Tree.insertNewSlot.
 *
 ...
 * lookup, and without any record of an obsolete offset. This makes the cost of
 * cleaning of deleted LNs very low. For example, if the log looks like this:
 *
 * 100 LNA 200 delete of LNA
 *
 ...
 * fetch error, when the file is deleted by the cleaner but the entry containing
 * the deleted LN has not been deleted by the INCompressor.
 *
 * PD is also set in replication rollback, when LNs are marked as invisible.
 *
 * When LSN locking was implemented (see CursorImpl.lockLN), the PD flag took on
 * additional meaning. PD is used to determine whether an LN is deleted without
 * fetching it, and therefore is relied on to be transactionally correct.
 *
 ...
 * one flag: simply the Deleted flag. The Deleted flag would be set in the same
 * way as PD is currently set, as well as the second use of KD described above
 * (when the LSN is NULL_LSN after an insertion error). The use of KD and PD for
 * invisible entries and recovery rollback should also be considered.
 *
 ...
 * example) that consider a slot deleted when KD is set. Since KD is rarely set
 * currently, this shouldn't have a noticeable performance impact.
 */
...
public class IN extends Node implements Comparable<IN>, LatchContext {
	 ...
	 *
	 * Note: A compact representation will be changed to the non-compact one, if
	 * needed, but in the current implementation, the reverse mutation (from
	 * long to compact) never takes place.
	 */
	long baseFileNumber;
	 ...
	 * L.prevFullLsn.
	 *
	 * (b) After logging a UIN or a full-BIN logrec, it is set to the LSN of the
	 * logrec written. This is done in IN.afterLog().
	 *
	 * Notice that this is a persistent field, but except from case (c), when
	 * reading a logrec L, it is set not to the value found in L, but to the lsn
	 * of L. This is why its read/write is managed by the INLogEntry class
	 ...
	 * follows:
	 *
	 * Let L be the most recently written logrec for this BIN instance. If L is
	 * a full-version logrec, lastDeltaVersion is NULL; otherwise it is the lsn
	 * of L.
	 *
	 ...
	 * It is set in 2 cases:
	 *
	 * (a) after "this" is created via reading a logrec L, lastDeltaVersion is
	 * set to L's lsn, if L is a BIN-delta logrec, or to NULL if L is a full-BIN
	 * logrec (this is done in IN.postFetch/RecoveryInit(), via
	 * BIN.setLastLoggedLsn()).
	 *
	 * (b) After we write a logrec L for this BIN instance, lastDeltaVersion is
	 * set to NULL if L is a full-BIN logrec, or to L's lsn, if L is a BIN-delta
	 * logrec (this is done in BIN.afterLog()).
	 *
	 ...
	 *
	 * @param db
	 *            is passed in order to initialize the database for an
	 ...
	 *             (fatal) if the parent latch is already held.
	 */
	 ...
	 *
	 * @param makeDirty
	 *            should normally be true, but may be false when an expired slot
	 *            containing the identifier key has been deleted.
	 */
	public final void setIdentifierKey(byte[] key, boolean makeDirty) {
		 ...
		 * budget. If we did, then it would look like this:
		 * 
		 * int oldIDKeySz = (identifierKey == null) ? 0 :
		 * MemoryBudget.byteArraySize(identifierKey.length);
		 * 
		 * int newIDKeySz = (key == null) ? 0 :
		 * MemoryBudget.byteArraySize(key.length); updateMemorySize(newIDKeySz -
		 * oldIDKeySz);
		 * 
		 */
		identifierKey = key;
	 ...
	 *
	 * @param key
	 *            is the key to set in the slot and is the LN key.
	 *
	 * @param data
	 *            If the data portion of a record must be embedded in this BIN,
	 ...
	 * key prefix is adjusted, if necessary.
	 *
	 * In case of insertion (indicated by a true value for the isInsertion
	 * param), it is assumed that the idx slot does not store any valid info, so
	 * any change to the key prefix (if any) is due to the insertion of this new
	 * new key and not to the removal of the current key at the idx slot.
	 *
	 * In case of update, the method does not check if the current key is indeed
	 * different from the new key; it just updates the key unconditionally. If
	 * the slot has embedded data, that data will also be updated (if the data
	 * param is not null), or be removed (if the data param is null). If the
	 * slot does not have embedded data and the data param is not null, the
	 * given data will be embedded.
	 *
	 ...
	 *
	 * The following transition is a NOOP and the LSN is not set: Any LSN to
	 * same value. The following transitions are allowed and cause the LSN to be
	 * set: Null LSN to transient LSN Null LSN to persistent LSN Transient LSN
	 * to persistent LSN Persistent LSN to new persistent LSN The following
	 * transitions should not occur and throw an exception: Transient LSN to
	 * null LSN Transient LSN to new transient LSN Persistent LSN to null LSN
	 ...
	 * children are INs (this is not a BIN), when this method returns null it is
	 * does not imply that the child is non-dirty, because dirty BINs are stored
	 * off-heap. To fetch the current version from off-heap cache in that case,
	 * call loadIN instead.
	 */
	 ...
	 * method is used during tree searches.
	 *
	 * On entry, the parent must be latched already.
	 *
	 * If the child must be fetched from the log, the parent is unlatched. After
	 * the disk read is done, the parent is relatched. However, due to splits,
	 * it may not be the correct parent anymore. If so, the method will return
	 * null, and the caller is expected to restart the tree search.
	 *
	 ...
	 * used by this method in determining if, after a disk read, "this" is still
	 * the correct parent for the child. "searchKey" may be null if the caller
	 * is doing a LEFT or RIGHT search.
	 */
	...
	private IN fetchINWithNoLatch(int idx, byte[] searchKey, SearchResult result, CacheMode cacheMode) {
		...
		if (child == null) {
			...
			try {
				 ...
				 * the idx position before P was unlatched above. Here, we view
				 * slots as independent objects, not identified by their
				 ...
				 * relatched, let S' be the slot at the idx position, if P still
				 * has an idx position. We consider the following 2 cases:
				 *
				 ...
				 *
				 * 2. There is no idx position in P or S'.LSN != S.LSN. In this
				 * case we cannot be sure if S' (if it exists) is the the
				 * correct slot to follow. So, we (re)search for K in P to check
				 * if P is still the correct parent and find the correct slot to
				 * follow. If this search lands on the 1st or last slot in P, we
				 * may return null because using the key info contained in P
				 * only, we do not know the full range of keys covered by those
				 * two slots. If null is returned, the caller is expected to
				 * restart the tree search from the root.
				 *
				 * Notice that the if conditions are necessary before calling
				 * findEntry(). Without them, we could get into an infinite loop
				 * of search re-tries in the scenario where nothing changes in
				 * the tree and findEntry always lands on the 1st or last slot
				 * in P. The conditions guarantee that we may restart the tree
				 * search only if something changes with S while P is unlatched
				 * (S moves to a different position or a different IN or it
				 * points to a different LSN).
				 * 
				 ...
				 *
				 * For DBs in DW mode, special care must be taken because splits
				 * are not immediately logged. So, for DW DBs, to avoid a call
				 * to findEntry() we require that not only S'.LSN == S.LSN, but
				 ...
				 * findEntry() call above). We check 5 cases:
				 *
				 * (a) There is already a cached child at the "idx" position. In
				 * this case, we return whatever is there because it has to be
				 * the most recent version of the appropriate child node. This
				 * is true even when a split or reverse split occurred. The
				 * check for isKeyInBounds above is critical in that case.
				 *
				 * (b) There is no cached child at the "idx" slot, but the slot
				 * LSN is not the same as the LSN we read from the log. This is
				 * the case if "idx" was changed by findEntry() or other threads
				 * fetched the same child as this thread, updated it, and then
				 * evicted it. The child we fetched is obsolete and should not
				 * be attached. For simplicity, we just return null in this
				 * (quite rare) case.
				 *
				 ...
				 * did not change because it wasn't logged. We determine whether
				 * the off-heap BIN has changed, and if so then null is
				 * returned. This is also rare.
				 *
	...
	/**
	 * Returns the idx-th child of "this" upper IN, fetching the child from the
	 * log and attaching it to its parent if it is not already attached.
	 *
	 ...
	 *
	 * @param idx
	 *            The slot of the child to fetch.
	 */
	 ...
	 * null if the child is not in the main or off-heap cache.
	 *
	 * On entry, the parent must be EX-latched already and it stays EX-latched
	 * for the duration of this method and on return (even in case of
	 * exceptions).
	 ...
	 *
	 * 1. if the LSN is null and the KnownDeleted flag is set; or 2. if the
	 * LSN's file has been cleaned and: a. the PendingDeleted or KnownDeleted
	 * flag is set, or b. the entry is "probably expired".
	 *
	 * Note that checking for PD/KD before calling this method is not sufficient
	 * to ensure that null is not returned, because null is also returned for
	 * expired records.
	 *
	...
	/**
	 * @param in
	 *            parent IN. Is null when root is fetched.
	 */
	 ...
	 * parameter to the Comparator.compare method. This allows a comparator to
	 * perform specialized searches, when passed down from upper layers.
	 *
	...
	public final int findEntry(byte[] key, boolean indicateIfDuplicate, boolean exact, Comparator<byte[]> comparator) {
		 ...
		 * special entry zero comparison.
		 *
		 * We always use inexact searching to get down to the BIN, and then call
		 * findEntry separately on the BIN if necessary. So the behavior of
		 * findEntry is different for BINs and INs, because it's used in
		 * different ways.
		 *
		 * Consider a tree where the lowest key is "b" and we want to insert
		 * "a". If we did the comparison (with exact == false), we wouldn't find
		 * the correct (i.e. the left) path down the tree. So the virtual key
		 * ensures that "a" gets inserted down the left path.
		 *
		 * The insertion case is a good specific example. findBinForInsert does
		 * inexact searching in the INs only, not the BIN.
		 *
	 ...
	 * is already latched by the caller.
	 *
	 ...
	 *
	 * See BIN.compress and INCompressor for a discussion about why slots can be
	 ...
	 *
	 * @param newLSN
	 *            The new on-disk position of the child.
	 *
	 * @param newVLSN
	 *            The VLSN of the logrec at the new position. For LN children
	 *            only.
	 *
	 * @param newSize
	 *            The size of the logrec at the new position. For LN children
	 *            only.
	 */
	 ...
	 *
	 * No need to do memory counting in this method because the BIN is not yet
	 ...
	 *
	 * @param newLN
	 *            The LN associated with the new record.
	 *
	 * @param newLSN
	 *            The LSN of the insertion logrec.
	 *
	 * @param newSize
	 *            The size of the insertion logrec.
	 *
	 * @param newKey
	 *            The value for the record's key. It is equal to the current key
	 *            value in the slot, but may not be identical to that value if a
	 *            custom comparator is used.
	 *
	 * @param newData
	 *            If the record's data must be embedded in this BIN, "data"
	 ...
	 *
	 * @param oldMemSize
	 *            If the child LN was cached before the update op, it has
	 ...
	 *
	 * @param newLSN
	 *            The LSN of the update logrec.
	 *
	 * @param newVLSN
	 *            The VLSN of the update logrec.
	 *
	 * @param newSize
	 *            The on-disk size of the update logrec.
	 *
	 * @param newKey
	 *            The new value for the record's key. It is equal to the current
	 *            value, but may not be identical to the current value if a
	 *            custom comparator is used. It may be null, if the caller knows
	 *            for sure that the key does not change.
	 *
	 * @param newData
	 *            If the record's data must be embedded in this BIN, "data"
	 *            stores the record's data. Null otherwise. See also comment for
	 ...
	 *
	 * @param oldMemSize
	 *            If the child LN was cached before the deletion, it has already
	 *            been updated in-place by the caller (the ln contents have been
	 *            deleted). In this case, oldMemSize stores the in-memory size
	 *            of the child LN before the update, and it is used to do memory
	 *            counting. Otherwise oldMemSize is 0 and the newly created LN
	 *            has not been attached to the tree; it will be attached later
	 *            by the caller, if needed.
	 *
	 * @param newLSN
	 *            The LSN of the deletion logrec.
	 *
	 * @param newVLSN
	 *            The VLSN of the deletion logrec.
	 *
	 * @param newSize
	 *            The on-disk size of the deletion logrec.
	 */
	 ...
	 *
	 * @param idx
	 *            The BIN slot for the record.
	 *
	 * @param lsn
	 *            The LSN of the new record version. It may be null in case of
	 *            undo, if the logrec that is being undone is an insertion and
	 *            the record did not exist at all in the DB before that
	 *            insertion.
	 *
	 * @param knownDeleted
	 *            True if the new version is a committed deletion.
	 *
	 * @param pendingDeleted
	 *            True if the new version is a deletion, which may or may not be
	 *            committed.
	 *
	 * @param key
	 *            The key of the new version. It is null only if we are undoing
	 ...
	 *
	 * @param data
	 *            The data of the new version. It is non-null if and only if the
	 *            new version must be embedded in the BIN.
	 *
	 * @param vlsn
	 *            The VLSN of the new version.
	 *
	 * @param logrecSize
	 *            The on-disk size of the logrec corresponding to the new
	 ...
	 *
	 * This method is called after the child node has been either (a) fetched in
	 * from disk and is not dirty, or (b) is a newly created instance that will
	 * be written out later by something like a checkpoint. In either case, the
	 * slot LSN does not need to be updated.
	 *
	 ...
	 *
	 * The most common caller of this method is the evictor. If the child being
	 * evicted was dirty, it has just been logged and the lsn of the slot must
	 * be updated.
	 */
	...
	final IN splitInternal(final IN parent, final int childIndex, final IN grandParent, final int maxEntries,
		...
		try {
			 ...
			 * add a new pointer to the new sibling.
			 *
			 * We must use the provisional logging for two reasons:
			 *
			 * 1) All three log entries must be read atomically. The parent must
			 * get logged last, as all referred-to children must precede it.
			 * Provisional entries guarantee that all three are processed as a
			 * unit. Recovery skips provisional entries, so the changed children
			 * are only used if the parent makes it out to the log.
			 *
			 * 2) We log all they way to the root to avoid the "great aunt"
			 * problem (see LevelRecorder), and provisional logging is necessary
			 * during a checkpoint for levels less than maxFlushLevel.
			 *
			 * We prohibit compression during logging because there should be at
			 * least one entry in each IN. Note the use of getKey(0) below.
			 */
	 ...
	 *
	 * @param child
	 *            The new child to be placed under the slot. May be the newly
	 *            created sibling or the pre-existing sibling.
	 * @param lsn
	 *            The new lsn of the child (the child was logged just before
	 *            calling this method, so its slot lsn must be updated)
	 * @param key
	 *            The new key for the slot. We should not actually update the
	 ...
	 *
	 * TODO: This method is no longer used for LN slot reuse, and freeing of the
	 * off-heap BIN could be done by the only caller, splitInternal, and then
	 ...
	 * often (e.g., every CRUD operation), since this could cause lots of memory
	 * allocations as the representations contract and expend, resulting in
	 * expensive GC.
	 *
	 ...
	 * possible.
	 *
	 * Used by the evictor to reclaim memory by some means short of evicting the
	 * entire node. If a positive value is returned, the evictor will postpone
	 * full eviction of this node.
	 */
	 ...
	 *
	 * Note: the method is overwritten by BIN and LN. BIN.isValidForDelete()
	 * will not fetch any child LNs. LN.isValidForDelete() simply returns false.
	 *
	...
	public final void verify(byte[] maxKey) throws EnvironmentFailureException {
		 ...
		 * null ? null : getKeyComparator());
		 * 
		 * byte[] key1 = null; for (int i = 1; i < nEntries; i++) { key1 =
		 * entryKeys.get(i); byte[] key2 = entryKeys.get(i - 1);
		 * 
		 * int s = Key.compareKeys(key1, key2, userCompareToFcn); if (s <= 0) {
		 * throw EnvironmentFailureException.unexpectedState ("IN " +
		 * getNodeId() + " key " + (i-1) + " (" + Key.dumpString(key2, 0) +
		 * ") and " + i + " (" + Key.dumpString(key1, 0) + ") are out of order"
		 * ); } }
		 * 
		 * boolean inconsistent = false; if (maxKey != null && key1 != null) {
		 * if (Key.compareKeys(key1, maxKey, userCompareToFcn) >= 0) {
		 * inconsistent = true; } }
		 * 
		 * if (inconsistent) { throw EnvironmentFailureException.unexpectedState
		 * ("IN " + getNodeId() + " has entry larger than next entry in parent."
		 * ); } } catch (DatabaseException DE) { DE.printStackTrace(System.out);
		 * }
		 *****************/
	}
	 ...
	 *
	 * It is called from IN.postFetch/RecoveryInit(). If the logrec we have just
	 ...
	 *
	 * A checkpoint or Database.sync starts The INList is traversed, dirty nodes
	 * are selected BINb is bypassed on the INList, since it's not dirty BINb is
	 * split, creating a new sibling, BINc, and dirtying INa INa is selected as
	 * a dirty node for the ckpt
	 *
	 * If this happens, INa is in the selected dirty set, but not its dirty
	 * child BINb and new child BINc.
	 *
	 * In a durable db, the existence of BINb and BINc are logged anyway. But in
	 * a deferred write db, there is an entry that points to BINc, but no logged
	 * version.
	 *
	 * This will not cause problems with eviction, because INa can't be evicted
	 * until BINb and BINc are logged, are non-dirty, and are detached. But it
	 * can cause problems at recovery, because INa will have a null LSN for a
	 * valid entry, and the LN children of BINc will not find a home. To prevent
	 * this, search for all dirty children that might have been missed during
	 * the selection phase, and write them out. It's not sufficient to write
	 * only null-LSN children, because the existing sibling must be logged lest
	 * LN children recover twice (once in the new sibling, once in the old
	 * existing sibling.
	 *
	 * TODO: Would the problem above be solved by logging dirty nodes using a
	 * tree traversal (post-order), rather than using the dirty map?
	 *
	 * Overriden by BIN class.
	 */
	 ...
	 *
	 * If 'node' is non-null, 'logEntry' must be null. If 'node' is null,
	 * 'logEntry' and 'parent' must be non-null.
	 *
	 * When 'logEntry' is non-null we are logging an off-heap BIN, and it is not
	 * resident in the main cache. The lastFull/DeltaLsns are not updated here,
	 * and this must be done instead by the caller.
	 *
	 * When 'node' is non-null, 'parent' may or may not be null. It must be
	 * non-null when logging provisionally, since obsolete LSNs are added to the
	 * parent's collection.
	 */
	private static long logInternal(final IN node, INLogEntry<?> logEntry, final boolean allowDeltas,
			final boolean allowCompress, final Provisional provisional, final boolean backgroundIO, final IN parent) {
		 ...
		 * unchanged.
		 *
		 * For full version logging: + Count lastFullVersion and
