...
public final class FileSummaryLN extends LN {
     ...
     * For the old version of the LN there will be a single record per file. If
     * this is a version 0 log entry, the key is a string. However, such an LN
     * may be migrated by the cleaner, in which case the version will be 1 or
     * greater [#13061]. In the latter case, we can distinguish a string key by:
     * 1) If the key is not 8 bytes long, it has to be a string key. 2) If the
     * key is 8 bytes long, but bytes[4] is ascii "0" to "9", then it must be a
     * string key. bytes[4] to bytes[7] are a sequence number that is the number
     * of log entries counted. For this number to be greater than 0x30000000,
     * the binary value of 4 digits starting with ascii "0", over 400 million
     * log entries would have to occur in a single file; this should never
     * happen. Note that having to rely on method (2) is unlikely. A string key
     * will only be 8 bytes if the file number reach 8 decimal digits
     * (10,000,000 to 99,999,999). This is a very large file number and unlikely
     * to have occurred using JE 1.7.1 or earlier. In summary, the only time the
     * algorithm here could fail is if there were more than 400 million log
     * entries per file, and more than 10 million were written with JE 1.7.1 or
     * earlier.
     */
     ...
     *
     * @param sequence is a unique identifier for the LN for the given file, and
     *            must be greater than the last sequence.
     */
    ...
    /**
     * Return the correct log type for a FileSummaryLN. Note: FileSummaryLN will
    ...
    public void writeToLog(final ByteBuffer logBuffer, final int logVersion, final boolean forReplication) {
         ...
         * to the log, and reset the tracked summary. When deleting the LN, the
         * tracked summary is cleared explicitly and will be null.
         */
    ...
    public long getMemorySizeIncludedByParent() {
        return super.getMemorySizeIncludedByParent() + (MemoryBudget.FILESUMMARYLN_OVERHEAD - MemoryBudget.LN_OVERHEAD)
     ...
     * size. Must be called after LN is inserted into the BIN and logged, while
     * the cursor is still positioned on the inserted LN. The BIN must be
     * latched. [#17462]
     * <p>
     * The obsoleteOffsets memory size is not intially budgeted in the usual way
     * because PackedOffsets.pack (which changes the memory size) is called
     ...
     * (see CursorImpl.putInternal).
     * </p>
     * <p>
     * Note that the tree memory usage cannot be updated directly in getOffsets
     * because the tree memory usage must always be the sum of all IN sizes, and
     * it is reset to this sum each checkpoint.
     * </p>
     */
    @Override
