 ...
 * implementation of java.util.logging.ConsoleHandler. By default, the handler's
 * level is {@link Level#OFF}. To enable the console output, use the standard
...
public class ConsoleHandler extends java.util.logging.ConsoleHandler {
    ...
    public ConsoleHandler(Formatter formatter, EnvironmentImpl envImpl) {
        ...
        if (envImpl != null) {
            level = LoggerUtils.getHandlerLevel(envImpl.getConfigManager(), EnvironmentParams.JE_CONSOLE_LEVEL,
                    propertyName);
        } else {
