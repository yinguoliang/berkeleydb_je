...
/**
 * DbBackup is a helper class for stopping and restarting JE background activity
 * in an open environment in order to simplify backup operations. It also lets
 * the application create a backup which can support restoring the environment
 * to a specific point in time.
 * <p>
 * <b>Backing up without DbBackup</b>
 * <p>
 * Because JE has an append only log file architecture, it is always possible to
 * do a hot backup without the use of DbBackup by copying all log files (.jdb
 * files) to your archival location. As long as the log files are copied in
 * alphabetical order, (numerical in effect) <i>and</i> all log files are
 * copied, the environment can be successfully backed up without any need to
 * stop database operations or background activity. This means that your backup
 * operation must do a loop to check for the creation of new log files before
 * deciding that the backup is finished. For example:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * In the example above, the backup operation must be sure to copy file 5, which
 * came into existence after the backup had started. If the backup stopped
 * operations at file 4, the backup set would include only file 1 and 4,
 * omitting file 3, which would be an inconsistent set.
 * <p>
 ...
 * read/write protection, the application must pass DbBackup an open, read/write
 * environment handle.
 * <p>
 ...
 * the ongoing creation of new files. Also, there will be no need to check for a
 * newer version of the last file on the next backup.
 * <p>
 ...
 * backup, a checkpoint should be performed immediately before calling
 * {@link #startBackup}. This will reduce recovery time when opening the
 * environment with the restored log files. A checkpoint is performed explicitly
 * by calling {@link Environment#checkpoint} using a config object for which
 ...
 * verified before or during the copy. If a file is copied that is corrupt (due
 * to an earlier disk failure that went unnoticed, for example), the backup will
 * be invalid and provide a false sense of security.
 * <p>
 * The {@link LogVerificationInputStream example here} shows how to implement
 * the {@code myCopyFiles} method using {@link LogVerificationInputStream}. A
 * {@link LogVerificationReadableByteChannel} could also be used for higher
 * performance copying. A filter input stream is used to verify the file
 * efficiently as it is being read. If you choose to use a script for copying
 * files, the {@link DbVerifyLog} command line tool can be used instead.
 * <p>
 ...
 * <p>
 * Incremental backups are used to reduce the number of files copied during each
 * backup. Compared to a full backup, there are two additional pieces of
 * information needed for an incremental backup: the number of the last file in
 * the previous backup, and a list of the files in the environment directory at
 * the time of the current backup, i.e., the current snapshot. Their purpose is
 * explained below.
 * <p>
 ...
 * is used to avoid unused files after a restore, and may also be used to reduce
 * the size of the backup set. How to use this list is described below.
 * <p>
 ...
 * backup, then the list should be used to delete unused files from the backup
 * set. After copying all files during the backup, any file that is <em>not</em>
 * present in the list may then be deleted from the backup set. This both
 * reduces the size of the backup set, and ensures that unused files will not be
 * present in the backup set and therefore will not be restored.</li>
 * <li>If you need to keep all log files from each backup so you can restore to
 * more than one point in time, then the list for each backup should be saved
 * with the backup file set so it can be used during a restore. During the
 * restore, only the files in the list should be copied, starting with an empty
 * destination directory. This ensures that unused files will not be restored.
 * </li>
 * </ol>
 * <p>
 * The following two examples shows how to perform an incremental backup. In the
 * first example, the list of current files is used to delete files from the
 ...
 * written by the most recent backup, or a list written by an earlier backup. To
 * restore, the files in the list should be copied into an empty destination
 * directory.
 * <p>
 ...
 * <ul>
 * <li>the backup was created using DbBackup with JE 6.2 or earlier, OR</li>
 ...
 * restored files, the
 * {@link com.sleepycat.je.EnvironmentConfig#ENV_RECOVERY_FORCE_NEW_FILE}
 * parameter should be set to true when opening the JE Environment for the first
 * time after the restore. When this parameter is set to true, the last .jdb
 * file restored will not be modified when opening the Environment, and the next
 ...
 * after a restore, then the backup set that was restored may not be used as the
 * basis for future incremental backups. If a future incremental backup were
 * performed based on this backup set, it would be incomplete and data would be
 * lost if that incremental backup were restored.
 * <p>
 * When JE 6.3 or later is used to create the backup, and the backup is created
 * in a read-write environment (the usual case), this extra step is unnecessary.
 * In this case, {@link #startBackup} will have added an "immutable file" marker
 * to the last file in the backup and this will prevent that file from being
 * modified, just as if the {@code ENV_RECOVERY_FORCE_NEW_FILE} parameter were
 * set to true.
 */
public class DbBackup {
     ...
     * <p>
     * This is equivalent to using {@link #DbBackup(Environment,long)} and
     * passing {@code -1} for the {@code lastFileInPrevBackup} parameter.
     * </p>
     *
     * @param env with an open, valid environment handle. If the environment
     *            directory has read/write permissions, the environment handle
     *            must be configured for read/write.
     * @throws IllegalArgumentException if the environment directory has
     *             read/write permissions, but the environment handle is not
     ...
     *            directory has read/write permissions, the environment handle
     *            must be configured for read/write.
     * @param lastFileInPrevBackup the last file in the previous backup set when
     *            performing an incremental backup, or {@code -1} to perform a
     *            full backup. The first file in this backup set will be the
     *            file following {@code lastFileInPrevBackup}.
    ...
    private DbBackup(Environment env, EnvironmentImpl envImpl, long lastFileInPrevBackup) {
        ...
        /*
         * If the environment is writable, we need a r/w environment handle in
         * order to flip the file.
         */
     ...
     * <p>
     * This method determines the last file in the backup set, which is the last
     * log file in the environment at this point in time. Following this method
     * call, all new data will be written to other, new log files. In other
     * words, the last file in the backup set will not be modified after this
     * method returns.
     * </p>
     * <p>
     * <em>WARNING:</em> After calling this method, deletion of log files in the
     * backup set by the JE log cleaner will be disabled until
     * {@link #endBackup()} is called. To prevent unbounded growth of disk
     * usage, be sure to call {@link #endBackup()} to re-enable log file
     * deletion. Additionally, the Environment can't be closed until endBackup()
     * is called.
     * </p>
     *
     * @throws com.sleepycat.je.rep.LogOverwriteException if a replication
     *             operation is overwriting log files. The backup can not
     *             proceed because files may be invalid. The backup may be
    ...
    public synchronized void startBackup() throws DatabaseException {
        ...
        if (!envImpl.addDbBackup(this)) {
            throw envImpl.createLogOverwriteException("A replication operation is overwriting log files. The "
        ...
        try {
            ...
            if (envIsReadOnly) {
                 ...
                 * include all files. However, we can't add a marker to the last
                 * file in list, and therefore it will not be immutable after
                 * being restored to a read-write directory (unless the user
                 * sets ENV_RECOVERY_FORCE_NEW_FILE after restoring).
                 */
                lastFileInBackup = envImpl.getFileManager().getLastFileNum();
            } else {
                /*
                 * Flip the log so that all files in the backup list are
                 * immutable. But first, write an "immutable file" marker in the
                 * last file in the backup, so it cannot be modified after it is
                 * restored. Recovery enforces this rule.
                 */
                LogEntry marker = new SingleItemEntry<EmptyLogEntry>(LogEntryType.LOG_IMMUTABLE_FILE,
                        new EmptyLogEntry());
            ...
            snapshotFiles = envImpl.getFileManager().listFileNames(0, lastFileInBackup);
        } finally {
     ...
     *
     * @throws com.sleepycat.je.rep.LogOverwriteException if a replication
     *             operation has overwritten log files. Any copied files should
     *             be considered invalid and discarded. The backup may be
     *             attempted at a later time.
    ...
    public synchronized void endBackup() {
        ...
        if (invalid) {
            invalid = false;
            throw envImpl.createLogOverwriteException("A replication operation has overwritten log files from "
                    + "file " + rollbackStartedFileNumber + ". Any copied files "
                    + "should be considered invalid and discarded. The backup " + "may be attempted at a later time.");
        }
    }
     ...
     *
     * @return the file number of the last file in the current backup set. Save
     *         this value to reduce the number of files that must be copied at
     ...
     * called.
     * <p>
     * The file numbers returned are in the range from the constructor parameter
     * {@code lastFileInPrevBackup + 1} to the last log file at the time that
     * {@link #startBackup} was called.
     * </p>
     *
     * @return the names of all files to be copied, sorted in alphabetical
     *         order. The return values are generally simple file names, not
     *         full paths. However, if multiple data directories are being used
     *         (i.e. the {@link <a
     *         href="../EnvironmentConfig.html#LOG_N_DATA_DIRECTORIES">
     *         je.log.nDataDirectories</a>} parameter is non-0), then the file
     *         names are prepended with the associated "dataNNN/" prefix, where
     *         "dataNNN/" is the data directory name within the environment home
     *         directory and "/" is the relevant file separator for the
     *         platform.
     * @throws EnvironmentFailureException if an unexpected, internal or
     *             environment-wide failure occurs.
     * @throws IllegalStateException if a backup has not been started.
    ...
    public synchronized String[] getLogFilesInBackupSet() {
        checkBackupStarted();
        return envImpl.getFileManager().listFileNames(firstFileInBackup, lastFileInBackup);
    }
     ...
     * copied in the previous backup session. Can only be called in backup mode,
     * after startBackup() has been called.
     *
    ...
    public synchronized String[] getLogFilesInBackupSet(long lastFileInPrevBackup) {
        ...
        return fileManager.listFileNames(lastFileInPrevBackup + 1, lastFileInBackup);
    }
     ...
     * <p>
     * When performing an incremental backup, this method is called to determine
     * the files that would needed for a restore. As described in the examples
     * at the top of this class, this list can be used to avoid unused files
     * after a restore, and may also be used to reduce the size of the backup
     * set.
     * </p>
     * <p>
     * When performing a full backup this method is normally not needed, since
     * in that case it returns the same set of files that is returned by
     * {@link #getLogFilesInBackupSet()}.
     * </p>
     *
    ...
    /**
     * @hidden A test entry point used to simulate the environment is now
     *         rolling back, and this TestHook would invalidate the in progress
     *         DbBackups.
     */
