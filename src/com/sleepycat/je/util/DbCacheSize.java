 ...
 * number of records and database characteristics and DbCacheSize will return an
 * estimate of the cache size required for holding the database in memory. Based
 * on this information a JE main cache size can be chosen and then configured
 * using {@link EnvironmentConfig#setCacheSize} or using the
 * {@link EnvironmentConfig#MAX_MEMORY} property. An off-heap cache may also be
 * optionally configured using {@link EnvironmentConfig#setOffHeapCacheSize} or
 * using the {@link EnvironmentConfig#MAX_OFF_HEAP_MEMORY} property.
 * <h4>Importance of the JE Cache</h4> The JE cache is not an optional cache. It
 * is used to hold the metadata for accessing JE data. In fact the JE cache size
 * is probably the most critical factor to JE performance, since Btree nodes
 * will have to be fetched during a database read or write operation if they are
 * not in cache. During a single read or write operation, at each level of the
 * Btree that a fetch is necessary, an IO may be necessary at a different disk
 * location for each fetch. In addition, if internal nodes (INs) are not in
 * cache, then write operations will cause additional copies of the INs to be
 * written to storage, as modified INs are moved out of the cache to make room
 * for other parts of the Btree during subsequent operations. This additional
 * fetching and writing means that sizing the cache too small to hold the INs
 * will result in lower operation performance.
 * <p>
 * For best performance, all Btree nodes should fit in the JE cache, including
 * leaf nodes (LNs), which hold the record data, and INs, which hold record keys
 * and other metadata. However, because system memory is limited, it is
 * sometimes necessary to size the cache to hold all or at least most INs, but
 * not the LNs. This utility estimates the size necessary to hold only INs, and
 * the size to hold INs and LNs.
 * <p>
 * In addition, a common problem with large caches is that Java GC overhead can
 * become significant. When a Btree node is evicted from the JE main cache based
 * on JE's LRU algorithm, typically the node will have been resident in the JVM
 * heap for an extended period of time, and will be expensive to GC. Therefore,
 * when most or all LNs do <em>not</em> fit in the main cache, using
 * {@link CacheMode#EVICT_LN} can be beneficial to reduce the Java GC cost of
 * collecting the LNs as they are moved out of the main cache. With EVICT_LN,
 * the LNs only reside in the JVM heap for a short period and are cheap to
 * collect. A recommended approach is to size the JE main cache to hold only
 * INs, and size the Java heap to hold that amount plus the amount needed for GC
 * working space and application objects, leaving any additional memory for use
 * by the file system cache or the off-heap cache. Tests show this approach
 * results in lower GC overhead and more predictable latency.
 * <p>
 ...
 * memory. This JVM feature is enabled with the <a href=
 * "http://download.oracle.com/javase/7/docs/technotes/guides/vm/performance-enhancements-7.html#compressedOop">
 * Compressed Oops</a> (<code>-XX:+UseCompressedOops</code>) option, although in
 * modern JVMs it is on by default. Because of this factor, and because Java GC
 * overhead is usually higher with larger heaps, a maximum heap size slightly
 * less than 32GiB is recommended, along with Compressed Oops option.
 * <p>
 ...
 * 20GiB.
 * <p>
 * As of JE 6.4, an optional off-heap cache may be configured in addition to the
 * main JE cache. See {@link EnvironmentConfig#setOffHeapCacheSize} for
 ...
 * <p>
 * To reduce Java GC overhead, sometimes a small main cache is used along with
 * an off-heap cache. Note that it is important that the size the main cache is
 * at least large enough to hold all the upper INs (the INs at level 2 and
 * above). This is because the off-heap cache does not contain upper INs, it
 * only contains LNs and bottom internal nodes (BINs). When a level 2 IN is
 * evicted from the main cache, its children (BINs and LNs) in the off-heap
 * cache, if any, must also be evicted, which can be undesirable, especially if
 * the off-heap cache is not full. This utility displays the main cache size
 * needed to hold all upper INs, and displays a warning if this is smaller than
 * the main cache size specified.
 * <h4>Estimating the JE Cache Size</h4> Estimating JE in-memory sizes is not
 * straightforward for several reasons. There is some fixed overhead for each
 * Btree internal node, so fanout (maximum number of child entries per parent
 * node) and degree of node sparseness impacts memory consumption. In addition,
 * JE uses various compact in-memory representations that depend on key sizes,
 * data sizes, key prefixing, how many child nodes are resident, etc. The
 * physical proximity of node children also allows compaction of child physical
 * address values.
 * <p>
 * Therefore, when running this utility it is important to specify all
 * {@link EnvironmentConfig} and {@link DatabaseConfig} settings that will be
 * used in a production system. The {@link EnvironmentConfig} settings are
 * specified by command line options for each property, using the same names as
 * the {@link EnvironmentConfig} parameter name values. For example,
 * {@link EnvironmentConfig#LOG_FILE_MAX}, which influences the amount of memory
 * used to store physical record addresses, can be specified on the command line
 * as:
 * <p>
 ...
 * <ul>
 * <li>{@code -nodemax ENTRIES} corresponds to
 * {@link DatabaseConfig#setNodeMaxEntries}.</li>
 * <li>{@code -duplicates} corresponds to passing true to
 * {@link DatabaseConfig#setSortedDuplicates}. Note that duplicates are
 * configured for DPL MANY_TO_ONE and MANY_TO_MANY secondary indices.</li>
 * <li>{@code -keyprefix LENGTH} corresponds to passing true
 * {@link DatabaseConfig#setKeyPrefixing}. Note that key prefixing is always
 * used when duplicates are configured.</li>
 * </ul>
 * <p>
 * This utility estimates the JE cache size by creating an in-memory Environment
 * and Database. In addition to the size of the Database, the minimum overhead
 * for the Environment is output. The Environment overhead shown is likely to be
 * smaller than actually needed because it doesn't take into account use of
 * memory by JE daemon threads (cleaner, checkpointer, etc) the memory used for
 * locks that are held by application operations and transactions, the memory
 * for HA network connections, etc. An additional amount should be added to
 ...
 * different configuration parameters or different key and data sizes, run this
 * utility for each database/table and sum the sizes. If you are summing
 ...
 * parameters for this utility. If a representative data set can be created, one
 * approach is to use the {@link DbPrintLog} utility with the {@code -S} option
 * to find the average key and data size for all databases/tables, and use these
 * values as input parameters, as if there were only a single database/tables.
 * With this approach, it is important that the {@code
 * DatabaseConfig} parameters are the same, or at least similar, for all
 * databases/tables.
 * <h4>Key Prefixing and Compaction</h4> Key prefixing deserves special
 * consideration. It can significantly reduce the size of the cache and is
 * generally recommended; however, the benefit can be difficult to predict. Key
 * prefixing, in turn, impacts the benefits of key compaction, and the use of
 * the {@link EnvironmentConfig#TREE_COMPACT_MAX_KEY_LENGTH} parameter.
 * <p>
 ...
 * integers, and the {@link DatabaseConfig#setNodeMaxEntries maximum entries per
 * node} is 128 (the default value) then 6 or 7 of the 8 bytes of the key will
 * have a common prefix in each BIN. Of course, when records are deleted, the
 * number of prefixed bytes may be reduced because the range of key values in a
 * BIN will be larger. For this example we will assume that, on average, 5 bytes
 * in each BIN are a common prefix leaving 3 bytes per key that are unprefixed.
 * <p>
 ...
 * for key compaction (16 bytes). This means that each key will use 16 bytes of
 * memory, in addition to the amount used for the prefix for each BIN. The
 * per-key overhead could be reduced by changing the {@code
 * TREE_COMPACT_MAX_KEY_LENGTH} parameter to a smaller value, but care should be
 * taken to ensure the compaction will be effective as keys are inserted and
 * deleted over time.
 * <p>
 ...
 * argument.
 * <h4>Key Prefixing and Duplicates</h4> When
 * {@link DatabaseConfig#setSortedDuplicates duplicates} are configured for a
 * Database (including DPL MANY_TO_ONE and MANY_TO_MANY secondary indices), key
 * prefixing is always used. This is because the internal key in a duplicates
 * database BIN is formed by concatenating the user-specified key and data. In
 * secondary databases with duplicates configured, the data is the primary key,
 * so the internal key is the concatenation of the secondary key and the primary
 * key.
 * <p>
 * Key prefixing is always used for duplicates databases because prefixing is
 * necessary to store keys efficiently. When the number of duplicates per unique
 * user-specified key is more than the number of entries per BIN, the entire
 * user-specified key will be the common prefix.
 * <p>
 * For example, a database that stores user information may use email address as
 * the primary key and zip code as a secondary key. The secondary index database
 * will be a duplicates database, and the internal key stored in the BINs will
 * be a two part key containing zip code followed by email address. If on
 * average there are more users per zip code than the number of entries in a
 * BIN, then the key prefix will normally be at least as long as the zip code
 * key. If there are less (more than one zip code appears in each BIN), then the
 * prefix will be shorter than the zip code key.
 * <p>
 ...
 * <h4>Small Data Sizes and Embedded LNs</h4> Another special data
 * representation involves small data sizes. When the data size of a record is
 * less than or equal to {@link EnvironmentConfig#TREE_MAX_EMBEDDED_LN} (16
 * bytes, by default), the data is stored (embedded) in the BIN, and the LN is
 * not stored in cache at all. This increases the size needed to hold all INs in
 * cache, but it decreases the size needed to hold the complete data set. If the
 * data size specified when running this utility is less than or equal to
 * TREE_MAX_EMBEDDED_LN, the size displayed for holding INs only will be the
 * same as the size displayed for holdings INs and LNs.
 * <p>
 * See {@link EnvironmentConfig#TREE_MAX_EMBEDDED_LN} for information about the
 * trade-offs in using the embedded LNs feature.
 * <h4>Record Versions and Oracle NoSQL Database</h4> This note applies only to
 * when JE is used with Oracle NoSQL DB. In Oracle NoSQL DB, an internal JE
 * environment configuration parameter is always used:
 * {@code -je.rep.preserveRecordVersion true}. This allows using record versions
 * in operations such as "put if version", "delete if version", etc. This
 * feature performs best when the cache is sized large enough to hold the record
 * versions.
 * <p>
 ...
 * versions (but not the leaf nodes). This is the minimum recommended size when
 * the "... if version" operations are used.
 * <h4>Running the DbCacheSize utility</h4> Usage:
 * 
 * <pre>
 ...
 * are sizing the cache, as cache sizes will vary. You may also need to specify
 * -d32 or -d64 depending on your target, if the default JVM mode is not the
 * same as the mode to be used in production.
 * <p>
 * To take full advantage of JE cache memory, it is strongly recommended that
 * <a href=
 * "http://download.oracle.com/javase/7/docs/technotes/guides/vm/performance-enhancements-7.html#compressedOop">
 * compressed oops</a> (<code>-XX:+UseCompressedOops</code>) is specified when a
 * 64-bit JVM is used and the maximum heap size is less than 32 GB. As described
 * in the referenced documentation, compressed oops is sometimes the default JVM
 * mode even when it is not explicitly specified in the Java command. However,
 * if compressed oops is desired then it <em>must</em> be explicitly specified
 * in the Java command when running DbCacheSize or a JE application. If it is
 * not explicitly specified then JE will not aware of it, even if it is the JVM
 ...
 * <p>
 * The following example adds the use of an off-heap cache, where the main cache
 * size is specified to be 30MB.
 * 
 * <pre>
 * $ java -jar je-X.Y.Z.jar DbCacheSize -records 554719 -key 16 -data 100 \
 ...
 * off-heap LRU.</li>
 * <li>To cache only internal nodes, an off-heap cache is not needed since the
 * internal nodes take around 24MB, which when added to the 5MB overhead is less
 * than the 30MB main cache specified. This is why the number of bytes on the
 * second line is zero.</li>
 * <li>To cache all nodes, the main cache size specified should be used (25MB
 * added to the 5MB overhead is 30MB), and an off-heap cache of around 71MB
 * should be configured.</li>
 * </ul>
 ...
 * <ul>
 * <li>The following properties are always output (except allNodes, see below).
 * They describe the estimated size of the main cache.
 * <ul>
 * <li><strong>overhead</strong>: The environment overhead, as shown under
 * Environment Cache Overhead above.</li>
 * <li><strong>internalNodes</strong>: The Btree size in the main cache for
 * holding the internal nodes. This is the "Internal nodes only
 * " line above (followed by "MAIN cache" when {@code -offheap} is specified).
 * </li>
 * <li><strong>internalNodesAndVersions</strong>: The Btree size needed to hold
 * the internal nodes and record versions in the main cache. This value is zero
 * when {@code -offheap} is specified; currently JE does not cache record
 * versions off-heap unless their associated LNs are also cached off-heap, so
 * there is no way to calculate this property.</li>
 * <li><strong>allNodes</strong>: The Btree size in the main cache needed to
 * hold all nodes. This is the "Internal nodes and leaf nodes
 * " line above (followed by "MAIN cache" when {@code -offheap} is specified).
 * This property is not output unless {@code -data} is specified.</li>
 * </ul>
 ...
 * <ul>
 * <li><strong>minMainCache</strong>: The minimum size of the main cache needed
 * to hold all upper INs. When the {@code -maincache} value specified is less
 * than this minimum, not all internal nodes can be cached. See the discussion
 * further above.</li>
 * <li><strong>offHeapInternalNodes</strong>: The size of the off-heap cache
 * needed to hold the internal nodes. This is the "Internal nodes only: OFF_HEAP
 * cache" line above.</li>
 * <li><strong>offHeapAllNodes</strong>: The size of the off-heap cache needed
 * to hold all nodes. This is the "Internal nodes and leaf nodes: OFF_HEAP
 * cache" line above. This property is not output unless {@code -data} is
 * specified.</li>
 * </ul>
 * <li>The following properties are deprecated but are output for compatibility
 * with earlier releases.
 * <ul>
 * <li>minInternalNodes, maxInternalNodes, minAllNodes, and (when {@code -data}
 * is specified) maxAllNodes</li>
 * </ul>
 * </ul>
 *
...
public class DbCacheSize {
     ...
     * actual cache sizes during testing. [-measure] # Causes main program to
     * write a database to find # the actual cache size; default: do not
     * measure; # without -data, measures internal nodes only Only use -measure
     * without -orderedinsertion when record count is 100k or less, to avoid
     * endless attempts to find an unused key value via random number
     * generation. Also note that measured amounts will be slightly less than
     * calculated amounts because the number of prefix bytes is larger for
     * smaller key values, which are sequential integers from zero to max
     * records minus one.
     */
    ...
    /**
     * Runs DbCacheSize as a command line utility. For command usage, see
     * {@link DbCacheSize class description}.
     */
        ...
        try {
            ...
            if (offHeapCache) {
                ...
                if (mainCacheSize > 0 && mainCacheSize - envOverhead <= 1024 * 1024) {
                    ...
                    throw new IllegalArgumentException("The -maincache value must be at least 1 MiB larger"
                            + " than the environment overhead (" + INT_FORMAT.format(envOverhead) + ')');
                }
            }
            ...
            /*
             * Do not propagate exception thrown by Environment.close if another
    ...
    private long calcLevel2AndAboveSize() {
        ...
        return ((nUinNodes - nLevel2Nodes) * uinWithTargets) + (nLevel2Nodes * (uinNoTargets + uinOffHeapBINIds));
    }
    ...
    private void calcOffHeapNoLNsOrVLSNs() {
         ...
         */
        final long mainSpare = (mainDataSize > calcLevel2AndAboveSize()) ? (mainDataSize - calcLevel2AndAboveSize())
                : 0;
    ...
    private void calcOffHeapWithLNsAndVLSNs() {
        ...
        /*
         * If LNs are not stored separately (they are embedded or duplicates are
         * configured), then only internal nodes are relevant.
         */
        if (binOffHeapLNs == 0) {
         ...
         * LNs and how many off-heap LNs. The number that have main LNs is the
         * amount of main cache to spare (if all BINs had off-heap LNs) divided
         * by the added size required to hold the LNs in one BIN.
         */
        final long mainWithOffHeapLNIds = mainNoLNsOrVLSNs + (nBinNodes * binOffHeapLNIds);

        if (mainWithOffHeapLNIds <= mainDataSize) {

            final long mainSpare = (mainDataSize > mainNoLNsOrVLSNs) ? (mainDataSize - mainNoLNsOrVLSNs) : 0;
    ...
    private void calcTreeSizes(final Environment env) {
        ...
        if (nodeMaxEntries != binMaxEntries) {
            throw new IllegalArgumentException("-binmax not currently supported because a per-BIN max is"
        ...
        for (int i = 0; i < nodeAvg; i += 1) {
            ...
            final OperationResult result = db.put(null, keyEntry, dataEntry,
                    duplicates ? Put.NO_DUP_DATA : Put.NO_OVERWRITE, options);
        ...
        for (int i = 1; i < nodeAvg; i += 1) {
            ...
            final int result = in.insertEntry1(bin, bin.getKey(i), null, bin.getLsn(i), false/* blindInsertion */);
    ...
    private void setKeyData(final byte[] keyBytes, final int keyOffset, final DatabaseEntry keyEntry,
        ...
        if (keyPrefix + keyBytes.length > fullKey.length) {
            throw new IllegalArgumentException("Key doesn't fit, allowedLen=" + fullKey.length + " keyLen="
                    + keyBytes.length + " prefixLen=" + keyPrefix);
        }
    ...
    void printCacheSizes(final PrintStream out) {
        ...
        if (dataSize >= 0) {
            ...
            if (mainNoLNsOrVLSNs == mainWithLNsAndVLSNs && offHeapNoLNsOrVLSNs == offHeapWithLNsAndVLSNs) {
        ...
        if (offHeapCache && mainMinDataSize > mainDataSize) {
            out.println("\nWARNING: The information above applies to a data set of " + INT_FORMAT.format(records)
        ...
        if (btreeInfo) {
            ...
            if (offHeapCache) {
                ...
                out.println(line(nBinNodes - nMainBINsNoLNsOrVLSNs, "Internal nodes only, BINs" + offHeapSuffix));
                out.println(line(nMainBINsWithLNsAndVLSNs, "Internal nodes and leaf nodes, BINs" + mainSuffix));
                out.println(line(nBinNodes - nMainBINsWithLNsAndVLSNs,
                        "Internal nodes and leaf nodes, BINs" + offHeapSuffix));
                out.println(line(nMainLNsWithLNsAndVLSNs, "Internal nodes and leaf nodes, LNs" + mainSuffix));
                out.println(
                        line(records - nMainLNsWithLNsAndVLSNs, "Internal nodes and leaf nodes, LNs" + offHeapSuffix));
            }
        }
    ...
    void measure(final PrintStream out) {
        ...
        try {
            ...
            if (out != null) {
                out.println("Measuring with maximum cache size: " + INT_FORMAT.format(env.getConfig().getCacheSize())
                        + " and (for off-heap) main data size: " + INT_FORMAT.format(mainDataSize));
            }
            ...
            preloadMainNoLNsOrVLSNs = getStats(out, env, "Internal nodes only after preload (" + status + ")");
            ...
            if (!offHeapCache) {
                ...
                preloadMainNoLNsWithVLSNs = getStats(out, env,
                        "Internal nodes plus VLSNs after preload (" + status + ")");
            }
    ...
    private Environment openMeasureEnvironment(final boolean createNew, final boolean setMainSize) {
        ...
        if (setMainSize) {
             ...
             * sized log buffers so we can compare totals with the case where we
             * don't set the cache size.
             */
    ...
    private Environment openEnvironment(final EnvironmentConfig config, final boolean createNew) {
        ...
        if (replicated) {
            ...
            if (!repParams.isEmpty()) {
                throw new IllegalArgumentException(
                        "Cannot set replication params in a standalone " + "environment.  May add -replicated.");
            }
         ...
         * Since a tiny data set is use for estimating, and a small data set for
         * testing, we disable the compact representation when it is unlikely to
         * be effective.
         */
        final long fileSize = Integer.parseInt(newEnv.getConfig().getConfigParam(EnvironmentConfig.LOG_FILE_MAX));
    ...
    /**
     * Inserts records and ensures that no eviction occurs. LNs (and VLSNs) are
     * left intact.
     */
    private void insertRecords(final PrintStream out, final Environment env, final Database db) {
        ...
        if (keyPrefix == 0) {
             ...
             */
            final int nodeAvg = orderedInsertion ? nodeMaxEntries : ((nodeMaxEntries * DEFAULT_DENSITY) / 100);
            final int prevKey = lastKey - (nodeAvg * 2);
            final byte[] prevKeyBytes = padLeft(BigInteger.valueOf(prevKey).toByteArray(), maxKeyBytes);
            int calcPrefix = 0;
            while (calcPrefix < lastKeyBytes.length && calcPrefix < prevKeyBytes.length
     ...
     */
    private void readRecords(final PrintStream out, final Environment env, final Database db, final boolean readData) {
    ...
    private void checkForEviction(Environment env, int recNum) {
        ...
        if (stats.getOffHeapNodesTargeted() > 0) {
            ...
            throw new IllegalStateException(
                    "*** Ran out of off-heap cache at record " + recNum + " -- try increasing off-heap cache size ***");
        }
        if (stats.getNNodesTargeted() > 0) {
            getStats(System.out, env, "Out of main cache");
            throw new IllegalStateException(
                    "*** Ran out of main cache at record " + recNum + " -- try increasing Java heap size ***");
        }
    }
    ...
    /**
     * Pads the given array with zeros on the left, and returns an array of the
     * given size.
     */
     ...
     */
    private PreloadStatus preloadRecords(final PrintStream out, final Database db, final boolean loadLNs) {
        Thread thread = null;
        if (out != null) {
            thread = new Thread() {
                @Override
         ...
         * a little by design. We evict here to bring it down below the maximum,
         * and clear the stats so that the getStats method in this class doesn't
         * complain about the eviction later on.
         */
    ...
    private long getStats(final PrintStream out, final Environment env, final String msg) {
        ...
        if (out != null) {
            out.println("MainCache= " + INT_FORMAT.format(stats.getCacheTotalBytes()) + " Data= "
                    + INT_FORMAT.format(dataSize) + " BINs= " + INT_FORMAT.format(stats.getNCachedBINs()) + " UINs= "
                    + INT_FORMAT.format(stats.getNCachedUpperINs()) + " CacheMiss= "
                    + INT_FORMAT.format(stats.getNCacheMiss()) + " OffHeapCache= "
                    + INT_FORMAT.format(stats.getOffHeapTotalBytes()) + " OhLNs= "
                    + INT_FORMAT.format(stats.getOffHeapCachedLNs()) + " OhBIN= "
                    + INT_FORMAT.format(stats.getOffHeapCachedBINs()) + " OhBINDeltas= "
