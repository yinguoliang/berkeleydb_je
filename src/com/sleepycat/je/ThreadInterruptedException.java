 ...
 * a thread that is executing a JE operation, the state of the environment will
 * be undefined. That's because JE might have been in the middle of I/O activity
 ...
 * close the environment and re-open it before using it again. This is
 ...
 * {@code keepRunning} variable of some kind in each thread. Check this variable
 * in your threads, and return from the thread when it is false. Set it to false
 * when you want to stop the thread. If this thread is waiting to be woken up to
 ...
 * {@code ThreadInterruptedException} should be expected. Note that by shutting
 * down the environment abnormally, recovery time will be longer when the
