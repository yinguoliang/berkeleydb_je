 ...
 * {@link Environment#setMutableConfig} and is returned by
 * {@link Environment#getMutableConfig}.
 *
 * <p>
 * There are two types of mutable environment properties: per-environment handle
 * properties, and environment wide properties.
 * </p>
 *
 ...
 * instance. For example, to change the default transaction commit behavior for
 * a single environment handle, do this:
 * </p>
 *
 * <blockquote>
 * 
 ...
 *
 * <p>
 * The per-environment handle properties are listed below. These properties are
 * accessed using the setter and getter methods listed, as shown in the example
 * above.
 * </p>
 *
 * <ul>
 ...
 * <li>{@link #setTxnWriteNoSync}, {@link #getTxnWriteNoSync}
 * <em>deprecated</em></li>
 * </ul>
 *
 ...
 *
 * <p>
 * The environment-wide mutable properties are documented as such for each
 * EnvironmentConfig String constant.
 * </p>
 *
...
public class EnvironmentMutableConfig implements Cloneable, Serializable {
	 ...
	 * application and never used internally; internal code directly checks with
	...
	EnvironmentMutableConfig(Properties properties) throws IllegalArgumentException {
		...
		DbConfigManager.validateProperties(properties, false, // isRepConfigInstance
				getClass().getName());
	 ...
	 *            transaction commit. This means that transactions exhibit the
	 *            ACI (Atomicity, Consistency, and Isolation) properties, but
	 *            not D (Durability); that is, database integrity is maintained,
	 *            but if the JVM or operating system fails, it is possible some
	 *            number of the most recently committed transactions may be
	 *            undone during recovery. The number of transactions at risk is
	 *            governed by how many updates fit into a log buffer, how often
	 *            the operating system flushes dirty buffers to disk, and how
	 ...
	 *            ACI (Atomicity, Consistency, and Isolation) properties, but
	 *            not D (Durability); that is, database integrity is maintained,
	 *            but if the operating system fails, it is possible some number
	 *            of the most recently committed transactions may be undone
	 *            during recovery. The number of transactions at risk is
	 *            governed by how often the operating system flushes dirty
	 *            buffers to disk, and how often the database environment is
	 *            checkpointed.
	 *
	 *            <p>
	 *            The motivation for this attribute is to provide a transaction
	 *            that has more durability than asynchronous (nosync)
	 *            transactions, but has higher performance than synchronous
	 *            transactions.
	 *            </p>
	 *
	 *            <p>
	...
	public void setDurabilityVoid(Durability durability) {
		...
		if (durability == null) {
			...
			DbConfigManager.setVal(props, EnvironmentParams.JE_DURABILITY, durability.toString(), validateParams);
		}
	}
	...
	public void setCacheSizeVoid(long totalBytes) throws IllegalArgumentException {
		...
		DbConfigManager.setVal(props, EnvironmentParams.MAX_MEMORY, Long.toString(totalBytes), validateParams);
	}
	...
	public void setOffHeapCacheSizeVoid(long totalBytes) throws IllegalArgumentException {
		...
		DbConfigManager.setVal(props, EnvironmentParams.MAX_OFF_HEAP_MEMORY, Long.toString(totalBytes), validateParams);
	}
	 ...
	 * exceptions may repeat, the application should also choose how to handle a
	 * spate of exceptions. For example, the application may choose to act upon
	 * each notification, or it may choose to batch up its responses by
	 * implementing the listener so it stores exceptions, and only acts when a
	 * certain number have been received.
	 * 
	 * @param exceptionListener
	 *            the callback to be executed when an exception occurs.
	 *
	 * @return this
	 */
	 ...
	 *            in this environment. If {@code null} is specified,
	 *            {@link CacheMode#DEFAULT} will be used.
	 *
	 ...
	 *
	 * @param paramName
	 *            the configuration parameter name, one of the String constants
	 *            in this class
	 *
	 * @param value
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if the paramName or value is invalid.
	 */
	 ...
	 *
	 * @param paramName
	 *            a valid configuration parameter, one of the String constants
	 *            in this class.
	 * @return the configuration value.
	 * @throws IllegalArgumentException
	 *             if the paramName is invalid.
	 */
	 ...
	 * 
	 * @param handleConfigProps
	 *            are the config property values that were specified by
	 ...
	 *         properties, used by this class and EnvironmentConfig.
	 */
	@Override
	...
	/**
	 * Copies all mutable props to the given config object. Unchecked suppress
	 * here because Properties don't play well with generics in Java 1.5
	 */
	...
	/**
	 * Removes all immutable props. Unchecked suppress here because Properties
	...
	public String toString() {
		return " cacheSize=" + cacheSize + " offHeapCacheSize=" + offHeapCacheSize + " cacheMode=" + cacheMode
				+ " txnNoSync=" + txnNoSync + " txnWriteNoSync=" + txnWriteNoSync + " exceptionListener="
				+ (exceptionListener != null) + " map=" + props.toString();
	}
}
