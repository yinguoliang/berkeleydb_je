 ...
 *
 * <p>
 * A single Transaction may be used to protect operations for any number of
 * Databases in a given environment. However, a single Transaction may not be
 * used for operations in more than one distinct environment.
 * </p>
 *
 * <p>
 ...
 * Transaction.abort} or {@link Transaction#commit Transaction.commit} method is
 * called, the handle may not be accessed again, regardless of the success or
 * failure of the method, with one exception: the {@code abort} method may be
 * called any number of times to simplify error handling.
 * </p>
 *
 * <p>
...
public class Transaction {
	...
	public enum State {
		 ...
		 * for performing operations. This state is also indicated if
		 * {@link #isValid} returns true. For all other states, {@link #isValid}
		 * will return false.
		 */
		OPEN,
		 ...
		 * <p>
		 * This is an unusual situation and is normally due to a system failure,
		 * storage device failure, disk full condition, thread interrupt, or a
		 * bug of some kind. When a transaction is in this state, the
		 * Environment will have been {@link Environment#isValid() invalidated}
		 * by the error.
		 * <p>
		 ...
		 * the transaction is durable or not. Note that in the event of a system
		 * level failure, the reads themselves may be unreliable, e.g. the data
		 * may be in the file system cache but not on disk. Other applications
		 * may wish to repeat the transaction unconditionally, after resolving
		 * the error condition, particularly when the set of operations in the
		 * transaction is designed to be idempotent.
		 */
		POSSIBLY_COMMITTED,
		 ...
		 * is thrown by the {@code commit} method. For example, in a replicated
		 * environment, an
		 * {@link com.sleepycat.je.rep.InsufficientAcksException} may be thrown
		 * after the transaction is committed locally.
		 */
		COMMITTED,

		/**
		 * The transaction has been invalidated by an exception and cannot be
		 * committed. See {@link OperationFailureException} for a description of
		 * how a transaction can become invalid. The application is responsible
	...
	protected Transaction(Environment env, Txn txn) {
		...
		/*
		 * Copy the id to this wrapper object so the id will be available after
		 * the transaction is closed and the txn field is nulled.
		 */
		this.id = txn.getId();
	}
	 ...
	 * {@link Transaction} handle may not be accessed again, with one exception:
	 * the {@code abort} method itself may be called any number of times to
	 ...
	 *             if the environment has been closed, or cursors associated
	 *             with the transaction are still open.
	 */
	 ...
	 * environment. This method should only be called after the transaction has
	 * finished.
	 * <p>
	 ...
	 *
	 * @throws IllegalStateException
	 *             if the method is called before the transaction has committed
	 *             or aborted.
	 *
	 ...
	 * commit, the transaction will be committed synchronously to stable storage
	 * before the call returns. This means the transaction will exhibit all of
	 ...
	 * database integrity will be maintained, but it is possible this
	 ...
	 *
	 * <p>
	 * If the method encounters an error, the transaction <!-- and all child
	 ...
	 * After this method has been called, regardless of its return, the
	 * {@link Transaction} handle may not be accessed again, with one exception:
	 * the {@code abort} method may be called any number of times to simplify
	 * error handling.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @throws com.sleepycat.je.rep.InsufficientAcksException
	 *             if the master in a replicated environment did not receive
	 *             enough replica acknowledgments, although the commit succeeded
	 *             locally.
	 *
	 * @throws com.sleepycat.je.rep.ReplicaWriteException
	 *             if a write operation was performed with this transaction, but
	 *             this node is now a Replica.
	 *
	 ...
	 *
	 * @throws IllegalStateException
	 *             if the transaction or environment has been closed, or cursors
	 ...
	 * requirement overrides any default durability requirements associated with
	 * the environment. If the durability requirements cannot be satisfied, an
	 * exception is thrown to describe the problem. Please see
	 ...
	 *
	 * @throws com.sleepycat.je.rep.InsufficientReplicasException
	 *             if the master in a replicated environment could not contact
	 *             enough replicas to initiate the commit.
	 *
	 * @throws com.sleepycat.je.rep.InsufficientAcksException
	 *             if the master in a replicated environment did not receive
	 *             enough replica acknowledgments, althought the commit
	 *             succeeded locally.
	 *
	 * @throws com.sleepycat.je.rep.ReplicaWriteException
	 *             if a write operation was performed with this transaction, but
	 ...
	 * {@link com.sleepycat.je.EnvironmentConfig#setTxnNoSync
	 * EnvironmentConfig.setTxnNoSync} method. This behavior may also be set for
	 * a single transaction using the
	 * {@link com.sleepycat.je.Environment#beginTransaction
	 * Environment.beginTransaction} method. Any value specified to this method
	 ...
	 *
	 * @param durability
	 *            the durability to use for the commit
	 * @param explicitSync
	 *            true if the method was invoked from one of the sync-specific
	 *            APIs, false if durability was used explicitly. This parameter
	 *            exists solely to support mixed mode api usage checks.
	 *
	 * @throws IllegalArgumentException
	 *             via commit(Durability)
	 */
	 ...
	 *
	 * <p>
	 * If {@link #setTxnTimeout(long,TimeUnit)} has not been called to configure
	 * the timeout, the environment configuration value (
	 * {@link EnvironmentConfig#TXN_TIMEOUT} )is returned.
	 * </p>
	 *
	 * @param unit
	 ...
	 *
	 * @throws IllegalStateException
	 *             if the transaction or environment has been closed.
	 *
	 * @throws IllegalArgumentException
	 *             if the unit is null.
	 *
	 ...
	 *
	 * @param timeOut
	 *            The timeout value for the transaction lifetime, or zero to
	 ...
	 *
	 * <p>
	 * If {@link #setLockTimeout(long,TimeUnit)} has not been called to
	 * configure the timeout, the environment configuration value (
	 * {@link EnvironmentConfig#LOCK_TIMEOUT}) is returned.
	 * </p>
	 *
	 * @param unit
	 *            the {@code TimeUnit} of the returned value. May not be null.
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             via all API methods with a txn param
	 */
	Locker getLocker() throws DatabaseException {

		if (txn == null) {
			throw new IllegalArgumentException("Transaction " + id + " has been closed and is no longer usable.");
		}
		return txn;
	}
	...
	/**
	 * @throws EnvironmentFailureException
	 *             if the underlying environment is invalid, via all methods.
	 *
	 * @throws IllegalStateException
	 *             via all methods.
	 */
	...
	/**
	 * Returns whether this {@code Transaction} is open, which is equivalent to
	 * when {@link Transaction#getState} returns {@link Transaction.State#OPEN}.
	 * See {@link Transaction.State#OPEN} for more information.
	 *
	 * <p>
	 * When an {@link OperationFailureException}, or one of its subclasses, is
	 * caught, the {@code isValid} method may be called to determine whether the
	 * {@code Transaction} can continue to be used, or should be aborted.
	 * </p>
	 */
