 ...
 * Environment}.
 * <p>
 * The caller supplies the contents of the log file by passing arrays of bytes
 * in a series of calls to the {@link #verify} method, which verifies the
 ...
 * file. The primary intended use of this class is to verify the contents of log
 * files that are being copied as part of a programmatic backup. It is critical
 * that invalid files are not added to a backup set, since then both the live
 * environment and the backup will be invalid.
 *
...
public class LogVerifier {
     ...
     * LogVerificationException}. This should be a simple file name of the form
     *            {@code NNNNNNNN.jdb}, where NNNNNNNN is the file number in
     *            hexadecimal format.
    ...
    public LogVerifier(final EnvironmentImpl envImpl, final String fileName, final long fileNum) {
         ...
         */
        headerBuf = ByteBuffer.allocate(Math.max(LogEntryHeader.MAX_HEADER_SIZE, FileHeader.entrySize()));
     ...
     *             environment-wide failure occurs
     */
    public void verify(final byte[] buf, final int off, final int len) throws LogVerificationException {
    ...
    private int processFixedHeader(final byte[] buf, final int curOffset, final int remaining)
        ...
        if (headerBuf.position() == maxSize) {
            ...
            if (header.getPrevOffset() != prevEntryStart) {
                throw newVerifyException("Header prevOffset=0x" + Long.toHexString(header.getPrevOffset())
                        + " but prevEntryStart=0x" + Long.toHexString(prevEntryStart));
            }
            ...
            validator.update(headerBuf.array(), LogEntryHeader.CHECKSUM_BYTES, maxSize - LogEntryHeader.CHECKSUM_BYTES);
    ...
    private int processItem(final byte[] buf, final int curOffset, final int remaining)
        ...
        if (itemPosition == maxSize) {
            try {
                 ...
                 * lastEntryType.toStringNoVersion() + " log version: " +
                 * logVersion); System.out.println();
                 */
                throw newVerifyException(e);
            }
    ...
    private LogVerificationException newVerifyException(String reason, Throwable cause) {
        ...
        return new LogVerificationException("Log is invalid, fileName: " + fileName + " fileNumber: 0x"
                + Long.toHexString(fileNum) + " logEntryOffset: 0x" + Long.toHexString(entryStart) + " logEntrySize: "
                + logEntrySize + " verifyState: " + state + " reason: " + reason, cause);
    }
}
