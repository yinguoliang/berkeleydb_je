...
/**
 * A JE stat that maintains a map of individual {@link LongAvgRate} values which
...
public final class LongAvgRateMapStat extends MapStat<Long, LongAvgRate> {
    ...
    /**
     * The time the last stat was removed. This value is used to determine which
     * entries should be included when calling computeInterval. Synchronize on
    ...
    public synchronized LongAvgRate createStat(String key) {
        assert key != null;
        final LongAvgRate stat = new LongAvgRate(definition.getName() + ":" + key, periodMillis, reportTimeUnit);
     ...
     * updated with any values from both maps. Treats this map as newest if both
     * have the same timestamp. This method does not compute negative intervals,
     * since negation does not work properly for this non-additive stat. The
     * base argument must be a LongAvgRateMapStat.
     */
    @Override
     ...
     * latest changes, removing entries not in the argument, and adding ones not
     * in this instance.
     */
    private synchronized LongAvgRateMapStat updateLatest(final LongAvgRateMapStat latest) {

        synchronized (latest) {
            for (final Iterator<Entry<String, LongAvgRate>> i = statMap.entrySet().iterator(); i.hasNext();) {
