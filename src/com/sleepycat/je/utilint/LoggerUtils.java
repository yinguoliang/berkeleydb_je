...
/**
 * <b>Logging Architecture</b> =========================== JE uses the
 * java.util.logging package. The ability to dynamically specify logging levels
 * per component is important functionality for the system. Logging output is
 * directed to the console, to the je.info files, and in special cases, to a
 * MemoryHandler. The latter is meant for debugging and field support. Logging
 * output from multiple environments may end up going to the same handler,
 * either because a single process is executing multiple environments, or
 * because the output of multiple environments, such as a replication group, is
 * combined in a single display. Because of that, it's important for logging
 * output to be prefixed with an environment id so it can be distinguished by
 * environment. Loggers managed by java.util.logging.LogManager are supposed to
 * be maintained with a weak reference by the LogManager. In our experience,
 * loggers do not seem to be released, and seem to accumulate in memory. Because
 * of that, we create a single logger per JE class, rather than a logger per
 * class instance. The latter would be more convenient, because we wish to use
 * environment specific information, such as the environment name as a prefix,
 * or the location of the je.info file, when creating output. Restricting
 * ourselves to a single per-class logger requires that we keep the logger and
 * its associated handlers and formatters stateless, because the logger may be
 ...
 * per-thread map to store state information needed by the logger. This state
 * information is: - the environment impl from the envMap(from which one can
 * obtain the prefix and the console, file and memory handlers to use) - or if
 * the environment impl is null because the component executes without an
 * environment, the output will go to only a console handler. It will use a
 * particular formatter to prefix the output with a useful id. This is obtained
 * from the formatter map. With this scheme, a JE process has a maximum of - N
 * loggers, where N is the number of classes which get loggers - 3 handlers *
 * number of environments, because each environment creates a Console, File and
 * Memory handler. <b>How To Use Logging in a JE Class</b>
 * ======================================= Creating a Logger: There are three
 * kinds of loggers that a class may chose to use. 1. A class with a reference
 * to EnvironmentImpl or RepImpl should use LoggerUtils.getLogger(Class<?>) to
 * create a logger which prefixes its output with an environment id. When a
 * logger is obtained this way, the logger should not be used directly. Instead,
 * LoggerUtils provides several methods like this: LoggerUtils.severe() equals
 * to logger.severe LoggerUtils.warning() equals to logger.warning etc
 * LoggerUtils.logMsg(Logger, EnvironmentImpl, Level, String) equals to
 * logger.log(Level, String) 2. A class without an EnvironmentImpl which still
 * has some kind of custom information to prepend to the logging output should
 * use LoggerUtils.getFormatterNeeded(). For example,
 ...
 * use: LoggerUtils.logMsg(Logger, Formatter, Level, String) where the formatter
 * is the one created by the using class. 3. A logger without an EnvironmentImpl
 * does not prefix or customize the logging output, and uses
 * LoggerUtils.getLoggerFixedPrefix to create a logger. In this case, use the
 * usual java.util.logging.Logger logging methods. Note: there are some JE
 * classes which only conditionally reference an environment. In that case, the
 * environment must also conditionally create a logger, and then use the wrapper
 * methods which use both an environmentImpl and a formatter. For example: if
 * (envImpl != null) { logger = LoggerUtils.getLogger(getClass()); } else {
 * logger = LoggerUtils.getLoggerFormatterNeeded(); } formatter = new
 * Formatter(.....); Then use LoggerUtils.logMsg(Logger, EnvironmentImpl,
 * Formatter, Level, String) instead of Logger.log(Level, String)
 */
public class LoggerUtils {

    /*
     * Environment state to be used by a logger. Must be set and released per
     * logger call.
     */
    static final Map<Thread, EnvironmentImpl> envMap              = new ConcurrentHashMap<Thread, EnvironmentImpl>();

    /*
     * Formatter state to be used by a logger. Must be set and released per
     * logger call. Used by logging calls that do not have an available
     * environment.
     */
     ...
     * environment identifier. Use this for classes which have a reference to an
     * EnvironmentImpl (or RepImpl). When a logger is obtained this way, the
     * logger should not be used directly. Instead, the wrapper methods in
     * LoggerUtils which put and remove the environment from the envMap must be
     * used, so that the logging output can be properly prefixed and redirected
    ...
    public static Logger getLogger(Class<?> cl) {
        ...
        /*
         * [#18277] Add null check of logger.getHandlers() because the Resin app
         * server's implementation of logging can return null instead of an
         * empty array.
         */
        ...
        if (handlers != null) {
            for (Handler h : handlers) {
                 ...
                 * do not have a custom prefix use the ConsoleHandler directly.
                 * Having ConsoleRedirectHandler extend ConsoleHandler lets us
                 * have a model where the user only have to set
                 * com.sleepycat.je.util.ConsoleHandler in their logging
                 * properties file.
                 */
     ...
     * output is prefixed in a custom way, using the formatter map to access the
     * proper state. This should be used by a class that does not have an
     * EnvironmentImpl, but still wishes to prepend some kind of custom prefix
     * to the logging output. When a logger is obtained this way, the logger
     * should not be used directly. Instead, the wrapper methods in LoggerUtils
     * which use a Formatter parameter, and put and remove the environment from
     * the formatterMap must be used, so that the logging output can be properly
    ...
    private static Logger createLogger(String className) {
         ...
         * handlers. This was not useful because of the need to specify handlers
         * per environment, and also caused a process monitor echo issue within
         * NoSQL DB.
         */
     ...
     * use.
     */
    public static void logMsg(Logger useLogger, EnvironmentImpl envImpl, Formatter formatter, Level logLevel,
                              String msg) {
        if (envImpl != null) {
     ...
     */
    public static void traceAndLogException(EnvironmentImpl envImpl, String sourceClass, String sourceMethod,
    ...
    /**
     * Records a message both to the java.util.logging loggers and through the
     * trace system which writes to the .jdb files. The logLevel parameter only
     * applies to the java.util.logging system. Trace messages are
     * unconditionally written to the .jdb files. Because of that, this method
     * should be used sparingly, for critical messages.
     */
    ...
    public static Level getHandlerLevel(DbConfigManager configManager, ConfigParam param, String levelName) {
        ...
        /*
         * If the params are not set, and levels are set in the properties file,
         * then set the level from properties file.
         */
    ...
    /**
     * Logs a full thread dump as if jstack were piped to the je.info file. Only
     * one dump per EnvironmentImpl lifetime is allowed. Allowing multiple dumps
     * can causes them to be interleaved, and risks filling the je.info files
     * with repeated dumps. The envImpl should be invalidated when this method
     * is called, so one dump should be enough.
     */
