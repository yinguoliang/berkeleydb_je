 ...
 * supported. TODO: We no longer support Java 6 so much of this mechanism can be
 * removed. Other code can assume that a FileStore is always available. Also, if
 * an IOException is thrown by FileStore methods, instead of tolerating this we
 * should invalidate the Environment, since this is standard JE policy. However,
 * we may want to leave the factory interface in place to support testing with
...
public abstract class FileStoreInfo {
    ...
    interface Factory {
        ...
        /** @see #getInfo */
        abstract FileStoreInfo factoryGetInfo(String file) throws IOException;
    }
    ...
    private static Factory createFactory() {
        try {
            ...
            return new FailingFactory(
                    new UnsupportedOperationException("FileStoreInfo is only supported for Java 7 and later"));
        }
        try {
            return Class.forName(JAVA7_FILE_STORE_FACTORY_CLASS).asSubclass(Factory.class).newInstance();
        } catch (Exception e) {
            return new FailingFactory(new IllegalStateException(
     ...
     * about file stores. Returns normally if called on a Java 7 runtime or
     * later, otherwise throws {@link UnsupportedOperationException}.
     *
     ...
     * the file store associated with the specified file. Throws
     * {@link UnsupportedOperationException} if called on a Java runtime prior
     * to Java 7. Equal objects will be returned for all files associated with
     * the same file store.
     *
     * @param file the file
     ...
     *             attempting to use reflection to access the underlying
     *             implementation
     * @throws IOException if an I/O error occurs
     */
    public static final FileStoreInfo getInfo(final String file) throws IOException {
