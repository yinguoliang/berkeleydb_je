 ...
 * performed using C(2) through C(n), in that order. The Btree lookups are exact
 * searches to determine whether the candidate record also contains secondary
 * keys K(2) through K(n). The lookups are equivalent to a
 * {@link Cursor#getSearchBoth Cursor.getSearchBoth} operation on the secondary
 * index. The primary record itself is not retrieved and the primary database is
 * not accessed.</li>
 * <li>If any lookup in step 2 fails, the algorithm advances to the next
 * candidate record using C(1). Lookups are performed in the order of the cursor
 * array, and the algorithm proceeds to the next C(1) candidate key as soon as a
 * single lookup fails.</li>
 * <li>If all lookups in step 2 succeed, then the matching key and/or data is
 * returned by the {@code getNext} method. If the
 * {@link #getNext(DatabaseEntry,DatabaseEntry,LockMode)} method signature is
 * used, then the primary database is read to obtain the record data, as if
 * {@link Cursor#getSearchKey Cursor.getSearchKey} were called for the primary
 * database. If the {@link #getNext(DatabaseEntry,LockMode)} method signature is
 * used, then only the primary key is returned and the primary database is not
 * accessed.</li>
 * <li>The algorithm ends when C(1) has no more candidate records with K(1), and
 * the {@code getNext} method will then return
 * {@link com.sleepycat.je.OperationStatus#NOTFOUND OperationStatus.NOTFOUND}.
 * </li>
 * </ol>
 */
public class JoinCursor implements ForwardCursor, Closeable {
    ...
    JoinCursor(final Database primaryDb, final Cursor[] cursors, final JoinConfig configParam)
        ...
        if (!config.getNoSort()) {
             ...
             * so that countEstimate is called only once per cursor.
             */
     ...
     * secondary cursors passed to {@link Database#join Database.join}.
     * </p>
     * <p>
     ...
     * specified via the {@code CursorConfig} or {@code LockMode} parameters.
     * </p>
     *
     ...
     * </p>
     */
    public OperationStatus getNext(final DatabaseEntry key, final DatabaseEntry data, LockMode lockMode) {
     ...
     * implementation.
     */
    private OperationResult retrieveNext(final DatabaseEntry keyParam, final DatabaseEntry dataParam,
        ...
        outerLoop: while (true) {
            ...
            if (!cursorFetchedFirst[0]) {
                ...
                result = secCursor.retrieveNext(firstSecKey, candidateKey, lockMode, cacheMode, GetMode.NEXT_DUP);
            }
            if (result == null) {
            ...
            for (int i = 1; i < secCursors.length; i += 1) {
                ...
                if (!cursorFetchedFirst[i]) {
                    ...
                    if (readUncommitted && result == null) {
                        ...
                        result = secCursor.retrieveNext(secKey, scratchEntry, lockMode, cacheMode, GetMode.NEXT_DUP);
                        if (result == null) {
                ...
                result = secCursor.search(scratchEntry, candidateKey, lockMode, cacheMode, SearchMode.BOTH, true);
                if (result == null) {
            ...
            if (dataParam != null) {
                if (!secCursors[0].readPrimaryAfterGet(priDb, firstSecKey, candidateKey, dataParam, lockMode,
