 ...
 * is thrown if the value is not valid for that attribute.
 * </p>
 * <p>
 * Most parameters are described by the parameter name String constants in this
 * class. These parameters can be specified or individually by calling
 * {@link #setConfigParam}, through a Properties object passed to
 * {@link #EnvironmentConfig(Properties)}, or via properties in the
 ...
 * </pre>
 * <p>
 * Some commonly used environment attributes have convenience setter/getter
 * methods defined in this class. For example, to change the default lock
 * timeout setting for an environment, the application can instead do the
 * following:
 * </p>
 * 
 * <pre class=code>
 ...
 * <li>Any configuration parameters not set by the application are set to system
 * defaults, described along with the parameter name String constants in this
 * class.</li>
 * </ol>
 * <p>
 * However, a small number of parameters do not have string constants in this
 * class, and cannot be set using {@link #setConfigParam}, a Properties object,
 * or the je.properties file. These parameters can only be changed via the
 * following setter methods:
 * </p>
 * <ul>
 ...
 * environment properties. Immutable properties may be specified when the first
 * Environment handle (instance) is opened for a given physical environment.
 * When more handles are opened for the same environment, the following rules
 * apply:
 * </p>
 * <ol>
 ...
 * changed using {@link Environment#setMutableConfig}. See
 * {@link EnvironmentMutableConfig} for a list of mutable properties; all other
 ...
 * </p>
 * <h4>Getting the Current Environment Properties</h4> To get the current "live"
 * properties of an environment after constructing it or changing its
 * properties, you must call {@link Environment#getConfig} or
 ...
 * <p>
 * Several environment and transaction configuration properties are time
 * durations. For these properties, a time unit is specified along with an
 * integer duration value.
 * </p>
 * <p>
 * When specific setter and getter methods exist for a time duration property,
 * these methods have a {@link TimeUnit} argument. Examples are
 * {@link #setLockTimeout(long,TimeUnit)} and {@link #getLockTimeout(TimeUnit)}.
 * Note that the {@link TimeUnit} argument may be null only when the duration
 * value is zero; there is no default unit that is used when null is specified.
 * </p>
 * <p>
 * When a time duration is specified as a string value, the following format is
 * used.
 * </p>
 *
 * <pre>
 *    {@code <value> [ <whitespace> <unit> ]}
 * </pre>
 * <p>
 * The {@code <value>} is an integer. The {@code <unit>} name, if present, must
 * be preceded by one or more spaces or tabs.
 * </p>
 * <p>
 * The following {@code <unit>} names are allowed. Both {@link TimeUnit} names
 * and IEEE standard abbreviations are allowed. Unit names are case insensitive.
 * </p>
 * <table border="true">
 * <tr>
 * <th>IEEE abbreviation</th>
 * <th>TimeUnit name</td>
 * <th>Definition</th>
 * </tr>
 * <tr>
 ...
 * </pre>
 * <p>
 * The maximum duration value is currently Integer.MAX_VALUE milliseconds. This
 * translates to almost 25 days (2147483647999999 ns, 2147483647999 us,
 * 2147483647 ms, 2147483 s, 35791 min, 596 h).
 * </p>
 * <p>
 * Note that when the {@code <unit>} is omitted, microseconds is implied. This
 * default is supported for compatibility with JE 3.3 and earlier. In JE 3.3 and
 * earlier, explicit time units were not used and durations were always
 * implicitly specified in microseconds. The older methods that do not have a
 * {@link TimeUnit} argument, such as {@link #setLockTimeout(long)} and
 * {@link #getLockTimeout()}, use microsecond durations and have been
 * deprecated.
 * </p>
 */
public class EnvironmentConfig extends EnvironmentMutableConfig {
    ...
    /**
     * Configures the memory available to the database system, in bytes. See
     ...
     * <p>
     * The system will evict database objects when it comes within a prescribed
     * margin of the limit.
     * </p>
     * <p>
     ...
     * calculation and sets the cache size explicitly.
     * </p>
     * <p>
     * The following details apply to setting the cache size to a percentage of
     * the JVM heap size byte size (this parameter) as well as to a byte size (
     * {@link #MAX_MEMORY}
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * If {@link #SHARED_CACHE} is set to true, MAX_MEMORY and
     * MAX_MEMORY_PERCENT specify the total size of the shared cache, and
     * changing these parameters will change the size of the shared cache.
     * </p>
     * <p>
     * When using the shared cache feature, new environments that join the cache
     * may alter the cache percent setting if their configuration is set to a
     * different value.
     * </p>
     * <p>
     * To take full advantage of JE cache memory, it is strongly recommended
     * that <a href=
     * "http://download.oracle.com/javase/7/docs/technotes/guides/vm/performance-enhancements-7.html#compressedOop">
     * compressed oops</a> (<code>-XX:+UseCompressedOops</code>) is specified
     * when a 64-bit JVM is used and the maximum heap size is less than 32 GB.
     * As described in the referenced documentation, compressed oops is
     * sometimes the default JVM mode even when it is not explicitly specified
     * in the Java command. However, if compressed oops is desired then it
     * <em>must</em> be explicitly specified in the Java command when running
     * DbCacheSize or a JE application. If it is not explicitly specified then
     * JE will not aware of it, even if it is the JVM default setting, and will
     * not take it into account when calculating cache memory sizes.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     * The off-heap cache is used to hold record data and Btree nodes when these
     * are evicted from the "main cache" because it overflows. Eviction occurs
     * according to an LRU algorithm and takes into account the user- specified
     * {@link CacheMode}. When the off-heap cache overflows, eviction occurs
     * there also according to the same algorithm.
     * <p>
     ...
     * <p>
     * The use of the file system cache has performance benefits, but also has
     * its own drawbacks: 1) There is a significant redundancy between the main
     * cache and the file system cache because all data and Btree information
     * that is logged (written) by JE appears in the file system and may also
     * appear in the main cache. 2) It is not possible for <em>dirty</em> Btree
     * information to be placed in the file system cache without logging it,
     * this logging may be otherwise unnecessary, and the logging creates
     * additional work for the JE cleaner; in other words, the size of the main
     * cache alone determines the maximum size of the in-memory "dirty set".
     * <p>
     * The off-heap cache is stored outside the Java heap using a native
     * platform memory allocator. The current implementation relies on internals
     * that are specific to the Oracle and IBM JDKs; however, a memory allocator
     * interface that can be implemented for other situations is being
     * considered for a future release. Records and Btree objects are serialized
     * when they are placed in the off-heap cache, and they must be materialized
     * when they are moved back to the main cache in order to access them. This
     * serialization and materialization adds some CPU overhead and thread
     * contention, as compared to accessing data directly in the main cache. The
     * off-heap cache can contain dirty Btree information, so it can be used to
     * increase the maximum size of the in-memory "dirty set".
     * <p>
     * NOTE: If an off-heap cache is configured but cannot be used because that
     * native allocator is not available in the JDK that is used, an
     * {@code IllegalStateException} will be thrown by the {@link Environment}
     * or {@link com.sleepycat.je.rep.ReplicatedEnvironment} constructor. In the
     * current release, this means that the {@code sun.misc.Unsafe} class must
     * contain the {@code allocateMemory} method and related methods, as defined
     * in the Oracle JDK.
     * <p>
     ...
     * materialization overhead is not increased. In this case, the use of the
     * off-heap cache is clearly beneficial, and using the off-heap cache
     ...
     * processes, while the off-heap cache does not. Therefore, it is best to be
     * conservative about leaving memory free for other uses, and it is not a
     * good idea to size the off-heap cache such that all machine memory will be
     * allocated. If off-heap allocations or other allocations fail because
     ...
     * <p>
     * By default this parameter is false and this environment uses a private
     * cache. If this parameter is set to true, this environment will use a
     * cache that is shared with all other open environments in this process
     * that also set this parameter to true. There is a single shared cache per
     * process.
     * </p>
     * <p>
     * By using the shared cache, multiple open environments will make better
     * use of memory because the cache LRU algorithm is applied across all
     * information in all environments sharing the cache. For example, if one
     * environment is open but not recently used, then it will only use a small
     * portion of the cache, leaving the rest of the cache for environments that
    ...
    /**
     * If true, a checkpoint is forced following recovery, even if the log ends
     * with a checkpoint.
     * <p>
     * <table border="1">
     * <tr>
    ...
    /**
     * By default, if a checksum exception is found at the end of the log during
     * Environment startup, JE will assume the checksum is due to previously
     * interrupted I/O and will quietly truncate the log and restart. If this
     * property is set to true, when a ChecksumException occurs in the last log
     * file during recovery, instead of truncating the log file, and
     * automatically restarting, attempt to continue reading past the corrupted
     * record with the checksum error to see if there are commit records
     * following the corruption. If there are, throw an
     * EnvironmentFailureException to indicate the presence of committed
     * transactions. The user may then need to run DbTruncateLog to truncate the
     * log for further recovery after doing manual analysis of the log. Setting
     * this property is suitable when the application wants to guard against
     * unusual cases.
     * <p>
     * <table border="1">
     * <tr>
     ...
     * threads can perform I/O. If zero (the default) then no limitation on I/O
     * is enforced.
     * <p>
     * <table border="1">
     * <tr>
     ...
     * {@link #ENV_BACKGROUND_WRITE_LIMIT} or {@link #ENV_BACKGROUND_READ_LIMIT}
     * is reached. If {@link #ENV_BACKGROUND_WRITE_LIMIT} and
     * {@link #ENV_BACKGROUND_READ_LIMIT} are zero, this setting is not used.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * <p>
     * This should be set to true when transactional guarantees such as
     * atomicity of multiple operations and durability are important.
     * </p>
     * <p>
     ...
     * Transactional product. Note that if transactions are not used, specifying
     * true does not create additional overhead in the environment.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     * <p>
     * If true, create the environment with record locking. This property should
     * be set to false only in special circumstances when it is safe to run
     * without record locking.
     * </p>
     * <p>
     * This configuration option should be used when locking guarantees such as
     * consistency and isolation are not important. If locking mode is disabled
     * (it is enabled by default), the cleaner is automatically disabled. The
     * user is responsible for invoking the cleaner and ensuring that there are
     * no concurrent operations while the cleaner is running.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     * <p>
     * A read-only environment has several limitations and is recommended only
     * in special circumstances. Note that there is no performance advantage to
     * opening an environment read-only.
     * </p>
     * <p>
     ...
     * time may open the environment read-write. See
     ...
     * </p>
     * <ul>
     * <li>In the read-only environment no writes may be performed, as expected,
     * and databases must be opened read-only using
     * {@link DatabaseConfig#setReadOnly}.</li>
     ...
     * cannot be flushed to disk. This means that the read-only environment may
     * not be suitable for operations that use large amounts of memory, and poor
     * performance may result if this is attempted.</li>
     ...
     * </ul>
     * <p>
     * For these reasons, it is recommended that a read-only environment be used
     * only for short periods and for operations that are not performance
     ...
     * single application so that they can be performed in the JVM process where
     * the environment is open read-write.
     * </p>
     * <p>
     * In most applications, opening an environment read-only can and should be
     * avoided.
     * </p>
     * <p>
     * <table border="1">
    ...
    /**
     * If true, use latches instead of synchronized blocks to implement the lock
     * table and log write mutexes. Latches require that threads queue to obtain
     * the mutex in question and therefore guarantee that there will be no mutex
     * starvation, but do incur a performance penalty. Latches should not be
     * necessary in most cases, so synchronized blocks are the default. An
     * application that puts heavy load on JE with threads with different thread
     * priorities might find it useful to use latches. In a Java 5 JVM, where
     * java.util.concurrent.locks.ReentrantLock is used for the latch
     ...
     * be detected. Latches are held internally for very short durations. If due
     * to unforeseen problems a deadlock occurs, a timeout will occur after the
     * duration specified by this parameter. When a latch timeout occurs:
     * <ul>
    ...
    /**
     * The interval added to the system clock time for determining that a record
     * may have expired. Used when an internal integrity error may be present,
     * but may also be due to a record that expired and the system clock was
     * moved back.
     * <p>
     * For example, say a record expires and then the clock is moved back by one
     * hour to correct a daylight savings time error. Because the LN and BIN
     * slot for an expired record are purged separately (see
     ...
     * extended down time.
     * <p>
     * WARNING: Disabling expiration is intended for special-purpose access for
     * data recovery only. When this parameter is set to false, records that
     * have expired may or may not have been purged, so they may or may not be
     * accessible. In addition, it is possible for the key and data of a record
     * to expire independently, so the key may be accessible (if the data is not
     * requested by the read operation), while the record will appear to be
     * deleted when the data is requested. The same thing is true of primary and
     * secondary records, which are also purged independently. A record may be
     * accessible by primary key but not secondary key, and vice-versa.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * the internal nodes for all duplicates databases. Preloading each database
     * individually gives a performance advantage if the JE cache is roughly
     * large enough to contain the internal nodes for a single duplicates
     * database.
     * <p>
     * <table border="1">
     * <tr>
     ...
     * <td>No</td>
     * <td>0</td>
     * <td>{@value com.sleepycat.je.config.EnvironmentParams#LOG_MEM_SIZE_MIN}
     * </td>
     * <td>-none-</td>
     * </tr>
     * </table>
     ...
     * <td>No</td>
     * <td>
     * {@value com.sleepycat.je.config.EnvironmentParams#NUM_LOG_BUFFERS_DEFAULT}
     * </td>
     * <td>2</td>
     * <td>-none-</td>
     * </tr>
    ...
    /**
     * The maximum starting size of a JE log buffer. JE silently restricts this
     * value to be no more than the configured maximum log file size
     * (je.log.fileMax).
     * <p>
     * <table border="1">
     * <tr>
     ...
     * <p>
     * If 0, all log files (*.jdb) will reside in the environment home directory
     * passed to the Environment constructor. A non-zero value indicates the
     * number of environment subdirectories to use for holding the environment
     * log files.
     * <p>
     * If data subdirectories are used (i.e. je.log.nDataDirectories > 0), this
     * parameter must be set when the environment is initially created. Like the
     * environment home directory, each and every one of the dataNNN/
     * subdirectories must also be present and writable. This parameter must be
     * set to the same value for all subsequent openings of the environment or
     ...
     * EnvironmentFailureException will be thrown, and the Environment will fail
     * to be opened.
     * <p>
     * This parameter should be set using the je.properties file rather than the
     * EnvironmentConfig. If not, JE command line utilities that open the
     ...
     *             beyond that of a simple RAID configuration, and will be
     *             removed in the next release, which is slated for mid-April,
     *             2017.
     */
    ...
    /**
     * If true, perform a checksum verification just before and after writing to
     * the log. This is primarily used for debugging.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
    ...
    /**
     * If true, periodically detect unexpected file deletions. Normally all file
     * deletions should be performed as a result of JE log cleaning. If an
     * external file deletion is detected, JE assumes this was accidental. This
     * will cause the environment to be invalidated and all methods will throw
    ...
    /**
     * If the time taken by an fsync exceeds this limit, a WARNING level message
     * is logged. If this parameter set to zero, a message will not be logged.
     * By default, this parameter is 5 seconds.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
    ...
    /**
     * The time interval in nanoseconds during which transactions may be grouped
     * to amortize the cost of write and/or fsync when a transaction commits
     * with SyncPolicy#SYNC or SyncPolicy#WRITE_NO_SYNC on the local machine.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * {@link Durability.SyncPolicy#COMMIT_NO_SYNC NO_SYNC} or
     * {@link Durability.SyncPolicy#COMMIT_WRITE_NO_SYNC WRITE_NO_SYNC}
     * durability, and making the transaction durable with respect to the
     * storage device. To provide this guarantee, a JE background thread is used
     * to flush any data buffered by JE to the file system, and also perform an
     * fsync to force any data buffered by the file system to the storage
     * device. If this parameter is set to zero, this JE background task is
     * disabled and no such guarantee is provided.
     * <p>
     ...
     * LOG_FLUSH_SYNC_INTERVAL}, provides a fallback for flushing to the storage
     * device, in case the file system is not adequately configured.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * buffered by JE to the file system. If this parameter is set to zero, this
     * JE background task is disabled and no such guarantee is provided.
     * <p>
    ...
    /**
     * If true (default is true) the Write Queue is used for file I/O operations
     * which are blocked by concurrent I/O operations.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * <p>
     * If true (the default), the verifier runs according to the schedule given
     * by {@link #VERIFY_SCHEDULE}. Each time the verifier runs, it performs
     * checksum verification if the {@link #VERIFY_LOG} setting is true. Btree
     ...
     * exception can call the new
     * {@link EnvironmentFailureException#isCorrupted()} method to determine
     ...
     * <p>
     * See https://en.wikipedia.org/wiki/Cron#Configuration_file Note that times
     * and dates are specified in local time, not UTC time.
     * <p>
     * If the verifier is already running at the scheduled time, the scheduled
     * run is skipped.
     * <p>
     * <table border="1">
     * <tr>
    ...
    /**
     * Whether the background verifier should verify checksums in the log, as if
     * the {@link DbVerifyLog} utility were run.
     * <p>
     * If true, the entire log is read sequentially and verified. The size of
     * the read buffer is determined by LOG_ITERATOR_READ_SIZE.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
    ...
    /**
     * @hidden Whether the background verifier should perform Btree
     *         verification, as if the {@link DbVerify} utility were run.
     *         <p>
     *         If true, the Btree of all databases, external and internal, is
     *         verified. The in-memory cache is used for verification and
     *         internal data structures are checked. References to data records
     *         (log sequence numbers, or LSNs) are checked to ensure they do not
     *         refer to deleted files -- this is the most common type of
     *         corruption. Additional checks are performed, depending on the
     *         settings for {@link #VERIFY_SECONDARIES},
     *         {@link #VERIFY_DATA_RECORDS}, and
     *         {@link #VERIFY_OBSOLETE_RECORDS}.
     *         <p>
     *         <table border="1">
     *         <tr>
     *         <td>Name</td>
    ...
    /**
     * @hidden Whether to verify secondary index references during Btree
     *         verification.
     *         <p>
     *         An index record contains a reference to a primary key, and the
     *         verification involves checking that a record for the primary key
     *         exists.
     *         <p>
     *         Note that secondary index references are verified only for each
     *         {@link SecondaryDatabase} (and
     *         {@link com.sleepycat.persist.SecondaryIndex SecondaryIndex}) that
     *         is currently open. The relationship between a secondary and
     *         primary database is not stored persistently, so JE is not aware
     *         of the relationship unless the secondary database has been opened
     *         by the application.
     *         <p>
     *         <table border="1">
     *         <tr>
     ...
     *         verification.
     *         <p>
     *         Regardless of this parameter's value, the Btree reference to the
     *         data record (the log sequence number, or LSN) is is checked to
     *         ensure that it doesn't refer to a file that has been deleted by
     *         the JE cleaner -- this sort of "dangling reference" is the most
     *         common type of corruption. If this parameter value is true, the
     *         LN is additionally fetched from disk (if not in cache) to verify
     *         that the LSN refers to a valid log entry. Because LNs are often
     *         not cached, this can cause expensive random IO, and the default
     *         value for this parameter is false for this reason. Some
     *         applications may choose to set this parameter to true, for
     *         example, when using a storage device with fast random IO(an SSD).
     *         <p>
     *         Note that Btree internal nodes (INs) are always fetched from disk
     *         during verification, if they are not in cache, and this can
     *         result in random IO. Verification was implemented with the
     ...
     *         references(log sequence numbers, or LSNs) to obsolete records. If
     *         such a reference is incorrect and the record at the LSN is
     ...
     *         <p>
     *         If this parameter's value is true, all active LSNs in the Btree
     *         are checked to ensure they are not in the cleaner's set of
     *         obsolete LSNs. To perform this check efficiently, the set of all
     *         obsolete LSNs must be fetched from disk and kept in memory during
     *         the verification run, and the default value for this parameter is
     *         false for this reason. Some applications may choose to set this
     *         parameter to true, when the use of more Java heap memory is worth
     *         the additional safety measure.
     *         <p>
     *         <table border="1">
     *         <tr>
     *         <td>Name</td>
    ...
    /**
     * The maximum number of entries in an internal btree node. This can be set
     * per-database using the DatabaseConfig object.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     *             {@link DatabaseConfig#setNodeMaxEntries} should be used
     *             instead.
     */
     ...
     * the record to be embedded in its parent LN. Normally, records (key-value
     * pairs) are stored on disk as individual byte sequences called LNs (leaf
     * nodes) and they are accessed via a Btree. The nodes of the Btree are
     * called INs (Internal Nodes) and the INs at the bottom layer of the Btree
     * are called BINs (Bottom Internal Nodes). Conceptually, each BIN contains
     * an array of slots. A slot represents an associated data record. Among
     * other things, it stores the key of the record and the most recent disk
     * address of that record. Records and INs share the disk space (are stored
     * in the same kind of files), but LNs are stored separately from BINs,
     * i.e., there is no clustering or co-location of a BIN and its child LNs.
     * <p>
     * With embedded LNs, a whole record may be stored inside a BIN (i.e., a BIN
     * slot may contain both the key and the data portion of a record).
     ...
     * may contain both embedded and non-embedded records. The "embeddedness" of
     * a record is a dynamic property: a size-changing update may turn a
     ...
     * and written to disk. This is because LNs also act as log records, which
     * are needed during recovery and/or transaction abort to undo/redo
     ...
     * <p>
     * Obviously, embedding records has the performance advantage that no extra
     * disk read is needed to fetch the record data (i.e., the LN) during read
     * operations. This is especially true for operations like cursor scans and
     * for random searches within key ranges whose containing BINs can fit in
     * the JE cache (in other words when there is locality of reference).
     * Furthermore, embedded records do not need to be migrated during cleaning;
     * they are considered obsolete by default, because they will never be
     * needed again after their containing log file is deleted. This makes
     * cleaning faster, and more importantly, avoids the dirtying of the parent
     * BINs, which would otherwise cause even more cleaning later.
     * <p>
     * On the other hand, embedded LNs make the BINs larger, which can lead to
     * more cache eviction of BINs and the associated performance problems. When
     * eviction does occur, performance can deteriorate as the size of the data
     * portion of the records grows. This is especially true for insertion-only
     * workloads. Therefore, increasing the value of TREE_MAX_EMBEDDED_LN beyond
     * the default value of 16 bytes should be done "carefully": by considering
     * the kind of workloads that will be run against BDB-JE and their relative
     * importance and expected response times, and by running performance tests
     * with both embedded and non-embedded LNs.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * including internal nodes and record keys and data. If the specified value
     * is larger than the size initially available in the cache, it will be
     * truncated to the amount available.
     * <p>
     ...
     * <p>
     * In the Btree, the JE in-memory cache, the default representation for keys
     * uses a byte array object per key. The per-key object overhead of this
     * approach ranges from 20 to 32 bytes, depending on the JVM platform.
     * </p>
     * <p>
     * To reduce memory overhead, a compact representation can instead be used
     * where keys will be represented inside a single byte array instead of
     * having one byte array per key. Within the single array, all keys are
     ...
     * reduces Java GC activity as well as memory overhead.
     * </p>
     * <p>
     * In order for the compact representation to reduce memory usage, all keys
     * in a database, or in a Btree internal node, must be roughly the same
     * size. The more fully populated the internal node, the more the savings
     * with this representation since the single byte array is sized to hold the
     * maximum number of keys in the internal node, regardless of the actual
     * number of keys that are present.
     * </p>
     * <p>
     * It's worth noting that the storage savings of the compact representation
     * are realized in addition to the storage benefits of key prefixing (if it
     * is configured), since the keys stored in the key array are the smaller
     * key values after the prefix has been stripped, reducing the length of the
     * key and making it more likely that it's small enough for this specialized
     * representation. This configuration parameter (
     * {@code TREE_COMPACT_MAX_KEY_LENGTH}) is the maximum key length, not
     * including the common prefix, for the keys in a Btree internal node stored
     * using the compact representation. See
     * {@link DatabaseConfig#setKeyPrefixing}.
     * </p>
     * <p>
     * The compact representation is used automatically when both of the
     * following conditions hold.
     * </p>
     * <ul>
     * <li>All keys in a Btree internal node must have an unprefixed length that
     * is less than or equal to the length specified by this parameter (
     ...
     * default representation. In that case, the default representation will be
     * used.</li>
     * </ul>
     * <p>
     * If this configuration parameter is set to zero, the compact
     * representation will not be used.
     * </p>
     * <p>
     * The default value of this configuration parameter is 16 bytes. The
     * potential drawbacks of specifying a larger length are:
     * </p>
     * <ul>
     * <li>Insertion and deletion for larger keys move bytes proportional to the
     * storage length of the keys.</li>
     * <li>With the compact representation, all operations create temporary byte
     * arrays for each key involved in the operation. Larger byte arrays mean
     * more work for the Java GC, even though these objects are short lived.
     * </li>
     * </ul>
     * <p>
     * Mutation of the key representation between the default and compact
     * approaches is automatic on a per-Btree internal node basis. For example,
     * if a key that exceeds the configured length is added to a node that uses
     * the compact representation, the node is automatically mutated to the
     * default representation. A best effort is made to prevent frequent
     * mutations that could increase Java GC activity.
     * </p>
     * <p>
     * To determine how often the compact representation is used in a running
     * application, see {@link EnvironmentStats#getNINCompactKeyIN}.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
    ...
    /**
     * @deprecated as of 3.3.87. Compression of the root node no longer has any
     *             benefit and this feature has been removed. This parameter has
     *             no effect.
     */
     ...
     * of bytes below {@link #MAX_MEMORY}. No more than 50% of je.maxMemory will
     * be evicted per eviction cycle, regardless of this setting.
     * <p>
     * When using the shared cache feature, the value of this property is
     * applied the first time the cache is set up. New environments that join
     * the cache do not alter the cache setting.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * For example, if this parameter is 5, then when the cache size is 5% over
     * its maximum or 105% full, critical eviction will start.
     * <p>
     * Critical eviction is eviction performed in application threads as part of
     * normal database access operations. Background eviction, on the other
    ...
    /**
     * @deprecated as of JE 4.1, since the single evictor thread has been
     *             replaced be a more robust thread pool.
     */
    ...
    /**
     * The number of LRU lists in the main JE cache. Ideally, all nodes managed
     * by an LRU eviction policy should appear in a single LRU list, ordered by
     * the "hotness" of each node. However, such a list is accessed very
     * frequently by multiple threads, and can become a synchronization
     * bottleneck. To avoid this problem, the evictor can employ multiple LRU
     * lists. The nLRULists parameter specifies the number of LRU lists to be
     * used. Increasing the number of LRU lists alleviates any potential
     * synchronization bottleneck, but it also decreases the quality of the LRU
     * approximation.
     * <p>
     * <table border="1">
     * <tr>
     ...
     * <p>
     * These threads help keep memory usage within cache bounds, offloading work
     * from application threads.
     * <p>
     * {@link #EVICTOR_CORE_THREADS}, {@link #EVICTOR_MAX_THREADS} and
     * {@link #EVICTOR_KEEP_ALIVE} are used to configure the core, max and
     * keepalive attributes for the
     * {@link java.util.concurrent.ThreadPoolExecutor} which implements the
     ...
     * from application threads. If the eviction thread pool receives more work,
     * it will allocate up to this number of threads. These threads will
     * terminate if they are idle for more than the time indicated by
    ...
    /**
     * Allow Bottom Internal Nodes (BINs) to be written in a delta format during
     * eviction. Using a delta format will improve write and log cleaning
     * performance, but may reduce read performance if BINs are not maintained
     * in cache memory.
     * <p>
     * <table border="1">
     * <tr>
    ...
    /**
     * The number of LRU lists in the off-heap JE cache. Ideally, all nodes
     * managed by an LRU eviction policy should appear in a single LRU list,
     * ordered by the "hotness" of each node. However, such a list is accessed
     * very frequently by multiple threads, and can become a synchronization
     * bottleneck. To avoid this problem, the evictor can employ multiple LRU
     * lists. The nLRULists parameter specifies the number of LRU lists to be
     * used. Increasing the number of LRU lists alleviates any potential
     * synchronization bottleneck, but it also decreases the quality of the LRU
     ...
     * from application threads.
     * <p>
     * {@link #OFFHEAP_CORE_THREADS}, {@link #OFFHEAP_MAX_THREADS} and
     * {@link #OFFHEAP_KEEP_ALIVE} are used to configure the core, max and
     * keepalive attributes for the
     * {@link java.util.concurrent.ThreadPoolExecutor} which implements the
     * eviction thread pool.
     * <p>
    ...
    /**
     * The checkpointer wakeup interval in microseconds. By default, this is
     * inactive and we wakeup the checkpointer as a function of the number of
     * bytes written to the log ({@link #CHECKPOINTER_BYTES_INTERVAL}).
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * to the log. If zero (and by default) it is set to the
     * {@link #LOG_FILE_MAX} value divided by four.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * disabled, and cleaning and checkpointing will occur only via
     * {@link #CLEANER_BYTES_INTERVAL}, {@link #CHECKPOINTER_BYTES_INTERVAL},
     * and {@link #CHECKPOINTER_WAKEUP_INTERVAL}.
     * </p>
     * <p>
     * For example, if a database were removed or truncated, or large records
     * were deleted, the amount written to the log may not exceed
     * CLEANER_BYTES_INTERVAL. If writing were to stop at that point, no
     * cleaning would occur, if it were not for the wakeup interval.
     * </p>
     * <p>
     ...
     * cleaned and a checkpoint is needed to reclaim space, and write operations
     * have stopped, a checkpoint will be scheduled when the
     ...
     * {@link Environment#checkpoint} is called.
     * </p>
     * <p>
     * In test environments it is fairly common for application writing to stop,
     * and then to expect cleaning to occur as a result of the last set of
     * operations. This situation may also arise in production environments, for
     * example, during repair of an out-of-disk situation.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     * delete/update), the size of the previous version of the record is unknown
     * and therefore the cleaner's utilization calculations may be incorrect.
     * Setting this parameter to true will cause a record to be read during a
     * blind delete/update, in order to determine its size. This will ensure
     * that the cleaner's utilization calculations are correct, but will cause
     * more (potentially random) IO.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * successful cleaning. This parameter may be set to false for diagnosing
     * log cleaning problems. For example, if a bug causes a LOG_FILE_NOT_FOUND
     * exception, when reproducing the problem it is often necessary to avoid
     * deleting files so they can be used for diagnosis. When this parameter is
     * false:
     * <ul>
     * <li>Rather than delete files that are successfully cleaned, the cleaner
     * renames them.</li>
     * <li>When renaming a file, its extension is changed from ".jdb" to ".del"
     * and its last modification date is set to the current time.</li>
     * <li>Depending on the setting of the {@link #CLEANER_USE_DELETED_DIR}
     * parameter, the file is either renamed in its current data directory (the
     * default), or moved into the "deleted" sub-directory.</li>
     * </ul>
     * <p>
     * When this parameter is set to false, disk usage may grow without bounds
     * and the application is responsible for removing the cleaned files. It may
     * be necessary to write a script for deleting the least recently cleaned
     * files when disk usage is low. The .del extension and the last
     ...
     * {@code CLEANER_USE_DELETED_DIR} applies only when
     * {@link #CLEANER_EXPUNGE} is false. When {@link #CLEANER_EXPUNGE} is true,
     ...
     * "*.del". In this case, the "deleted" sub-directory must have been created
     * by the application before opening the Environment. This allows the
     * application to control permissions on this sub-directory. When multiple
     * data directories are used ({@link #LOG_N_DATA_DIRECTORIES}), a "deleted"
     * sub-directory must be created under each data directory. Note that
     * {@link java.io.File#renameTo(File)} is used to move the file, and this
     * method may or may not support moving the file to a different volume (when
     * the "deleted" directory is a file system link) on a particular platform.
     * <p>
     * When {@code CLEANER_USE_DELETED_DIR} is false (and {@code
     * CLEANER_EXPUNGE} is false), the cleaner will change the file extension of
     * successfully cleaned data files from ".jdb" to ".del", but will not move
     ...
     * {@link #LOG_ITERATOR_READ_SIZE} value is used.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * percentage of the cache. The default value is 2% of {@link #MAX_MEMORY}.
     * If 0 and {@link #SHARED_CACHE} is true, use 2% divided by N where N is
     * the number of environments sharing the global cache.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * forcing the cleaning of a large number of log files. File numbers are in
     * hex and are comma separated or hyphen separated to specify ranges, e.g.:
     * '9,a,b-d' will clean 5 files.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
    ...
    /**
     * All log files having a log version prior to the specified version will be
     * cleaned at a time when no other log cleaning is necessary. Intended for
     * use in upgrading old format log files forward to the current log format
     * version, e.g., to take advantage of format improvements; note that log
     * upgrading is optional. The default value zero (0) specifies that no
     * upgrading will occur. The value negative one (-1) specifies upgrading to
     ...
     *             Btree splits. To reduce a cleaner backlog, configure more
     *             cleaner threads.
     */
     ...
     * @deprecated This parameter is ignored and proactive migration is no
     *             longer supported due to its negative impact on eviction and
     *             checkpointing. To reduce a cleaner backlog, configure more
     *             cleaner threads.
     */
     ...
     *             supported due to its negative impact on eviction and
     *             checkpointing. To reduce a cleaner backlog, configure more
     *             cleaner threads.
     */
     ...
     * application has multiple threads performing concurrent JE operations. It
     * should be set to a prime number, and in general not higher than the
    ...
    /**
     * Whether to perform deadlock detection when a lock conflict occurs. By
     * default, deadlock detection is enabled (this parameter is true) in order
     * to reduce thread wait times when there are deadlocks.
     * <p>
     * Deadlock detection is performed as follows.
     * <ol>
     * <li>When a lock is requested by a record read or write operation, JE
     * checks for lock conflicts with another transaction or another thread
     * performing a non-transactional operation. If there is no conflict, the
     * lock is acquired and the operation returns normally.</li>
     * <li>When there is a conflict, JE performs deadlock detection. However,
     * before performing deadlock detection, JE waits for the
     * {@link #LOCK_DEADLOCK_DETECT_DELAY} interval, if it is non-zero. This
     * delay is useful for avoiding the overhead of deadlock detection when
     * normal, short-lived contention (not a deadlock) is the reason for the
     * conflict. If the lock is acquired during the delay, the thread wakes up
     * and the operation returns normally.</li>
     * <li>If a deadlock is detected, {@link DeadlockException} is thrown in one
     * of the threads participating in the deadlock, called the "victim". The
     * victim is chosen at random to prevent a repeated pattern of deadlocks,
     * called "live lock". A non-victim thread that detects a deadlock will
     * notify the victim and perform short delays, waiting for the deadlock to
     * be broken; if the lock is acquired, the operation returns normally.</li>
     * <li>It is possible for live lock to occur in spite of using random victim
     * selection. It is also possible that a deadlock is not broken because the
     * victim thread is unresponsive or the application fails to close a cursor
     * or to commit or abort a transaction. In these cases, if the lock or
     * transaction timeout expires without acquiring the lock, a
     * {@code DeadlockException} is thrown for the last deadlock detected, in
     * the thread that detected the deadlock. In this case,
     * {@code DeadlockException} may be thrown by more than one thread
     * participating in the deadlock.</li>
     * <li>When no deadlock is detected, JE waits for the lock or transaction
     * timeout to expire. If the lock is acquired during this delay, the thread
     * wakes up and the operation returns normally.</li>
     * <li>When the lock or transaction timeout expires without acquiring the
     * lock, JE checks for deadlocks one final time. If a deadlock is detected,
     * {@code DeadlockException} is thrown; otherwise,
     * {@link LockTimeoutException} or {@link TransactionTimeoutException}is
     * thrown.</li>
     * </ol>
     * <p>
     ...
     * (Degree 3) isolation. By setting Serializable isolation, phantoms will be
     * prevented. By default transactions provide Repeatable Read isolation. The
     * default is false for the database environment.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * lock was taken. The default is false, and true should only be used during
     * debugging because of the added memory/processing cost. This parameter is
    ...
    /**
     * @deprecated in favor of <code>FILE_LOGGING_LEVEL</code> As of JE 4.0, use
     *             the standard java.util.logging configuration methodologies.
     *             To enable logging output to the je.info files, set
     *             com.sleepycat.je.util.FileHandler.level = {@literal <LEVEL>}
     *             through the java.util.logging configuration file, or through
     *             the java.util.logging.LogManager. To set the handler level
     *             programmatically, set
     *             "com.sleepycat.je.util.FileHandler.level" in the
     *             EnvironmentConfig object.
     */
    public static final String                           TRACE_FILE                             = "java.util.logging.FileHandler.on";

    /**
     * @deprecated in favor of <code>CONSOLE_LOGGING_LEVEL</code> As of JE 4.0,
     *             use the standard java.util.logging configuration
     *             methodologies. To enable console output, set
     *             com.sleepycat.je.util.ConsoleHandler.level =
     *             {@literal <LEVEL>} through the java.util.logging
     *             configuration file, or through the
     *             java.util.logging.LogManager. To set the handler level
     *             programmatically, set
     *             "com.sleepycat.je.util.ConsoleHandler.level" in the
     *             EnvironmentConfig object.
     */
     ...
     *             separated from the java.util.logging mechanism. This
     ...
     *             configuration methodologies. To set the FileHandler output
     *             file size, set com.sleepycat.je.util.FileHandler.limit =
     *             {@literal <NUMBER>} through the java.util.logging
     *             configuration file, or through the
     ...
     * @deprecated As of JE 4.0, use the standard java.util.logging
     *             configuration methodologies. To set the FileHandler output
     *             file count, set com.sleepycat.je.util.FileHandler.count =
     *             {@literal <NUMBER>} through the java.util.logging
     *             configuration file, or through the
     *             java.util.logging.LogManager.
     */
    ...
    /**
     * @deprecated As of JE 4.0, use the standard java.util.logging
     *             configuration methodologies. Set logging levels using class
     *             names through the java.util.logging configuration file, or
     ...
     * <p>
     * Setting this parameter in the je.properties file or through
     * {@link EnvironmentConfig#setConfigParam} is analogous to setting the
     * property in the java.util.logging properties file or MBean. It is
     * preferred to use the standard java.util.logging mechanisms for
     * configuring java.util.logging.Handler, but this JE parameter is provided
     * because the java.util.logging API doesn't provide a method to set handler
     ...
     * file, which is in the Environment home directory. Value should be one of
     ...
     *             com.sleepycat.je.recovery.level = {@literal <LEVEL>} through
     ...
     *             com.sleepycat.je.evictor.level = {@literal <LEVEL>} through
     *             the java.util.logging configuration file, or through the
     *             java.util.logging.LogManager.
     */
     ...
     *             com.sleepycat.je.cleaner.level = {@literal <LEVEL>} through
     *             the java.util.logging configuration file, or through the
     *             java.util.logging.LogManager.
     */
     ...
     * format and written to the log file at a user specified interval. The
     * logging occurs per-Environment when the Environment is opened in
     * read/write mode. Statistics are written to a filed named je.stat.csv.
     * Successively older files are named by adding "0", "1", "2", etc into the
    ...
    /**
     * Log file maximum row count for Stat collection. When the number of rows
     * in the statistics file reaches the maximum row count, the file is closed,
     * rotated out, and a new file opened. The name of the log file is
     * je.stat.csv. Successively older files are named by adding "0", "1", "2",
     * etc into the file name. The file name format is je.stat.[version
     * number].csv.
     * <p>
     * <table border="1">
     * <tr>
    ...
    /**
     * For unit testing, to prevent using the utilization profile and expiration
     * profile DB.
     */
     ...
     *            non-transactional operations, or zero to disable lock
     *            timeouts.
    ...
    public void setLockTimeoutVoid(long timeout, TimeUnit unit) throws IllegalArgumentException {
        ...
        DbConfigManager.setDurationVal(props, EnvironmentParams.LOCK_TIMEOUT, timeout, unit, validateParams);
    }
     ...
     *             {@link #setLockTimeout(long, TimeUnit)}.
     */
    ...
    public void setTransactionalVoid(boolean transactional) {
        ...
        DbConfigManager.setBooleanVal(props, EnvironmentParams.ENV_INIT_TXN, transactional, validateParams);
    }
    ...
    public void setLockingVoid(boolean locking) {
        ...
        DbConfigManager.setBooleanVal(props, EnvironmentParams.ENV_INIT_LOCKING, locking, validateParams);
    }
    ...
    public void setTxnTimeoutVoid(long timeout, TimeUnit unit) throws IllegalArgumentException {
        ...
        DbConfigManager.setDurationVal(props, EnvironmentParams.TXN_TIMEOUT, timeout, unit, validateParams);
    }
    ...
    public void setTxnSerializableIsolationVoid(boolean txnSerializableIsolation) {
        ...
        DbConfigManager.setBooleanVal(props, EnvironmentParams.TXN_SERIALIZABLE_ISOLATION, txnSerializableIsolation,
                validateParams);
    }
    ...
    public void setSharedCacheVoid(boolean sharedCache) {
        ...
        DbConfigManager.setBooleanVal(props, EnvironmentParams.ENV_SHARED_CACHE, sharedCache, validateParams);
    }
     ...
     * setting this to a string unique to each Environment may make it easier to
     * diagnose certain exception conditions as well as thread dumps.
     *
     * @return this
     */
     ...
     * java.util.logging.Loggers instantiated by this Environment. This lets the
     * application specify a handler which
     * <ul>
     ...
     * </ul>
     * Note that {@link Handler} is not serializable, and the logging handler
     * should be set within the same process.
     */
    ...
    public EnvironmentConfig setConfigParam(String paramName, String value) throws IllegalArgumentException {
        ...
        DbConfigManager.setConfigParam(props, paramName, value, false, /* requireMutablity */
                validateParams, false /* forReplication */, true /* verifyForReplication */);
        return this;
    }
     ...
     * ProgressListener.progress() method is called at different stages of the
     * recovery process. See {@link RecoveryProgress} for information about
     * those stages.
     * <p>
     * When using progress listeners, review the information at
     * {@link ProgressListener#progress} to avoid any unintended disruption to
     * environment startup.
     * 
     * @param progressListener The ProgressListener to callback during
     *            environment startup (recovery).
     */
    public EnvironmentConfig setRecoveryProgressListener(final ProgressListener<RecoveryProgress> progressListener) {
     ...
     * @hidden The void return setter for use by Bean editors.
     */
    public void setRecoveryProgressListenerVoid(final ProgressListener<RecoveryProgress> progressListener) {
    ...
    public String toString() {
        return " nodeName=" + nodeName + " allowCreate=" + allowCreate + " recoveryProgressListener="
                + (recoveryProgressListener != null) + " classLoader=" + (classLoader != null) + " customStats="
                + (customStats != null) + super.toString();
    }
}
