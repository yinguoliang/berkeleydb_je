...
public class EnvironmentConfig extends EnvironmentMutableConfig {
	 ...
	 * sequences called LNs (leaf nodes) and they are accessed via a Btree. The
	 * nodes of the Btree are called INs (Internal Nodes) and the INs at the
	 * bottom layer of the Btree are called BINs (Bottom Internal Nodes).
	 * Conceptually, each BIN contains an array of slots. A slot represents an
	 * associated data record. Among other things, it stores the key of the
	 * record and the most recent disk address of that record. Records and INs
	 * share the disk space (are stored in the same kind of files), but LNs are
	 * stored separately from BINs, i.e., there is no clustering or co-location
	 * of a BIN and its child LNs.
	 * <p>
	 ...
	 *
	 * Ideally, all nodes managed by an LRU eviction policy should appear in a
	 * single LRU list, ordered by the "hotness" of each node. However, such a
	 * list is accessed very frequently by multiple threads, and can become a
	 * synchronization bottleneck. To avoid this problem, the evictor can employ
	 * multiple LRU lists. The nLRULists parameter specifies the number of LRU
	 * lists to be used. Increasing the number of LRU lists alleviates any
	 * potential synchronization bottleneck, but it also decreases the quality
	 ...
	 * prevented. By default transactions provide Repeatable Read isolation.
	 *
	 ...
	 *
	 * @param properties
	 *            Supported properties are described in this class
	 *
	 * @throws IllegalArgumentException
	 *             If any properties read from the properties param are invalid.
	 */
	 ...
	 *
	 * @param allowCreate
	 *            If true, the database environment is created if it doesn't
	 *            already exist.
	 *
	 * @return this
	 */
	 ...
	 *
	 * @param timeout
	 *            The lock timeout for all transactional and non-transactional
	 *            operations, or zero to disable lock timeouts.
	 *
	 * @param unit
	 *            the {@code TimeUnit} of the timeout value. May be null only if
	 *            timeout is zero.
	 *
	 * @return this
	 *
	 * @throws IllegalArgumentException
	 *             if the value of timeout is invalid
	 *
	 ...
	 *
	 * @param unit
	 *            the {@code TimeUnit} of the returned value. May not be null.
	 *
	 ...
	 *
	 * @param readOnly
	 *            If true, configure the database environment to be read only,
	 *            and any attempt to modify a database will fail.
	 *
	 * @return this
	 */
	 ...
	 *
	 * @param locking
	 *            If false, configure the database environment for no locking.
	 *            The default is true.
	 *
	 * @return this
	 */
	 ...
	 *
	 * @param timeout
	 *            The transaction timeout. A value of 0 turns off transaction
	 *            timeouts.
	 *
	 * @param unit
	 *            the {@code TimeUnit} of the timeout value. May be null only if
	 ...
	 *            The ProgressListener to callback during environment startup
	 *            (recovery).
	 */
