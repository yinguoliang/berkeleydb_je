...
/**
 * Implements the Replica side of the handshake protocol between the Replica and
 * the Feeder. The FeederReplicaHandshake class takes care of the other side.
 *
 * @see <a href=
...
public class ReplicaFeederHandshake {
    ...
    /**
     * If the nodeType is an Arbiter, the SNTPRequest message is sent but the
     * clock skew is not checked.
     */
    ...
    /**
     * Set the current log version to a different value, for testing. Specifying
     * {@code 0} reverts to the standard value.
     *
    ...
    private Protocol negotiateProtocol() throws IOException {
        ...
        final Protocol defaultProtocol = Protocol.getProtocol(repImpl, replicaNameIdPair, clock,
        ...
        if (message instanceof DuplicateNodeReject) {
            throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.HANDSHAKE_ERROR,
                    "A replica with the name: " + replicaNameIdPair + " is already active with the Feeder:"
                            + feederNameIdPair);
        }

        FeederProtocolVersion feederVersion = ((FeederProtocolVersion) message);
        feederNameIdPair = feederVersion.getNameIdPair();
        Protocol configuredProtocol = Protocol.get(repImpl, replicaNameIdPair, clock, feederVersion.getVersion(),
                getCurrentProtocolVersion(), groupFormatVersion);
        LoggerUtils.fine(logger, repImpl,
                "Feeder id: " + feederVersion.getNameIdPair() + "Response message: " + feederVersion.getVersion());
        ...
        if (configuredProtocol == null) {
            ...
            throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.PROTOCOL_VERSION_MISMATCH,
                    "Incompatible protocol versions. " + "Protocol version: " + feederVersion.getVersion()
                            + " introduced in JE version: " + Protocol.getProtocolJEVersion(feederVersion.getVersion())
                            + " requested by the Feeder: " + feederNameIdPair + " is not supported by this Replica: "
                            + replicaNameIdPair + " with protocol version: " + defaultProtocol.getVersion()
    ...
    public Protocol execute() throws IOException, ProtocolException {
        ...
        /**
         * Note whether log entries with later log versions need to be converted
         * to log version 12 to work around [#25222].
         */
        ...
        LoggerUtils.info(logger, repImpl, "Replica-feeder " + feederNameIdPair.getName() + " handshake completed.");
        return protocol;
    }
    ...
    private void verifyVersions() throws IOException {
        ...
        protocol.write(protocol.new ReplicaJEVersions(getCurrentJEVersion(), getCurrentLogVersion()), namedChannel);
        ...
        if (feederJEVersions.getLogVersion() > getCurrentLogVersion()) {
            throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.HANDSHAKE_ERROR,
                    " Feeder: " + feederNameIdPair + ". " + "Feeder log version " + feederJEVersions.getLogVersion()
                            + " is not known to the replica, whose current log version is " + getCurrentLogVersion());
        }
    }
    ...
    private void verifyMembership() throws IOException {
        ...
        Message message = protocol.new NodeGroupInfo(groupName, repGroup.getUUID(), replicaNameIdPair,
                repImpl.getHostName(), repImpl.getPort(), nodeType, repImpl.isDesignatedPrimary(),
                getCurrentJEVersion());
     ...
     * time based consistency requirements requested by transactions. The intent
     * of this check is to draw the attention of the application or the
     * administrators to the skew, not correct it.
     * <p>
     * The scheme implemented below is a variation on the scheme used by
     * <a href="http://tools.ietf.org/html/rfc2030">SNTP</a> protocol. The
     * Feeder plays the role of the SNTP server and the replica the role of the
     * client in this situation. The mechanism used here is rough and does not
     * guarantee the detection of a clock skew, especially since it's a one time
     * check done each time a connection is re-established with the Feeder. The
     * clocks could be in sync at the time of this check and drift apart over
     * the lifetime of the connection. It's also for this reason that we do not
     * store the skew value and make compensations using it when determining
     * replica consistency.
     * <p>
    ...
    private void checkClockSkew() throws IOException, ProtocolException {
        ...
        do {
            if (checkClockSkew) {
                ...
                isLast = (++sampleCount >= CLOCK_SKEW_MAX_SAMPLE_SIZE) || (clockDelay <= CLOCK_SKEW_MIN_DELAY_MS);
            } else {
                isLast = true;
            }
        ...
        LoggerUtils.logMsg(logger, repImpl, (Math.abs(clockDelta) >= maxClockDelta) ? Level.SEVERE : Level.FINE,
                "Round trip delay: " + clockDelay + " ms. " + "Clock delta: " + clockDelta + " ms. "
                        + "Max permissible delta: " + maxClockDelta + " ms.");

        if (Math.abs(clockDelta) >= maxClockDelta) {
            throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.HANDSHAKE_ERROR,
                    "Clock delta: " + clockDelta + " ms. " + "between Feeder: " + feederNameIdPair.getName()
                            + " and this Replica exceeds max permissible delta: " + maxClockDelta + " ms.");
        }
    }
}
