...
/**
 * Holds information seen by the ReplicaSyncupReader when scanning a replica's
 * log for a matchpoint.
 */
public class MatchpointSearchResults implements Serializable {
    ...
    /*
     * Track whether we passed a checkpoint which deleted cleaned log files. If
     * so, we cannot do a hard recovery.
     */
    private boolean                   passedCheckpointEnd;

    /*
     * If we skip a gap in the log file when searching for a matchpoint, we no
     * longer can be sure if we have passed checkpoints.
     */
    private boolean                   passedSkippedGap;

    /*
     * We save a set number of passed transactions for debugging information.
     * The list is limited in size in case we end up passing a large number of
     * transactions on our hunt for a matchpoint. Note that we save both durable
     * and non-durable transactions to aid with debugging.
     */
    private final List<PassedTxnInfo> passedTxns;
    private final int                 passedTxnLimit;

    /*
     * We need to keep the penultimate passed txn so we can readjust the passed
     * transaction information when the matchpoint is found. For example,
     * suppose the log contains: txn A commit txn B commit txn C commit and txn
     * A commit is the matchpoint. The way the loop works, we'll have
     * earliestPassedTxn = txnA, and penultimate = txn B. If the matchpoint is
     * txnA, the log will be truncated at txnB, and we should reset
     * earliestPassedTxn = txnB. Note that numPassedDurableCommits is a subset
     * of numPassedCommits
     */
    ...
    /**
     * If we see a checkpoint end record, see if it is a barrier to rolling
     * back, and advance the file reader position.
     */
     ...
     * adjust the debugging list of passed transactions. The matchpoint entry is
     * just before the truncation point, and does not get truncated.
     */
    ...
    void notePassedCommits(TxnCommit commit, VLSN commitVLSN, long commitLSN) {
        ...
        if ((commitVLSN != null) && !commitVLSN.isNull()) {
            ...
            if (commit.hasLoggedEntries() && ((dtvlsn.getSequence() == VLSN.UNINITIALIZED_VLSN_SEQUENCE)
    ...
    private void processDTVLSN(VLSN txnEndVLSN, final long txnEndDTVLSN) {
        if (dtvlsn.isNull()) {
            /*
             * The first commit/abort record, set the dtvlsn to a non null value
             */
             ...
             */
            if ((txnEndDTVLSN > dtvlsn.getSequence()) && (dtvlsn.getSequence() != VLSN.UNINITIALIZED_VLSN_SEQUENCE)) {
                throw new IllegalStateException("DTVLSNs should only decrease with decreasing VLSNs." + " prev:"
                        + dtvlsn + " next:" + txnEndDTVLSN + " commit VLSN:" + txnEndVLSN);
            }
        }
    }
    ...
    public String toString() {
        return "matchpointLSN=" + DbLsn.getNoFormatString(matchpointLSN) + " passedCkpt=" + passedCheckpointEnd
                + " passedTxnLimit=" + passedTxnLimit + " passedTxns=" + passedTxns + " earliestTxn="
                + earliestPassedTxn + " penultimateTxn=" + penultimatePassedTxn + " numPassedCommits="
                + numPassedCommits + " numPassedDurableCommits=" + numPassedDurableCommits + " passedSkippedGap="
                + passedSkippedGap;
    }

    /**
     * If 1 or more commits was passed, construct a message that can be used by
     * RollbackException and RollbackProhibitedException.
     */
    public String getRollbackMsg() {
        ...
        if (numPassedDurableCommits == 0) {
            return " " + numPassedCommits + " total commits to the earliest point indicated by transaction "
                    + earliestPassedTxn;
        }

        return " " + numPassedCommits + " total commits(" + numPassedDurableCommits + " of which were durable) "
                + "to the earliest point indicated by transaction " + earliestPassedTxn;
    }
    ...
    public static class PassedTxnInfo implements Serializable {
        ...
        @Override
        public String toString() {
            return "id=" + id + " time=" + time + " vlsn=" + vlsn + " lsn=" + DbLsn.getNoFormatString(lsn) + " durable="
                    + durableCommit;
        }
    }
}
