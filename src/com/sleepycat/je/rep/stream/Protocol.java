 ...
 * defines a set of basic message operation codes that are to be used by its
 * subclasses. From Feeder to Replica Heartbeat -> HeartbeatResponse Commit ->
 * Ack Commit+ -> GroupAck Entry ShutdownRequest -> ShutdownResponse Note: in
 * the future, we may want to support bulk entry messages From Replica to Feeder
 * The following subset of messages represents the handshake protocol that
 * precedes the transmission of replication log entries. ReplicaProtocolVersion
 * -> FeederProtocolVersion | DuplicateNodeReject ReplicaJEVersions ->
 * FeederJEVersions | JEVersionsReject NodeGroupInfo -> NodeGroupInfoOK |
 * NodeGroupInfoReject SNTPRequest -> SNTPResponse -> HeartbeatResponse A
 * HeartbeatResponse is not strictly a response message and may also be sent
 ...
 * timed out if the feeder or the replica replay are otherwise busy. Note that
 * there may be multiple SNTPRequest/SNTPResponse message pairs that are
 * exchanged as part of a single handshake. So a successful handshake requested
 * sequence generated by the Replica looks like: ReplicaProtocolVersion
 * ReplicaJEVersions MembershipInfo [SNTPRequest]+ The following messages
 * constitute the syncup and the transmission of log entries. EntryRequest ->
 * Entry | EntryNotFound | AlternateMatchpoint RestoreRequest -> RestoreResponse
 * StartStream The Protocol instance has local state in terms of buffers that
 * are reused across multiple messages. A Protocol instance is expected to be
 * used in strictly serial fashion. Consequently, there is an instance for each
 * Replica to Feeder connection, and two instances per Feeder to Replica
 * connection: one for the InputThread and one for the OutputThread.
 */
public class Protocol extends BaseProtocol {

    /**
     * Normally the op code should fall in the range defined in BaseProtocol,
     * however following op code inherit from original implementation of stream
     * protocol thus we just copy them here for backward compatibility.
     */
    public final static MessageOp    REPLICA_PROTOCOL_VERSION = new MessageOp((short) 1, ReplicaProtocolVersion.class);

    public final static MessageOp    FEEDER_PROTOCOL_VERSION  = new MessageOp((short) 2, FeederProtocolVersion.class);

    public final static MessageOp    DUP_NODE_REJECT          = new MessageOp((short) 3, DuplicateNodeReject.class);

    public final static MessageOp    REPLICA_JE_VERSIONS      = new MessageOp((short) 4, ReplicaJEVersions.class);

    public final static MessageOp    FEEDER_JE_VERSIONS       = new MessageOp((short) 5, FeederJEVersions.class);

    public final static MessageOp    JE_VERSIONS_REJECT       = new MessageOp((short) 6, JEVersionsReject.class);

    public final static MessageOp    MEMBERSHIP_INFO          = new MessageOp((short) 7, NodeGroupInfo.class);

    public final static MessageOp    MEMBERSHIP_INFO_OK       = new MessageOp((short) 8, NodeGroupInfoOK.class);

    public final static MessageOp    MEMBERSHIP_INFO_REJECT   = new MessageOp((short) 9, NodeGroupInfoReject.class);

    public final static MessageOp    SNTP_REQUEST             = new MessageOp((short) 10, SNTPRequest.class);

    public final static MessageOp    SNTP_RESPONSE            = new MessageOp((short) 11, SNTPResponse.class);

    /** All message ops needed by protocol */
    private static final MessageOp[] ALL_MESSAGE_OPS          = { REPLICA_PROTOCOL_VERSION, FEEDER_PROTOCOL_VERSION,
            DUP_NODE_REJECT, REPLICA_JE_VERSIONS, FEEDER_JE_VERSIONS, JE_VERSIONS_REJECT, MEMBERSHIP_INFO,
            MEMBERSHIP_INFO_OK, MEMBERSHIP_INFO_REJECT, SNTP_REQUEST, SNTP_RESPONSE, ENTRY, START_STREAM, HEARTBEAT,
            HEARTBEAT_RESPONSE, COMMIT, ACK, ENTRY_REQUEST, ENTRY_NOTFOUND, RESTORE_REQUEST, RESTORE_RESPONSE,
            ALT_MATCHPOINT, SHUTDOWN_REQUEST, SHUTDOWN_RESPONSE, GROUP_ACK };
     ...
     *            entries
     */
    private Protocol(final RepImpl repImpl, final NameIdPair nameIdPair, final Clock clock, final int protocolVersion,
                     final int maxProtocolVersion, final int streamLogVersion,
                     @SuppressWarnings("unused") final int groupFormatVersion) {

        super(repImpl, nameIdPair, protocolVersion, maxProtocolVersion, streamLogVersion, ALL_MESSAGE_OPS,
     ...
     */
    public static Protocol get(final RepNode repNode, final int protocolVersion, final int maxProtocolVersion) {
        return get(repNode.getRepImpl(), repNode.getNameIdPair(), repNode.getClock(), protocolVersion,
                maxProtocolVersion, repNode.getGroup().getFormatVersion());
    }

    public static Protocol get(final RepImpl repImpl, final NameIdPair nameIdPair, final Clock clock,
                               final int protocolVersion, final int maxProtocolVersion, final int groupFormatVersion) {
        return get(repImpl, nameIdPair, clock, protocolVersion, maxProtocolVersion, LogEntryType.LOG_VERSION,
                groupFormatVersion);
    }
    ...
    public static Protocol get(final RepNode repNode, final int protocolVersion, final int maxProtocolVersion,
        ...
        return get(repNode.getRepImpl(), repNode.getNameIdPair(), repNode.getClock(), protocolVersion,
                maxProtocolVersion, streamLogVersion, repNode.getGroup().getFormatVersion());

    }

    public static Protocol get(final RepImpl repImpl, final NameIdPair nameIdPair, final Clock clock,
                               final int protocolVersion, final int maxProtocolVersion, final int streamLogVersion,
        ...
        return get(repImpl, nameIdPair, clock, protocolVersion, minProtocolVersion, maxProtocolVersion,
                streamLogVersion, groupFormatVersion);
    }

    /**
     * Returns a protocol object using the specified minimum and maximum values,
     * returning null if no supported version is found. Use this method for
     * testing when the RepGroupImpl object is not available.
     */
    static Protocol get(final RepNode repNode, final int protocolVersion, final int minProtocolVersion,
                        final int maxProtocolVersion, final int streamLogVersion) {
        ...
        return get(repNode.getRepImpl(), repNode.getNameIdPair(), repNode.getClock(), protocolVersion,
                minProtocolVersion, maxProtocolVersion, streamLogVersion, formatVersion);
    }

    private static Protocol get(final RepImpl repImpl, final NameIdPair nameIdPair, final Clock clock,
                                final int protocolVersion, final int minProtocolVersion, final int maxProtocolVersion,
                                final int streamLogVersion, final int groupFormatVersion) {
         ...
         */
        return new Protocol(repImpl, nameIdPair, clock, protocolVersion, maxProtocolVersion, streamLogVersion,
                groupFormatVersion);
    }
    ...
    static Protocol getProtocol(final RepNode repNode, final int protocolVersion) {
        ...
        return getProtocol(repNode.getRepImpl(), repNode.getNameIdPair(), repNode.getClock(), protocolVersion,
                formatVersion);
    }

    static Protocol getProtocol(final RepImpl repImpl, final NameIdPair nameIdPair, final Clock clock,
                                final int protocolVersion, final int groupFormatVersion) {

        return new Protocol(repImpl, nameIdPair, clock, protocolVersion, protocolVersion, LogEntryType.LOG_VERSION,
                groupFormatVersion);
    }
     ...
     *         of the protocol
     */
    private static boolean isSupportedVersion(final int protocolVersion, final int minProtocolVersion,
    ...
    /**
     * The replica sends the feeder its protocol version. IMPORTANT: This
    ...
    /**
     * The feeder sends the replica its proposed version. IMPORTANT: This
     * message must not change.
     */
    ...
    public class SNTPResponse extends HandshakeMessage {
        ...
        /*
         * Initialized when the message is serialized to ensure it's as accurate
         * as possible.
         */
    ...
    public class NodeGroupInfo extends HandshakeMessage {
        ...
        /**
         * A string version of the JE version running on this node, or the empty
         * string if not known.
         */
        private final String     jeVersion;

        NodeGroupInfo(final String groupName, final UUID uuid, final NameIdPair nameIdPair, final String hostName,
                      final int port, final NodeType nodeType, final boolean designatedPrimary,
        ...
        public ByteBuffer wireFormat() {
            ...
            if (!repGroupV3 && nodeType.compareTo(NodeType.ELECTABLE) > 0) {
                throw new IllegalStateException("Node type not supported before group version 3: " + nodeType);
            }
     ...
     * contains the group's UUID and the replica's NameIdPair. The group UUID is
     * used to update the replica's notion of the group UUID on first joining.
     * The NameIdPair is used to update the replica's node ID for a secondary
     * node, which is not available in the RepGroupDB.
     */
