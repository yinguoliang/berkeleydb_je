 ...
 *
 * @see <a href=
 *      "https://sleepycat.oracle.com/pmwiki/pmwiki.php?n=JEHomePage.ReplicaFeederHandshake">
 *      FeederReplicaHandshake</a>
 */
public class FeederReplicaHandshake {
    ...
    /**
     * The negotiated highest version associated with log records to be sent by
     * this feeder in the HA stream.
     */
    ...
    /**
     * Returns the replica node ID. The returned value is only valid after the
     * handshake has been executed.
     *
    ...
    private int getCurrentLogVersion() {
        return (testCurrentLogVersion != 0) ? testCurrentLogVersion : LogEntryType.LOG_VERSION_HIGHEST_REPLICABLE;
    }
     ...
     * the replica. This check requires the log version of the replicas to be
     * greater than or equal to
     * {@value LogEntryType#LOG_VERSION_REPLICATE_OLDER} . Allowing the replica
     * version to be behind the feeder version supports replication during
     * upgrades where the feeder is upgraded before the replica. [#22336] This
     * check also requires that the JE version of the replica is at least the
     * minJEVersion specified by the RepGroupImpl, if any. This check makes sure
     * that nodes running an older software version cannot join the group after
     * a new and incompatible feature has been used.
     */
    private JEVersionsReject checkJECompatibility(final Protocol protocol, final JEVersions jeVersions) {
        ...
        if (replicaLogVersion < LogEntryType.LOG_VERSION_REPLICATE_OLDER - 1) {
            return protocol.new JEVersionsReject(
                    "Incompatible log versions. " + "Feeder log version: " + getCurrentLogVersion()
                            + ", Feeder JE version: " + getCurrentJEVersion() + ", Replica log version: "
                            + replicaLogVersion + ", Replica JE version: " + jeVersions.getVersion());
        }

        final JEVersion minJEVersion = repNode.getGroup().getMinJEVersion();
        if (minJEVersion.compareTo(jeVersions.getVersion()) > 0) {
            return protocol.new JEVersionsReject("Unsupported JE version. " + "Feeder JE version: "
                    + getCurrentJEVersion() + ", Feeder min JE version: " + minJEVersion + ", Replica JE version: "
    ...
    public Protocol execute() throws DatabaseException, IOException, ProtocolException, ExitException {
        ...
        final String versionMsg = " Replica " + replicaNameIdPair.getName() + " Versions" + " JE: "
                + replicaJEVersions.getVersion().getVersionString() + " Log: " + replicaJEVersions.getLogVersion()
        ...
        if (reject != null) {
            final String msg = "Version incompatibility: " + reject.getErrorMessage() + " with replica "
        ...
        /*
         * Use the minimum common log version. This ensures that the feeder can
         * generate it and that the replica can understand it.
         */
        streamLogVersion = Math.min(getCurrentLogVersion(), replicaJEVersions.getLogVersion());

        writeMessage(protocol, protocol.new FeederJEVersions(getCurrentJEVersion(), streamLogVersion));
        ...
        LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder-replica " + replicaNameIdPair.getName()
                + " handshake completed." + versionMsg + " Stream Log: " + protocol.getStreamLogVersion());
    ...
    private void verifyMembershipInfo(Protocol protocol) throws IOException, DatabaseException, ExitException {
        ...
        try {
            ...
            if (nodeGroup.getNodeId() != replicaNameIdPair.getId()) {
                throw new ExitException("The replica node ID sent during protocol negotiation: " + replicaNameIdPair
                        + " differs from the one sent in the MembershipInfo " + "request: " + nodeGroup.getNodeId());
            }
            ...
            if (nodeGroup.getNodeType().hasTransientId()) {
                 ...
                 * Otherwise, fall through, and the subsequent code will notice
                 * the incompatible node type.
                 */
                if (node == null) {
                ...
                if (nodeGroup.getNodeType().isArbiter()) {
                    ...
                    if (arbMembers.size() > 0) {
                        throw new ExitException("An Arbiter node already exists in the " + "replication group.");
                    }
                }
                try {

                    /*
                     * It is possible that the thread that added the node to the
                     * group database has not added the member to the in memory
                     * representation. We retry several times to give the other
                     * thread a chance to update the in memory structure.
                     */
                    ...
                    if (node == null) {
                        throw EnvironmentFailureException
                                .unexpectedState("Node: " + nodeGroup.getNameIdPair() + " not found");
                    }
                ...
                throw new ExitException("Node: " + nodeGroup.getNameIdPair() + " is no longer a member of the group."
     ...
     *
     * @throws ExitException if the configuration in the group db differs from
     *             the supplied config
     */
    private void doGroupChecks(NodeGroupInfo nodeGroup, final RepGroupImpl group) throws ExitException {
        ...
        if (nodeGroup.isDesignatedPrimary() && repNode.getRepImpl().isDesignatedPrimary()) {
            throw new ExitException(
                    "Conflicting Primary designations. Feeder node: " + repNode.getNodeName() + " and replica node: "
                            + nodeGroup.getNodeName() + " cannot simultaneously be designated primaries");
        }

        if (!nodeGroup.getGroupName().equals(group.getName())) {
            throw new ExitException("The feeder belongs to the group: " + group.getName() + " but replica id"
                    + replicaNameIdPair + " belongs to the group: " + nodeGroup.getGroupName());
        }

        if (!RepGroupImpl.isUnknownUUID(nodeGroup.getUUID()) && !nodeGroup.getUUID().equals(group.getUUID())) {
            throw new ExitException("The environments have the same name: " + group.getName()
                    + " but represent different environment instances." + " The environment at the master has UUID "
                    + group.getUUID() + ", while the replica " + replicaNameIdPair.getName() + " has UUID: "
                    + nodeGroup.getUUID());
        }
    }
    ...
    private void doNodeChecks(NodeGroupInfo nodeGroup, RepNodeImpl node) throws ExitException {
        ...
        if (!nodeGroup.getHostName().equals(node.getHostName())) {
            throw new ExitException(
                    "Conflicting hostnames for replica id: " + replicaNameIdPair + " Feeder thinks it is: "
                            + node.getHostName() + " Replica is configured to use: " + nodeGroup.getHostName());
        }

        if (nodeGroup.port() != node.getPort()) {
            throw new ExitException(
                    "Conflicting ports for replica id: " + replicaNameIdPair + " Feeder thinks it uses: "
                            + node.getPort() + " Replica is configured to use: " + nodeGroup.port());
        }

        if (!((NodeType.ELECTABLE == node.getType()) || (NodeType.SECONDARY == node.getType())
                || (NodeType.EXTERNAL == node.getType()) || (NodeType.ARBITER == node.getType())
        ...
        if (!nodeGroup.getNodeType().equals(node.getType())) {
            throw new ExitException("Conflicting node types for: " + replicaNameIdPair + " Feeder thinks it uses: "
                    + node.getType() + " Replica is configured as type: " + nodeGroup.getNodeType());
        }
        replicaNode = node;
    }
     ...
     */
    private void maybeUpdateJEVersion(final NodeGroupInfo nodeGroup, final RepGroupImpl group, final RepNodeImpl node)
            throws ExitException {

        if ((group.getFormatVersion() >= RepGroupImpl.FORMAT_VERSION_3) && (nodeGroup.getJEVersion() != null)
             ...
             * may mean that a quorum is not available. Saving the JE version is
             * only an optimization, so it is OK if this attempt fails to be
             * persistent.
             */
            try {
    ...
    private Protocol negotiateProtocol() throws IOException, ExitException {
        ...
        if ((dup != null) || (message.getNameIdPair().getName().equals(feederNameIdPair.getName()))) {
            ...
            writeMessage(defaultProtocol, defaultProtocol.new DuplicateNodeReject(
                    "This node: " + replicaNameIdPair + " is already in active use at the feeder "));
            SocketAddress dupAddress = namedChannel.getChannel().getSocketChannel().socket().getRemoteSocketAddress();

            throw new ExitException("A replica with the id: " + replicaNameIdPair
                    + " is already active with this feeder. " + " The duplicate replica resides at: " + dupAddress);
        }
