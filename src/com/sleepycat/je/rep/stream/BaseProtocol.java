...
/**
 * Defines the base protocol of messages used to set up a stream between source
 * and target. Note BaseProtocol represents a set of basic protocol operations
 * intended to be used by subclasses. For a complete description of message
 * operations used in JE HA protocol, please see the Protocol class in the same
 * package.
 *
...
public abstract class BaseProtocol extends BinaryProtocol {
    ...
    /*
     * Note that the GROUP_ACK response message was introduced in version 5, but
     * is disabled by default via RepParams.REPLICA_ENABLE_GROUP_ACKS. It can be
     * enabled when we can increase the protocol version number.
     */
    ...
    /*
     * Following ops are core replication stream post-handshake messages defined
     * in streaming protocol and are intended to be used in subclasses. Note
     * these msg op codes inherit from original implementation of stream
     * protocol. Due to backward compatibility requirement, we keep them
     * unchanged and directly copy them here.
     */
    public final static MessageOp ENTRY                           = new MessageOp((short) 101, Entry.class);

    public final static MessageOp START_STREAM                    = new MessageOp((short) 102, StartStream.class);

    public final static MessageOp HEARTBEAT                       = new MessageOp((short) 103, Heartbeat.class);

    public final static MessageOp HEARTBEAT_RESPONSE              = new MessageOp((short) 104, HeartbeatResponse.class);

    public final static MessageOp COMMIT                          = new MessageOp((short) 105, Commit.class);

    public final static MessageOp ACK                             = new MessageOp((short) 106, Ack.class);

    public final static MessageOp ENTRY_REQUEST                   = new MessageOp((short) 107, EntryRequest.class);

    public final static MessageOp ENTRY_NOTFOUND                  = new MessageOp((short) 108, EntryNotFound.class);

    public final static MessageOp ALT_MATCHPOINT                  = new MessageOp((short) 109,
            AlternateMatchpoint.class);

    public final static MessageOp RESTORE_REQUEST                 = new MessageOp((short) 110, RestoreRequest.class);

    public final static MessageOp RESTORE_RESPONSE                = new MessageOp((short) 111, RestoreResponse.class);

    public final static MessageOp SHUTDOWN_REQUEST                = new MessageOp((short) 112, ShutdownRequest.class);

    public final static MessageOp SHUTDOWN_RESPONSE               = new MessageOp((short) 113, ShutdownResponse.class);

    public final static MessageOp GROUP_ACK                       = new MessageOp((short) 114, GroupAck.class);
     ...
     * <p>
     * This field is only set to true by the replica, which only reads and
     ...
     */
    BaseProtocol(final RepImpl repImpl, final NameIdPair nameIdPair, final int protocolVersion,
                 final int maxProtocolVersion, final int streamLogVersion, final MessageOp[] protocolOps,
    ...
    /**
     * Returns a BaseProtocol object configured that implements the specified
     * (supported) protocol version, with enforced message operation code
     * validity check. This constructor enforces checking validity of message
     * operation code. It should be used in any subclass except the legacy HA
     * protocol in je.stream.Protocol.
     *
     * @param repImpl the node using the protocol
     * @param nameIdPair name-id pair of the node using the protocol
     * @param protocolVersion the version of the protocol that must be
     ...
     * @param protocolOps the message operations that make up this protocol
     */
    protected BaseProtocol(final RepImpl repImpl, final NameIdPair nameIdPair, final int protocolVersion,
                           final int maxProtocolVersion, final int streamLogVersion, final MessageOp[] protocolOps) {
    ...
    /**
     * StartStream indicates that the replica would like the feeder to start the
     * replication stream at the proposed vlsn.
     */
            ...
            try {
                ...
                if (ois != null) {
                    try {
                        ...
                        logger.finest("exception raised when closing the " + "object input stream object "
                ...
                try {
                    ...
                    if (oos != null) {
                        try {
                            ...
                            logger.finest("exception raised when closing the " + "object output stream object "
                                    + e.getLocalizedMessage());
                        }
                    }
                }
    ...
    public class Commit extends Entry {
        ...
        public Commit(final boolean needsAck, final SyncPolicy replicaSyncPolicy, final OutputWireRecord wireRecord) {
            super(wireRecord);
        ...
        @Override
        public ByteBuffer wireFormat() {
            final int bodySize = super.getWireSize() + 1 /* needsAck */
        ...
        private Commit(final boolean needsAck, final SyncPolicy replicaSyncPolicy, final ByteBuffer buffer)
     ...
     * for the replica, which will be registered as this replica's local CBVLSN.
     * This will contribute to the global CBVLSN calculation. The feeder when
     * sending this response must, if it is also the master, update the
     * membership table to set the local CBVLSN for the requesting node thus
     * ensuring that it can continue the replication stream at this VLSN (or
     * higher) when it retries the syncup operation.
     */
    public class RestoreResponse extends SimpleMessage {
        ...
        private int getGroupFormatVersion() {
            return (getVersion() < VERSION_5) ? RepGroupImpl.FORMAT_VERSION_2 : RepGroupImpl.MAX_FORMAT_VERSION;
        }
     ...
     * format was changed.
     */
    protected void writeOutputWireRecord(final OutputWireRecord record, final ByteBuffer messageBuffer) {
            ...
            for (MessageOp op : protocolOps) {
                if (!isValidMsgOpCode(op.getOpId())) {
                    throw EnvironmentFailureException
                            .unexpectedState("Op id: " + op.getOpId() + " is out of allowed range inclusively ["
                                    + MIN_MESSAGE_OP_CODE_IN_SUBCLASS + ", " + MAX_MESSAGE_OP_CODE_IN_SUBCLASS + "]");
                }
            }
        }
