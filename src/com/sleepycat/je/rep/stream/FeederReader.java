...
/**
 * The FeederReader is a flavor of VLSNReader which supports replication stream
 * feeding. It assumes that reading will always go forward in the log. Special
 * features are: - The reader can read either from a log buffer or from the
 * file. Sometimes log entries are logged but are not yet available on disk. In
 * general, it's better to read from the log buffers rather then the file. - The
 * reader can block for a given time period, waiting for the next vlsn to appear
 */
public class FeederReader extends VLSNReader {
    ...
    /*
     * A constantly resetting counter of hits in the log item cache. This serves
     * as state that lets the FeederReader know that its position in the log
     * files might have become stale, due to cache hits. Because it's reset
     ...
     * will not use the vlsnIndex LogItem cache. Should only be used for unit
     * tests!
     */
    private final boolean            bypassCache;

    public FeederReader(EnvironmentImpl envImpl, VLSNIndex vlsnIndex, long startLsn, int readBufferSize,
                        NameIdPair nameIdPair) {

        this(envImpl, vlsnIndex, startLsn, readBufferSize, nameIdPair, false /* bypassCache */);
    }
    ...
    FeederReader(EnvironmentImpl envImpl, VLSNIndex vlsnIndex, long startLsn, int readBufferSize) {
        ...
        this(envImpl, vlsnIndex, startLsn, readBufferSize, new NameIdPair("test Node", 0), true /* bypassCache */);
    }

    private FeederReader(EnvironmentImpl envImpl, VLSNIndex vlsnIndex, long startLsn, int readBufferSize,
                         NameIdPair nameIdPair, boolean bypassCache) {
        super(envImpl, vlsnIndex, true, // forward
                startLsn, readBufferSize, nameIdPair, DbLsn.NULL_LSN); // finishLsn
     ...
     * the constructor, because the Feeder is set up to require the construction
     * of the source before we know where to start.
     *
     * @throws IOException
     */
     ...
     * specified by "waitTime". Where possible, the FeederReader fetches the log
     * record from the cache within the VLSNIndex. (See the VLSNIndex for a
     * description of this two level cache). If the requested VLSN is not
     * available from the cache, the reader fetches the item from the JE log --
     * either from the log buffers or from disk. The FeederReader is like a
     * cursor on the log, and retains a position in the log. When there are log
     * item cache hits, the FeederReader's position can fall behind, because it
     * is being bypassed. It is possible for log cleaning to take place between
     * the point of the FeederReader's stale position and the end of the log. If
     * so, the FeederReader must not attempt to scan from its current position,
     * because it might run afoul of gap created by the cleaned and delete log
     * files. When there have been log item cache hits, the FeederReader must
     * jump its position forward using the vlsnIndex mappings to safely skip
     * over any cleaned gaps in the log.
     */
    public OutputWireRecord scanForwards(VLSN vlsn, int waitTime) throws InterruptedException {
         ...
         * entry. If there are no cache hits and the reader has been supplying
         * log entries sequentially, we know that it is already positioned at
         * the immediately preceding log entry and that we can scan from there
         * to the current requested record. In that case, we are only hoping
         * that the VLSNIndex can supply the exact location of the current
         * requested record in order to reduce the scanning. If there have been
         * cache hits, the reader's current position is some unknown distance
         * back. In that case, scanning from the current position could run into
         * a cleaned gap in the log files, and could fail. Because of that, we
         * must reposition to a VLSN that is <= to the current requested VLSN.
         * We know that such a VLSN must exist and have a valid lsn mapping,
         * because the begin and end point in the vlsn range always exists.
         */
        long repositionLsn;
        if (prevCacheHits > 0) {
             ...
             * at a point in the log that is before the current target VLSN, but
             * after any available mappings. For example, suppose the VLSNIndex
             * has VLSNs 10, 50, 100. Suppose the reader is positioned at VLSN
             * 20, and we have supplied VLSNs 21->40 from the cache. VLSN 41 has
             * not hit in the cache, and we must fetch the log record from disk.
             * We do not want to slide the FeederReader from its current
             * position at 21 back to VLSN 10.
             */
        ...
        try {
            ...
            throw new EnvironmentFailureException(envImpl, EnvironmentFailureReason.LOG_CHECKSUM,
                    "trying to reposition FeederReader to " + DbLsn.getNoFormatString(repositionLsn) + " prevWindow="
                            + window,
                    e);
             ...
             */
            throw new LogFileNotFoundException(vlsn, "Trying to reposition FeederReader to "
                    + DbLsn.getNoFormatString(repositionLsn) + " for vlsn:" + vlsn + " prevWindow=" + window, e);
        }
    ...
    static class SwitchWindow extends ReadWindow {
        ...
        /*
         * Reposition to the specified file, and fill starting at targetOffset.
         * For this use case, we are always going forwards, and
        ...
        private boolean fillFromLogBuffer(long windowFileNum, long targetOffset) throws DatabaseException {
            ...
            try {
                ...
                /*
                 * Put the logBuffer's contents into wholeContents, and position
                 * wholeContents at the desired target offset. If this logBuffer
                 * had been the currentWriteBuffer, it's positioned for writing
                 * and must be flipped for reading.
                 */
                ...
                int copyLength = (availableContentLen > readBuffer.capacity()) ? readBuffer.capacity()
                        : availableContentLen;
         ...
         * file (next largest number) if needed. Unlike other file readers, we
         * are reading log files that are concurrently growing, so this read
         * window must also know to look in the log buffers. The contract
         * between the feeder reader and the VLSNIndex lets us assume that the
         * feeder reader is only active when it is sure that there is more data
         * available somewhere -- whether it's in the log buffers, write queue,
         * or on disk.
         *
        ...
        protected boolean fillNext(boolean singleFile, int bytesNeeded)
            ...
            /*
             * Try to fill the window by asking for the next offset from the log
             * buffers.
             */
            ...
            /*
             * If that didn't work, there are these possible reasons why: a -
             * it's a valid offset, but it's no longer in a log buffer, it was
             * written to disk. b - it's not a valid offset, because the log
             * file flipped. In both cases, go to the FileManager and see if
             * there's more log to be found.
             */
            ...
            try {
                ...
                /*
                 * Remember that the nextFile may not be fileNum + 1 if there
                 * has been log cleaning.
                 */
                ...
                if (fillFromLogBuffer(nextFile, FileManager.firstLogEntryOffset())) {
                    /*
                     * We filled the read buffer, and jumped to a new file.
                     */
                    return true;
                }
                ...
                e.printStackTrace();
                throw EnvironmentFailureException
                        .unexpectedException("Problem in ReadWindow.fill, reading from  = " + currentFileNum(), e);
     ...
     * found during a forward scan of the feeder reader. This is to be expected
