...
/**
 * Format for log entries sent across the wire for replication. In most cases,
 * the bytes are read directly from the log and never need to be serialized into
 * the backing object. Note that the ByteBuffer held within the OutputWireRecord
 * has a limited lifetime. Often it's just sliced, rather than copied from the
 * underlying buffer.
 */
public class OutputWireRecord extends WireRecord {
     ...
     */
    OutputWireRecord(final EnvironmentImpl envImpl, final LogEntryHeader header, final ByteBuffer readerBuffer) {
        super(header);
    ...
    OutputWireRecord(final EnvironmentImpl envImpl, final LogItem logItem) {
        ...
        assert entryBuffer.limit() == header.getItemSize() : "Limit:" + entryBuffer.limit() + " size:"
                + header.getItemSize();
    }
        ...
        if (sharedEntry == null) {
            ...
            if (!entryType.isReplicationPossible()) {
                throw EnvironmentFailureException
                        .unexpectedState("Log entry type does not support replication: " + entryType);
            }
     ...
     * entry will only be created once, at most. When a LogItem from the
     * LogItemCache was used to construct this record, we cache the materialized
     * entry in LogItem to try to avoid redundant materialization in multiple
     * feeders.
     */
    public synchronized ReplicableLogEntry instantiateEntry() throws DatabaseException {
        ...
        if (!(entry instanceof ReplicableLogEntry)) {
            throw EnvironmentFailureException
                    .unexpectedState("Log entry type does not support replication: " + entry.getClass().getName());
        }
     ...
     *         the InputWireRecord. The comparison will disregard portions of
     *         the logEntry that may be different, such at timestamps on a
     *         Commit entry. Must be called before the entryBuffer that backs
     *         this OutputWireRecord is reused.
     ...
     * should be used. This method caches its result, so that it can be called
     * twice (from getEntrySize and writeToWire) and will not repeat the
     * calculation or return different results. If true is returned,
     * reSerializedSize will also be set.
     */
    private boolean willReSerialize(final int logVersion) {
        ...
        if (header.getVersion() < 8) {
            ...
            if (logEntry != null) {
                ...
                /*
                 * If we must materialize the entry in order to re-serialize it,
                 * then we must make a best guess about whether this is
                 * worthwhile by examining the entry in serialized format.
                 */
                reserialize = getSharedEntry().isReplicationFormatWorthwhile(entryBuffer, header.getVersion(),
                        logVersion);
            }
        }
    ...
    private boolean isOldFormatRequired(final int logVersion) {
        ...
        oldFormatRequired =
                ...
                /* it is older than the entry version, */
                        ...
                        /*
                         * and it is older than the entry class's last format
                         * change
                         */
    ...
    /**
     * Returns the ID of the database containing the associated entry if it is a
     * replicable entry. Returns null for non-replicable entries and for entries
     ...
     * numberspace. It seems a little fragile to test this here, using
     * instanceof to decide what to test. It would be cleaner to put this
     * validity check as part of the implementation of a log entry. But this is
     * a HA related check, and we want to keep the core code as independent as
     * possible. The check is here rather than in some other test code because
     * it needs to access the deserialized log entry. We don't want to provide a
     * method which returns a logEntry, because in general an OutputWireRecord
     * should not instantiate the log entry.
