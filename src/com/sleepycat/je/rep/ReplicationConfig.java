 ...
 * {@link ReplicatedEnvironment} construction. The set methods of this class
 * validate the configuration values when the method is invoked. An
 * IllegalArgumentException is thrown if the value is not valid for that
 * attribute.
 * <p>
 * Note that ReplicationConfig only describes those attributes which must be set
 * at {@code ReplicatedEnvironment} construction time, while its superclass
 ...
 * <ol>
 * <li>Configuration parameters specified in {@literal 
 * <environmentHome>/je.properties} take first precedence.</li>
 * <li>Configuration parameters set in the ReplicationConfig object used at
 * {@code ReplicatedEnvironment} construction are next.</li>
 ...
 * <h4>Getting the Current ReplicatedEnvironment Properties</h4> To get the
 * current "live" properties of a replicated environment after constructing it
 * or changing its properties, you must call
 * {@link ReplicatedEnvironment#getRepConfig} or
 * {@link ReplicatedEnvironment#getRepMutableConfig}. The original
 * ReplicationConfig or ReplicationMutableConfig object used to set the
 * properties is not kept up to date as properties are changed, and does not
 * reflect property validation or properties that are computed.
 */
public class ReplicationConfig extends ReplicationMutableConfig implements RepConfigProxy {
    ...
    /**
     * The name for the replication group. The name should consist of letters,
     * digits, and/or hyphen ("-"), underscore ("_"), or period (".").
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     */
    public static final String                         GROUP_NAME                    = EnvironmentParams.REP_PARAM_PREFIX
            + "groupName";
     ...
     * <p>
     * Note that the node name is immutable. Normally the host name should not
     * be used as the node name, unless you intend to reuse the host name when a
     * machine fails and is replaced, or the node is upgraded to new hardware.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     */
    public static final String                         NODE_NAME                     = EnvironmentParams.REP_PARAM_PREFIX
            + "nodeName";
     ...
     */
    public static final String                         NODE_TYPE                     = EnvironmentParams.REP_PARAM_PREFIX
            + "nodeType";
     ...
     */
    @Deprecated
    public static final String                         HELPER_HOSTS                  = EnvironmentParams.REP_PARAM_PREFIX
            + "helperHosts";
     ...
     * </p>
     */
    public static final String                         DEFAULT_PORT                  = EnvironmentParams.REP_PARAM_PREFIX
            + "defaultPort";

    /**
     * Names the hostname and port associated with this node in the replication
     * group, e.g. je.rep.nodeHostPort=foo.com:5001.
     * <p>
     * The hostname is defaulted to "localhost" to make it easy to prototype and
     * to execute the examples, but the user should be very sure to set a
     ...
     */
    public static final String                         NODE_HOST_PORT                = EnvironmentParams.REP_PARAM_PREFIX
            + "nodeHostPort";
     ...
     * </p>
     */
    public static final String                         BIND_INADDR_ANY               = EnvironmentParams.REP_PARAM_PREFIX
            + "bindInaddrAny";

    /**
     * The default consistency policy used by a replica. Only two policies are
     * meaningful as properties denoting environment level default policies:
     * {@link NoConsistencyRequiredPolicy} and {@link TimeConsistencyPolicy}.
     ...
     * 
     * <pre>
     *   {@code TimeConsistencyPolicy(<permissibleLag>,<timeout>)}
     * </pre>
     * 
     * where {@code <permissibleLag>} and {@code <timeout>} are
     * {@link <a href="../EnvironmentConfig.html#timeDuration">Time Duration
     * Properties</a>}.
     * <p>
     * For example, a time based consistency policy with a lag of one second and
     * a timeout of one hour is denoted by the string:
     ...
     *      Properties</a>
     */
    public static final String                         CONSISTENCY_POLICY            = EnvironmentParams.REP_PARAM_PREFIX
            + "consistencyPolicy";
     ...
     *      Properties</a>
     */
    public static final String                         REP_STREAM_TIMEOUT            = EnvironmentParams.REP_PARAM_PREFIX
            + "repStreamTimeout";

    /**
     * The cost of replaying the replication stream as compared to the cost of
     * performing a network restore, represented as a percentage. Specifies the
     * relative cost of using a log file as the source of transactions to replay
     * on a replica as compared to using the file as part of a network restore.
     * This parameter is used to determine whether a cleaned log file that could
     * be used to support replay should be removed because a network restore
     * would be more efficient. The value is typically larger than 100, to
     * represent that replay is usually more expensive than network restore for
     * a given amount of log data due to the cost of replaying transactions. If
     * the value is 0, then the parameter is disabled, and no log files will be
     * retained based on the relative costs of replay and network restore.
     * <p>
     * Note that log files are always retained if they are known to be needed to
     * support replication for electable replicas that have been in contact with
     * the master within the {@link #REP_STREAM_TIMEOUT} period, or by any
     * replica currently performing replication. This parameter only applies to
     * the retention of additional files that might be useful to secondary nodes
     * that are out of contact, or to electable nodes that have been out of
     * contact for longer than REP_STREAM_TIMEOUT.
     * </p>
     * <p>
     * To disable the retention of these additional files, set this parameter to
     * zero.
     * </p>
     * <p>
     * <table border="1">
     ...
     */
    public static final String                         REPLAY_COST_PERCENT           = EnvironmentParams.REP_PARAM_PREFIX
            + "replayCostPercent";
     ...
     * prevents the retention of log files as determined by the
     * {@link #REPLAY_COST_PERCENT} parameter if retaining the files would
     * reduce free space below the specified percentage. If the value is 0, then
     * the parameter is disabled, and decisions about which log files to remove
     ...
     * </p>
     * <p>
     * To disable the retention of these additional files, set
     * {@link #REPLAY_COST_PERCENT} to zero.
     * </p>
     * <p>
     * <table border="1">
     * <tr>
     ...
     */
    public static final String                         REPLAY_FREE_DISK_PERCENT      = EnvironmentParams.REP_PARAM_PREFIX
            + "replayFreeDiskPercent";
     ...
     *      Properties</a>
     */
    public static final String                         REPLAY_TXN_LOCK_TIMEOUT       = EnvironmentParams.REP_PARAM_PREFIX
            + "replayTxnLockTimeout";
     ...
     *             {@link ReplicationMutableConfig#REPLAY_MAX_OPEN_DB_HANDLES}.
     */
    @Deprecated
     ...
     *      Properties</a>
     * @deprecated replaced by
     *             {@link ReplicationMutableConfig#REPLAY_DB_HANDLE_TIMEOUT}.
     */
    @Deprecated
     ...
     *      Properties</a>
     */
    public static final String                         ENV_CONSISTENCY_TIMEOUT       = EnvironmentParams.REP_PARAM_PREFIX
            + "envConsistencyTimeout";
     ...
     * two node group, if this node is the designated <code>Primary</code>, the
     * <code>Primary</code> will be <code>activated</code>, and the
     ...
     */
    public static final String                         REPLICA_ACK_TIMEOUT           = EnvironmentParams.REP_PARAM_PREFIX
            + "replicaAckTimeout";

    /**
     * @hidden The amount of time that the
     *         {@link com.sleepycat.je.Transaction#commit(com.sleepycat.je.Durability)}
     *         on the Master will wait for acknowledgments from an Arbiter. This
     *         wait occurs after waiting for the REPLICA_ACK_TIMEOUT period and
     *         not receiving the required number of acknowledgments. If the
     *         Master does not receive a sufficient number of acknowledgments
     *         within this timeout period, the <code>commit()</code> will throw
     ...
     *      Properties</a>
     */
    public static final String                         ARBITER_ACK_TIMEOUT           = EnvironmentParams.REP_PARAM_PREFIX
            + "arbiterAckTimeout";
     ...
     * on the Master will wait for a sufficient number of electable Replicas, as
     ...
     * <code>beginTransaction()</code> will proceed normally instead of throwing
     * an exception.
     * <p>
     * <table border="1">
     * <tr>
     ...
     * @see ReplicationMutableConfig#DESIGNATED_PRIMARY
     */
    public static final String                         INSUFFICIENT_REPLICAS_TIMEOUT = EnvironmentParams.REP_PARAM_PREFIX
     ...
     * </p>
     */
    public static final String                         MAX_MESSAGE_SIZE              = EnvironmentParams.REP_PARAM_PREFIX
            + "maxMessageSize";
     ...
     *      Properties</a>
     */
    public static final String                         MAX_CLOCK_DELTA               = EnvironmentParams.REP_PARAM_PREFIX
            + "maxClockDelta";
     ...
     * @see ReplicationMutableConfig#DESIGNATED_PRIMARY
     */
    public static final String                         ELECTIONS_PRIMARY_RETRIES     = EnvironmentParams.REP_PARAM_PREFIX
            + "electionsPrimaryRetries";
     ...
     * the partition containing a minority of electable nodes, and the side with
     * the majority has elected a new master. Rebroadcasting election results on
     * a periodic basis ensures that the obsolete master is brought up to date
     * after the network partition has been resolved. As a result of the update,
     * the environment at the obsolete master will transition into a replica
     * state.
     * <p>
     ...
     * </p>
     */
    public static final String                         ELECTIONS_REBROADCAST_PERIOD  = EnvironmentParams.REP_PARAM_PREFIX
            + "electionsRebroadcastPeriod";
     ...
     * transactions that were successfully committed with a durability requiring
     * acknowledgments from at least a simple majority of nodes. If the number
     * of durable committed transactions targeted for rollback exceeds this
     * parameter, a {@link RollbackProhibitedException} will be thrown.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     */
    public static final String                         TXN_ROLLBACK_LIMIT            = EnvironmentParams.REP_PARAM_PREFIX
            + "txnRollbackLimit";
     ...
     * false timeouts, if the network is experiencing transient problems, or the
     * Java GC is responsible for long pauses. In the latter case, it's
     ...
     * @since 4.0.100
     */
    public static final String                         FEEDER_TIMEOUT                = EnvironmentParams.REP_PARAM_PREFIX
            + "feederTimeout";
     ...
     * promptly in some circumstances and the overall time needed to failover to
     * a new master will be reduced. However, it increases the chances of false
     * timeouts, if the network is experiencing transient problems, or the Java
     * GC is responsible for long pauses. In the latter case, it's generally
     * better to tune the GC to avoid such pauses.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * @since 4.0.100
     */
    public static final String                         REPLICA_TIMEOUT               = EnvironmentParams.REP_PARAM_PREFIX
            + "replicaTimeout";

    /**
     * The size of the the TCP receive buffer associated with the socket used by
     * the replica to transfer the replication stream.
     * <p>
     ...
     * will be limited to this maximum value. For example, on Linux you may need
     * to set the kernel parameter: net.core.rmem_max property using the
     ...
     * @since 5.0.37
     */
    public static final String                         REPLICA_RECEIVE_BUFFER_SIZE   = EnvironmentParams.REP_PARAM_PREFIX
            + "replicaReceiveBufferSize";
     ...
     * <p>
     * The {@link ReplicatedEnvironmentStats#getNReplayGroupCommitMaxExceeded()}
     ...
     */
    public static final String                         REPLICA_MAX_GROUP_COMMIT      = EnvironmentParams.REP_PARAM_PREFIX
            + "replicaMaxGroupCommit";
     ...
     * that commit throughput could be improved by increasing the time interval.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     */
    public static final String                         REPLICA_GROUP_COMMIT_INTERVAL = EnvironmentParams.REP_PARAM_PREFIX
            + "replicaGroupCommitInterval";

    /**
     * The maximum amount of time for the internal housekeeping, like elections,
     * syncup with the master, etc. to be accomplished when opening a new handle
     * to an environment.
     * <p>
     * This timeout does not encompass the time spent making the node consistent
     * with the master, if it is a Replica. The timeout associated with making a
     * replica consistent is normally determined by the
     ...
     *      Properties</a>
     */
    public static final String                         ENV_SETUP_TIMEOUT             = EnvironmentParams.REP_PARAM_PREFIX
            + "envSetupTimeout";

    /**
     * @hidden
     * @deprecated For internal use only. When set to <code>true</code>, it
     *             permits opening of a ReplicatedEnvironment handle in the
     *             {@link ReplicatedEnvironment.State#UNKNOWN} state, if a
     *             Master could not be determined within the timeout specified
     *             by {@link ReplicationConfig#ENV_SETUP_TIMEOUT}. If it's
     *             false, an <code>
     * UnknownMasterException</code> exception is thrown upon expiration of the
     *             timeout.
     *             <p>
     *             A ReplicatedEnvironment handle in the
     *             {@link ReplicatedEnvironment.State#UNKNOWN} state can only be
     *             used to initiate read operations with an appropriately
     *             relaxed {@link NoConsistencyRequiredPolicy}; write operations
     *             will fail with a <code>ReplicaWriteException</code>. The
     *             handle will transition to a <code>Master</code> or
     *             <code>Replica</code> state when it can contact a sufficient
     ...
     */
    @Deprecated
    public static final String                         ALLOW_UNKNOWN_STATE_ENV_OPEN  = EnvironmentParams.REP_PARAM_PREFIX
            + "allowUnknownStateEnvOpen";
     ...
     * {@code Master} or {@code Replica} state when it can contact a sufficient
     * number of other nodes in the replication group.
     * <p>
     ...
     * ReplicatedEnvironment constructor will throw
     * {@link UnknownMasterException}.
     * <p>
     * <table border="1">
     * <tr>
     * <td>Name</td>
     ...
     * @since 5.0.33
     */
    public static final String                         ENV_UNKNOWN_STATE_TIMEOUT     = EnvironmentParams.REP_PARAM_PREFIX
            + "envUnknownStateTimeout";
     ...
     * (text encoding) for node names and host names. This is incorrect, in that
     * it requires that the JVM for all nodes in a replication group have the
     * same default charset.
     * <p>
     * When this parameter is set to <code>false</code>, the UTF-8 charset is
     * always used in the replication protocol. In other words, the JVM default
     * charset has no impact on the replication protocol.
     * <p>
     ...
     * <li>all node names and host names contain only ASCII characters, and the
     * default charset on all JVMs is a superset of ASCII.</li>
     * </ul>
     * <p>
     * In JE 5.1, the default value for this parameter will be changed to false.
     * In preparation for this, impacted applications should explicitly set the
     * parameter to false at the next available opportunity. For applications
     * not yet deployed, this should be done now. For deployed applications, a
     * hot upgrade may not be performed when changing the parameter. Instead, a
     * cold upgrade must be performed: all nodes must be stopped and upgraded
     * before bringing them up again. In other words, for impacted applications
     * the value of this configuration parameter must be the same for all
     * running nodes in a replication group.
     * <p>
     ...
     * </table>
     */
    public static final String                         PROTOCOL_OLD_STRING_ENCODING  = EnvironmentParams.REP_PARAM_PREFIX
            + "protocolOldStringEncoding";

    /**
     * @hidden For internal use only. When set to <code>true</code> the
     *         ReplicatedEnvironment will be used by the Arbiter. *
     *         <p>
     *         <table border="1">
     *         <tr>
     *         <td>Name</td>
     ...
     *         </table>
     */
    public static final String                         ARBITER_USE                   = EnvironmentParams.REP_PARAM_PREFIX
            + "arbiterUse";
     ...
     * </p>
     */
    public static final String                         ARBITER_OUTPUT_QUEUE_SIZE     = EnvironmentParams.REP_PARAM_PREFIX
            + "arbiterOutputQueueSize";
     ...
     * configuration. This field should be typed as ReplicationNetworkConfig,
     * but because this class is serializable and field is not transient,
     * javadoc wants to describe it as part of the javadoc output, but the fact
     * that the type is hidden causes javadoc to encounter a
     * NullPointerException. As a temporary measure, this class is typed as
     * Object in order to avoid the javadoc error. When ReplicationNetworkConfig
     * becomes public, the type of the field should be changed to
     * ReplicationNetworkConfig. This will not cause any problems with
     * cross-release serialization as long as we take care that only
     * ReplicationNetworkConfig instances are assigned to this field.
     */
    private Object                                     repNetConfig                  = ReplicationNetworkConfig
            .createDefault();
    ...
    /**
     * Creates a ReplicationConfig initialized with the system default settings
     * and the specified group name, node name, and hostname/port values.
     * <p>
     * Note that the node name is immutable. Normally the host name should not
     * be used as the node name, unless you intend to reuse the host name when a
     * machine fails and is replaced, or the node is upgraded to new hardware.
    ...
    /**
     * For internal use only. Returns a boolean that specifies if we need to
     * convert the existing logs to replicated format.
     *
    ...
    /**
     * For internal use only. If set to true, this environment should be
     * converted to replicated format.
     *
     ...
     * the group. When the node is instantiated and joins the replication group,
     * a check is done to ensure that the name is unique, and a
     * {@link RestartRequiredException} is thrown if it is not.
     * <p>
     * The name should consist of letters, digits, and/or hyphen ("-"),
     * underscore ("_"), or period (".").
     * <p>
    ...
    /**
     * Sets the hostname and port associated with this node. The hostname and
     * port combination are denoted by a string of the form:
     * 
     * <pre>
     *  hostname[:port]
    ...
    public void setReplicaAckTimeoutVoid(long replicaAckTimeout, TimeUnit unit) {
        DbConfigManager.setDurationVal(props, RepParams.REPLICA_ACK_TIMEOUT, replicaAckTimeout, unit, validateParams);
    }
    ...
    public void setMaxClockDeltaVoid(long maxClockDelta, TimeUnit unit) throws IllegalArgumentException {
        ...
        DbConfigManager.setDurationVal(props, RepParams.MAX_CLOCK_DELTA, maxClockDelta, unit, validateParams);
    }

    /**
     * Sets the consistency policy to be associated with the configuration. This
     * policy acts as the default policy used to govern the consistency
     * requirements when starting new transactions. See the
     * {@link <a href="{@docRoot}../ReplicationGuide/consistency.html">overview
     * on consistency in replicated systems</a>} for more background.
     * <p>
     * 
    ...
    public void setConsistencyPolicyVoid(ReplicaConsistencyPolicy policy) {
        ...
        DbConfigManager.setVal(props, RepParams.CONSISTENCY_POLICY, RepUtils.getPropertyString(policy), validateParams);
    }

    /**
     * Returns the default consistency policy associated with the configuration.
     * <p>
     * If the user does not set the default consistency policy through
     * {@link ReplicationConfig#setConsistencyPolicy}, the system will use the
     * policy defined by {@link ReplicationConfig#CONSISTENCY_POLICY}.
     *
    ...
    /**
     * @hidden SSL deferred Get the replication service net configuration
     *         associated with this ReplicationConfig.
     */
    ...
    /**
     * @hidden SSL deferred Set the replication service net configuration
     *         associated with this ReplicationConfig.
     * @param netConfig the new ReplicationNetworkConfig to be associated with
     *            this ReplicationConfig. This must not be null.
    ...
    /**
     * @hidden For use by this class and by
     *         ReplicatedEnvironment.setupRepConfig() Moves any properties that
     *         belong to ReplicationNetworkConfig to repNetConfig. This is
     *         intended to be called after a bulk property load.
     */
    void propagateRepNetProps() {
        ...
        /*
         * Construct a new properties set that includes both the properties that
         * we hold directly, plus any properties stored on an existing
         ...
         * non-property fields that should be retained from the original, so use
         * the orignal object and just change the properties.
         */
    ...
    /**
     * @hidden For internal use only: Internal convenience method. Returns the
     *         set of sockets associated with helper nodes. This method should
     *         only be used when the configuration object is known to have an
     *         authoritative value for the helper hosts values. In a replication
     ...
     *         This method should only be used when the configuration object is
     *         known to have an authoritative value for its socket value. In a
     *         replication node, the je.properties file may override the values
    ...
    public int getNodePort() {
        ...
        String portString = (colonToken >= 0) ? hostAndPort.substring(colonToken + 1)
     ...
     * {@link ProgressListener} to provide feedback on replication stream
     * sync-up. The ProgressListener.progress() method is called at different
     * stages of the syncup process. See {@link SyncupProgress} for information
     * about those stages.
     * <p>
     ...
     * @since 5.0
     */
    public ReplicationConfig setSyncupProgressListener(final ProgressListener<SyncupProgress> progressListener) {
     ...
     * @hidden The void return setter for use by Bean editors.
     */
    public void setSyncupProgressListenerVoid(final ProgressListener<SyncupProgress> progressListener) {
    ...
    /**
     * @hidden Installs a callback to be notified when JE is about to modify
     *         previously written log files.
     */
    ...
    /**
     * @hidden Configures a filter object that is transmitted to the remote
     *         Feeder as part of the replica feeder syncup. The remote feeder
     *         then invokes this filter on each record before it sends the
     *         record to the replica. The filter can be used to filter out
     *         replication records at the feeder itself and this eliminate the
     *         feeder to replica transmission overhead for records in which it
     *         has no interest.
     */
    ...
    /**
     * @hidden For internal use only. Performs the checks need to ensure that
     *         this is a valid replicated environment configuration. This method
     *         must only be invoked after all the appropriate fields are set.
     */
