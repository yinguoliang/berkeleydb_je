...
/**
 * @hidden For internal use only. Defines the protocol used by the Monitor to
 *         keep informed about group changes, and a node joins/leaves the group.
 *         The Master uses the protocol to inform all Monitors about group
 *         change and node join/leave change. GCHG -> no response expected from
 *         the monitor. JoinGroup -> no response expected from the monitor.
...
public class Protocol extends TextProtocol {
    ...
    public class GroupChange extends ChangeEvent {
        ...
        @Override
        public String wireFormat() {
            final int repGroupVersion = (Double.parseDouble(sendVersion) <= Double.parseDouble(REP_GROUP_V2_VERSION))
                    ? RepGroupImpl.FORMAT_VERSION_2 : RepGroupImpl.MAX_FORMAT_VERSION;
            return super.wireFormat() + opType.toString() + SEPARATOR + group.serializeHex(repGroupVersion);
        }
    }
    ...
    private abstract class MemberEvent extends ChangeEvent {
        ...
        @Override
        public String wireFormat() {
            return super.wireFormat() + masterName + SEPARATOR + Long.toString(joinTime);
        }
    }
    ...
    public class LeaveGroup extends MemberEvent {
        ...
        public LeaveGroup(String nodeName, String masterName, LeaveReason leaveReason, long joinTime, long leaveTime) {
            super(nodeName, masterName, joinTime);
        ...
        @Override
        public String wireFormat() {
            return super.wireFormat() + SEPARATOR + leaveReason.toString() + SEPARATOR + Long.toString(leaveTime);
        }
    }
}
