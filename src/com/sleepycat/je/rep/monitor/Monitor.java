 ...
 * members of the replication group. The information provided by the monitor can
 * be used to route update requests to the node that is currently the master and
 * distribute read requests across the other members of the group.
 * <p>
 ...
 * <p>
 * Applications with direct access to a
 * {@link com.sleepycat.je.rep.ReplicatedEnvironment ReplicatedEnvironment} can
 * use its {@link <a href="{@docRoot}/../ReplicationGuide/replicawrites.html">
 ...
 *      Nodes</a>
 * @see <a href="{@docRoot}../examples/je/rep/quote/package-summary.html">je.rep
 *      .quote Examples</a>
 */
public class Monitor {
    ...
    /*
     * The groupMembers set saves the names of all nodes known to be part of the
     * group. 1. If a node is not in the set, it means this node has not been
     * added to the group or has been removed from the group already. Note that
     * a node not in the groupMembers set may have an entry in the joinEvents
     * map if a join event arrived for the node without an associated group add
     * event, possibly because of an event delivery failure. 2. If a node is in
     * the set, but does not have an entry in the joinEvents map, this node may
     * have closed itself (due to either crash or normal close) or has issued an
     * ADD GroupChangeEvent. However, it hasn't issued a JoinGroupChangeEvent
     * yet (because ADD GroupChangeEvent and JoinGroupEvent are notified in two
     * phases). 3. If a node is in the set, and is also in the joinEvents map,
     * the node has already issued a JoinGroupEvent.
     */
    private final Set<String>                               groupMembers  = Collections
     ...
     */
    private final ConcurrentHashMap<String, JoinGroupEvent> joinEvents    = new ConcurrentHashMap<String, JoinGroupEvent>();

    /*
     * A thread which proactively checks on group status. TODO: ideally
     * implementation would be changed to use an ExecutorService rather than a
     * Thread.
     */
    ...
    /**
     * Deprecated as of JE5. Creates a monitor instance using a
     * {@link ReplicationConfig}. Monitor-specific properties that are not
     * available in ReplicationConfig use default settings.
     *
    ...
    public Monitor(MonitorConfig monitorConfig) {
        ...
        repGroupAdmin = new ReplicationGroupAdmin(groupName, monitorConfig.getHelperSockets(), channelFactory);
    ...
    /**
     * Registers the monitor with the group so that it can be kept informed of
     * the outcome of elections and group membership changes. The monitor, just
     * like a replication node, is identified by its nodeName. The Monitor uses
     * the helper nodes to locate a master with which it can register itself. If
     * the helper nodes are not available the registration will fail.
     * <p>
    ...
    public ReplicationNode register() throws EnvironmentFailureException {
        ...
        RepNodeImpl monitorNode = new RepNodeImpl(nameIdPair, NodeType.MONITOR, monitorConfig.getNodeHostname(),
                monitorConfig.getNodePort(), JEVersion.CURRENT_VERSION);
     ...
     * <p>
     * Once the registration has been completed, the Monitor can start listening
     * even if none of the other nodes in the group are available. It will be
     * contacted automatically by the other nodes as they come up.
     * <p>
     * If the group has a Master, invoking <code>startListener</code> results in
     * a synchronous callback to the application via the
     * {@link MonitorChangeListener#notify(NewMasterEvent)} method. If there is
     * no Master at this time, the callback takes place asynchronously, after
     * the method returns, when a Master is eventually elected.
     * <p>
     * Starting the listener will start the underlying ping thread, which
     * proactively checks group status for changes that might have been missed
     * when this Monitor instance has lost network connectivity or is down.
     *
    ...
    public void startListener(MonitorChangeListener newListener) throws DatabaseException, IOException {
        ...
        if (newListener == null) {
            throw new IllegalArgumentException(
                    "A MonitorChangeListener must be associated with " + " this Monitor when invoking this method");
        }
        ...
        Protocol electionProtocol = new Protocol(TimebasedProposalGenerator.getParser(), MasterValue.getParser(),
                monitorConfig.getGroupName(), nameIdPair, null, channelFactory);
        ...
        try {
            ...
            /*
             * In the absence of a network failure, the query should result in a
             * call to the notify method of MonitorChangeListener.
             */
    ...
    private void notifyJoinGroupEventsForActiveNodes(ReplicationGroup group) {
        ...
        for (ReplicationNode repNode : group.getNodes()) {
            ...
            /*
             * Note any existing nodes with persistent id as group members.
             * Nodes w/ transient id never send add or remove events, so they
             * are not included in groupMembers.
             */
            ...
            MessageExchange me = stateProtocol.new MessageExchange(repNode.getSocketAddress(),
                    NodeStateService.SERVICE_NAME, stateProtocol.new NodeStateRequest(repNode.getName()));
            me.run();
            ...
            if (resp instanceof NodeStateResponse) {
                ...
                notifyJoin(
                        new JoinGroupEvent(response.getNodeName(), response.getMasterName(), response.getJoinTime()));
            }
        }
    }
     ...
     * successful election. This method relies on the helper nodes supplied to
     * the monitor and queries them for the master. This method is useful when a
     * Monitor first starts up and the Master needs to be determined. Once a
     * Monitor is registered and the Listener has been started, it's kept up to
     * date via events that are delivered to the Listener.
     *
     * @return the id associated with the master replication node.
     * @throws UnknownMasterException if the master could not be determined from
     *             the set of helpers made available to the Monitor.
     * @throws EnvironmentFailureException if an unexpected, internal or
     *             environment-wide failure occurs.
     ...
     * querying the helpers to determine the master and then obtaining the group
    ...
    synchronized void notifyGroupChange(GroupChangeEvent event) {
        ...
        switch (event.getChangeType()) {
            ...
            case ADD:
                ...
                break;
            default:
                throw new IllegalArgumentException("Unrecognized GroupChangeType: " + event.getChangeType());
        }
        ...
        public void notify(Proposal proposal, Value value) {
            ...
            try {
                ...
                LoggerUtils.logMsg(logger, formatter, Level.INFO,
                        "Monitor notified of new Master: " + currentMasterName);
                ...
                LoggerUtils.logMsg(logger, formatter, Level.SEVERE,
    ...
    /*
     * PingThread periodically queries the replication group state in order to
     * proactively find group changes that this monitor may have missed if the
     * monitor was down or had network connectivity problems. Any missed changes
     * are propagated as the appropriate type of events. PingThread takes these
     * steps: 1. Get the current group information. If not available, use the
     * last valid group information. Get information for both removed and
     * non-removed nodes. 2. Walk through the removed nodes set. If there is a
     * node which is in the removed nodes set but still contained in
     * groupMembers, send a REMOVE GroupChangeEvent. 2a. Also walk through the
     * group members remembered by the monitor, and send REMOVE GroupChangeEvent
     * for each one that is not recorded in the current group info for removed
     * or unremoved nodes, to account for deleted nodes. 3. Walk through all
     * unremoved nodes. There are three cases: a. If a node is reachable (it
     * acks state request), but it's not in groupMembers, emit an ADD
     * GroupChangeEvent. Only do this for electable and monitor nodes --
     * secondary nodes don't send these events. b. If a node is reachable, but
     * joinEvents.containsKey(node name) returns false, send a JoinGroupEvent.
     * c. If a node is unreachable, but joinEvents.containsKey(node name)
     * returns true, we want to send a LeaveGroupEvent, but guard against this
     * being a transient situation. We do some retries and if we get the same
     * result for all retries, send a missed LeaveGroupEvent. 3a. Also walk
     * through all nodes removed nodes, and do cases 3.b. and 3.c. for those
     * nodes 4. Walk through all nodes in joinEvents, and perform 3.c. for ones
     * which are not present as removed or unremoved nodes in the group. These
     * nodes are secondary nodes, which are removed from the group when the node
     * becomes unavailable.
     */
    private class PingThread extends Thread {
        ...
        /**
         * Ping all nodes to find out about possible missed events. Manufacture
         * notifications for any missed events.
         */
        private void queryNodes() {
            /* Get the current valid group information. */
            final ReplicationGroup repGroup = getValidGroup();

            final Set<? extends ReplicationNode> removedNodes = repGroup.getRepGroupImpl().getRemovedNodes();

            /* Send missed REMOVE GroupChangeEvents for removed nodes. */
            for (final ReplicationNode repNode : removedNodes) {
                notifyGroupChange(new GroupChangeEvent(repGroup, repNode.getName(), GroupChangeType.REMOVE));
            }

            /*
             * Also send REMOVE GroupChangeEvents for nodes in groupMembers that
             * are no longer in the replication group, to account for deleted
             * nodes, which will not be remembered as removed nodes
             */
             ...
             * shutdown or network failure. TODO: Provide for distinguishing
             * secondary and deleted electable nodes.
             */
        ...
        private void pingNode(ReplicationNode repNode, ReplicationGroup repGroup, boolean removed) {
            ...
            try {
                ...
                if (!groupMembers.contains(name) && !repNode.getType().isSecondary() && !repNode.getType().isExternal()
                        && !removed) {
        ...
        private void notifyMissedLeaveEvents(String name) {
            ...
            if (missedLeaveEvents.get(name) == retries) {
                notifyLeave(new LeaveGroupEvent(name, event.getMasterName(), LeaveReason.ABNORMAL_TERMINATION,
                        event.getJoinTime().getTime(), System.currentTimeMillis()));
            }
