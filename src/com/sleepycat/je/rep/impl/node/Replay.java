...
/**
 * Replays log records from the replication stream, and manages the transactions
 * for those records. The Replay module has a lifetime equivalent to the
 * environment owned by this replicator. Its lifetime is longer than the
 * feeder/replica stream. For example, suppose this is nodeX: t1 - Node X is a
 * replica, node A is master. Replay X is alive t2 - Node X is a replica, node B
 * takes over as master. X's Replay module is still alive and has the same set
 * of active txns. It doesn't matter to X that the master has changed. t3 - Node
 * X becomes the master. Now its Replay unit is cleared, because anything
 * managed by the Replay is defunct.
 */
public class Replay {
    ...
    /**
     * If a commit replay operation takes more than this threshold, it's logged.
     * This information helps determine whether ack timeouts on the master are
     * due to a slow replica, or the network.
     */
     ...
     * Note that there is an interesting relationship between ActiveTxns and the
     * txn collection managed by the environment TxnManager. ActiveTxns is
     * effectively a subset of the set of txns held by the TxnManager.
     * ReplayTxns must be sure to register and unregister themselves from
     * ActiveTxns, just as all Txns must register and unregister with the
     * TxnManager's set. One implementation alternative to having an ActiveTxns
     * map here is to search the TxnManager set (which is a set, not a map) for
     * a given ReplayTxn. Another is to subclass TxnManager so that replicated
     * nodes have their own replayTxn map, just as XATxns have a XID->Txn map.
     * Both alternatives seemed too costly in terms of performance or elaborate
     * in terms of code to do for the current function. It seems clearer to make
     * the ActiveTxns a map in the one place that it is currently used. This
     * choice may change over time, and should be reevaluated if the
     * implementation changes. The ActiveTxns key is the transaction id. These
     * transactions are closed when: - the replay unit executes a commit
     * received over the replication stream - the replay unit executes an abort
     * received over the replication stream - the replication node realizes that
     * it has just become the new master, and was not previously the master.
     ...
     * another part by another. The map is synchronized, so simple get/put
     * operations do not require additional synchronization. However, iteration
     * requires synchronization and copyActiveTxns can be used in most cases.
     */
    ...
    /**
     * All writes (predominantly acks) are queued here, so they do not block the
     * replay thread.
     */
    ...
    public Replay(RepImpl repImpl, @SuppressWarnings("unused") NameIdPair nameIdPair) {
         ...
         * messages in the face of a network drop and 2X to allow for additional
         * headroom and minimize the chances that the replay might be blocked
         * due to the limited queue length.
         */
        ...
        /*
         * The Replay module manages all write transactions and mimics a writing
         * application thread. When the node comes up, it populates the
         * activeTxn collection with ReplayTxns that were resurrected at
         * recovery time.
         */
        activeTxns = new SimpleTxnMap<>(1024);

        /*
         * Configure the data entry used for deletion to avoid fetching the old
         * data during deletion replay.
         */
        ...
        replayLoggingThresholdNs = MILLISECONDS.toNanos(configManager.getDuration(RepParams.REPLAY_LOGGING_THRESHOLD));
    }
    ...
    /**
     * Actions that must be taken before the recovery checkpoint, whether the
     * environment is read/write or read/only.
     */
    public void preRecoveryCheckpointInit(RecoveryInfo recoveryInfo) {
        for (Txn txn : recoveryInfo.replayTxns.values()) {

            /*
             * ReplayTxns need to know about their owning activeTxn map, so they
             * can remove themselves at close. We are casting upwards, because
             * the non-HA code is prohibited from referencing Replication
             * classes, and the RecoveryInfo.replayTxns collection doesn't know
             * that it's got ReplayTxns.
             */
     ...
     * Replay transactions need only be aborted by the node that has become the
     * new master (who was previously a Replica). The replay transactions on the
     * other replicas who have not changed roles are resolved by the abort
    ...
    public void abortOldTxns() throws DatabaseException {
        ...
        for (ReplayTxn replayTxn : copyActiveTxns().values()) {
             ...
             * despite being a ReplayTxn; it will be corrected when it's written
             * to the log.
             */
    ...
    private void updateCommitStats(final boolean needsAck, final SyncPolicy syncPolicy, final long startTimeNanos,
                                   final long masterCommitTimeMs, final long replicaCommitTimeMs) {

        final long now = System.nanoTime();
        final long commitNanos = now - startTimeNanos;

        if (commitNanos > ackTimeoutLogThresholdNs && logger.isLoggable(Level.INFO)) {
            LoggerUtils.info(logger, repImpl, "Replay commit time: " + (commitNanos / 1000000)
                    + " ms exceeded log threshold: " + (ackTimeoutLogThresholdNs / 1000000));
        }
    ...
    public void replayEntry(long startNs, Protocol.Entry entry)
        ...
        if (!wireRecord.getVLSN().follows(lastReplayedVLSN)) {
            throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.UNEXPECTED_STATE,
                    "Rep stream not sequential. Current VLSN: " + lastReplayedVLSN + " next log entry VLSN: "
                            + wireRecord.getVLSN());
        }
        ...
        try {
            ...
            if (LOG_TXN_COMMIT.equalsType(entryType)) {
                ...
                final SyncPolicy implSyncPolicy = needsAck ? groupCommit.getImplSyncPolicy(txnSyncPolicy)
                        : noAckSyncPolicy;
                ...
                repTxn.commit(implSyncPolicy, new ReplicationContext(lastReplayedVLSN), commit.getMasterNodeId(),
                        dtvlsn);

                final long masterCommitTimeMs = commit.getTime().getTime();
                lastReplayedTxn = new TxnInfo(lastReplayedVLSN, masterCommitTimeMs);

                updateCommitStats(needsAck, implSyncPolicy, startNs, masterCommitTimeMs, repTxn.getEndTime());
                ...
                if (logger.isLoggable(Level.FINEST)) {
                    LoggerUtils.finest(logger, repImpl, "abort called for " + txnId + " masterId="
                            + abort.getMasterNodeId() + " repContext=" + abortContext);
                }
            ...
            if (elapsedNs > replayLoggingThresholdNs) {
                LoggerUtils.info(logger, repImpl,
                        "Replay time for entry type:" + LogEntryType.findType(entryType) + " "
                                + NANOSECONDS.toMillis(elapsedNs) + "ms " + "exceeded threshold:"
                                + NANOSECONDS.toMillis(replayLoggingThresholdNs) + "ms");
            }
        }
    }

    /**
     * Update the replica's in-memory DTVLSN using the value in the commit/abort
     * entry. In the normal course of events, DTVLSNs should not decrease.
     * However, there is just one exception: if the rep stream transitions from
     * a post to a pre-dtvlsn stream, it will transition from a positive to the
     * UNINITIALIZED_VLSN_SEQUENCE. A transition from a pre to a post-dtvlsn
     * transition (from zero to some positive value), observes the
     * "DTVLSNs should not decrease" rule automatically.
     *
    ...
    private long updateDTVLSN(final TxnEnd txnEnd) {
        ...
        if (txnDTVLSN == VLSN.UNINITIALIZED_VLSN_SEQUENCE) {
            ...
            if (prevDTVLSN != VLSN.UNINITIALIZED_VLSN_SEQUENCE) {
                LoggerUtils.info(logger, repImpl, "Transitioned to pre DTVLSN stream." + " DTVLSN:" + prevDTVLSN
        ...
        if (txnDTVLSN < lastReplayedDTVLSN) {
            String msg = "DTVLSNs must be in ascending order in the stream. " + " prev DTVLSN:" + lastReplayedDTVLSN
                    + " next DTVLSN:" + txnDTVLSN + " at VLSN: " + lastReplayedVLSN.getSequence();
        ...
        if ((lastReplayedDTVLSN == VLSN.UNINITIALIZED_VLSN_SEQUENCE) && (txnDTVLSN > 0)) {
            LoggerUtils.info(logger, repImpl,
                    "Transitioned to post DTVLSN stream." + " DTVLSN:" + txnDTVLSN + " at VLSN:" + lastReplayedVLSN);
        }
    ...
    void queueAck(final long txnId) throws IOException {
        try {
            ...
            /*
             * Have the higher levels treat it like an IOE and exit the thread.
             */
    ...
    private void logReplay(ReplayTxn repTxn, boolean needsAck, SyncPolicy syncPolicy) {
        ...
        if (needsAck) {
            LoggerUtils.fine(logger, repImpl, "Replay: got commit for txn=" + repTxn.getId()
                    + ", ack needed, replica sync policy=" + syncPolicy + " vlsn=" + lastReplayedVLSN);
        } else {
            LoggerUtils.fine(logger, repImpl,
                    "Replay: got commit for txn=" + repTxn.getId() + " ack not needed" + " vlsn=" + lastReplayedVLSN);
        }
    }

    /**
     * Returns true if there are no other activeTxns that have also modified the
     * membership database and are still open, since they could potentially hold
     * write locks that would block the read locks acquired during the refresh
     * operation.
     *
     ...
     * replica. We just want to ensure that if this node becomes the master, its
     * sequences are in sync with what came before in the replication stream,
     * and ids are not incorrectly reused.
     */
    ...
    public ReplayTxn getReplayTxn(long txnId, boolean registerTxnImmediately) throws DatabaseException {
        ...
        synchronized (activeTxns) {
            ...
            if (useTxn == null) {
                ...
                /*
                 * Durability will be explicitly specified when ReplayTxn.commit
                 * is called, so TransactionConfig.DEFAULT is fine.
                 */
    ...
    /**
     * Replays the NameLN. Note that the operations: remove, rename and truncate
     * need to establish write locks on the database. Any open handles are
     * closed by this operation by virtue of the ReplayTxn's importunate
     * property. The application will receive a LockPreemptedException if it
     * subsequently accesses the database handle.
     */
    private void applyNameLN(ReplayTxn repTxn, InputWireRecord wireRecord) throws DatabaseException {
        ...
        final DbOpReplicationContext repContext = new DbOpReplicationContext(wireRecord.getVLSN(), nameLNEntry);
        ...
        try {
            switch (opType) {
                case CREATE: {
                    ...
                    dbImpl = repImpl.getDbTree().createReplicaDb(repTxn, databaseName, dbConfig, nameLN, repContext);

                    /*
                     * We rely on the RepGroupDB.DB_ID value, so make sure it's
                     * what we expect for this internal replicated database.
                     */
                    if ((dbImpl.getId().getId() == RepGroupDB.DB_ID)
                            && !DbType.REP_GROUP.getInternalName().equals(databaseName)) {
                        throw EnvironmentFailureException.unexpectedState("Database: "
                                + DbType.REP_GROUP.getInternalName() + " is associated with id: "
                                + dbImpl.getId().getId() + " and not the reserved database id: " + RepGroupDB.DB_ID);
                    }
                ...
                case REMOVE: {
                    ...
                    try {
                        ...
                        throw EnvironmentFailureException.unexpectedState("Database: " + dbImpl.getName() + " Id: "
                                + nameLN.getId() + " not found on the Replica.");
                    }
                    break;
                }
                ...
                case TRUNCATE: {
                    ...
                    try {
                        TruncateDbResult result = repImpl.getDbTree().truncateReplicaDb(repTxn, databaseName, false,
                                nameLN, repContext);
                ...
                case RENAME: {
                    dbImpl = repImpl.getDbTree().getDb(nameLN.getId());
                    try {
                        dbImpl = repImpl.getDbTree().renameReplicaDb(repTxn, dbImpl.getName(), databaseName, nameLN,
                                repContext);
                        TriggerManager.runRenameTriggers(repTxn, dbImpl, databaseName);
                    } catch (DatabaseNotFoundException e) {
                        throw EnvironmentFailureException.unexpectedState("Database rename from: " + dbImpl.getName()
                                + " to " + databaseName + " failed, name not found on the Replica.");
                    }
                    break;
                }
                ...
                default:
                    throw EnvironmentFailureException.unexpectedState("Illegal database op type of " + opType.toString()
    ...
    private void applyLN(final ReplayTxn repTxn, final InputWireRecord wireRecord) throws DatabaseException {

        final LNLogEntry<?> lnEntry = (LNLogEntry<?>) wireRecord.getLogEntry();
        final DatabaseId dbId = lnEntry.getDbId();

        /*
         * If this is a change to the rep group db, remember at commit time, and
         * refresh this node's group metadata.
         */
        ...
        try (final Cursor cursor = DbInternal.makeCursor(dbImpl, repTxn, null /* cursorConfig */)) {
            ...
            if (dbImpl.getSortedDuplicates()
                ...
                throw EnvironmentFailureException.unexpectedState(dbImpl.getEnv(),
                        "[#25288] emb=" + lnEntry.isEmbeddedLN() + " key=" + Key.getNoFormatString(lnEntry.getKey())
                                + " data=" + Key.getNoFormatString(ln.getData()) + " vlsn=" + ln.getVLSNSequence());
            }
            ...
            if (ln.isDeleted()) {
                ...
                result = DbInternal.searchForReplay(cursor, replayKeyEntry, delDataEntry, LockMode.RMW, SearchMode.SET);
                ...
                result = DbInternal.putForReplay(cursor, replayKeyEntry, replayDataEntry, ln, lnEntry.getExpiration(),
                        lnEntry.isExpirationInHours(), PutMode.OVERWRITE, repContext);
            }

            if (result == null) {
                throw new EnvironmentFailureException(repImpl, EnvironmentFailureReason.LOG_INCOMPLETE,
     ...
     * entry represented by the matchpoint VLSN. Effectively truncate these
     * rolled back log entries by making them invisible. Flush the log first, to
     * make sure these log entries are out of the log buffers and are on disk,
     * so we can reliably find them through the FileManager. Rollback steps are
     * described in
     * https://sleepycat.oracle.com/trac/wiki/Logging#Recoverysteps. In summary,
     * 1. Log and fsync a new RollbackStart record 2. Do the rollback in memory.
     * There is no need to explicitly log INs made dirty by the rollback
     * operation. 3. Do invisibility masking by overwriting LNs. 4. Fsync all
     * overwritten log files at this point. 5. Write a RollbackEnd record, for
     * ease of debugging Note that application read txns can continue to run
     * during syncup. Reader txns cannot access records that are being rolled
     * back, because they are in txns that are not committed, i.e, they are
     * write locked. The rollback interval never includes committed txns, and we
     * do a hard recovery if it would include them.
     */
    public void rollback(VLSN matchpointVLSN, long matchpointLsn) {
        ...
        try {
             ...
             * entries will be flushed to disk and the TxnChain will not have to
             * worry about entries that are in log buffers when constructing the
             * rollback information.
             */
            LogManager logManager = repImpl.getLogManager();
            LogEntry rollbackStart = SingleItemEntry.create(LogEntryType.LOG_ROLLBACK_START,
                    new RollbackStart(matchpointVLSN, matchpointLsn, localActiveTxns.keySet()));
            long rollbackStartLsn = logManager.logForceFlush(rollbackStart, true, // fsyncRequired,
            ...
            /*
             * 3 & 4 - Mark the rolled back log entries as invisible. But before
             * doing so, invoke any registered rewrite listeners, so the
             * application knows that existing log files will be modified. After
             * all are done, fsync the set of files. By waiting, some may have
             * made it out on their own.
             */
            ...
            logManager.logForceFlush(
                    SingleItemEntry.create(LogEntryType.LOG_ROLLBACK_END,
                            new RollbackEnd(matchpointLsn, rollbackStartLsn)),
                    true, // fsyncRequired
                    ReplicationContext.NO_REPLICATE);

            /*
             * Restart the backup service only if all the steps of the rollback
             * were successful.
             */
            ...
            LoggerUtils.info(logger, repImpl, "Rollback to matchpoint " + matchpointVLSN + " at "
                    + DbLsn.getNoFormatString(matchpointLsn) + " status=" + rollbackStatus);
        }
    }
    ...
    /**
     * Make a copy of activeTxns to avoid holding its mutex while iterating. Can
     * be used whenever the cost of the HashMap copy is not significant.
     */
     ...
     * The Replicator is closing down and Replay will not be invoked again.
     */
    public void close() {
     ...
     * for details. This method is invoked after each log entry is read from the
     * replication stream.
     *
     ...
     * immediately, but their acknowledgments are delayed until after either the
     * REPLICA_GROUP_COMMIT_INTERVAL (the max amount the first transaction in
     * the group is delayed) has expired, or the size of the group (as specified
     * by REPLICA_MAX_GROUP_COMMIT) has been exceeded.
     */
    private class GroupCommit {
        ...
        private GroupCommit(DbConfigManager configManager) {
            ...
            final long groupCommitIntervalMs = configManager.getDuration(RepParams.REPLICA_GROUP_COMMIT_INTERVAL);
        ...
        /**
         * Returns the sync policy to be implemented at the replica. If group
         * commit is active, and SYNC is requested it will return NO_SYNC
         * instead to delay the fsync.
         *
        ...
        private SyncPolicy getImplSyncPolicy(SyncPolicy txnSyncPolicy) {
            return ((txnSyncPolicy == SyncPolicy.SYNC) && isEnabled()) ? SyncPolicy.NO_SYNC : txnSyncPolicy;
        }

        /**
         * Buffers the acknowledgment if the commit calls for a sync, or if
         * there are pending acknowledgments to ensure that acks are sent in
         * order.
         *
         ...
         */
        private final boolean bufferAck(long nowNs, ReplayTxn ackTxn, SyncPolicy txnSyncPolicy) throws IOException {
        ...
        private final void flushPendingAcks(long nowNs) throws IOException {
            ...
            if ((nPendingAcks == 0) || ((nPendingAcks != pendingCommitAcks.length)
