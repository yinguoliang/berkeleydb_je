 ...
 * must be done through the APIs provided by this class. The database is simply
 * a representation of the RepGroup. Each entry in the database represents a
 * node in RepGroup; the key is the String node name, and the data is the
 * serialized ReplicationNode. There is a special entry keyed by GROUP_KEY that
 * holds the contents of the RepGroup (excluding the nodes) itself. The database
 * may be modified concurrently by multiple transactions as a master processes
 * requests to update it. It may also be accessed by multiple overlapping
 * transactions as a Replica replays the rep stream. These updates need to be
 * interleaved with operations like getGroup() that create copies of the
 * RepGroup instance. To avoid deadlocks, entries in the database are accessed
 * in order of ascending key. GROUP_KEY in particular is associated with the
 * lowest key value so that it's locked first implicitly as part of any
 * iteration and any other modifications to the database must first lock it
 * before making changes to the group itself. An instance of this class is
 * created as part of a replication node and is retained for the entire lifetime
 * of that node.
 */
public class RepGroupDB {
    ...
    /*
     * The key used to store group-wide information in the database. It must be
     * the lowest key in the database, so that it's locked first during database
     * iteration.
     */
    ...
    private final static HashMap<String, Object> lockMap                   = new HashMap<String, Object>();

    /* The fixed DB ID associated with the internal rep group database. */
    public final static long                     DB_ID                     = DbTree.NEG_DB_ID_START - 1;

    /*
     * Number of times to retry for ACKs on the master before returning to to
     * the Replica, which will then again retry on some periodic basis.
     */
    private final static int                     QUORUM_ACK_RETRIES        = 5;

    /* Convenience Durability and Config constants. */
    private final static Durability              QUORUM_ACK_DURABILITY     = new Durability(SyncPolicy.SYNC,
            SyncPolicy.SYNC, ReplicaAckPolicy.SIMPLE_MAJORITY);

    private final static TransactionConfig       QUORUM_ACK                = new TransactionConfig();
    ...
    private final static Durability              NO_ACK_DURABILITY         = new Durability(SyncPolicy.SYNC,
            SyncPolicy.SYNC, ReplicaAckPolicy.NONE);

    private final static Durability              NO_ACK_NO_SYNC_DURABILITY = new Durability(SyncPolicy.NO_SYNC,
     ...
     * this time, since the state of the node master/replica is not known at the
     * time the replication node (and consequently this instance) is created.
     * 
     ...
     * using NO_CONSISTENCY. This method can read the database directly, and can
     * be used when the replicated environment is detached and the RepNode is
     * null. It's for the latter reason that the method reads uncommitted data.
     * In detached mode, there may be transactions on the database that were in
     * progress when the node was last shutdown. These transactions may have
     * locks which will not be released until after the node is re-attached and
     * the replication stream is resumed. Using uncommitted reads avoids use of
     ...
     * <p>
     * If this method returns successfully, nodes that are running a JE version
     * older than the one specified will not be permitted to join the
     ...
     * <p>
     * The update attempts to obtain acknowledgments from a simple majority, to
     * make sure that future masters agree that the update has taken place, but
    ...
    public void setMinJEVersion(final JEVersion newMinJEVersion)
        ...
        try {
             ...
             * lost.
             */
            LoggerUtils.info(logger, repImpl, "Proceeding without acks for minimum JE version");
        } finally {
            if (txn != null) {
                txn.abort();
            }
     ...
     */
    static private Cursor makeCursor(DatabaseImpl dbImpl, Txn txn, CursorConfig cursorConfig) {
            ...
            while (mcursor.getNext(keyEntry, value, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
                ...
                if (GROUP_KEY.equals(key)) {
                    ...
                    if (!group.getName().equals(groupName)) {
                        throw EnvironmentFailureException.unexpectedState("The argument: " + groupName
                                + " does not match the expected group name: " + group.getName());
                    }
     ...
     * member database. If it isn't, enter it into the database. If the database
     * does not exist, create it as well.
     * <p>
    ...
    public void addFirstNode() throws DatabaseException {
        ...
        ensureMember(
                new RepNodeImpl(nodeName, repImpl.getHostName(), repImpl.getPort(), repImpl.getCurrentJEVersion()));
    }
            ...
            if (status == OperationStatus.SUCCESS) {
                ...
                if (miInDb.equivalent(ensureNode)) {
                    ...
                    LoggerUtils.warning(logger, repImpl,
                            "Incompatible node descriptions. " + "Membership database definition: " + miInDb.toString()
                                    + " Transient definition: " + ensureNode.toString());
                    if (ensureNode.getType() != miInDb.getType()) {
                        throw EnvironmentFailureException
                                .unexpectedState("Conflicting node types for node " + ensureNode.getName()
                                        + ": expected " + ensureNode.getType() + ", found " + miInDb.getType());
                    }
                    throw EnvironmentFailureException.unexpectedState("Incompatible node descriptions for node: "
                            + ensureNode.getName() + ", node ID: " + ensureNode.getNodeId());
                }
                LoggerUtils.info(logger, repImpl, "Present but not ack'd node: " + ensureNode.getNodeId()
    ...
    public void removeMember(final RepNodeImpl removeNode, final boolean delete) {
        LoggerUtils.info(logger, repImpl, (delete ? "Deleting node: " : "Removing node: ") + removeNode.getName());

        if (removeNode.getType().hasTransientId()) {
            throw new IllegalArgumentException(
                    "Attempt to call removeMember on a node with type " + removeNode.getType() + ": " + removeNode);
        }
        ...
        TwoPhaseUpdate twoPhaseUpdate = new TwoPhaseUpdate(removeNode, true) {
            ...
            void phase2Body() {
                ...
                try {
                    final OperationStatus status = cursor.getSearchKey(nodeNameKey, new DatabaseEntry(), LockMode.RMW);
                    if (status != OperationStatus.SUCCESS) {
                        throw EnvironmentFailureException
                                .unexpectedState("Node ID: " + removeNode.getNameIdPair() + " not present in group db");
                    }
                    cursor.delete();
                } finally {
                    cursor.close();
    ...
    private void createMember(final RepNodeImpl node)
            throws InsufficientReplicasException, InsufficientAcksException, DatabaseException {

        LoggerUtils.fine(logger, repImpl, "Adding node: " + node.getNameIdPair());

        twoPhaseMemberUpdate(node, true);

        LoggerUtils.info(logger, repImpl, "Successfully added node:" + node.getNameIdPair() + " HostPort = "
                + node.getHostName() + ": " + node.getPort() + " [" + node.getType() + "]");
    }

    /*
     * Update a current rep node information in the RepGroupDB. <p>This method
     * should not be called for nodes with transient IDs.
     * @param node the new node information
     * @param quorumAck whether to require acknowledgments from a quorum
     * @throws InsufficientReplicasException upon failure of 2p member update
     * @throws InsufficientAcksException upon failure of 2p member update
     */
    public void updateMember(final RepNodeImpl node, final boolean quorumAck) throws DatabaseException {
        ...
        LoggerUtils.info(logger, repImpl, "Successfully updated node: " + node.getNameIdPair() + " Hostport = "
                + node.getHostName() + ": " + node.getPort() + " [" + node.getType() + "]");
    }

    /**
     * Implements the two phase update of membership information. In the first
     * phase the master repeatedly tries to commit the "put" operation until it
     * gets a Quorum of acks, ensuring that the operation has been made durable.
     * Nodes that obtain this entry will start using it in elections. However,
     * the node itself will not participate in elections until it has
     * successfully completed phase 2. In the second phase, the entry for the
     * member is updated to note that a quorum of acks was received. Failure
     * leaves the database with the member info absent, or present but without
     * the update to quorumAcks indicating that a quorum has acknowledged the
     * change.
     *
     ...
     */
    private void twoPhaseMemberUpdate(final RepNodeImpl node, final boolean quorumAck) throws DatabaseException {
     ...
     * the node is a secondary, arbiter, or external node. Also updates the rep
     * node's transient group information about the global CBVLSN. If it
     * encounters contention, it returns false, and the caller must retry at
     * some later point in time. Note that changes to the local CBVLSN do not
     * update the group version number since they do not impact group
     * membership.
     *
     ...
     * @throws DatabaseException
     */
    public boolean updateLocalCBVLSN(final NameIdPair nameIdPair, final VLSN newCBVLSN, final NodeType nodeType)
            throws DatabaseException {
        ...
        DatabaseEntry value = new DatabaseEntry();
        final RepGroupImpl.BarrierState barrierState = new RepGroupImpl.BarrierState(newCBVLSN,
        ...
        try {
            ...
            /*
             * No database update for secondary, arbiter, or external nodes, but
             * set ok to true so that the rep node's group information is
             * updated.
             */
            ...
            if (status != OperationStatus.SUCCESS) {
                throw EnvironmentFailureException
                        .unexpectedState("Node ID: " + nameIdPair + " not present in group db");
            }
            ...
            if (status != OperationStatus.SUCCESS) {
                throw EnvironmentFailureException.unexpectedState(
                        "Node ID: " + nameIdPair + " stored localCBVLSN could not be updated. Status: " + status);
            }
            LoggerUtils.fine(logger, repImpl, "Local CBVLSN updated to " + newCBVLSN + " for node " + nameIdPair);
            ok = true;
        } finally {
            if (cursor != null) {
                cursor.close();
     ...
     */
    private RepGroupImpl fetchGroupObject(final Txn txn, final DatabaseImpl groupDbImpl, final LockMode lockMode)
            throws DatabaseException {
        ...
        try {
            ...
            if (status != OperationStatus.SUCCESS) {
                throw EnvironmentFailureException
                        .unexpectedState("Group entry key: " + GROUP_KEY + " missing from group database");
            }
        } finally {
            if (cursor != null) {
                cursor.close();
     ...
     */
    private void saveNodeObject(Txn txn, RepNodeImpl node, DatabaseImpl groupDbImpl, RepGroupImpl repGroup)
            throws DatabaseException {
    ...
    public static class GroupBinding extends TupleBinding<RepGroupImpl> {
        ...
        public RepGroupImpl entryToObject(TupleInput input) {
            ...
            return new RepGroupImpl(name, uuid, formatVersion, input.readInt(), input.readInt(),
                    ((formatVersion < RepGroupImpl.FORMAT_VERSION_3) ? RepGroupImpl.MIN_FORMAT_VERSION_JE_VERSION
     ...
     * version, and the node's format version is checked on reading to make sure
     * it is not newer than the current group format version, although they
     * could have an older format version if they have not been saved recently.
     * <p>
     * Prior to RepGroupImpl version 3, the second field was always the ordinal
     * value of the node type, which was either 0 or 1. Starting with version 3,
     * values greater than 1 are treated as the rep group version of the format
     * used to write the node binding, with the node type following in the next
     * field, and the jeVersion field added at the end.
     */
    public static class NodeBinding extends TupleBinding<RepNodeImpl> {

        /** The approximate maximum size of the serialized form. */
        static final int         APPROX_MAX_SIZE  = 40 +                        /*
                                                                                 * node
                                                                                 * name
                                                                                 * (guess)
                                                                                 */
        ...
        public RepNodeImpl entryToObject(final TupleInput input) {
            ...
            if (!v2 && (versionOrNodeType > groupFormatVersion)) {
                throw new IllegalStateException(
                        "Node entry version " + versionOrNodeType + " for node " + nameIdPair.getId()
                                + " is illegal because it is newer than group version " + groupFormatVersion);
            }
            final int nodeTypeNum = v2 ? versionOrNodeType : input.readByte();
            return new RepNodeImpl(nameIdPair, NodeType.values()[nodeTypeNum], input.readBoolean(), input.readBoolean(),
                    input.readString(), input.readInt(), new BarrierState(new VLSN(input.readLong()), input.readLong()),
                    input.readInt(), v2 ? null : parseJEVersion(input.readString()));
        }
         ...
         * format version.
         */
        public static boolean supportsObjectToEntry(final RepNodeImpl node, final int groupFormatVersion) {
        ...
        public void objectToEntry(final RepNodeImpl mi, final TupleOutput output) {
            if (!supportsObjectToEntry(mi, groupFormatVersion)) {
                throw new IllegalArgumentException(
                        "Node type " + mi.getType() + " is not supported for group version " + groupFormatVersion);
            }
        ...
        private void phase1() throws DatabaseException {
            ...
            for (int i = 0; i < QUORUM_ACK_RETRIES; i++) {
                ...
                try {
                    ...
                    /* Commit was aborted. */
                    LoggerUtils.warning(logger, repImpl, "Phase 1 retry; for node: " + node.getName()
                            + " insufficient active replicas: " + e.getMessage());
                    continue;
                } catch (InsufficientAcksException e) {
                    phase1Exception = e;
                    /* Local commit completed but did not get enough acks. */
                    LoggerUtils.warning(logger, repImpl,
                            "Phase 1 retry; for node: " + node.getName() + " insufficient acks: " + e.getMessage());
                    continue;
                    ...
                    deadlockHandler();
                    LoggerUtils.warning(logger, repImpl,
                            "Phase 1 retry; for node: " + node.getName() + " deadlock exception: " + e.getMessage());
                    continue;
                } catch (DatabaseException e) {
    ...
    /**
     * An internal API used to obtain group information by opening a stand alone
     * environment handle and reading the RepGroupDB. Used for debugging and
     * utilities.
     *
    ...
    /**
     * Deletes all the current members from the rep group database and creates a
     * new group, with just the member supplied via the configuration. This
     * method exists to support the utility {@link DbResetRepGroup}
     * <p>
     * The changes proceed in three steps: 1) Determine the node id sequence
     * number. This is to ensure that rep node ids are not reused. Old rep node
     * ids are present in the logs as commit records. 2) A new group object,
     * with the node id sequence number determined in step 1), is created and
     * all existing nodes are deleted. 3) The first node is added to the rep
     * group.
     *
    ...
    public void reinitFirstNode(VLSN lastOldVLSN) {
        ...
        RepNodeImpl firstNode = new RepNodeImpl(nodeName, hostname, port, repImpl.getCurrentJEVersion());
        final BarrierState barrierState = new BarrierState(lastOldVLSN, System.currentTimeMillis());
