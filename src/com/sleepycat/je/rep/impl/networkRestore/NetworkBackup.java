 ...
 * deleted or are renamed, depending on how the backup operation was configured.
 * <p>
 * Renamed backup files have the following syntax: NNNNNNNN.bup.<backup number>
...
public class NetworkBackup {
     ...
     * retained under a different name (with a BUP_SUFFIX), or whether it should
     * be deleted.
     */
     ...
     *            file manager because it is used only for logging and
     *            environment invalidation. A network backup may be invoked by
     *            unit tests without an enclosing environment.
     * @param minVLSN the VLSN that should be covered by the server. It ensures
     *            that the log files are sufficiently current for this client's
     *            needs.
     * @throws IllegalArgumentException if the environment directory is not
     *             valid. When used internally, this should be caught
     *             appropriately.
     */
    public NetworkBackup(InetSocketAddress serverSocket, int receiveBufferSize, File envDir, NameIdPair clientNameId,
                         boolean retainLogfiles, int serverLoadThreshold, VLSN minVLSN, RepImpl repImpl,
                         FileManager fileManager, LogManager logManager, DataChannelFactory channelFactory,
        ...
        statistics = new StatGroup(NetworkBackupStatDefinition.GROUP_NAME, NetworkBackupStatDefinition.GROUP_DESC);
     ...
     */
    public NetworkBackup(InetSocketAddress serverSocket, File envDir, NameIdPair clientNameId, boolean retainLogfiles,
                         FileManager fileManager, LogManager logManager, DataChannelFactory channelFactory)
            throws DatabaseException {

        this(serverSocket, 0, envDir, clientNameId, retainLogfiles, Integer.MAX_VALUE, VLSN.NULL_VLSN, null,
                fileManager, logManager, channelFactory, new Properties());
    }
     ...
     */
    public String[] execute() throws IOException, DatabaseException, ServiceConnectFailedException,
            LoadThresholdExceededException, InsufficientVLSNRangeException {

        try {
            channel = channelFactory.connect(serverAddress,
                    new ConnectOptions().setTcpNoDelay(true).setReceiveBufferSize(receiveBufferSize)
            ...
            LoggerUtils.info(logger, repImpl,
                    "Restoring from:" + serverAddress + " Allocated network receive buffer size:"
                            + channel.getSocketChannel().socket().getReceiveBufferSize() + "(" + receiveBufferSize + ")"
                            + " candidate log file count:" + fileNames.length);

            getFiles(fileNames);
            cleanup(fileNames);
            assert fileManager.listJDBFiles().length == fileNames.length : "envDir=" + envDir + " list="
                    + Arrays.asList(fileManager.listJDBFiles()) + " fileNames=" + Arrays.asList(fileNames);
            ...
            LoggerUtils.info(logger, repImpl,
                    "Backup file total: " + backupFileCount.get() + ".  Files actually fetched: " + fetchCount.get()
                            + ".  Files skipped(available locally): " + skipCount.get()
                            + ".  Local files renamed/deleted: " + disposedCount.get());
        }
    }
    ...
    private void checkServer()
            throws IOException, ProtocolException, LoadThresholdExceededException, InsufficientVLSNRangeException {
        ...
        if ((!VLSN.NULL_VLSN.equals(minVLSN))
                && ((resp.getRangeFirst().compareTo(minVLSN) > 0) || (resp.getRangeLast().compareTo(minVLSN)) < 0)) {
            throw new InsufficientVLSNRangeException(minVLSN, resp.getRangeFirst(), resp.getRangeLast());
        }
        if (resp.getActiveFeeders() > serverLoadThreshold) {
            LoadThresholdExceededException exception = new LoadThresholdExceededException(serverLoadThreshold,
     ...
     * the SHA1 if it's not already cached requires a pass over the log file.
     * Note that the server will always send back the SHA1 value if it has it
     * cached.
     */
        ...
        for (final String fileName : fileNames) {
            ...
            if (file.exists() && (fileLength == file.length())) {
                ...
                if (statResp.getDigestSHA1().length == 0) {
                    ...
                    statResp = protocol.read(channel, Protocol.FileInfoResp.class);
                }
                final byte digest[] = LogFileFeeder.getSHA1Digest(file, fileLength).digest();
                if (Arrays.equals(digest, statResp.getDigestSHA1())) {
                    LoggerUtils.info(logger, repImpl, "File: " + file.getCanonicalPath() + " length: " + fileLength
     ...
     * true name only after its digest has been verified. This method is
     * protected to facilitate error testing.
     */
    protected void getFile(File file) throws IOException, ProtocolException, DigestException {
        ...
        /*
         * Delete the tmp file if it already exists. Use the full path of this
         * file in case the environment uses multiple data directories.
         */
        ...
        try {
            ...
            if (logger.isLoggable(Level.INFO)) {
                LoggerUtils.info(logger, repImpl,
                        String.format(
                                "Fetched log file: %s, size: %,d bytes," + " %s bytes," + " %s bytes,"
                                        + " %s bytes/second",
                                file.getName(), fileResp.getFileLength(), transferredBytes, expectedBytes,
                                transferRate));
            }
        } finally {
            fileStream.close();
         ...
         * file creation can safely be called multiple times; the file will only
         * be created the first time.
         */
        restoreMarker.createMarkerFile(RestoreRequired.FailureType.NETWORK_RESTORE, exceptionProperties);
        ...
        if (!file.setLastModified(fileResp.getLastModifiedTime())) {
            throw EnvironmentFailureException.unexpectedState("File.setlastModifiedTime() for:" + file + " and time "
        ...
        if (retainLogfiles) {
            ...
            try {
                ...
                throw EnvironmentFailureException.unexpectedState(
                        "Could not rename log file " + file.getPath() + " because of exception: " + e.getMessage());
            }
    ...
    private String[] getFileList() throws IOException, ProtocolException {
        ...
        FileListResp fileListResp = protocol.read(channel, Protocol.FileListResp.class);
    ...
    private Protocol checkProtocol(Protocol candidateProtocol) throws IOException, ProtocolException {
        ...
        if (serverVersion.getVersion() != candidateProtocol.getVersion()) {
            String message = "Server requested protocol version:" + serverVersion.getVersion()
                    + " but the client version is " + candidateProtocol.getVersion();
    ...
    /*
     * @hidden A test entry point used to simulate a slow network restore.
     */
    ...
    /**
     * Exception indicating that the digest sent by the server did not match the
     * digest computed by the client, that is, the log file was corrupted during
     * transit.
     */
    ...
    /**
     * Exception indicating that the server vlsn range did not cover the VLSN of
     * interest.
     */
    @SuppressWarnings("serial")
    public static class InsufficientVLSNRangeException extends Exception {
        ...
        public String getMessage() {
            return "Insufficient VLSN range. Needed VLSN: " + minVLSN + " Available range: " + "[" + rangeFirst + ", "
                    + rangeLast + "]";
        }
    }
    ...
    public static class LoadThresholdExceededException extends Exception {
        ...
        @Override
        public String getMessage() {
            return "Active server threshold: " + threshold + " exceeded. " + "Active servers: " + activeServers;
        }
    }
}
