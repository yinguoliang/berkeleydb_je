 ...
 * hit in the cache, preventing a lookup in the log buffers or log files. The
 * log item cache is made up of weak references so there is never any guarantee
 * that even the most recent 32 entries are in there.
 */
class LogItemCache {
    ...
    /*
     * Soft reference to array, so that the LogItems can be released when under
     * GC pressure.
     */
    private volatile SoftReference<AtomicReference<LogItem>[]> cacheReference = new SoftReference<AtomicReference<LogItem>[]>(
            null);
     ...
     * allocated twice and introduce a cache miss. This occurrence is infrequent
     * enough that it's not worth the overhead of a sync mechanism.
     *
