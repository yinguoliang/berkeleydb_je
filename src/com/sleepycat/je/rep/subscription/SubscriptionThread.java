...
class SubscriptionThread extends StoppableThread {
    ...
    SubscriptionThread(ReplicatedEnvironment env, VLSN reqVLSN, SubscriptionConfig config, SubscriptionStat stats,
                       Logger logger) {
    ...
    public void run() {
        ...
        LoggerUtils.info(logger, envImpl, "Start subscription from VLSN " + reqVLSN + " from feeder at "
        ...
        try {
            ...
            while (!isShutdown()) {
                try {
                    ...
                    if (!auxThreadCreated) {
                        LoggerUtils.fine(logger, envImpl, "Create auxiliary msg processing " + "and output threads");
                        createAuxThread();
                    ...
                    if (numRetry == maxRetry) {
                        LoggerUtils.info(logger, envImpl,
                                "Reaching the max retry " + maxRetry + " to connect feeder " + config.getFeederHost()
                        ...
                        numRetry++;
                        LoggerUtils.fine(logger, envImpl, "Fail to connect feeder at " + config.getFeederHost()
                                + " sleep for " + e.getRetrySleepMs() + " ms and re-connect again");
            ...
            if (messageProcThread.isAlive()) {
                try {
                    ...
                    LoggerUtils.fine(logger, envImpl, "exception in shutting down msg proc " + "thread "
            ...
            storedException = e;
            LoggerUtils.info(logger, envImpl, "unable to subscribe from requested VLSN " + e.getRefreshVLSN() + "\n"
                    + LoggerUtils.getStackTrace(e));
            status = SubscriptionStatus.VLSN_NOT_AVAILABLE;
        } catch (EnvironmentFailureException e) {
            storedException = e;
            LoggerUtils.warning(logger, envImpl,
                    "unable to sync up with feeder due to EFE " + e.getMessage() + "\n" + LoggerUtils.getStackTrace(e));
    ...
    /**
     * shutdown the subscriber and all auxiliary threads, close channel to the
     * Feeder.
     */
    void shutdown() {
        ...
        if (messageProcThread != null) {
            try {
                ...
                LoggerUtils.warning(logger, envImpl, "error in shutdown msg proc thread: " + e.getMessage()
                        + ", continue shutdown the" + " subscription thread.");
            } finally {
        ...
        if (outputThread != null) {
            try {
                ...
                LoggerUtils.warning(logger, envImpl, "error in shutdown output thread: " + e.getMessage()
                        + ", continue shutdown " + "subscription thread.");
            } finally {
                outputThread = null;
            }
        }
     ...
     */
    void offer(Object message) throws InterruptedException {

        RepImpl repImpl = (RepImpl) envImpl;

        while (!isShutdown() && /* stop enqueue msg if thread is shutdown */
                !inputQueue.offer(message, SubscriptionConfig.QUEUE_POLL_INTERVAL_MS, TimeUnit.MILLISECONDS)) {
            /* Offer timed out. */
            if (!messageProcThread.isAlive()) {
                LoggerUtils.info(logger, repImpl, "Thread consuming input queue is gone, start" + " shutdown process");
                throw new GroupShutdownException(logger, repImpl, config.getFeederHost(), stats.getHighVLSN(), 0);
            } else {
     ...
     */
    private void initializeConnection() throws InternalException, EnvironmentFailureException, ConnectionException {

        /* open a channel to feeder */
        LoggerUtils.fine(logger, envImpl,
                "subscription " + config.getSubNodeName() + " open channel and handshake with feeder");
        try {

            openChannel();
            ReplicaFeederHandshake handshake = new ReplicaFeederHandshake(
                    new SubFeederHandshakeConfig(config.getNodeType()));

            protocol = handshake.execute();

            LoggerUtils.fine(logger, envImpl,
                    "subscription " + config.getSubNodeName() + " sync-up with feeder at vlsn: " + reqVLSN);
            SubscriberFeederSyncup syncup = new SubscriberFeederSyncup(namedChannel, protocol, config.getFeederFilter(),
            ...
            if (!startVLSN.equals(VLSN.NULL_VLSN)) {
                ...
                LoggerUtils.info(logger, envImpl,
                        "Subscription " + config.getSubNodeName() + " successfully connect to feeder at "
                                + config.getFeederHost() + ":" + config.getFeederPort() + ", reqVLSN: " + reqVLSN
                                + ", start VLSN: " + startVLSN);
            } else {
                throw new InsufficientLogException((RepImpl) envImpl, reqVLSN);
            }
        } catch (IOException e) {
            throw new ConnectionException("Unable to connect due to " + e.getMessage() + ",  will retry later.",
                    config.getSleepBeforeRetryMs(), e);
        } catch (EnvironmentFailureException e) {
            logger.warning("Fail to handshake with feeder: " + e.getMessage());
            throw e;
        } catch (BinaryProtocol.ProtocolException e) {
            final String msg = ("Unable to connect to feeder " + config.getFeederHost() + " due to protocol exception "
                    + e.getMessage());
    ...
    private void createAuxThread() {
        ...
        outputThread = new SubscriptionOutputThread(repImpl, outputQueue, protocol, namedChannel.getChannel(), stats);
        outputThread.start();
        LoggerUtils.fine(logger, envImpl, "output thread created for subscription " + config.getSubNodeName());
        /* start thread to consume data in input queue */
        messageProcThread = new SubscriptionProcessMessageThread(repImpl, inputQueue, config, stats, logger);
     ...
     */
    private NamedChannel openChannel() throws ConnectionException, InternalException {
        ...
        try {
            ...
            LoggerUtils.fine(logger, envImpl,
                    "channel opened to service " + FeederManager.FEEDER_SERVICE + "@" + config.getFeederHost()
                            + "[address: " + config.getFeederHostAddr() + " port: " + config.getFeederPort() + "]");
            ...
            throw new ConnectionException(
                    ...
                    config.getSleepBeforeRetryMs(), cause);
            ...
            throw new InternalException("Subscription " + config.getSubNodeName() + "failed to handshake for service "
                    + FeederManager.FEEDER_SERVICE + " with feeder " + config.getFeederHost(), cause);
        }
        LoggerUtils.info(logger, envImpl,
                "Subscription " + config.getSubNodeName() + " has successfully created a channel to feeder at "
                        + config.getFeederHost() + ":" + config.getFeederPort());
    ...
    /**
     * Internal loop to dequeue message from channel to the feeder, process
     * shutdown and heartbeat messages, and relay data operations to the input
     * queue to be consumed by input thread.
     *
     ...
     */
    private void loopInternal() throws InternalException, GroupShutdownException {
        ...
        try {
            ...
            LoggerUtils.info(logger, envImpl,
                    "Start reading messages from feeder " + config.getFeederHost() + ":" + config.getFeederPort());
            while (!isShutdown()) {
                ...
                if (messageOp == Protocol.HEARTBEAT) {
                    LoggerUtils.finest(logger, envImpl, "receive heartbeat from " + namedChannel.getNameIdPair());
                    queueAck(ReplicaOutputThread.HEARTBEAT_ACK);
                } else if (messageOp == Protocol.SHUTDOWN_REQUEST) {

                    LoggerUtils.info(logger, envImpl,
                            "Receive shutdown request from feeder " + config.getFeederHost() + ", shutdown subscriber");

                    /*
                     * create a shutdown request, make it in the queue so client
                     * is able to see that in callback, and throw an exception.
                    ...
                    Exception exp = new GroupShutdownException(logger, repImpl, config.getFeederHost(),
                            stats.getHighVLSN(), req.getShutdownTimeMs());
                    offer(exp);
                    throw exp;
                } else {
    ...
    private void queueAck(Long xid) throws IOException {
        try {
            ...
            /*
             * If interrupted while waiting, have the higher levels treat it
             * like an IOE and exit the thread.
             */
    ...
    private class ConnectionException extends RuntimeException {
        ...
        public ConnectionException(String message, long retrySleepMs, Throwable cause) {
            super(message, cause);
    ...
    private class SubscriptionThreadExceptionHandler implements UncaughtExceptionHandler {
        ...
        public void uncaughtException(Thread t, Throwable e) {
            logger.severe("Error { " + e.getMessage() + " } in SubscriptionThread {" + t
                    + " } was uncaught.\nstack trace:\n" + LoggerUtils.getStackTrace(e));
        }
    }
}
