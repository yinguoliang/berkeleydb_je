 ...
 * its consistency policy. If the Replica cannot catch up in the time defined by
 * its <code>ReplicaConsistencyPolicy</code>, it will throw this exception from
 ...
 * given the load being placed on the system and the hardware resources that are
 * available to service the load. The exception may also indicate that there is
 * a network related issue that is preventing the Replica from communicating
 ...
 * constraints can be satisfied more easily. For example, in a
 * {@link <a href= "{@docRoot}/../ReplicationGuide/lifecycle.html#twonode"> two
 * node replication group</a>}, if the primary goes down, the application may
 * want the secondary node to continue to service read requests, and will lower
 * the consistency requirement on that node in order to maintain read
 * availability.
 *
...
public class ReplicaConsistencyException extends OperationFailureException {
     ...
     */
    public ReplicaConsistencyException(ReplicaConsistencyPolicy consistencyPolicy, String rnName,
                                       boolean unknownMaster) {
        /* No need to set abortOnly, beginTransaction will fail. */
        super(null /* locker */, false /* abortOnly */,
                "Unable to achieve consistency at rep node:" + rnName + ", despite waiting for "
