 ...
 * this node before allowing the application's
 * {@link com.sleepycat.je.Environment#beginTransaction(com.sleepycat.je.Transaction, com.sleepycat.je.TransactionConfig)
 * Environment.beginTransaction()} operation on the Replica to proceed.
 * <p>
 * For example, suppose the application is a web application where a replicated
 * group is implemented within a load balanced web server group. Each request to
 * the web server consists of an update operation followed by read operations
 * (say from the same client), The read operations naturally expect to see the
 * data from the updates executed by the same request. However, the read
...
public class CommitPointConsistencyPolicy implements ReplicaConsistencyPolicy {
     ...
     * Replica that uses this consistency policy is allowed to start only after
     * the transaction identified by the <code>commitToken</code> has been
     * committed on the Replica. The
     * {@link com.sleepycat.je.Environment#beginTransaction( com.sleepycat.je.Transaction, com.sleepycat.je.TransactionConfig)
     * Environment.beginTransaction()} will wait for at most
     * <code>timeout</code> for the Replica to catch up. If the Replica has not
     * caught up in this period, the <code>beginTransaction()</code> method will
     * throw a {@link ReplicaConsistencyException}.
     *
     * @param commitToken the token identifying the transaction
     * @param timeout the maximum amount of time that the transaction start will
     *            wait to allow the Replica to catch up.
     ...
     *         replication stream to the point identified by the commit token or
     *         past it. If it has not, the method waits until the constraint is
     *         satisfied, or the timeout period has expired, whichever event
     *         takes place first.
     */
    @Override
    public void ensureConsistency(EnvironmentImpl envImpl) throws InterruptedException, ReplicaConsistencyException {
        ...
        if (!commitToken.getRepenvUUID().equals(repImpl.getRepNode().getUUID())) {
            throw new IllegalArgumentException("Replication environment mismatch. "
                    + "The UUID associated with the commit token is: " + commitToken.getRepenvUUID()
                    + " but this replica environment has the UUID: " + repImpl.getRepNode().getUUID());
        }
