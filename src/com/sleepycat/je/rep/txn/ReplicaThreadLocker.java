 ...
 * operation on a Replica, for a replicated DB. Like ReadonlyTxn, it enforces
 * read-only semantics and implements the ReplicaConsistencyPolicy. Unlike
 * ReadonlyTxn, the environment default ReplicaConsistencyPolicy is enforced
 * rather than the policy specified via TransactionConfig, and this policy is
 * enforced via the openCursorHook rather than the txnBeginHook.
 */
public class ReplicaThreadLocker extends ThreadLocker {
     ...
     */
    @Override
    public LockResult lockInternal(final long lsn, final LockType lockType, final boolean noWait,
                                   final boolean jumpAheadOfWaiters, final DatabaseImpl database) {
     ...
     * we allow the operation (e.g., for a non-replicated DB), then be sure to
     * call the base class method to prepare to undo in the (very unlikely)
    ...
    /**
     * Verifies that consistency requirements are met before allowing the cursor
     * to be opened.
     */
    @Override
