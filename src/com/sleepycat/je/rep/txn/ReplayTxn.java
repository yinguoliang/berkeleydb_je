...
public class ReplayTxn extends Txn {
    ...
    /*
     * The last VLSN applied by this txn. Used for sanity checking. This field
     * is currently not precisely set when a transaction is resurrected. The
     * sanity check could be made more robust by setting this field at
     * resurrection.
     */
     ...
     * the activeTxns map.
     */
    public ReplayTxn(EnvironmentImpl envImpl, TransactionConfig config, long txnId, SimpleTxnMap<ReplayTxn> activeTxns,
                     Logger logger)
    ...
    public ReplayTxn(EnvironmentImpl envImpl, TransactionConfig config, long txnId, Logger logger)
        ...
        super(envImpl, config, null, // ReplicationContext set later
                txnId); // mandatedId
    ...
    /*
     * A ReplayTxn always writes the node ID of the master which generated the
     * commit or abort.
     */
    @Override
    ...
    /**
     * Utility method to validate DTVLSN. It ensures that the DTVLSN is not null
     * and that DTVLSN(vlsn) <= VLSN.
     */
    private long validateDTVLSN(VLSN txnVLSN, long checkDTVLSN) {
        ...
        if (txnVLSN == null) {
             ...
             * consequently does not have a pre-assigned vlsn; a VLSN will be
             * assigned when the abort is actually written
             */
            return checkDTVLSN;
        }
     ...
     * @throws DatabaseException
     */
    public long commit(SyncPolicy syncPolicy, ReplicationContext clientRepContext, int commitMasterNodeId,
                       long commitDTVLSN)
            throws DatabaseException {
         ...
         * will return this value and write the originating node's id into the
         * commit record on this log.
         */
    ...
    public long commit() {
        throw EnvironmentFailureException
                .unexpectedState("Replay Txn abort semantics require use of internal commit api");
    }
    ...
    public void abort() {
        throw EnvironmentFailureException
                .unexpectedState("Replay Txn abort semantics require use of internal abort api");
    }
     ...
     * this method returns false when no LN has been logged by the txn. But when
     * replaying a Master txn, we need to guarantee that the VLSN is logged on
     * the Replica. [#20543]
     */
    @Override
     ...
     * transaction are marked obsolete. Note that this is by no means a complete
     * implementation of what would be needed to support user visible
     * savepoints. This method only rolls back write operations and doesn't
     * handle other types of state, like read locks and open cursors. There are
     * several key assumptions: - the transaction does not hold read locks. -
     * the transaction will either be resumed, and any rolled back operations
     * will be repeated, or the transaction will be aborted in its entirety. If
     * all operations in the transaction are rolled back, this transaction is
     * also unregistered and closed. Rolling back a log entry through rollback
     * is akin to truncating the transactional log. The on-disk entries should
     * not be referred to by anything in the in-memory tree or the transaction
     * chain. JE's append only storage and the fact that the transactional log
     * entries are intertwined through the physical log prohibits any log
     * truncation. To mimic log truncation, any rolled back log entry is marked
     * as obsolete. Since only the last version of any data record is alive, any
     * future uses of this transaction must use the obsoleteDupsAllowed option
     * (see Txn.countObsoleteExact) to prevent asserts about duplicate obsolete
     * offsets. For example, suppose the transaction logs this: 100 LNa
     * (version1) 200 LNa (version2) 300 LNa (version3) At this point in time,
     * LSN 100 and 200 are obsolete. Now, suppose we roll back to LSN 100. LSNs
     * 200 and 300 are marked obsolete by the rollback.(although LSN 200 was
     * already obsolete). It is true that for an instance in time LSN 100 is
     * incorrectly marked as obsolete, when it's really alive. But this
     * transaction is going to either abort or resume exactly as it was before,
     * so LSN 100 is going to be obsolete again. Suppose txn.abort() is called.
     * The abort() logic will mark LSN 100 as obsolete, since it is the latest
     * version of the record in the transaction. Using the obsoleteDupsAllowed
     * option avoids an assertion on the double recording of LSN 100.
    ...
    public Collection<Long> rollback(long matchpointLsn) throws DatabaseException {
        ...
        /*
         * We don't expect there to be any database handles associated with a
         * ReplayTxn, because only DatabaseImpls are used. Because of that,
         * there should be no cleanup needed.
         */
        if (openedDatabaseHandles != null) {
            throw EnvironmentFailureException.unexpectedState("Replay Txn " + getId() + " has a openedDatabaseHandles");
        }

        /*
         * There is no need to call cleanupDatabaseImpls if the txn still holds
         * locks. The operations in this txn will either be entirely aborted, or
         * will be repeated, so any cleanup will happen when the txn ends.
         */
        return rollbackLsns;
    }
     ...
     * predecessor. This was done to streamline abort processing, so that an
     * undo reverts directly to the original version rather than stepping
     * through all the intermediates. The intermediates are skipped. However,
     * undo to a matchpoint may need to stop at an intermediate point, so we
     * need to create a true chain of versions. To do so, we read the
     * transaction backwards from the last logged LSN to reconstruct a
     * transaction chain that links intermediate versions of records. For
     * example, suppose our transaction looks like this and that we are undoing
     * up to LSN 250 lsn=100 node=A (version 1) lsn=200 node=B (version 1) <--
     * matchpointLsn lsn=300 node=C (version 1) lsn=400 node=A (version 2)
     * lsn=500 node=B (version 2) lsn=600 node=A (version 3) lsn=700 node=A
     * (version 4) To setup the old versions, We walk from LSN 700 -> 100 700
     * (A) rolls back to 600 600 (A) rolls back to 400 500 (B) rolls back to 200
     * 400 (A) rolls back to 100 300 (C) rolls back to an empty slot (NULL_LSN).
     ...
     * entries are no longer valid. Lastly, the appropriate write locks must be
     * released.
     * 
    ...
    private void undoWrites(long matchpointLsn, List<Long> rollbackLsns) throws DatabaseException {
        ...
        /*
         * Generate a map of node->List of intermediate LSNs for this node. to
         * re-create the transaction chain.
         */
        ...
        try {
            while ((undoLsn != DbLsn.NULL_LSN) && DbLsn.compareTo(undoLsn, matchpointLsn) > 0) {
                 ...
                 */
                RecoveryManager.rollbackUndo(logger, Level.FINER, location, undo.db, undo.logEntry, undoLsn, revertTo);
            ...
            /*
             * Correct the fields which hold LSN and VLSN state that may now be
             * changed.
             */
            ...
            LoggerUtils.traceAndLogException(envImpl, "Txn", "undo", "For LSN=" + DbLsn.getNoFormatString(undoLsn), e);
            throw e;
        ...
        if (lastLoggedLsn == DbLsn.NULL_LSN) {
             ...
             * abort which is needed. Set database state for deletes before
    ...
    private void countObsoleteInexact(long undoLsn, UndoReader undo) {
        ...
        envImpl.getLogManager().countObsoleteNode(undoLsn, null, /* type */
                undo.logEntrySize, undo.db, false /* countExact */);
    }
    ...
    /**
     * Returns the time when this transaction was committed or aborted. Returns
     * zero if the transaction is still in progress.
     *
    ...
    public void setLastAppliedVLSN(VLSN justApplied) {
        if (justApplied.compareTo(lastApplied) <= 0) {
            throw EnvironmentFailureException.unexpectedState(
                    "Txn " + getId() + " attempted VLSN = " + justApplied + " txnLastApplied = " + lastApplied);
        }
     ...
     * collection. That collection is only needed by an application-facing Txn,
     * so that those database handles can be invalidated if needed. ReplayTxn is
     * not application-facing, and uses DatabaseImpls rather than Databases.
     */
     ...
     * call lock() on the lsn, but that path is not available because we do not
     * have a handle on a databaseImpl.
     */
    public void stealLockFromMasterTxn(Long lsn) {
        ...
        /*
         * Assert, and if something strange happened, opt to invalidate the
         * environment and wipe the slate clean.
         */
        if (!result.success) {
            throw EnvironmentFailureException.unexpectedState(envImpl,
                    "Transferring from master to replica state, txn " + getId() + " was unable to transfer lock for "
                            + DbLsn.getNoFormatString(lsn) + ", lock grant type=" + result.lockGrant);
        }
