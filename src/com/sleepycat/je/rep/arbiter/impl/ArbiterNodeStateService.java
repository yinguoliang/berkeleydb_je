...
/**
 * The service registered by an Arbiter to answer the state request from another
 * node.
 */
public class ArbiterNodeStateService extends ExecutingService {
    ...
    public ArbiterNodeStateService(ServiceDispatcher dispatcher, ArbiterImpl arbImpl) {
        ...
        protocol = new NodeStateProtocol(arbImpl.getGroupName(), arbImpl.getNameIdPair(), arbImpl.getRepImpl(),
    ...
    public ResponseMessage process(NodeStateRequest stateRequest) {
        return protocol.new NodeStateResponse(arbImpl.getNameIdPair().getName(),
                arbImpl.getMasterStatus().getNodeMasterNameId().getName(), arbImpl.getJoinGroupTime(),
