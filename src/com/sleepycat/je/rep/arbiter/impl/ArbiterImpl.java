...
/**
 * The implementation of the Arbiter. The Arbiter is a participant in elections
 * and may acknowledge transaction commits.
 * <p>
 * The Arbiter persists the Arbiter's replication group node identifier and the
 * highest commit VLSN that has been acknowledged. Currently the Feeder sends
 * commit acknowledgment requests to the Arbiter if the replication factor is
 * two and the other Rep node is not available (RepImpl.preLogCommitHook).
 * <p>
 * The VLSN is used in response to an election promise request. The priority of
 * the response is lower than a RepNode. This allows the RepNode to be selected
 * if the VLSN is equal. An Arbiter may not "win" an election. If the Arbiter's
 * VLSN is the highest in the election, the election result is ignored. A NULL
 * nodeid in a promise response is used to identify a promise response is from
 * an Arbiter.
 * <p>
 * Two pieces of information are persisted by the Arbiter. The replication group
 * node identifier is persisted because this information is located in the group
 * database and the Arbiter does not have a copy of the group database. The
 * other is the high VLSN of a Arbiter ACKed commit.
 * <p>
 * In the future, the algorithm could be changed to request commit
 * acknowledgments when the replication factor is greater than two. This would
 * allow for better write availability when the replication factor is an even
 * number.
 */
...
public class ArbiterImpl extends StoppableThread {
    ...
    /*
     * The latch used to wait for the ArbiterAcker to establish a connection
     * with the Master.
     */
    ...
    /**
     * The Arbiter implementation. Uses the following replication parameters:
     * RepParams.GROUP_NAME The replication group name.
     * RepParams.ENV_UNKNOWN_STATE_TIMEOUT Timeout used for being in the unknown
     * state. RepParams.NODE_HOST_PORT The host name and port associated with
     * this node.
     *
    ...
    public ArbiterImpl(File arbiterHome, RepImpl repImpl)
        ...
        try {
            ...
            throw EnvironmentFailureException.unexpectedException(repImpl,
                    "Problem attempting to join on " + getSocket(), ioe);
        }
    }
    ...
    public StatGroup loadStats(StatsConfig config) {
        ...
        StatGroup trackerStats = arbiterVLSNTracker == null ? ArbiterVLSNTracker.loadEmptyStats()
    ...
    private void initialize() throws IOException {
        ...
        currentState = new AtomicReference<ReplicatedEnvironment.State>(ReplicatedEnvironment.State.UNKNOWN);
    ...
    private void utilityServicesShutdown() {
        ...
        if (binaryStateService != null) {
            try {
                ...
                LoggerUtils.info(logger, repImpl, "Error shutting down binaryStateService " + e.getMessage());
            }
        }
    ...
    public void shutdown() {
        ...
        try {
            ...
            LoggerUtils.info(logger, repImpl, "Error shutting down monitor event manager " + e.getMessage());
        }
    ...
    public void refreshHelperHosts() {
        final Set<InetSocketAddress> helpers = new HashSet<InetSocketAddress>(repImpl.getHelperSockets());
    ...
    /**
     * Communicates with existing nodes in the group in order figure out who is
     * the master. In the case where the local node does not appear to be in the
     * (local copy of the) GroupDB, typically because the node is starting up
     * with an empty env directory. It could be that this is a new node (never
     * before been part of the group). Or it could be a pre-existing group
     * member that has lost its env dir contents and wants to be restored via a
     * Network Restore operation.
     * <p>
     * We query the designated helpers for the Master information. The helpers
     * are the ones that were identified via the node's configuration.
     * <p>
    ...
    private void queryGroupForMembership() throws InterruptedException {
        ...
        LoggerUtils.fine(logger, repImpl, "New node " + nameIdPair.getName() + " located master: " + groupMasterNameId);
    }
    ...
    private boolean checkGroupMasterIsAlive(NameIdPair groupMasterNameId) {
        ...
        try {
            ...
            protocol.write(protocol.new BinaryNodeStateRequest(groupMasterNameId.getName(),
                    repImpl.getConfigManager().get(RepParams.GROUP_NAME)), channel);
        ...
        public void notify(Proposal proposal, Value value) {
            ...
            try {
                ...
                LoggerUtils.logMsg(logger, formatter, Level.FINE,
                        "Arbiter notified of new Master: " + currentMasterName);
                masterStatus.setGroupMaster(currentValue.getHostName(), currentValue.getPort(),
    ...
    private class MasterSuggestionGenerator implements Acceptor.SuggestionGenerator {
        ...
        public Ranking getRanking(Proposal proposal) {
            return new Ranking(arbiterVLSNTracker.getDTVLSN().getSequence(), arbiterVLSNTracker.get().getSequence());
