...
/**
 * The ArbiterAcker is used to acknowledge transactions. A feeder connection is
 * established with the current master. Commit and Heartbeat messages are sent
 * by the master. The ArbiterAcker responds and persistently tracks the high
 * VLSN of the commit messages that it acknowledges. There are configuration
 * parameters that are used. RepParams.REPLICA_MESSAGE_QUEUE_SIZE used for the
 * replay queue size and in the computation of the output queue size.
 ...
 * used for the datachannel buffer size. RepParams.REPSTREAM_OPEN_TIMEOUT used
 * for the datachannel open timeout. RepParams.MAX_CLOCK_DELTA - used for
 * ReplicaFeederHandshake maximum clock delta. RepParams.HEARTBEAT_INTERVAL
 * heartbeat interval in millis. RepParams.ENABLE_GROUP_ACKS enables output
 * thread ack grouping. The main Arbiter thread reads messages from the feeder
 * channel and queues the message on the request queue. The request thread reads
 * entries from the request queue. The request thread may queue an entry on the
 * output queue. The ArbiterOutputThread reads from the output queue and writes
 * to the network channel. read from network -> ArbiterAcker main thread ->
 * requestQueue requestQueue -> RequestThread -> outputQueue outputQueue ->
 * ArbiterOutputThread -> writes to network
 */
class ArbiterAcker {
    ...
    /*
     * The maximum number of entries pulled out of the request queue that are
     * grouped together. There is at most one write to the data file for this
     * group.
     */
    ...
    ArbiterAcker(ArbiterImpl arbiterImpl, RepImpl repImpl) {
        ...
        final int requestQueueSize = repImpl.getConfigManager().getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);
         ...
         * that the RequestThread can completely process the buffered messages
         * in the face of a network drop and 2X to allow for additional headroom
         * and minimize the chances that the operation might be blocked due to
         * the limited queue length.
         */
        final int outputQueueSize = 2 * repImpl.getConfigManager().getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);
    ...
    /**
     * The core Arbiter control loop. The loop exits when it encounters one of
     * the following possible conditions: 1) The connection to the master can no
     * longer be maintained, due to connectivity issues, or because the master
     * has explicitly shutdown its connections due to an election. 2) The node
     * becomes aware of a change in master, that is, assertSync() fails. 3) The
     * loop is interrupted, which is interpreted as a request to shutdown the
     * Arbiter node as a whole. 4) It fails to establish its node information in
     * the master as it attempts to join the replication group for the first
     * time. Normal exit from this run loop results in the Arbiter node retrying
     * finding the group master. A thrown exception, on the other hand, results
     * in the Arbiter node as a whole terminating its operation and no longer
     * participating in the replication group, that is, it enters the DETACHED
     * state.
     *
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    LoggerUtils.fine(logger, repImpl, "Retry #: " + retryCount + "/" + e.retries
                            + " Will retry Arbiter loop after " + e.retrySleepMs + "ms. ");
    ...
    protected void doRunArbiterLoopInternalWork() throws Exception {
        ...
        arbiterOutputThread = new ArbiterOutputThread(repImpl, outputQueue, protocol, arbiterFeederChannel.getChannel(),
    ...
    private void loopExitCleanup() {
        ...
        if (shutdownException != null) {
            if (shutdownException instanceof RetryException) {
                ...
                LoggerUtils.info(logger, repImpl, "Exiting inner Arbiter loop." + " Master requested shutdown.");
            } else {
    ...
    private void createArbiterFeederChannel() throws IOException, ConnectRetryException {
        ...
        try {
            ...
            dataChannel = repImpl.getChannelFactory().connect(arbiterImpl.getMasterStatus().getNodeMaster(),
                    connectOpts);

            arbiterFeederChannel = new NamedChannelWithTimeout(repImpl, logger, arbiterImpl.getChannelTimeoutTask(),
                    dataChannel, timeoutMs);
    ...
    private GroupShutdownException processShutdown(ShutdownRequest shutdown) throws IOException {
        ...
        return new GroupShutdownException(logger, repImpl, masterHostName, arbiterImpl.getArbiterVLSNTracker().get(),
        ...
        for (int i = 0; i < groupMessages.size(); i++) {
            ...
            if (messageOp == Protocol.SHUTDOWN_REQUEST) {
                ...
                if (LOG_TXN_COMMIT.equalsType(entryType)) {
                    ...
                    /*
                     * The Arbiter, unlike Replicas, does not receive commits in
                     * ascending VLSN order, so discard lower DTVLSNs.
                     */
    ...
    class RequestThread extends StoppableThread {
        ...
        public void run() {
            ...
            try {
                while (true) {
                    final Message message = requestQueue.poll(REQUEST_QUEUE_POLL_INTERVAL_NS, TimeUnit.NANOSECONDS);
                ...
                LoggerUtils.info(logger, repImpl, "ArbiterAcker thread exiting with exception:" + e.getMessage());
            }
        }
