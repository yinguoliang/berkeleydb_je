...
/**
 * Thrown by {@link Environment#beginTransaction} and {@link Transaction#commit}
 * when these operations are initiated at a Master which is not in contact with
 * a quorum of Replicas as determined by the {@link ReplicaAckPolicy} that is in
...
public class InsufficientReplicasException extends OperationFailureException {
     ...
     */
    public InsufficientReplicasException(Locker locker, ReplicaAckPolicy ackPolicy, int requiredAckCount,
                                         Set<String> availableReplicas) {
        super(locker, true /* abortOnly */, makeMsg(ackPolicy, requiredAckCount, availableReplicas), null /* cause */);
    ...
    /**
     * Returns the number of nodes (including the master) that were required to
     * be active in order to satisfy the Replica ack policy.
     *
    ...
    private static String makeMsg(ReplicaAckPolicy commitPolicy, int requiredAckCount, Set<String> availableReplicas) {

        String errorPrefix = "Commit policy: " + commitPolicy.name() + " required " + requiredAckCount + " replica"
        ...
        switch (availableReplicas.size()) {
            ...
            default:
                return errorPrefix + " Only the following " + availableReplicas.size()
                        + " replicas listed here were available: " + availableReplicas;
        }
    }
}
