 ...
 * to a replicated environment. The class supplies methods that can be used to
 * list group members, remove members, update network addresses, and find the
 * current master. Information is found and updated by querying nodes in the
 * group. Because of that, ReplicationGroupAdmin can only obtain information
 * when there is at least one node alive in the replication group.
 */
public class ReplicationGroupAdmin {
     ...
     * @param helperSockets the sockets on which it can contact helper nodes in
     *            the replication group to carry out admin services.
     */
    ...
    public ReplicationGroupAdmin(String groupName, Set<InetSocketAddress> helperSockets,
        ...
        electionsProtocol = new Protocol(TimebasedProposalGenerator.getParser(), MasterValue.getParser(), groupName,
                NameIdPair.NOCHECK, null /* repImpl */, channelFactory);
    ...
    /**
     * Sets the helper sockets being used to contact a replication group member,
     * in order to query for the information.
     *
     * @param helperSockets the sockets on which it can contact helper nodes in
     *            the replication group to carry out admin services.
     */
    ...
    /**
     * Returns the socket address associated with the node that's currently the
     * master.
     *
    ...
    private InetSocketAddress getMasterSocket() throws UnknownMasterException, EnvironmentFailureException {
        ...
        MasterValue masterValue = Learner.findMaster(electionsProtocol, helperSockets, logger, null, formatter);
    ...
    /**
     * @hidden Internal implementation class. Ensures that this monitor node is
     *         a member of the replication group, adding it to the group if it
     *         isn't already.
    ...
    public ReplicationNode ensureMonitor(RepNodeImpl monitor)
        ...
        if (!monitor.getType().isMonitor()) {
            throw EnvironmentFailureException.unexpectedState("Node type must be Monitor not: " + monitor.getType());
        }

        MasterValue masterValue = Learner.findMaster(electionsProtocol, helperSockets, logger, null, formatter);
        EnsureOK okResp = (EnsureOK) doMessageExchange(groupProtocol.new EnsureNode(monitor), EnsureOK.class);

        monitor.getNameIdPair().update(okResp.getNameIdPair());
        return new RepNodeImpl(new NameIdPair(masterValue.getNodeName()), NodeType.ELECTABLE, masterValue.getHostName(),
                masterValue.getPort(),
    ...
    /**
     * Removes this node from the group, so that it is no longer a member of the
     * group. When removed, it will no longer be able to connect to a master,
     * nor can it participate in elections. If the node is a
     * {@link com.sleepycat.je.rep.monitor.Monitor} it will no longer be
     * informed of election results. Once removed, a node cannot be added again
     * to the group under the same node name.
     * <p>
     ...
     *             <code>nodeName</code> is not a member of the replication
     *             group
     ...
     *             environment-wide failure occurs.
     * @see <a href=
     *      "{@docRoot}/../ReplicationGuide/utilities.html#node-addremove"
    ...
    public void removeMember(String nodeName)
            throws UnknownMasterException, MemberNotFoundException, MasterStateException, EnvironmentFailureException {
    ...
    /**
     * @hidden internal, for use in disaster recovery [#23447] Deletes this node
     *         from the group, so that it is no longer a member of the group.
     *         When deleted, it will not connect to a master, or participate in
     *         elections until the environment is reopened. If the node is a
     *         {@link com.sleepycat.je.rep.monitor.Monitor} it will no longer be
     *         informed of election results. Unlike removed nodes, deleted nodes
     *         are completely removed from the group, so they can be added again
     *         to the group under the same node name.
     *         <p>
     *         The node being deleted must be shut down before this call is
     *         issued.
     *         <p>
     *         If the node is an active <code>Replica</code> the master will
     *         terminate its connection with the node.
     *         <p>
     *         An active Master cannot be deleted. It must first be shutdown, or
     *         transition to the <code>Replica</code> state before it can be
     ...
     *             <code>nodeName</code> is not a member of the replication
     *             group
     * @throws MasterStateException if the member being deleted is currently the
     *             Master
     * @throws EnvironmentFailureException if an unexpected, internal or
     *             environment-wide failure occurs.
     */
    public void deleteMember(String nodeName) throws UnknownMasterException, MemberActiveException,
            MemberNotFoundException, MasterStateException, EnvironmentFailureException {
    ...
    public ReplicationGroup getGroup() throws UnknownMasterException, EnvironmentFailureException {
        ...
        GroupResponse resp = (GroupResponse) doMessageExchange(groupProtocol.new GroupRequest(), GroupResponse.class);
     ...
     * node and <code>state</code> of the application where the node is running
     * in.
     *
     ...
     * hostname/port. If the log files are available at the node, they will be
     * reused. A network restore operation may need to be initiated by the
     * application to copy over any needed log files if no log files are
     ...
     * @throws EnvironmentFailureException if an unexpected, internal or
     *             environment-wide failure occurs
     * @throws MasterStateException if the member being updated is currently the
     *             master
     * @throws MemberNotFoundException if the node denoted by
     *             <code>nodeName</code> is not a member of the replication
     *             group
     ...
     *             alive
     * @throws UnknownMasterException if the master was not found
     * @see DbResetRepGroup DbResetRepGroup, which can be used in a related but
     *      different use case to copy and move a group.
     */
    /*
     * TODO: EXTERNAL is hidden for now. The doc need updated to include
     * EXTERNAL when it becomes public.
     */
    public void updateAddress(String nodeName, String newHostName, int newPort) throws EnvironmentFailureException,
            MasterStateException, MemberNotFoundException, ReplicaStateException, UnknownMasterException {
    ...
    public String transferMaster(Set<String> nodeNames, int timeout, TimeUnit timeUnit, boolean force)
        ...
        final RequestMessage transferMaster = groupProtocol.new TransferMaster(nodeNameList, timeoutMillis, force);
    ...
    /*
     * Check that the specified node is an appropriate target. For example, make
     * sure it's a valid node in the group, and it's not the same as the
     * original node.
     */
    private RepNodeImpl checkMember(String nodeName, String masterErrorMessage, boolean electableOnly)
        ...
        /* Creation is not yet acknowledged */
            ...
            throw new MemberNotFoundException("Node: " + nodeName + " is not a member of the " + "group: " + groupName);
        }
    ...
    private ResponseMessage doMessageExchange(RequestMessage request, Class<?> respClass)
            throws EnvironmentFailureException, MasterStateException, MemberNotFoundException, UnknownMasterException {

        /* Do the communication. */
        final InetSocketAddress masterAddress = getMasterSocket();
        final MessageExchange me = groupProtocol.new MessageExchange(masterAddress, GroupService.SERVICE_NAME, request);
        me.run();
