...
/**
 * Plays the role of a Proposer in the consensus algorithm. Note that the
 * Proposer also plays the role of a distinguished learner and informs all other
...
public abstract class Proposer {
    ...
    public Proposer(Elections elections, NameIdPair nameIdPair) {
        ...
        statistics = new StatGroup(ProposerStatDefinition.GROUP_NAME, ProposerStatDefinition.GROUP_DESC);
     ...
     * @return the results from running Phase1, or null if phase 1 did not get a
     *         promise from the majority of acceptors.
    ...
    private Phase1Result phase1(QuorumPolicy quorumPolicy, Proposal proposal) throws HigherNumberedProposal {
        ...
        LoggerUtils.logMsg(logger, elections.getRepImpl(), formatter, Level.FINE, "Phase 1 proposal: " + proposal);
        ...
        FutureTrackingCompService<MessageExchange> compService = Utils.broadcastMessage(acceptorSockets,
                Acceptor.SERVICE_NAME, elections.getProtocol().new Propose(proposal), elections.getThreadPool());
     ...
     * @throws HigherNumberedProposal
     */
    private Phase1Result tallyPhase1Results(Proposal currentProposal,
                                            final FutureTrackingCompService<MessageExchange> compService)
            throws HigherNumberedProposal {

        final Phase1Result result = new Phase1Result();

        new Utils.WithFutureExceptionHandler<MessageExchange>(compService, 2 * elections.getProtocol().getReadTimeout(),
            ...
            protected void processResponse(MessageExchange me) {
                ...
                if (op == elections.getProtocol().REJECT) {
                    ...
                    if ((result.acceptProposal == null)
                            || ((acceptProposal != null) && (acceptProposal.compareTo(result.acceptProposal) > 0))) {
                    ...
                    LoggerUtils.logMsg(logger, elections.getRepImpl(), formatter, Level.WARNING,
     ...
     * @throws HigherNumberedProposal
     */
    private Phase2Result phase2(QuorumPolicy quorumPolicy, Proposal proposal, Value phase2Value,
                                Set<InetSocketAddress> promisories)
            throws HigherNumberedProposal {

        LoggerUtils.logMsg(logger, elections.getRepImpl(), formatter, Level.FINE,
        ...
        FutureTrackingCompService<MessageExchange> compService = Utils.broadcastMessage(promisories,
                Acceptor.SERVICE_NAME, accept, elections.getThreadPool());
     ...
     *             encountered
     */
    private Phase2Result tallyPhase2Results(final QuorumPolicy quorumPolicy,
                                            final FutureTrackingCompService<MessageExchange> compService)
            throws HigherNumberedProposal {

        final Phase2Result result = new Phase2Result();
        try {
            new Utils.WithFutureExceptionHandler<MessageExchange>(compService,
                    2 * elections.getProtocol().getReadTimeout(), TimeUnit.MILLISECONDS, logger, elections.getRepImpl(),
                    null) {
     ...
     * @return the candidate value for Phase2, or null, if no suitable value was
    ...
    public WinningProposal issueProposal(QuorumPolicy quorumPolicy, RetryPredicate retryPredicate)
        ...
        while (retryPredicate.retry()) {
            try {
                ...
                final Phase2Result result2 = phase2(quorumPolicy, proposal, phase2Value, result1.promisories);
                if (result2 == null) {
                    continue;
                }

                LoggerUtils.logMsg(logger, elections.getRepImpl(), formatter, Level.INFO,
    ...
    private static class Phase1Result {
        ...
        /*
         * The highest accepted proposal that was received from the promisories.
         * It can be null.
         */
    ...
    public interface Proposal extends Comparable<Proposal> {
        ...
        /**
         * The string representation of the proposal. It must not contain the
