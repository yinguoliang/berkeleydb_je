 ...
 * operations. Note that this class and its subclasses are not synchronized.
 * There must be one instance of this class per thread of control. IMPORTANT:
 * Please avoid all uses of ByteBuffer.get/put when serializing message fields
 * of types: long, int and short to avoid byte order issues. Use
 * LogUtils.read/write methods instead, since they use a single canonical
 * byte-independent representation.
 */
public abstract class BinaryProtocol {

    protected static final int          MESSAGE_HEADER_SIZE  = 2
            /* Message op id (short) */ + 4 /* message size (int) */;
    ...
    public final MessageOp              INCOMPATIBLE_VERSION = new MessageOp((short) 1003, IncompatibleVersion.class);
     ...
     *            implemented/simulated by this protocol when communicating with
     *            the recipient.
     */
    protected BinaryProtocol(NameIdPair nameIdPair, int codeVersion, int configuredVersion, EnvironmentImpl envImpl) {
        ...
        stats = new StatGroup(BinaryProtocolStatDefinition.GROUP_NAME, BinaryProtocolStatDefinition.GROUP_DESC);
        ...
        for (MessageOp op : new MessageOp[] { CLIENT_VERSION, SERVER_VERSION, INCOMPATIBLE_VERSION, PROTOCOL_ERROR }) {
    ...
    /*
     * The shared read and write buffers that are reused. There are two buffers,
     * so that reading and writing can proceed in parallel.
     */
     ...
     * later releases, a config param was added to force UTF-8 to be used. In JE
     * 5.1 and later, the default will be to use UTF-8. [#20967]
     *
    ...
    public static class MessageOp {
        ...
        public MessageOp(short opId, Class<? extends Message> messageClass) {
            ...
            try {
                constructor = messageClass.getConstructor(messageClass.getEnclosingClass(), ByteBuffer.class);
    ...
    private MessageOp getOp(ByteBuffer messageBuffer) {
        ...
        if (messageOp == null) {
            throw EnvironmentFailureException.unexpectedState(envImpl,
                    "Unknown message op id:" + opId + " Known op ids:" + Arrays.toString(ops.keySet().toArray()));
        }
        return messageOp;
    }
    ...
    interface WireFormatable {
        ...
        /*
         * Returns a ByteBuffer holding the message in a representation suitable
         * for use in a network transmission. The buffer is flipped and ready
         * for relative reads.
         */
    ...
    /**
     * Fills a dedicated empty buffer with bytes read from the channel. It flips
     * the buffer after it has been filled, so it's ready for reading.
     *
     * @param channel the channel to be read
     * @param buffer the buffer to be filled
     * @throws IOException if the errors were encountered while reading from the
     *             channel or the buffer could not be filled with the expected
     *             number of bytes
     */
     ...
     *            ReadableByteChannel rather than the more obvious SocketChannel
        ...
        try {
            ...
            if (messageBodySize > 0) {
                if (messageBodySize > maxMessageSize) {
                    throw EnvironmentFailureException.unexpectedState("Message op: " + op + " Body size: "
            ...
            if (messageBodySize < 0) {
                throw EnvironmentFailureException
                        .unexpectedState("Message op: " + op + " Body size: " + messageBodySize);
            }
            /* No body */
    ...
    public <T extends Message> T read(ReadableByteChannel channel, Class<T> cl) throws IOException, ProtocolException {
         ...
         * convenient to deal with responses when there can be multiple possible
         * but related responses.
         */
    ...
    private void write(Message message, WritableByteChannel channel, NameIdPair destinationNameIdPair)
        ...
        if (logger.isLoggable(Level.FINER)) {
            if (destinationNameIdPair == NameIdPair.NULL) {
                ...
                LoggerUtils.logMsg(logger, envImpl, formatter, Level.FINER, "Sent " + message + " to " + channel);
            } else {
    ...
    /**
     * Buffers the serialized form of the message (if possible) for later writes
     * to the network. If buffering the message would result in an overflow, the
     * current contents of the buffer are flushed before the message is added to
     * the buffer. If the size of the message exceeds the size of the buffer,
     * the message is flushed directly to the network.
     *
     ...
     *            It's best for performance if this is a direct byte buffer, to
     *            avoid another copy when the buffer is finally flushed.
     * @param nMessages the number of messages currently in the buffer,
     *            including this message, that is, nMessages is always > 0 upon
     *            entry.
     ...
     */
    public int bufferWrite(WritableByteChannel channel, ByteBuffer batchWriteBuffer, int nMessages, Message message)
            throws IOException {
        ...
        if (batchWriteBuffer.remaining() < messageBuffer.limit()) {
            ...
            if (batchWriteBuffer.remaining() < messageBuffer.limit()) {
                /*
                 * Buffer is too small for message, so write it directly. This
                 * write must always be preceded by a buffer flush.
                 */
     ...
     */
    public void flushBufferedWrites(WritableByteChannel channel, ByteBuffer batchWriteBuffer, int nMessages)
            throws IOException {
    ...
    private void flushBuffer(WritableByteChannel channel, ByteBuffer bb) throws IOException {
         ...
         * example is when the peer fails in the middle of a write; in this case
         * there is a "successful" partial write and the next write will result
         * in an I/O exception. There may be other such examples as well, hence
         * the defensive write loop below which tries to continue writes to
         * their logical conclusion or to an IOE.
         */
        while (bb.remaining() > 0) {
            ...
            if (bytes == 0) {
                ...
                /*
                 * This should not happen since it's a blocking channel, but the
                 * java doc is vague on this subject, so yield control if we are
                 * not making progress.
                 */
                Thread.yield();
            }
    ...
    /**
     * Base message class for all messages exchanged in the protocol. Serialized
     * layout of a message: - opType (short) - size of the body of the message
     * (int) - body of the message - message specific fields
     * <p>
    ...
    protected abstract class SimpleMessage extends Message {
         ...
         * reason that the method is associated with the Handshake message class
         * and not the Message class.
         *
        ...
        protected void putWireFormat(final ByteBuffer buffer, final Object obj) {
            ...
            if (cl == Long.class) {
                ...
                /*
                 * A string is stored with its length followed by its contents.
                 */
    ...
    static public class ProtocolException extends InternalException {
        ...
        public String getMessage() {
            return (unexpectedMessage != null)
                    ? ("Expected message type: " + cl + " but found: " + unexpectedMessage.getClass() +
