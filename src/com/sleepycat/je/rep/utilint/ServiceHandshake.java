 ...
 * service to be accessed and authentication for access to the service. The
 * service initialization protocol looks like one of these flows. In the flows,
 * the notation byte(X) indicates that X is a 1-byte network field and 2byte(X)
 * indicates that X is a 2-byte network field. If No authentication is
 * configured: Client Dispatcher -> "Service:" + byte(len) + <service name> <-
 * byte(OK|BUSY|UNKNOWN_SERVICE) If authentication is configured: Client
 * Dispatcher -> "Service:" + byte(len) + <service name> <- byte(AUTHENTICATE)
 * -> "Authenticate:" + byte(<len>) + <mechanism list> <- "Mechanism:" + 2byte(
 * <len>) + <server-selected mech name> + ":" + <auth mech params> ->
 * <Mechanism-specific payload> <- byte(OK|BUSY|UNKNOWN_SERVICE|INVALID) The
 * code here is organized into client-side and server-side functions.
...
public class ServiceHandshake {
    ...
    static class ServerHandshake {
        ...
        /**
         * Callback for underlying init operations upon successful discovery of
        ...
        /**
         * Process the handshake incrementally, as channel is presumed to be in
         * non-blocking mode. This function returns one of the following:
         * InitResult.READ: handshake is not complete; more data needed
         * InitResult.FAIL: handshake was rejected or otherwise failed the
         * underlying data channel has been closed. InitResult.DONE: handshake
         * is complete, and any authentication has been performed. A Response
         * still needs to be sent to the client. Caller must do this after any
         * additional validation that may be required.
         */
        InitResult process() throws IOException {
            ...
            if (channel.isTrustCapable()) {
                ...
                logMsg(Level.WARNING, false, // noteError
            ...
            if (currentOp instanceof RequireAuthenticateOp || authInfo == null) {
                /*
                 * Either we've just successfully completed authentication or no
    ...
    static class ClientHandshake {
        ...
        /**
         * Process the entire handshake sequence and report the final response
         * code received from the service dispatcher.
         */
    ...
    abstract static class ServerInitOp {
         ...
         * This function returns one of the following: InitResult.READ:
         * operation is not complete; more data needed InitResult.FAIL:
         * operation was rejected or otherwise failed the underlying data
         * channel has been closed. InitResult.DONE: operation has completed
         */
        abstract InitResult processOp(DataChannel channel) throws IOException;
        ...
        protected InitResult fillBuffer(DataChannel channel, ByteBuffer buffer) throws IOException {
            ...
            while (buffer.remaining() > 0) {
                ...
                if (count < 0) {
                    /* Premature EOF */
                    initState.logMsg(Level.WARNING, true, // noteError
                            "Premature EOF on channel: " + channel + ", service: " + initState.getServiceName()
                                    + " - read() returned: " + count);
        ...
        /**
         * Helper function to write the contents of the buffer. We make the
         * simplifying assumption here that we will be able to write as much as
            ...
            while (buffer.remaining() > 0) {
                ...
                if (count < 0) {
                    ...
                    if (tryCount > CHANNEL_WRITE_ATTEMPT_LIMIT) {
                        initState.logMsg(Level.WARNING, true, // noteError
                                "Failed to write to channel. " + "Send buffer size: "
        ...
        void closeChannel(Channel channel) {
            if (channel != null) {
                try {
                    ...
                    initState.logMsg(Level.WARNING, false, // noteError
    ...
    /**
     * The base class of elemental service initialization protocol operations
     * for the client side. The operation is assumed to be in blocking I/O mode.
     */
    abstract static class ClientInitOp {
        ...
        /**
         * Process the operation in support of the handshake. This operation may
         * consist of read and/or write operations. This function returns one of
         * the following: InitResult.FAIL: operation was rejected or otherwise
         * failed. The underlying communication channel needs to be closed by
         * the caller. InitResult.DONE: operation has completed. The most recent
         * Response from the server received during this operation is available
         * for inspection. Note that this may return DONE with the response set
         * to a failure response value (e.g. INVALID)
         */
        abstract InitResult processOp(IOAdapter ioAdapter) throws IOException;
    ...
    /**
     * Operations to communicate the requested service name. This is the initial
     * portion of the message and if we have a connection from a foreign entity,
     * this is most likely to detect an error.
     */
    ...
    /**
     * Server-side: expect client to provide a service name. Expected data
     * format: Literal: "Service:" in ASCII encoding Length: 1 byte Service
     * Name: <Length> bytes in ASCII encoding
     */
    static class ReceiveNameOp extends ServerInitOp {
        ...
        InitResult processOp(DataChannel channel) throws IOException {
            ...
            if (buffer.capacity() == INITIAL_BUFFER_SIZE) {
                 ...
                 * "Service:"<Length> Make sure that it has the right format.
                 */
            ...
            /*
             * If we made it here, we have a complete service request message.
             * Extract the service name from the buffer
             */
    ...
    /**
     * Client-side: Send the initial service request to the server and await a
     * Response.
     */
    static class SendNameOp extends ClientInitOp {
         ...
         * ServiceDispatcher.
         *
         * @param serviceName the service that is being requested. The service
         *            name must be less than SERVICE_NAME_LIMIT in size.
    ...
    /*
     * Top-level operations for handling service authentication. This class is
     * used upon receipt of a service request which we have responded to with an
     * AUTHENTICATE. The operation has two sub-phases: 1) negotiate an
     * authentication methods, 2) complete the negotiated method.
     */
    ...
    /**
     * Server side operation to communicate the need for authentication to the
     * client, negotiate an authentication mechanism and then complete the
     * authentication.
     */
    static class RequireAuthenticateOp extends ServerInitOp {
        ...
        InitResult processOp(DataChannel channel) throws IOException {
            ...
            if (authOp == null) {
                ...
                final String authResponseStr = selectedAuth.getMechanismName() + ":" + selectedAuth.getServerParams();
    ...
    /**
     * Client side operation to request authentication. This is done in response
     * to a message from the server notifying us that authentication is
     * required. The implementation of processOp() aggregates functionality from
     * SendRequestOp, which tells the server what authentication we can provide,
     * and then when we've agreed with the server on how to communicate, another
     * ClientOp is created that provides the actual authentication
     * implementation.
     */
    static class DoAuthenticateOp extends ClientInitOp {
        ...
        InitResult processOp(IOAdapter ioAdapter) throws IOException {
            ...
            /*
             * Now use the authOp determined by sendOp() to perform the actual
    ...
    /**
     * Server side Authentication request setup. Expect an authentication
     * request that looks like: Authenticate:<len><mechanism list> where len is
     * a 1-byte length and mechanism list is a comma-separated list of available
     * authentication mechanisms. Upon completion, selectedMechanism is non-null
     * if the client supports an authentication mechanism in common with us.
     */
    static class ExpectAuthRequestOp extends ServerInitOp {
        ...
        /**
         * Return the negotiated authentication mechanism. Returns null if no
         * common authentication mechanism found.
         */
    ...
    static class SendRequestOp extends ClientInitOp {
        ...
        InitResult processOp(IOAdapter ioAdapter) throws IOException {
            ...
            /*
             * response is PROCEED, so find out what mechanism we are to use.
             */
            ...
            if (!arraysEqual(AUTH_MECH_PREFIX_BYTES, mechPrefix, AUTH_MECH_PREFIX_BYTES.length)) {
                throw new IOException("Unexpected service authenticate response: " + encodeBytes(mechPrefix));
            }
            ...
            /*
             * The mechStr is a concatenation of mechanism name and mechanism
             * parameters, if required, separated by a ':'.
             */
         ...
         * ServiceDispatcher. Looks like: Authenticate:<len><mech list>
         *
    ...
    /**
     * Returns a comma-delimited list of authentication mechanism names from the
     * authList argument
     */
    ...
    /**
     * Find the first Authentication instance whose mechanism name matches one
     * of the entries in mechList.
     */
     ...
     * authentication mechanisms available on the client are available on the
     * dispatcher.
     */
     ...
     * @param len the number of bytes to compare - must be less than or equal to
     *            the length of the shorter of array1, array2
