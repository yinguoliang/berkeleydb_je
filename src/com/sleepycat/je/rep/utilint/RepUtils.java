...
public class RepUtils {
     ...
     */
    private static final Map<String, ConsistencyPolicyFormat<?>> consistencyPolicyFormats = new HashMap<String, ConsistencyPolicyFormat<?>>();
    static {
     ...
     * of a thread to cause the thread waiting on the queue to wake up and take
     * notice.
     */
     ...
     *
     * @param name must be the first part of the policy string with a non-letter
     *            delimiter following it, or must be the entire policy string.
     * @param format to register.
     */
    public static void addConsistencyPolicyFormat(final String name, final ConsistencyPolicyFormat<?> format) {
     ...
     * internal handler that formats and parses the string representation of the
     * policy. Only a fixed number of string-representable policies are
    ...
    private static class TimeConsistencyPolicyFormat implements ConsistencyPolicyFormat<TimeConsistencyPolicy> {
        ...
        public String policyToString(final TimeConsistencyPolicy policy) {
            return policy.getName() + "(" + policy.getPermissibleLag(TimeUnit.MILLISECONDS) + " ms,"
                    + policy.getTimeout(TimeUnit.MILLISECONDS) + " ms)";
        }
    ...
    /**
     * Converts a policy into a string suitable for use as a property value in a
     * je.properties file or elsewhere.
     *
     ...
     *             property value format, via ReplicationConfig(Properties) ctor
     *             and setter.
    ...
    public static ReplicaConsistencyPolicy getReplicaConsistencyPolicy(String propertyValue)
        ...
        for (final Map.Entry<String, ConsistencyPolicyFormat<?>> entry : consistencyPolicyFormats.entrySet()) {
            final String name = entry.getKey();
            if (upperCasePropertyValue.equals(name)
                    || (upperCasePropertyValue.startsWith(name) && upperCasePropertyValue.length() > name.length()
    ...
    public static class ExceptionAwareCountDownLatch extends CountDownLatch {
         ...
         *
         * @param e the exception to be wrapped in a DatabaseException and
         *            thrown.
         */
     ...
     */
    private static DatabaseException prepareTerminatingException(final Exception e, final EnvironmentImpl envImpl) {

        if (e == null) {
            return null;
        }

        final DatabaseException de = (e instanceof DatabaseException) ? ((DatabaseException) e)
    ...
    public static void shutdownChannel(DataChannel channel) {
        ...
        /*
         * For SSL, shutting down the socket before shutting down the channel is
         * a no-no. That results in SSLExceptions being thrown due to missing
         * close_notify alerts.
         */
        try {
            channel.close();
     ...
     * @return the connected channel
     */
    public static DataChannel openBlockingChannel(InetSocketAddress addr, DataChannelFactory factory,
    ...
    /**
     * Chains an old outstanding exception to the tail of a new one, so it's not
     * lost.
     *
    ...
    public static String writeTimesString(StatGroup stats) {
        ...
        return String.format(" write time: %, dms Avg write time: %,dus", nWriteNanos / 1000000, avgWriteNanos / 1000);
    }
     ...
     *         found in the file.
     * @throws IllegalArgumentException if the accessPropsFile contains invalid
     *             settings.
     */
    ...
    /**
     * A simple debugging utility used to obtain information about the execution
     * environment that's only available through some system utility, like
     * netstat, or jps, etc. It's up to the caller to ensure the availability of
     * the utility and ensure that it's on the search path.
     *
