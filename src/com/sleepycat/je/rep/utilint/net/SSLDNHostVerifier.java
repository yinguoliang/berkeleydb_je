 ...
 * designed for the case where it is expected that the server's certificate does
 * not match the host name, but instead, contains a well-known distinguished
 * name (DN). This check verifies that the DN matches expectations.
 * <p>
...
public class SSLDNHostVerifier extends SSLDNMatcher implements HostnameVerifier {
     ...
     *            requires a Java regular expression to be applied to the
     *            subject common name.
     */
     ...
     *
     * @param targetHost the intended target of a network connection This
     *            parameter is not used by this implementation.
     * @param sslSession the SSLSession that has been set up for the connection
     * @return true if sslSession indicates that the connection has been made to
     *         the correct host
     */
    @Override
