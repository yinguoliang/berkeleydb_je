...
public class PreloadConfig implements Cloneable {
	 ...
	 *
	 * @param maxBytes
	 *            If the maxBytes parameter is non-zero, a preload will stop
	 *            when the cache contains this number of bytes.
	 *
	 * @return this
	 */
	 ...
	 *
	 * @param maxMillisecs
	 *            If the maxMillisecs parameter is non-zero, a preload will stop
	...
	/**
	 * Preload progress listeners report this phase value, along with a count of
	 * the number if times that the preload has fetched from disk.
	 */
	 ...
	 * {@link ProgressListener} to provide feedback on preload progress. The
	 * ProgressListener.progress() method is called each time the preload mush
	 * fetch a btree node or data record from disk.
	 * <p>
	 ...
	 *            The ProgressListener to callback during preload.
	 */
	 ...
	 * in general improve performance at the expense of memory. Each LSN uses 16
	 * bytes of memory.
	 *
	 * @param lsnBatchSize
	 *            the maximum number of LSNs to accumulate and sort per batch.
	 *
	 * @return this
	 */
	...
	/**
	 * Preload is implemented to optimize I/O cost by fetching the records of a
	 * Database by disk order, so that disk access is are sequential rather than
	 * random. LSNs (log sequence numbers) are the disk addresses of database
	 * records. Setting this value causes the preload to process batches of LSNs
	 * rather than all in-memory LSNs at one time, which bounds the memory usage
	 * of the preload processing, at the expense of preload performance.
	 *
	 * @return the maximum number of LSNs to be sorted that this preload is
	 *         configured for.
	 */
	...
	/**
	 * Set the maximum amount of non JE Cache Memory that preload can use at one
	 * time. The default is an unlimited amount of memory. Setting this lower
	 * causes the preload to use less memory, but generally results in slower
	 * performance. Setting this higher will often improve performance at the
	 * expense of higher memory utilization.
	 *
	 * @param internalMemoryLimit
	 *            the maximum number of non JE Cache bytes to use.
	 *
	 * @return this
	 */
