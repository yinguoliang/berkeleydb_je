 ...
 * ReentrantLock, which is extended for a few reasons: 1) To prevent reentrant
 * use, since latches are not used reentrantly in JE. This increases reliability
 * by detecting accidental reentrant calls. Checks for reentrancy are
 * unconditional, i.e., checked in production. 2) To support instrumentation for
 * debugging (see LatchSupport). 3) Automatic use of configured latch timeout.
 * 4) Built-in thread interrupt handling. Latches are expected to be held for
 * short, defined periods of time. No deadlock detection is provided so it is
 * the caller's responsibility to sequence latch acquisition in an ordered
 * fashion to avoid deadlocks.
 */
public interface Latch {
    ...
    /**
     * Acquires a latch for exclusive/write access, but do not block if it's not
     * available.
     *
    ...
    /**
     * Releases the latch. If there are other thread(s) waiting for the latch,
     * one is woken up and granted the latch. If the latch was not owned by the
     * caller, just return.
     */
    void releaseIfOwner();
