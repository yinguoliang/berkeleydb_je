...
public class DiskOrderedCursorConfig implements Cloneable {
	 ...
	 * data. The default value is false (key + data). If keyOnly is true, the
	 * performance of the disk ordered scan will be better, because the Cursor
	 * only descends to the BIN level.
	 *
	 * @param keysOnly
	 ...
	 * Returns false if it is configured to return keys + data.
	 *
	...
	/**
	 * Specify whether the DiskOrderedCursor should scan the BINs only. If true,
	 * the performance of the disk ordered scan will be better, because LNs are
	 * not read from disk. However, in this case, the data portion of a record
	 * will be returned only if it is embedded in the BIN; otherwise only the
	 ...
	 * returning all record keys and only those record data that are embedded in
	 * the BINs.
	 *
	...
	/**
	 * Set the maximum amount of JE Cache Memory that the DiskOrderedScan can
	 * use at one time. The default is an unlimited amount of memory. Setting
	 * this lower causes the DiskOrderedScan to use less memory, but it sorts
	 * and processes LSNs more frequently thereby generally causing slower
	...
	/**
	 * Returns the maximum amount of JE Cache Memory that the DiskOrderedScan
	 * can use at one time.
	 *
	...
	/**
	 * Set the queue size for entries being passed between the DiskOrderedCursor
	 * producer thread and the application's consumer thread. If the queue size
	 * reaches this number of entries, the producer thread will block until the
	 * application thread removes one or more entries (by calling
	 * ForwardCursor.getNext(). The default is 1000.
	 *
	 * @param queueSize
	 *            the maximum number of entries the queue can hold before the
