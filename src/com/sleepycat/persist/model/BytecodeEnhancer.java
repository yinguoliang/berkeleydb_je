 ...
 * persistent if it contains the @Entity or @Persistent annotations. A resulting
 * enhanced class implements the com.sleepycat.persist.impl.Enhanced interface.
 * <p>
...
class BytecodeEnhancer extends ClassVisitor {
    ...
    private void sortFields() {
        ...
        /*
         * System.out.println("PriKey: " + priKeyField); System.out.println(
         * "SecKeys: " + secKeyFields); System.out.println("NonKeys: " +
         * nonKeyFields); //
         */
    }

    /**
     * Outputs code in a static block to register the prototype instance: static
     * { EnhancedAccessor.registerClass(TheClassName, new TheClass()); // or for
     * an abstract class: EnhancedAccessor.registerClass(TheClassName, null); }
     */
    private void genStaticBlock() {
        ...
        mv.visitMethodInsn(INVOKESTATIC, "com/sleepycat/persist/impl/EnhancedAccessor", "registerClass",
    ...
    /**
     * public Object bdbNewInstance() { return new TheClass(); // or if
     * abstract: return null; }
     */
    private void genBdbNewInstance() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbNewInstance", "()Ljava/lang/Object;", null, null);
        mv.visitCode();
        if (isAbstract) {
            mv.visitInsn(ACONST_NULL);
    ...
    /**
     * public Object bdbNewArray(int len) { return new TheClass[len]; // or if
     * abstract: return null; }
     */
    private void genBdbNewArray() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbNewArray", "(I)Ljava/lang/Object;", null, null);
        mv.visitCode();
        if (isAbstract) {
            mv.visitInsn(ACONST_NULL);
            mv.visitInsn(ARETURN);
    ...
    /**
     * public boolean bdbIsPriKeyFieldNullOrZero() { return theField == null; //
     * or zero or false, as appropriate // or if no primary key but has
     * superclass: return super.bdbIsPriKeyFieldNullOrZero(); }
     */
     ...
     * output.writeKeyObject(theField, format); // or output.writeInt(theField);
     * // and other simple types // or if no primary key but has superclass:
     * return super.bdbWritePriKeyField(output, format); }
     */
    private void genBdbWritePriKeyField() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbWritePriKeyField",
                "(Lcom/sleepycat/persist/impl/EntityOutput;" + "Lcom/sleepycat/persist/impl/Format;)V", null, null);
        mv.visitCode();
        if (priKeyField != null) {
            if (!genWriteSimpleKeyField(mv, priKeyField)) {
                ...
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeKeyObject",
            ...
            mv.visitVarInsn(ALOAD, 1);
            mv.visitVarInsn(ALOAD, 2);
            mv.visitMethodInsn(INVOKESPECIAL, superclassName, "bdbWritePriKeyField",
     ...
     * theField = (TheFieldClass) input.readKeyObject(format); // or theField =
     * input.readInt(); // and other simple types // or if no primary key but
     * has superclass: super.bdbReadPriKeyField(input, format); }
     */
    private void genBdbReadPriKeyField() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbReadPriKeyField",
                "(Lcom/sleepycat/persist/impl/EntityInput;" + "Lcom/sleepycat/persist/impl/Format;)V", null, null);
        mv.visitCode();
        if (priKeyField != null) {
            if (!genReadSimpleKeyField(mv, priKeyField)) {
                ...
                mv.visitVarInsn(ALOAD, 1);
                mv.visitVarInsn(ALOAD, 2);
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readKeyObject",
            ...
            mv.visitVarInsn(ALOAD, 1);
            mv.visitVarInsn(ALOAD, 2);
            mv.visitMethodInsn(INVOKESPECIAL, superclassName, "bdbReadPriKeyField",
     ...
     * output.writeInt(secKeyField1); output.writeObject(secKeyField2, null); //
     * etc }
     */
    private void genBdbWriteSecKeyFields() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbWriteSecKeyFields",
                "(Lcom/sleepycat/persist/impl/EntityOutput;)V", null, null);
        mv.visitCode();

        /*
         * In JE 5.0, String is treated as primitive type, so String does not
         * need to be registered. [#19247]
         */
        ...
        if (hasPersistentSuperclass) {
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKESPECIAL, superclassName, "bdbWriteSecKeyFields",
    ...
    /**
     * public void bdbReadSecKeyFields(EntityInput input, int startField, int
     * endField, int superLevel) { input.registerPriKeyObject(priKeyField); //
     * if an object // if has super: if (superLevel != 0) {
     * super.bdbReadSecKeyFields(..., superLevel - 1); } if (superLevel <= 0) {
     * switch (startField) { case 0: secKeyField1 = input.readInt(); if
     * (endField == 0) break; case 1: secKeyField2 = (String)
     * input.readObject(); if (endField == 1) break; case 2: secKeyField3 =
     * input.readInt(); } } }
     */
    private void genBdbReadSecKeyFields() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbReadSecKeyFields",
    ...
    /**
     * output.registerPriKeyObject(priKeyField); // or
     * input.registerPriKeyObject(priKeyField);
     */
    private void genRegisterPrimaryKey(MethodVisitor mv, boolean input) {
        String entityInputOrOutputClass = input ? "com/sleepycat/persist/impl/EntityInput"
                : "com/sleepycat/persist/impl/EntityOutput";
        mv.visitVarInsn(ALOAD, 1);
        mv.visitVarInsn(ALOAD, 0);
        mv.visitFieldInsn(GETFIELD, className, priKeyField.name, priKeyField.type.getDescriptor());
        mv.visitMethodInsn(INVOKEINTERFACE, entityInputOrOutputClass, "registerPriKeyObject", "(Ljava/lang/Object;)V");
    }
    ...
    private void genRegisterPrimaryStringKey(MethodVisitor mv) {
        ...
        mv.visitVarInsn(ALOAD, 0);
        mv.visitFieldInsn(GETFIELD, className, priKeyField.name, priKeyField.type.getDescriptor());
        mv.visitMethodInsn(INVOKEINTERFACE, entityInputOrOutputClass, "registerPriStringKeyObject",
                "(Ljava/lang/Object;)V");
    }

    /**
     * public void bdbWriteNonKeyFields(EntityOutput output) { // like
     * bdbWriteSecKeyFields but does not call registerPriKeyObject }
     */
    private void genBdbWriteNonKeyFields() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbWriteNonKeyFields",
                "(Lcom/sleepycat/persist/impl/EntityOutput;)V", null, null);
        mv.visitCode();
        if (!isCompositeKey) {
            if (hasPersistentSuperclass) {
                mv.visitVarInsn(ALOAD, 0);
                mv.visitVarInsn(ALOAD, 1);
                mv.visitMethodInsn(INVOKESPECIAL, superclassName, "bdbWriteNonKeyFields",
                        "(Lcom/sleepycat/persist/impl/EntityOutput;)V");
            }
            for (FieldInfo field : nonKeyFields) {
                genWriteField(mv, field);
    ...
    /**
     * public void bdbReadNonKeyFields(EntityInput input, int startField, int
     * endField, int superLevel) { // like bdbReadSecKeyFields but does not call
     * registerPriKeyObject }
     */
    private void genBdbReadNonKeyFields() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbReadNonKeyFields",
                "(Lcom/sleepycat/persist/impl/EntityInput;III)V", null, null);
        mv.visitCode();
        if (!isCompositeKey) {
    ...
    /**
     * public void bdbWriteCompositeKeyFields(EntityOutput output, Format[]
     * formats) { output.writeInt(compositeKeyField1);
     * output.writeKeyObject(compositeKeyField2, formats[1]); // etc }
     */
    ...
    /**
     * public void bdbReadCompositeKeyFields(EntityInput input, Format[]
     * formats) { compositeKeyField1 = input.readInt(); compositeKeyField2 =
     * input.readKeyObject(formats[1]); }
     */
    private void genBdbReadCompositeKeyFields() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbReadCompositeKeyFields",
                "(Lcom/sleepycat/persist/impl/EntityInput;" + "[Lcom/sleepycat/persist/impl/Format;)V", null, null);
        mv.visitCode();
        if (isCompositeKey) {
            for (int i = 0; i < nonKeyFields.size(); i += 1) {
    ...
    /**
     * output.writeInt(field); // and other primitives // or
    ...
    private void genWriteField(MethodVisitor mv, FieldInfo field) {
        ...
        if (field.isString) {
             ...
             */
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeString",
                    "(Ljava/lang/String;)Lcom/sleepycat/bind/tuple/TupleOutput;");
            mv.visitInsn(POP);
        } else if (sort == Type.OBJECT || sort == Type.ARRAY) {
            mv.visitInsn(ACONST_NULL);
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeObject",
                    "(Ljava/lang/Object;Lcom/sleepycat/persist/impl/Format;)V");
        } else {
    ...
    /**
     * Generates writing of a simple type key field, or returns false if the key
     * field is not a simple type (i.e., it is a composite key type).
     * output.writeInt(theField); // and other primitives // or
     * output.writeInt(theField.intValue()); // and other simple types // or
     * returns false
     */
    private boolean genWriteSimpleKeyField(MethodVisitor mv, FieldInfo field) {
        ...
        if (sort != null) {
            ...
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeBigInteger",
                    "(Ljava/math/BigInteger;)Lcom/sleepycat/bind/tuple/TupleOutput;");
            mv.visitInsn(POP);
        } else if (fieldClassName.equals(BigDecimal.class.getName())) {
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeSortedBigDecimal",
    ...
    /**
     * // if has super: if (superLevel != 0) { super.bdbReadXxxKeyFields(...,
     * superLevel - 1); }
     */
    private void genReadSuperKeyFields(MethodVisitor mv, boolean areSecKeyFields) {
        if (hasPersistentSuperclass) {
            ...
            mv.visitMethodInsn(INVOKESPECIAL, superclassName, name, "(Lcom/sleepycat/persist/impl/EntityInput;III)V");
            mv.visitLabel(next);
        }
    }

    /**
     * public void bdbReadXxxKeyFields(EntityInput input, int startField, int
     * endField, int superLevel) { // ... if (superLevel <= 0) { switch
     * (startField) { case 0: keyField1 = input.readInt(); if (endField == 0)
     * break; case 1: keyField2 = (String) input.readObject(); if (endField ==
     * 1) break; case 2: keyField3 = input.readInt(); } }
     */
    ...
    /**
     * field = input.readInt(); // and other primitives // or field =
     * (FieldClass) input.readObject();
     */
    private void genReadField(MethodVisitor mv, FieldInfo field) {
        ...
        if (field.isString) {
             ...
             * format ID for String data. [#19247]
             */
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readStringObject",
                    "()Ljava/lang/Object;");
            mv.visitTypeInsn(CHECKCAST, getTypeInstName(field.type));
        } else if (isRefType(field.type)) {
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readObject",
                    "()Ljava/lang/Object;");
            mv.visitTypeInsn(CHECKCAST, getTypeInstName(field.type));
        } else {
    ...
    /**
     * Generates reading of a simple type key field, or returns false if the key
     * field is not a simple type (i.e., it is a composite key type). field =
     * input.readInt(); // and other primitives // or field =
     * Integer.valueOf(input.readInt()); // and other simple types // or returns
     * false
     */
    private boolean genReadSimpleKeyField(MethodVisitor mv, FieldInfo field) {
        ...
        if (sort != null) {
            ...
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readString",
                    "()Ljava/lang/String;");
        } else if (fieldClassName.equals(BigInteger.class.getName())) {
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readBigInteger",
                    "()Ljava/math/BigInteger;");
        } else if (fieldClassName.equals(BigDecimal.class.getName())) {
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readSortedBigDecimal",
    ...
    /**
     * public Object bdbGetField(Object o, int field, int superLevel, boolean
     * isSecField) { if (superLevel > 0) { // if has superclass: return
     * super.bdbGetField (o, field, superLevel - 1, isSecField); } else if
     * (isSecField) { switch (field) { case 0: return Integer.valueOf(f2); case
     * 1: return f3; case 2: return f4; } } else { switch (field) { case 0:
     * return Integer.valueOf(f5); case 1: return f6; case 2: return f7; } }
     * return null; }
     */
    private void genBdbGetField() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbGetField", "(Ljava/lang/Object;IIZ)Ljava/lang/Object;", null,
                null);
        mv.visitCode();
        mv.visitVarInsn(ILOAD, 3);
        Label l0 = new Label();
        ...
        if (hasPersistentSuperclass) {
            ...
            mv.visitInsn(ISUB);
            mv.visitVarInsn(ILOAD, 4);
            mv.visitMethodInsn(INVOKESPECIAL, className, "bdbGetField", "(Ljava/lang/Object;IIZ)Ljava/lang/Object;");
            mv.visitInsn(ARETURN);
        } else {
            mv.visitJumpInsn(GOTO, l1);
        }
    ...
    /**
     * mv.visitVarInsn(ILOAD, 2); Label l0 = new Label(); Label l1 = new
     * Label(); Label l2 = new Label(); mv.visitTableSwitchInsn(0, 2,
     * TheDefLabel, new Label[] { l0, l1, l2 }); mv.visitLabel(l0);
     * mv.visitVarInsn(ALOAD, 0); mv.visitFieldInsn(GETFIELD, TheClassName,
     * "f2", "I"); mv.visitMethodInsn(INVOKESTATIC, "java/lang/Integer",
     * "valueOf", "(I)Ljava/lang/Integer;"); mv.visitInsn(ARETURN);
     * mv.visitLabel(l1); mv.visitVarInsn(ALOAD, 0); mv.visitFieldInsn(GETFIELD,
     * TheClassName, "f3", "Ljava/lang/String;"); mv.visitInsn(ARETURN);
     * mv.visitLabel(l2); mv.visitVarInsn(ALOAD, 0); mv.visitFieldInsn(GETFIELD,
     * TheClassName, "f4", "Ljava/lang/String;"); mv.visitInsn(ARETURN);
     */
    ...
    /**
     * public void bdbSetField(Object o, int field, int superLevel, boolean
     * isSecField, Object value) { if (superLevel > 0) { // if has superclass:
     * super.bdbSetField (o, field, superLevel - 1, isSecField, value); } else
     * if (isSecField) { switch (field) { case 0: f2 = ((Integer)
     * value).intValue(); case 1: f3 = (String) value; case 2: f4 = (String)
     * value; } } else { switch (field) { case 0: f5 = ((Integer)
     * value).intValue(); case 1: f6 = (String) value; case 2: f7 = (String)
     * value; } } }
     */
    private void genBdbSetField() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbSetField", "(Ljava/lang/Object;IIZLjava/lang/Object;)V", null,
                null);
        mv.visitCode();
        mv.visitVarInsn(ILOAD, 3);
        Label l0 = new Label();
        ...
        if (hasPersistentSuperclass) {
            ...
            mv.visitMethodInsn(INVOKESPECIAL, className, "bdbSetField", "(Ljava/lang/Object;IIZLjava/lang/Object;)V");
        }
        mv.visitInsn(RETURN);
        mv.visitLabel(l0);
        mv.visitVarInsn(ILOAD, 4);
    ...
    /**
     * public void bdbSetPriField(Object o, Object value) { if (priKeyField !=
     * null) { thisField = (TheFieldClass) value; } else if (super != null) { //
     * if has superclass: super.bdbSetPriField(o, value) } }
     */
    private void genBdbSetPriField() {
        MethodVisitor mv = cv.visitMethod(ACC_PUBLIC, "bdbSetPriField", "(Ljava/lang/Object;Ljava/lang/Object;)V", null,
                null);
        mv.visitCode();
        if (priKeyField != null) {
            mv.visitVarInsn(ALOAD, 0);
            ...
            mv.visitVarInsn(ALOAD, 1);
            mv.visitVarInsn(ALOAD, 2);
            mv.visitMethodInsn(INVOKESPECIAL, superclassName, "bdbSetPriField",
     ...
     * Label(); Label l2 = new Label(); mv.visitTableSwitchInsn(0, 2,
     * TheDefLabel, new Label[] { l0, l1, l2 }); mv.visitLabel(l0);
     * mv.visitVarInsn(ALOAD, 0); mv.visitVarInsn(ALOAD, 5);
     * mv.visitTypeInsn(CHECKCAST, "java/lang/Integer");
     * mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/Integer", "intValue",
     * "()I"); mv.visitFieldInsn(PUTFIELD, TheClassName, "f2", "I");
     ...
     * mv.visitLabel(l2); mv.visitVarInsn(ALOAD, 0); mv.visitVarInsn(ALOAD, 5);
     * mv.visitTypeInsn(CHECKCAST, "java/lang/String");
    ...
    private void genWritePrimitive(MethodVisitor mv, int sort) {
        switch (sort) {
            case Type.BOOLEAN:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeBoolean",
                        "(Z)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.CHAR:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeChar",
                        "(I)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.BYTE:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeByte",
                        "(I)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.SHORT:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeShort",
                        "(I)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.INT:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeInt",
                        "(I)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.LONG:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeLong",
                        "(J)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.FLOAT:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeSortedFloat",
                        "(F)Lcom/sleepycat/bind/tuple/TupleOutput;");
                break;
            case Type.DOUBLE:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityOutput", "writeSortedDouble",
    ...
    private void genReadPrimitive(MethodVisitor mv, int sort) {
        switch (sort) {
            ...
            case Type.FLOAT:
                ...
                break;
            case Type.DOUBLE:
                mv.visitMethodInsn(INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readSortedDouble",
                        "()D");
                break;
            default:
                throw DbCompat.unexpectedState(String.valueOf(sort));
     ...
     * name for an object type and the descriptor for an array type. Must not be
     * called for a non-reference type.
     */
     ...
     * float or double, this method will compare it to zero and leave the result
     * as an int operand.
     */
    ...
    static boolean isSimpleRefType(String className) {
        return (PRIMITIVE_WRAPPERS.containsKey(className) || className.equals(BigInteger.class.getName())
                || className.equals(BigDecimal.class.getName()) || className.equals(Date.class.getName())
