...
class SubIndex<PK, E> implements EntityIndex<PK, E> {
    ...
    <SK> SubIndex(SecondaryIndex<SK, PK, E> secIndex, EntityBinding entityBinding, SK key) throws DatabaseException {
    ...
    /* <!-- begin JE only --> */
    public OperationResult delete(Transaction txn, PK key, WriteOptions options) throws DatabaseException {

        return deleteInternal(txn, key, OpWriteOptions.make(options)).jeResult;
    }
    /* <!-- end JE only --> */

    private OpResult deleteInternal(Transaction txn, PK key, OpWriteOptions options) throws DatabaseException {
        ...
        try {
            ...
            if (DbCompat.IS_JE) {
                ...
                OperationResult result = cursor.get(keyEntry, pkeyEntry, dataEntry, Get.SEARCH_BOTH, readOptions);
                if (result != null) {
            ...
            /* <!-- end JE only --> */
            OperationStatus status = cursor.getSearchBoth(keyEntry, pkeyEntry, dataEntry,
    ...
    public EntityCursor<PK> keys(Transaction txn, PK fromKey, boolean fromInclusive, PK toKey, boolean toInclusive,
                                 CursorConfig config)
            throws DatabaseException {
    ...
    public EntityCursor<E> entities(Transaction txn, PK fromKey, boolean fromInclusive, PK toKey, boolean toInclusive,
                                    CursorConfig config)
            throws DatabaseException {
    ...
    private <V> EntityCursor<V> cursor(Transaction txn, PK fromKey, boolean fromInclusive, PK toKey,
                                       boolean toInclusive, ValueAdapter<V> adapter, CursorConfig config)
            throws DatabaseException {
    ...
    private <V> EntityCursor<V> cursor(Transaction txn, KeyRange pkeyRange, ValueAdapter<V> adapter,
                                       CursorConfig config)
            throws DatabaseException {
    ...
    public synchronized SortedMap<PK, E> sortedMap() {
        if (map == null) {
            map = (SortedMap) ((StoredSortedMap) secIndex.sortedMap()).duplicatesMap(keyObject, pkeyBinding);
        }
        return map;
    }
}
