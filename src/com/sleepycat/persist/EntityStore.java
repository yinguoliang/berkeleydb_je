 ...
 * <p>
 * {@code EntityStore} objects are thread-safe. Multiple threads may safely call
 * the methods of a shared {@code EntityStore} object.
 * </p>
 * <p>
 * See the <a href="package-summary.html#example">package summary example</a>
 * for an example of using an {@code EntityStore}.
 * </p>
 * <p>
 * Before creating an <code>EntityStore</code> you must create an
 * {@link Environment} object using the Berkeley DB engine API. The environment
 * may contain any number of entity stores and their associated databases, as
 * well as other databases not associated with an entity store.
 * </p>
 * <p>
 * An entity store is based on an {@link EntityModel}: a data model which
 * defines persistent classes (<em>entity classes</em>), primary keys, secondary
 * keys, and relationships between entities. A primary index is created for each
 * entity class. An associated secondary index is created for each secondary
 * key. The {@link Entity}, {@link PrimaryKey} and {@link SecondaryKey}
 * annotations may be used to define entities and keys.
 * </p>
 * <p>
 * To use an <code>EntityStore</code>, first obtain {@link PrimaryIndex} and
 ...
 * </p>
 * <p>
 * Although not normally needed, you can also use the entity store along with
 * the {@link com.sleepycat.je Base API}. Methods in the {@link PrimaryIndex}
 * and {@link SecondaryIndex} classes may be used to obtain databases and
 * bindings. The databases may be used directly for accessing entity records.
 * The bindings should be called explicitly to translate between
 * {@link com.sleepycat.je.DatabaseEntry} objects and entity model objects.
 * </p>
 * <p>
 * Each primary and secondary index is associated internally with a
 * {@link Database}. With any of the above mentioned use cases, methods are
 * provided that may be used for database performance tuning. The
 * {@link #setPrimaryConfig setPrimaryConfig} and {@link #setSecondaryConfig
 * setSecondaryConfig} methods may be called anytime before a database is opened
 * via {@link #getPrimaryIndex getPrimaryIndex} or {@link #getSecondaryIndex
 * getSecondaryIndex}. The {@link #setSequenceConfig setSequenceConfig} method
 * may be called anytime before {@link #getSequence getSequence} is called or
 * {@link #getPrimaryIndex getPrimaryIndex} is called for a primary index
 * associated with that sequence.
 * </p>
 * <!-- begin JE only -->
 * <h3>Database Names</h3>
 * <p>
 * The database names of primary and secondary indices are designed to be unique
 * within the environment and identifiable for debugging and use with tools such
 * as {@link com.sleepycat.je.util.DbDump} and
 * {@link com.sleepycat.je.util.DbLoad}.
 * </p>
 * <p>
 ...
 * EntityStore} and ENTITY_CLASS is name of the class passed to
 * {@link #getPrimaryIndex getPrimaryIndex}.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * Where KEY_NAME is the secondary key name passed to {@link #getSecondaryIndex
 * getSecondaryIndex}.
 * </p>
 * <p>
 ...
 * <li>If you create other databases in the same environment that are not part
 * of an <code>EntityStore</code>, to avoid naming conflicts the other database
 * names should not begin with <code>"persist#"</code>.</li>
 * <li>If you are using {@link com.sleepycat.je.util.DbDump} or
 * {@link com.sleepycat.je.util.DbLoad} to perform a backup or copy databases
 * between environments, knowing the database names can be useful. Normally you
 * will dump or load all database names starting with
 * <code>"persist#STORE_NAME#"</code>.</li>
 * </ul>
 * <p>
 * If you are copying all databases in a store as mentioned in the last point
 * above, there is one further consideration. There are two internal databases
 * that must be kept with the other databases in the store in order for the
 * store to be used. These contain the data formats and sequences for the store:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * These databases must normally be included with copies of other databases in
 * the store. They should not be modified by the application.
 * </p>
 * <p>
...
/* <!-- end JE only --> */
{
     ...
     *            environment. An empty string is allowed. Named stores may be
     *            used to distinguish multiple sets of persistent entities for
     *            the same entity classes in a single environment. Underlying
     *            database names are prefixed with the store name.
     * @param config the entity store configuration, or null to use default
     *            configuration properties.
     * @throws StoreExistsException when the
     *             {@link StoreConfig#setExclusiveCreate ExclusiveCreate}
     *             configuration parameter is true and the store's internal
     *             catalog database already exists.
     * @throws StoreNotFoundException when when the
     *             {@link StoreConfig#setAllowCreate AllowCreate} configuration
     *             parameter is false and the store's internal catalog database
     *             does not exist.
     * @throws IncompatibleClassException if an incompatible class change has
     *             been made and mutations are not configured for handling the
     *             change. See {@link com.sleepycat.persist.evolve Class
     *             Evolution} for more information. <!-- begin JE only -->
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#readFailures">
     *             Read Operation Failures</a> occurs. If the store does not
     *             exist and the {@link StoreConfig#setAllowCreate AllowCreate}
     *             parameter is true, then one of the
     ...
     * necessary.
     * <p>
     * If they are not already open, the primary and secondary databases for the
     * entity class are created/opened together in a single internal
     ...
     *            the corresponding primitive wrapper class if the primary key
     ...
     *             referenced by it are not persistent, or the primary key class
     *             does not match the entity's primary key field, or if metadata
     *             for the entity or primary key is invalid. <!-- begin JE only
     *             -->
     * @throws IndexNotAvailableException in a replicated environment if this
     *             Replica's persistent classes have been upgraded to define a
     *             new index, but the Master has not yet been upgraded.
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#readFailures">
     *             Read Operation Failures</a> occurs. If the index does not
     *             exist and the {@link StoreConfig#setReadOnly ReadOnly}
     *             parameter is false, then one of the
     *             <a href="../je/OperationFailureException.html#writeFailures">
     *             Write Operation Failures</a> may also occur.
     * @throws EnvironmentFailureException if an unexpected, internal or
     *             environment-wide failure occurs. <!-- end JE only -->
     * @throws DatabaseException the base class for all BDB exceptions.
     */
    public <PK, E> PrimaryIndex<PK, E> getPrimaryIndex(Class<PK> primaryKeyClass, Class<E> entityClass)
            throws DatabaseException {

        try {
            return store.getPrimaryIndex(primaryKeyClass, primaryKeyClass.getName(), entityClass,
                    entityClass.getName());
     ...
     * <p>
     * <em>NOTE:</em> If the secondary key field is declared in a subclass of
     * the entity class, use {@link #getSubclassIndex} instead.
     * </p>
     * <p>
     * If a {@link SecondaryKey#relatedEntity} is used and the primary index for
     * the related entity is not already open, it will be opened by this method.
     * That will, in turn, open its secondary indices, which can cascade to open
     * other primary indices.
     * </p>
     *
     * @param primaryIndex the primary index associated with the returned
     *            secondary index. The entity class of the primary index, or one
     *            of its superclasses, must contain a secondary key with the
     *            given secondary key class and key name.
     * @param keyClass the class of the secondary key field, or the
     *            corresponding primitive wrapper class if the secondary key
     *            field type is a primitive.
     * @param keyName the name of the secondary key field, or the
     *            {@link SecondaryKey#name} if this name annotation property was
     *            specified.
     ...
     *             superclasses does not contain a key field of the given key
     *             class and key name, or if the metadata for the secondary key
     *             is invalid. <!-- begin JE only -->
     * @throws IndexNotAvailableException in a replicated environment if this
     *             Replica's persistent classes have been upgraded to define a
     *             new index, but the Master has not yet been upgraded.
     * @throws OperationFailureException if one of the
     ...
     * @throws DatabaseException the base class for all BDB exceptions.
     */
    public <SK, PK, E> SecondaryIndex<SK, PK, E> getSecondaryIndex(PrimaryIndex<PK, E> primaryIndex, Class<SK> keyClass,
                                                                   String keyName)
            throws DatabaseException {

        try {
            return store.getSecondaryIndex(primaryIndex, primaryIndex.getEntityClass(),
                    primaryIndex.getEntityClass().getName(), keyClass, keyClass.getName(), keyName);
        } catch (IndexNotAvailableException e) {
            if (!store.attemptRefresh()) {
                throw e;
            }
     ...
     *            index. The entity subclass must contain a secondary key with
     ...
     *             contain a key field of the given key class and key name, or
     *             if the metadata for the secondary key is invalid. <!-- begin
     *             JE only -->
     * @throws IndexNotAvailableException in a replicated environment if this
     *             Replica's persistent classes have been upgraded to define a
     *             new index, but the Master has not yet been upgraded.
    ...
    public <SK, PK, E1, E2 extends E1> SecondaryIndex<SK, PK, E2> getSubclassIndex(PrimaryIndex<PK, E1> primaryIndex,
                                                                                   ...
                                                                                   Class<SK> keyClass, String keyName)
            throws DatabaseException {
        ...
        try {
            return store.getSecondaryIndex(primaryIndex, entitySubclass, primaryIndex.getEntityClass().getName(),
     ...
     * conversion overhead. Evolution may be performed concurrently with normal
     * access to the store.
     * <p>
     * Conversion is performed one entity class at a time. An entity class is
     * converted only if it has {@link Mutations} associated with it via
     ...
     * @return the EvolveStats. <!-- begin JE only -->
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#writeFailures">
     ...
     * subclasses.
     * <p>
     * The primary database for the given entity class will be truncated and all
     * secondary databases will be removed. The primary and secondary databases
     * associated with the entity class must not be open except by this store,
     * since database truncation/removal is only possible when the database is
     * not open.
     * </p>
     * <p>
     * The primary and secondary databases for the entity class will be closed
     * by this operation and the existing {@link PrimaryIndex} and
     * {@link SecondaryIndex} objects will be invalidated. To access the
     * indexes, the user must call {@link #getPrimaryIndex} and
     * {@link #getSecondaryIndex} after this operation is complete.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Alternatively, individual databases may be configured for deferred write
     * using {@link DatabaseConfig#setDeferredWrite} along with
     * {@link #getPrimaryConfig} and {@link #setPrimaryConfig}. Caution should
     * be used when configuring only some databases for deferred-write, since
     * durability will be different for these databases than for other databases
     * in the same store.
     * </p>
     * <p>
     * This method is equivalent to calling {@link Database#sync} for each
     * deferred-write index Database that is open for this store.
     * <p>
     * Instead of calling this method, {@link Environment#sync} may be used. The
     * difference is that this method will only flush the databases for this
     ...
     * that were opened via this store. The caller must ensure that the primary
     * and secondary indices for the entity class are no longer in use.
     * <p>
     * The primary and secondary databases for the entity class will be closed
     * by this operation and the existing {@link PrimaryIndex} and
     * {@link SecondaryIndex} objects will be invalidated. To access the
     ...
     * <!-- begin JE only -->
     * <p>
     * WARNING: To prevent memory leaks, the application must call this method
     * even when the Environment has become invalid. While this is not necessary
     * for Database objects, it is necessary for EntityStore objects to prevent
     * the accumulation of memory in the global DPL metadata cache. <!-- end JE
     * only -->
     * <p>
     ...
     * inverse of the store {@link StoreConfig#setReadOnly ReadOnly}. setting.
     * </li>
     * </ul>
     *
     * @param name the sequence name, which is normally defined using the
     ...
     * <p>
     * To be compatible with the entity model and the Direct Persistence Layer,
     * the configuration should be retrieved using {@link #getSequenceConfig
     * getSequenceConfig}, modified, and then passed to this method. The
     * following configuration properties may not be changed:
     * </p>
     * <ul>
     ...
     * </p>
     * <p>
     * If the range is changed to include the value zero, see {@link PrimaryKey}
     * for restrictions.
     * </p>
     *
     * @param name the sequence name, which is normally defined using the
    ...
    /**
     * Returns the default primary database Berkeley DB engine API configuration
     * for an entity class.
     * <p>
     * The returned configuration is as follows. All other properties have
     * default values.
     * </p>
     * <ul>
     * <li>{@link DatabaseConfig#setTransactional Transactional} is set to match
     * {@link StoreConfig#setTransactional StoreConfig}.</li>
     ...
     * {@link StoreConfig#setReadOnly StoreConfig}.</li> <!-- begin JE only -->
     * *
     * <li>{@link DatabaseConfig#setDeferredWrite DeferredWrite} is set to match
     * {@link StoreConfig#setDeferredWrite StoreConfig}.</li>
     * <li>{@link DatabaseConfig#setTemporary Temporary} is set to match
     * {@link StoreConfig#setTemporary StoreConfig}.</li> <!-- end JE only --> *
    ...
    /**
     * Configures the primary database for an entity class using the Berkeley DB
     * engine API.
     * <p>
     * To be compatible with the entity model and the Direct Persistence Layer,
     * the configuration should be retrieved using {@link #getPrimaryConfig
     * getPrimaryConfig}, modified, and then passed to this method. The
     * following configuration properties may not be changed:
     * </p>
     * <ul>
     ...
     * begin JE only --> *
     * <li>{@link DatabaseConfig#setTemporary Temporary}</li> <!-- end JE only
     * --> *
     ...
     * ReadOnly} setting.</li>
     * <li>{@link DatabaseConfig#setReadOnly ReadOnly} is set to match the
     * primary database.</li> <!-- begin JE only --> *
     * <li>{@link DatabaseConfig#setDeferredWrite DeferredWrite} is set to match
     * the primary database.</li>
     * <li>{@link DatabaseConfig#setTemporary Temporary} is set to match
     * {@link StoreConfig#setTemporary StoreConfig}.</li> <!-- end JE only --> *
     ...
     * <li>{@link SecondaryConfig#setAllowPopulate AllowPopulate} is set to true
     * when a secondary key is added to an existing primary index.</li>
     * <li>{@link SecondaryConfig#setKeyCreator KeyCreator} or
     * {@link SecondaryConfig#setMultiKeyCreator MultiKeyCreator} is set to an
     * internal instance.</li>
     * <li>{@link SecondaryConfig#setForeignMultiKeyNullifier
     * ForeignMultiKeyNullifier} is set to an internal instance if
     * {@link SecondaryKey#onRelatedEntityDelete} is
     * {@link DeleteAction#NULLIFY}.</li>
     * </ul>
     *
     ...
     * <p>
     * To be compatible with the entity model and the Direct Persistence Layer,
     * the configuration should be retrieved using {@link #getSecondaryConfig
     * getSecondaryConfig}, modified, and then passed to this method. The
     * following configuration properties may not be changed:
     * </p>
     * <ul>
     * <li>{@link DatabaseConfig#setExclusiveCreate ExclusiveCreate}</li>
     ...
     * <li>{@link SecondaryConfig#setForeignKeyDatabase ForeignKeyDatabase}</li>
     * </ul>
     * <p>
     * In addition, {@link DatabaseConfig#setAllowCreate AllowCreate} must be
     * the inverse of {@code ReadOnly}
