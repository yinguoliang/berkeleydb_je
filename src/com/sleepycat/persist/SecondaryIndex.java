 ...
 * <p>
 * {@code SecondaryIndex} objects are thread-safe. Multiple threads may safely
 * call the methods of a shared {@code SecondaryIndex} object.
 * </p>
 * <p>
 * {@code SecondaryIndex} implements {@link EntityIndex} to map the secondary
 * key type (SK) to the entity type (E). In other words, entities are accessed
 * by secondary key values.
 * </p>
 * <p>
 * The {@link SecondaryKey} annotation may be used to define a secondary key as
 ...
 * </pre>
 * <p>
 * Before obtaining a {@code SecondaryIndex}, the {@link PrimaryIndex} must be
 * obtained for the entity class. To obtain the {@code SecondaryIndex} call
 ...
 * </pre>
 * <p>
 * Since {@code SecondaryIndex} implements the {@link EntityIndex} interface, it
 * shares the common index methods for retrieving and deleting entities, opening
 * cursors and using transactions. See {@link EntityIndex} for more information
 * on these topics.
 * </p>
 * <p>
 * {@code SecondaryIndex} does <em>not</em> provide methods for inserting and
 * updating entities. That must be done using the {@link PrimaryIndex}.
 * </p>
 * <p>
 * Note that a {@code SecondaryIndex} has three type parameters {@code <SK,
 * PK, E>} or in the example {@code <String, Long, Employee>} while a
 * {@link PrimaryIndex} has only two type parameters {@code <PK, E>} or
 * {@code <Long,
 * Employee>}. This is because a {@code SecondaryIndex} has an extra level of
 * mapping: It maps from secondary key to primary key, and then from primary key
 * to entity. For example, consider this entity:
 * </p>
 * <div>
 ...
 * </div>
 * <p>
 * The {@link PrimaryIndex} maps from id directly to the entity, or from primary
 * key 1 to the "Jane Smith" entity in the example. The {@code
 ...
 * in the primary index. The main mapping of a {@code SecondaryIndex} is to map
 * from secondary key (SK) to entity (E), or in the example, from the String
 * department key to the Employee entity. The {@code SecondaryIndex} itself, by
 * implementing {@code EntityIndex<SK, E>}, provides this mapping.
 * </p>
 * <p>
 * The second mapping provided by {@code SecondaryIndex} is from secondary key
 * (SK) to primary key (PK), or in the example, from the String department key
 * to the Long id key. The {@link #keysIndex} method provides this mapping. When
 * accessing the keys index, the primary key is returned rather than the entity.
 * When only the primary key is needed and not the entire entity, using the keys
 * index is less expensive than using the secondary index because the primary
 * index does not have to be accessed.
 * </p>
 * <p>
 * The third mapping provided by {@code SecondaryIndex} is from primary key (PK)
 * to entity (E), for the subset of entities having a given secondary key (SK).
 * This mapping is provided by the {@link #subIndex} method. A sub-index is
 * convenient when you are interested in working with the subset of entities
 * having a particular secondary key value, for example, all employees in a
 * given department.
 * </p>
 * <p>
 * All three mappings, along with the mapping provided by the
 * {@link PrimaryIndex}, are shown using example data in the {@link EntityIndex}
 ...
 * relationship. A single entity is related to a single secondary key value. For
 * example:
 * </p>
 *
 * <pre class="code">
 ...
 * </pre>
 * <p>
 * With a {@link Relationship#ONE_TO_ONE ONE_TO_ONE} relationship, the secondary
 * key must be unique; in other words, no two entities may have the same
 * secondary key value. If an attempt is made to store an entity having the same
 * secondary key value as another existing entity, a {@link DatabaseException}
 * will be thrown.
 * </p>
 * <p>
 ...
 * entity may have the same secondary key value. In this example, more than one
 * employee may belong to the same department.
 * </p>
 * <p>
 * The most convenient way to access the employees in a given department is by
 * using a sub-index. For example:
 * </p>
 *
 * <pre class="code">
 ...
 * same secondary key value. In this example, no two employees may have the same
 * email address. If an attempt is made to store an entity having the same
 * secondary key value as another existing entity, a {@link DatabaseException}
 * will be thrown.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * The secondary key field for a {@link Relationship#ONE_TO_MANY ONE_TO_MANY}
 * relationship must be an array or collection type. To access the email
 * addresses of an employee, simply access the collection field directly. For
 * example:
 * </p>
 *
 * <pre class="code">
 ...
 * <p>
 * In a {@link Relationship#MANY_TO_MANY MANY_TO_MANY} relationship, one or more
 * entities is related to one or more secondary key values. For example:
 * </p>
 *
 * <pre class="code">
 * {@literal @Entity}
 ...
 * secondary key is not required to be unique; in other words, more than one
 * entity may have the same secondary key value. In this example, more than one
 * employee may belong to the same organization.
 * </p>
 * <p>
 * The most convenient way to access the employees in a given organization is by
 * using a sub-index. For example:
 * </p>
 *
 * <pre class="code">
 ...
 * </pre>
 * <p>
 * The secondary key field for a {@link Relationship#MANY_TO_MANY MANY_TO_MANY}
 * relationship must be an array or collection type. To access the organizations
 * of an employee, simply access the collection field directly. For example:
 * </p>
 *
 * <pre class="code">
 * Employee employee = primaryIndex.get(1); // Get the entity by primary key
 ...
 * sufficient. But in other cases, you may wish to constrain the secondary keys
 * of one entity class to be valid primary keys of another entity class. For
 * example, a Department entity may also be defined:
 * </p>
 *
 * <pre class="code">
 * {@literal @Entity}
 ...
 * </pre>
 * <p>
 * You may wish to constrain the department field values of the Employee class
 * in the examples above to be valid primary keys of the Department entity
 * class. In other words, you may wish to ensure that the department field of an
 * Employee will always refer to a valid Department entity.
 * </p>
 * <p>
 * You can implement this constraint yourself by validating the department field
 * before you store an Employee. For example:
 * </p>
 *
 * <pre class="code">
 ...
 * </pre>
 * <p>
 * Or, instead you could define the Employee department field as a foreign key,
 * and this validation will be done for you when you attempt to store the
 ...
 * </pre>
 * <p>
 * The {@code relatedEntity=Department.class} above defines the department field
 * as a foreign key that refers to a Department entity. Whenever a Employee
 * entity is stored, its department field value will be checked to ensure that a
 * Department entity exists with that value as its primary key. If no such
 * Department entity exists, then a {@link DatabaseException} is thrown, causing
 * the transaction to be aborted (assuming that transactions are used).
 * </p>
 * <p>
 * This begs the question: What happens when a Department entity is deleted
 * while one or more Employee entities have department fields that refer to the
 * deleted department's primary key? If the department were allowed to be
 ...
 * updated to refer to a different existing department, or set to null. This is
 * the responsibility of the application.
 * </p>
 * <p>
 * There are two additional ways of handling deletion of a Department entity.
 * These alternatives are configured using the
 * {@link SecondaryKey#onRelatedEntityDelete} annotation property. Setting this
 * property to {@link DeleteAction#NULLIFY} causes the Employee department field
 * to be automatically set to null when the department they refer to is deleted.
 * This may or may not be desirable, depending on application policies. For
 * example:
 * </p>
 *
 * <pre class="code">
 ...
 * to is deleted. This is probably not desirable in this particular example, but
 * is useful for parent-child relationships. For example:
 * </p>
 *
 * <pre class="code">
 * {@literal @Entity}
 ...
 * Order entity is deleted, it may be useful to automatically delete its "child"
 * OrderItem entities.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * Which approach is best? The Many-to-One approach better handles large number
 * of entities on the to-Many side of the relationship because it doesn't store
 * a collection of keys as an entity field. With Many-to-One a Btree is used to
 * store the collection of keys and the Btree can easily handle very large
 * numbers of keys. With One-to-Many, each time a related key is added or
 * removed the entity on the One side of the relationship, along with the
 * complete collection of related keys, must be updated. Therefore, if large
 * numbers of keys may be stored per relationship, Many-to-One is recommended.
 * </p>
 * <p>
 ...
 * model. For example, if you think of a Department as containing employees and
 * you wish to modify the Department object each time an employee is added or
 * removed, then you may wish to store a collection of Employee keys in the
 ...
 * addresses and employees frequently add and remove email addresses. To avoid
 * the potential performance problems associated with updating the Employee
 * entity every time an email address is added or removed, you could create an
 * EmployeeEmailAddress entity and use a Many-to-One relationship as shown
 * below:
 * </p>
 *
 * <pre class="code">
 ...
 * </pre>
 * <p>
 * If instead a {@code Set<Long> members} key had been defined in the
 ...
 * <p>
 * If you have a Many-to-Many relationship with a large number of entities on
 * <em>both</em> sides of the relationship, you can avoid the potential
 * performance problems by using a <em>relationship entity</em>. A relationship
 * entity defines the relationship between two other entities using two
 * Many-to-One relationships.
 * </p>
 * <p>
 * Imagine a relationship between cars and trucks indicating whenever a
 * particular truck was passed on the road by a particular car. A given car may
 * pass a large number of trucks and a given truck may be passed by a large
 ...
 * </p>
 * <p>
 * To apply the relationship entity approach we define a new entity class named
 * CarPassedTruck representing a single truck passed by a single car. We remove
 * the secondary key from the Car class and use two secondary keys in the
 * CarPassedTruck class instead.
 * </p>
 *
 * <pre class="code">
 * {@literal @Entity}
 ...
 * performance problems mentioned above. Or, you may choose to use this approach
 * in order to store other information about the relationship. For example, if
 * for each car that passes a truck you wish to record how much faster the car
 * was going than the truck, then a relationship entity is the logical place to
 * store that property. In the example below the speedDifference property is
 * added to the CarPassedTruck class.
 * </p>
 *
 * <pre class="code">
 * {@literal @Entity}
 ...
 * key. These factors should be weighed against its advantages and the relevant
 * application access patterns should be considered.
 * </p>
 *
 * @author Mark Hayes
 */
public class SecondaryIndex<SK, PK, E> extends BasicIndex<SK, E> {
    ...
    /**
     * Creates a secondary index without using an <code>EntityStore</code>. When
     * using an {@link EntityStore}, call {@link EntityStore#getSecondaryIndex
     * getSecondaryIndex} instead.
     * <p>
     * This constructor is not normally needed and is provided for applications
     * that wish to use custom bindings along with the Direct Persistence Layer.
     * Normally, {@link EntityStore#getSecondaryIndex getSecondaryIndex} is used
     * instead.
     * </p>
     *
     * @param database the secondary database used for all access other than via
     *            a {@link #keysIndex}.
     * @param keysDatabase another handle on the secondary database, opened
     *            without association to the primary, and used only for access
     *            via a {@link #keysIndex}. If this argument is null and the
     *            {@link #keysIndex} method is called, then the keys database
     *            will be opened automatically; however, the user is then
     *            responsible for closing the keys database. To get the keys
     *            database in order to close it, call {@link #getKeysDatabase}.
     ...
     */
    public SecondaryIndex(SecondaryDatabase database, Database keysDatabase, PrimaryIndex<PK, E> primaryIndex,
                          Class<SK> secondaryKeyClass, EntryBinding<SK> secondaryKeyBinding)
    ...
    /**
     * Returns the underlying secondary database that is not associated with the
     * primary database and is used for the {@link #keysIndex}.
     *
     ...
     * <p>
     * Note the following in the unusual case that you are <em>not</em> using an
     * <code>EntityStore</code>: This method will open the keys database, a
     * second database handle for the secondary database, if it is not already
     * open. In this case, if you are <em>not</em> using an
     ...
     * keys database will be closed automatically by {@link EntityStore#close}.
     * </p>
     *
    ...
    public synchronized EntityIndex<SK, PK> keysIndex() throws DatabaseException {
        ...
        if (keysIndex == null) {
            if (keysDb == null) {
                ...
                keysDb = DbCompat.openDatabase(db.getEnvironment(), null /* txn */, DbCompat.getDatabaseFile(secDb),
                        secDb.getDatabaseName(), config);
                if (keysDb == null) {
                    throw new IllegalStateException("Could not open existing DB, file: "
                            + DbCompat.getDatabaseFile(secDb) + " name: " + secDb.getDatabaseName());
                }
            }
            keysIndex = new KeysIndex<SK, PK>(keysDb, keyClass, keyBinding, priIndex.getKeyClass(),
                    priIndex.getKeyBinding());
        }
        return keysIndex;
    }
     ...
     * <p>
     * When using a {@link Relationship#MANY_TO_ONE MANY_TO_ONE} or
     * {@link Relationship#MANY_TO_MANY MANY_TO_MANY} secondary key, the
     * sub-index represents the left (MANY) side of a relationship.
     * </p>
     *
     ...
     * <!-- begin JE only -->
     * 
     * @hidden <!-- end JE only --> For internal use only. Used for obtaining
     *         the auto-commit txn config from the store, which overrides this
     *         method to return it.
     */
    /* <!-- begin JE only --> */
    protected
