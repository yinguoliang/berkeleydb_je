 ...
 * <p>
 * {@code PrimaryIndex} objects are thread-safe. Multiple threads may safely
 * call the methods of a shared {@code PrimaryIndex} object.
 * </p>
 * <p>
 * {@code PrimaryIndex} implements {@link EntityIndex} to map the primary key
 * type (PK) to the entity type (E).
 * </p>
 * <p>
 * The {@link Entity} annotation may be used to define an entity class and the
 * {@link PrimaryKey} annotation may be used to define a primary key as shown in
 ...
 * </pre>
 * <p>
 * To obtain the {@code PrimaryIndex} for a given entity class, call
 * {@link EntityStore#getPrimaryIndex EntityStore.getPrimaryIndex}, passing the
 ...
 * </pre>
 * <p>
 * Note that {@code Long.class} is passed as the primary key class, but the
 ...
 * </p>
 * <p>
 * The {@code PrimaryIndex} provides the primary storage and access methods for
 * the instances of a particular entity class. Entities are inserted and updated
 * in the {@code PrimaryIndex} by calling a method in the family of {@link #put}
 * methods. The {@link #put} method will insert the entity if no entity with the
 * same primary key already exists. If an entity with the same primary key does
 * exist, it will update the entity and return the existing (old) entity. For
 * example:
 * </p>
 *
 * <pre class="code">
 ...
 * </pre>
 * <p>
 * The {@link #putNoReturn} method can be used to avoid the overhead of
 ...
 * </pre>
 * <p>
 * The {@link #putNoOverwrite} method can be used to ensure that an existing
 * entity is not overwritten. {@link #putNoOverwrite} returns true if the entity
 * was inserted, or false if an existing entity exists and no action was taken.
 * For example:
 * </p>
 *
 * <pre class="code">
 ...
 * entity class must have a distinct primary key value. Rather than assigning
 * the unique primary key values yourself, a <em>sequence</em> can be used to
 * assign sequential integer values automatically, starting with the value 1
 * (one). A sequence is defined using the {@link PrimaryKey#sequence} annotation
 ...
 * </pre>
 * <p>
 * The name of the sequence used above is "ID". Any name can be used. If the
 * same sequence name is used in more than one entity class, the sequence will
 * be shared by those classes, in other words, a single sequence of integers
 * will be used for all instances of those classes. See
 * {@link PrimaryKey#sequence} for more information.
 * </p>
 * <p>
 * Any method in the family of {@link #put} methods may be used to insert
 * entities where the primary key is assigned from a sequence. When the
 * {@link #put} method returns, the primary key field of the entity object will
 * be set to the assigned key value. For example:
 * </p>
 *
 * <pre class="code">
 * Employee employee;
 ...
 * assigning a new primary key? The answer is that the {@link #put} methods will
 * only assign a new key from the sequence if the primary key field is zero or
 * null (for reference types). If an entity with a non-zero and non-null key
 * field is passed to a {@link #put} method, any existing entity with that
 * primary key value will be updated. For example:
 * </p>
 *
 * <pre class="code">
 * Employee employee;
 ...
 * </pre>
 * <p>
 * Since {@code PrimaryIndex} implements the {@link EntityIndex} interface, it
...
public class PrimaryIndex<PK, E> extends BasicIndex<PK, E> {
     ...
     * Normally, {@link EntityStore#getPrimaryIndex getPrimaryIndex} is used
     * instead.
     * </p>
     * <p>
     * Note that when this constructor is used directly, primary keys cannot be
     * automatically assigned from a sequence. The key assignment feature
     ...
     */
    public PrimaryIndex(Database database, Class<PK> keyClass, EntryBinding<PK> keyBinding, Class<E> entityClass,
                        EntityBinding<E> entityBinding)
            throws DatabaseException {

        super(database, keyClass, keyBinding, new EntityValueAdapter(entityClass, entityBinding, false));
     ...
     * <p>
     * If a {@link PrimaryKey#sequence} is used and the primary key field of the
     * given entity is null or zero, this method will assign the next value from
     * the sequence to the primary key field of the given entity.
     * </p>
     * <p>
     ...
     * </p>
     *
     * @param entity the entity to be inserted or updated. <!-- begin JE only
     *            -->
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#writeFailures">
     *             Write Operation Failures</a> occurs.
     ...
     *         same primary key is already present. <!-- begin JE only -->
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#writeFailures">
     *             Write Operation Failures</a> occurs.
     * @throws EnvironmentFailureException if an unexpected, internal or
    ...
    /* <!-- begin JE only --> */
    /**
     * Inserts or updates an entity, using Put type and WriteOptions parameters,
     ...
     *         {@code Put.NO_OVERWRITE} is used, null is returned if an entity
     *         with the same primary key is already present. If
     *         {@code Put.OVERWRITE} is used, null is never returned.
     * @throws OperationFailureException if one of the
     *             <a href="../je/OperationFailureException.html#writeFailures">
     *             Write Operation Failures</a> occurs.
     * @throws EnvironmentFailureException if an unexpected, internal or
    ...
    /**
     * If we are assigning primary keys from a sequence, assign the next key and
