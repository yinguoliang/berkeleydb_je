...
public class PersistKeyBinding implements EntryBinding {
    ...
    public PersistKeyBinding(Catalog catalogParam, String clsName, boolean rawAccess) {
        ...
        if (!keyFormat.isSimple() && !keyFormat.isEnum() && !(keyFormat.getClassMetadata() != null
                && keyFormat.getClassMetadata().getCompositeKeyFields() != null)) {
            throw new IllegalArgumentException("Key class is not a simple type, an enum, or a composite "
                    + "key class (composite keys must include @KeyField " + "annotations): " + clsName);
        }
