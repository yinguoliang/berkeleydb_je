...
public class PersistEntityBinding implements EntityBinding {
     ...
     */
    public PersistEntityBinding(final PersistCatalog catalogParam, final String entityClassName,
                                final boolean rawAccess) {
     ...
     */
    private Object entryToObjectInternal(final DatabaseEntry key, final Object priKey, final DatabaseEntry data)
     ...
     * entities correctly, then reads the data and returns the entity. This is a
     * special case of EntityInput.readObject for a top level entity. Special
     * treatments are: - The formatId must be >= 0; since this is the top level
     * instance, it cannot refer to a visited object nor be a null reference. -
     * The entity is not checked for existence in the visited object set;
     * entities cannot be referenced by another entity. - Reader.readPriKey must
     * be called prior to calling Reader.readObject.
     */
    static Object readEntity(Catalog useCatalog, DatabaseEntry key, Object priKey, DatabaseEntry data,
                             boolean rawAccess)
            throws RefreshException {

        RecordInput dataInput = new RecordInput(useCatalog, rawAccess, null, 0, data.getData(), data.getOffset(),
                data.getSize());
        ...
        if (priKey == null) {
            ...
            RecordInput keyInput = new RecordInput(useCatalog, rawAccess, null, 0, key.getData(), key.getOffset(),
                    key.getSize());
     ...
     */
    private void objectToDataInternal(final Object entity, final DatabaseEntry data) throws RefreshException {
    ...
    /**
     * Writes the formatId and object, and returns the bytes. This is a special
     * case of EntityOutput.writeObject for a top level entity. Special
     * treatments are: - The entity may not be null. - The entity is not checked
     * for existence in the visited object set; entities cannot be referenced by
     * another entity.
     */
    ...
    private void objectToKeyInternal(final Object entity, final DatabaseEntry key) throws RefreshException {

        /*
         * Write the primary key field as a special case since the output format
         * is for a key binding, not entity data.
         */
        Format format = getValidFormat(entity);
        RecordOutput output = new RecordOutput(catalog, rawAccess);
    ...
    private Format getValidFormat(Object entity) throws RefreshException {
        ...
        if (format.getEntityFormat() != entityFormat) {
            throw new IllegalArgumentException("The entity class (" + format.getClassName()
                    + ") must be this entity class or a subclass of it: " + entityFormat.getClassName());
        }
