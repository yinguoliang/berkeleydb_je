 ...
 * <p>
 * Normally we read the set of all secondary key fields first and then the set
 * of all non-key fields, reading each set in order of field name. But when
 * reading an old format record we must account for the following class
 * evolution conversions:
 * </p>
 * <ul>
 ...
 * specific ranges of fields as well as all fields. For example, all fields up
 * to a deleted field could be read, and then all fields from the following
 * field onward.
 * </p>
 *
 * @author Mark Hayes
...
interface Accessor {
     ...
     * implemented by bytecode generation, is a compromise. We use reflection to
     * create multidimensional arrays. We could in the future generate code to
     * create arrays as they are encountered, if there is a need to avoid
    ...
    /**
     * Writes all secondary key field values to the given EntityOutput, writing
     ...
     *            EntityOutput.registerPriKeyObject before writing any other
     *            fields.
     *            </p>
     * @param output the output data to write to.
     */
     ...
     * MAX_FIELD_NUM for endField. Fields from super classes are read first.
     * </p>
     * <p>
     ...
     * @param endField the ending field index in the range of fields to read. To
     *            read all fields, the endField should be MAX_FIELD_NUM.
     * @param superLevel is a non-negative number to read the fields of the
     *            class that is the Nth super instance; or a negative number to
     *            read fields in all classes.
     */
    void readSecKeyFields(Object o, EntityInput input, int startField, int endField, int superLevel)
            throws RefreshException;

    /**
     * Writes all non-key field values to the given EntityOutput, writing fields
     * in super classes first and in name order within class.
     *
    ...
    /**
     * Reads a range of non-key field values from the given EntityInput, reading
     * fields in super classes first and in name order within class.
     * <p>
     * To read all fields, pass -1 for superLevel, zero for startField and
     * MAX_FIELD_NUM for endField. Fields from super classes are read first.
    ...
    /**
     * Reads all composite key field values from the given EntityInput, reading
     * in declared field number order.
     *
     ...
     * @param superLevel is a positive number to identify the field of the class
     *            that is the Nth super instance; or zero to identify the field
     *            in this class.
     * @param isSecField is true for a secondary key field or false for a
     *            non-key field.
     * @return the current field value, or null for a reference type field that
     *         is null.
     */
