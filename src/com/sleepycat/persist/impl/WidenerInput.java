 ...
 * Used to cause an Accessor to read a widened value. For non-key fields we
 * support all Java primitive widening: - byte to short, int, long, float,
 * double, BigInteger and BigDecimal - short to int, long, float, double,
 * BigInteger and BigDecimal - char to int, long, float, double, BigInteger and
 * BigDecimal - int to long, float, double, BigInteger and BigDecimal - long to
 * float, double, BigInteger and BigDecimal - float to double For non-key fields
 * we also support: - Java reference widening - primitive to primitive wrapper -
 * Java primitive widening to corresponding primitive wrappers - Java widening
 * of primitive wrapper to primitive wrapper For secondary keys fields we ONLY
 * support: - primitive to primitive wrapper But for primary keys and composite
 * key fields we ONLY support: - primitive to primitive wrapper - primitive
 * wrapper to primitive These conversions don't require any converter, since the
...
class WidenerInput extends AbstractInput {
    ...
    /**
     * Returns whether widening is supported by this class. If false is returned
     * by this method, then widening is disallowed and a field converter or
     * deleter is necessary.
     */
