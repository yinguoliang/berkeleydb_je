 ...
 * <p>
 * Note that the equals(), compareTo() and hashCode() methods only use the name
 * field in this class. Comparing two FieldInfo objects is only done when both
 * are declared in the same class, so comparing the field name is sufficient.
 * </p>
 *
 * @author Mark Hayes
 */
class FieldInfo implements RawField, Serializable, Comparable<FieldInfo> {
    ...
    void collectRelatedFormats(Catalog catalog, Map<String, Format> newFormats) {
        ...
        /*
         * Prior to intialization we save the newly created format in the format
         * field so that it can be used by class evolution. But note that it may
         * be replaced by the initialize method. [#16233]
         */
         ...
         * the generic types for this field, i.e., MyClass1 and MyClass2 will be
         * created here. [#19377]
         */
        Class cls = field.getType();
        if (format instanceof NonPersistentFormat
                && (java.util.Map.class.isAssignableFrom(cls) || java.util.Collection.class.isAssignableFrom(cls))) {
            if (field != null && field.getGenericType() instanceof ParameterizedType) {
                collectParameterizedTypeFormats(catalog, newFormats, (ParameterizedType) field.getGenericType());
            }
        }
    }

    /*
     * Create formats for the parameterized types, e.g., will create formats for
     * MyClass1 and MyClass2 when meeting Map<MyClass1, Set<MyClass2>>, where
     * MyClass1 and MyClass2 are instance of java.lang.Class.
     */
    private void collectParameterizedTypeFormats(Catalog catalog, Map<String, Format> newFormats,
    ...
    void initialize(Catalog catalog, EntityModel model, int initVersion) {
         ...
         * because nothing changed. [#16233] Note that the format field may be
         * null when used in a composite key format used as a key comparator
         * (via PersistComparator). In that case (null format), we must not
